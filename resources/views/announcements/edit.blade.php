...
<div class="max-w-4xl">
    ...
    <form id="announcementForm" method="POST" action="{{ route('announcements.update', $announcement) }}"
        class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="card">
            ...
            <div class="space-y-2">
                <label
                    ...
                    <input type="radio" name="display_mode" value="full" {{ old('display_mode',
                        $announcement->display_mode ?? 'full') === 'full' ? 'checked' : '' }}
                ...
                <label
                    class="flex items-start p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-primary transition-colors">
                    <input type="radio" name="display_mode" value="image_only" {{ old('display_mode',
                        $announcement->display_mode ?? 'full') === 'image_only' ? 'checked' : '' }}
                    class="mt-1 w-4 h-4 text-primary border-gray-300 focus:ring-primary display-mode-radio">
                    <div class="ml-3">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        ...
                        placeholder="Descripción breve del aviso (máximo 1000 caracteres)">{{ old('description', $announcement->description) }}</textarea>
                    @error('description')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div class="space-y-4">
                <div>
                    <input type="hidden" id="media_id" name="media_id"
                        value="{{ old('media_id', $announcement->media_id) }}">

                    <div id="selected-image-preview" class="{{ $announcement->media ? '' : 'hidden' }} mb-4">
                        <div class="relative inline-block">
                            <img id="preview-img" src="{{ $announcement->media?->url }}"
                                alt="{{ $announcement->media?->alt }}"
                    ...
                    <button type="button" id="select-image-btn" class="btn-secondary">
                        ...
                        {{ $announcement->media ? 'Cambiar Imagen' : 'Seleccionar Imagen' }}
                    </button>
                    @error('media_id')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
        ...
        <div class="card" id="cta-fields">
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input type="url" id="cta_url" name="cta_url" value="{{ old('cta_url', $announcement->cta_url) }}"
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="cta_new_tab" name="cta_new_tab" value="1" {{ old('cta_new_tab',
                        $announcement->cta_new_tab) ? 'checked' : '' }}
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select id="display_type" name="display_type" required
                        ...
                        <option value="global" {{ old('display_type', $announcement->display_type)==='global' ?
                            'selected' : '' }}>
                            Todas las páginas
                        </option>
                        <option value="homepage" {{ old('display_type', $announcement->display_type)==='homepage' ?
                            'selected' : '' }}>
                            Solo página de inicio
                        </option>
                        <option value="specific_pages" {{ old('display_type', $announcement->
                            display_type)==='specific_pages' ? 'selected' : '' }}>
                            Páginas específicas
                        </option>
                    </select>
                    @error('display_type')
                ...
                <div id="page-selector" class="{{ $announcement->display_type === 'specific_pages' ? '' : 'hidden' }}">
                    ...
                    <div class="max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-4 space-y-2">
                        ...
                        <div class="flex items-center">
                            ...
                            old('page_slugs'))) || (!old('page_slugs') && is_array($announcement->page_slugs) &&
                            in_array($page->slug, $announcement->page_slugs)) ? 'checked' : '' }}
                            class="w-4 h-4 text-primary border-gray-300 rounded focus:ring-primary">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <div class="space-y-2">
                        <label
                            class="flex items-start p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-primary transition-colors">
                            <input type="radio" name="schedule_type" value="manual" {{ old('schedule_type',
                                $announcement->schedule_type ?? 'manual') === 'manual' ? 'checked' : '' }}
                        ...
                        <label
                            class="flex items-start p-4 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-primary transition-colors">
                            <input type="radio" name="schedule_type" value="scheduled" {{ old('schedule_type',
                                $announcement->schedule_type ?? 'manual') === 'scheduled' ? 'checked' : '' }}
                            class="mt-1 w-4 h-4 text-primary border-gray-300 focus:ring-primary">
                            <div class="ml-3">
                ...
                <div id="schedule-dates"
                    class="grid grid-cols-1 md:grid-cols-2 gap-4 {{ old('schedule_type', $announcement->schedule_type ?? 'manual') === 'scheduled' ? '' : 'hidden' }}">
                    <div>
