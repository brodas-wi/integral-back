        ...
        <div class="card">
            <form action="{{ route('roles.update', $role) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $role->name) }}" {{ in_array(
        $role->name,
        ['admin', 'editor', 'viewer']
    ) ? 'readonly' : 'required' }} class="input-field @error('name') border-red-500 @enderror {{ in_array($role->name, [
        'admin',
        'editor',
        'viewer'
    ]) ? 'bg-gray-100 cursor-not-allowed' : '' }}" placeholder="admin, editor, moderator">
                    @if(in_array($role->name, ['admin', 'editor', 'viewer']))
                        <p class="text-xs text-gray-600 mt-1">El nombre de los roles del sistema no puede ser modificado</p>
                    @else
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="display_name" name="display_name"
                        value="{{ old('display_name', $role->display_name ?? ucfirst($role->name)) }}" required
