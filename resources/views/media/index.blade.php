  ...
  @if($media->count() > 0)
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      @foreach($media as $item)
        <div class="card group hover:shadow-lg transition-shadow flex flex-col h-full p-4" id="media-item-{{ $item->id }}">
          <a href="{{ route('media.show', $item) }}" class="block mb-3">
            <div
              ...
              @elseif($item->isPdf())
                ...
                <div class="absolute top-2 right-2 bg-red-100 text-red-700 px-2 py-1 rounded text-xs font-bold">
                  PDF
                </div>
              @else
                <i class="ri-file-excel-line text-7xl text-green-500"></i>
                <div class="absolute top-2 right-2 bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold">
                  {{ strtoupper(pathinfo($item->filename, PATHINFO_EXTENSION)) }}
                </div>
              @endif
            </div>
          ...
          <div class="flex-1 flex flex-col">
            ...
            <div class="flex items-center gap-2 mt-1 pt-3 border-t border-gray-100">
              ...
              @php
                ...
                $canDownload = auth()->user()->can('media.download') || auth()->user()->can('media.manage');
              ...
              @if($hasDropdownActions)
                <div class="dropdown" data-dropdown>
                  ...
                  <div class="dropdown-menu">
                    ...
                    @if($canDelete)
                      <button type="button" onclick="confirmDeleteMedia({{ $item->id }}, '{{ addslashes($item->filename) }}')"
