    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2">
        <div class="card">
          ...
          @if($media->isImage())
            <div class="bg-gray-100 rounded-lg p-4 flex items-center justify-center" style="max-height: 500px;">
              <img src="{{ $media->url }}" alt="{{ $media->alt ?? $media->filename }}"
                class="max-w-full max-h-full object-contain rounded-lg shadow-lg" style="max-height: 480px;">
            </div>
          @elseif($media->isPdf())
            <div class="bg-gray-100 rounded-lg overflow-hidden">
              <iframe src="{{ $media->url }}" class="w-full border-0" style="height: 600px;" frameborder="0"></iframe>
            </div>
          @else
            <div class="bg-gray-100 rounded-lg p-8 flex items-center justify-center" style="min-height: 300px;">
      ...
      <div class="lg:col-span-1">
        <div class="card">
          <div class="flex items-center justify-center w-16 h-16 bg-primary bg-opacity-10 rounded-full mx-auto mb-4">
            @if($media->isImage())
          ...
          <div class="space-y-1.5">
            ...
            @if($media->isImage() && $media->width)
              <div class="flex items-center justify-between text-sm text-gray-600">
                ...
                <span class="font-semibold text-secondary">{{ $media->width }}×{{ $media->height }}px</span>
              </div>
            @endif

            <div class="flex items-start justify-between text-sm text-gray-600 gap-2">
              <span class="flex-shrink-0">MIME:</span>
              <span class="font-semibold text-secondary text-right text-xs break-all">{{ $media->mime_type }}</span>
            </div>
          ...
          <div class="mt-4 pt-4 border-t border-gray-100">
            ...
            <div class="flex items-center gap-2">
              ...
              <button onclick="copyMediaUrl()" class="btn-secondary btn-sm w-[38px] p-0 flex items-center justify-center"
                title="Copiar URL">
