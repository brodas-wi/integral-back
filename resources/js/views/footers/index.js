        ...
        btn.addEventListener("click", () => {
            ...
            if (isActive && pageCount > 0) {
                window.showConfirmModal({
                    title: "Desactivar footer en uso",
                    message: `Este footer está vinculado a <strong>${pageCount} página(s)</strong>. ¿Seguro que deseas desactivarlo?`,
...
function initDelete() {
    document.querySelectorAll("[data-delete-footer]").forEach((btn) => {
        btn.addEventListener("click", () => {
            const id = btn.dataset.footerId;
            ...
            if (pageCount > 0) {
                showNotification(
                    `No se puede eliminar: este footer está vinculado a ${pageCount} página(s). Desvincula las páginas primero.`,
                    "error",
                );
                return;
            }
...
async function doDelete(id, btn) {
    try {
        const res = await fetch(buildUrl(`footers/${id}`), {
            method: "DELETE",
            headers: {
                "X-CSRF-TOKEN": CSRF.getAttribute("content"),
                Accept: "application/json",
        ...
        const data = await res.json();
        if (data.success) {
            document.getElementById(`footer-row-${id}`)?.remove();
            showNotification(data.message, "success");
        } else {
