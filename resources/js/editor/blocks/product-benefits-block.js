const PB_STYLES = `
<style>
.pb-section{position:relative;width:100%;background:#E97300;border-top-left-radius:clamp(60px,8vw,140px);padding:clamp(2.5rem,5vw,4rem) clamp(1.5rem,4vw,4rem);box-sizing:border-box;overflow:hidden;}
.pb-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:2rem;margin-top:2.5rem;}
.pb-item{display:flex;flex-direction:column;align-items:center;text-align:center;gap:0.75rem;}
.pb-badge{width:4rem;height:4rem;border-radius:9999px;background:rgba(255,255,255,0.2);display:flex;align-items:center;justify-content:center;flex-shrink:0;}
...
function buildProductBenefitsHTML() {
...
export const productBenefitsBlock = {
    ...
    label: "Sección: beneficios con íconos (fondo naranja)",
    category: "Detalle de Producto",
    media: iconProductBenefits,
    content: buildProductBenefitsHTML(),
};