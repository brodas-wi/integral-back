...
function buildImageTitleBannerHTML(data, uid) {
    ...
    const topNotchStyle = `position:absolute;top:0;right:calc(-1 * ${notchSize});width:${notchSize};height:${notchSize};background:radial-gradient(circle at bottom right,transparent ${notchSize},#fff 0);`;

    const titleStyle = `margin:0;color:#003B71;font-weight:800;font-size:clamp(1.375rem,3vw,2.25rem);line-height:1.25;`;

    const responsiveCss = `@media(max-width:768px){#it-root-${uid}{padding:1.5rem 1.25rem;}#it-root-${uid} .it-wrap-${uid}{display:flex;flex-direction:column;gap:0.75rem;}#it-root-${uid} .it-box-${uid}{order:1;position:static;max-width:100%;width:100%;border-radius:20px;padding:1.25rem 1.5rem 0.625rem 1.5rem;justify-content:center;text-align:center;}#it-root-${uid} .it-img-${uid}{order:2;aspect-ratio:16/9;min-height:200px;}#it-root-${uid} .it-notch-${uid}{display:none;}}`;

    return `<section id="it-root-${uid}" style="${sectionStyle}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false"><div class="it-wrap-${uid}" style="position:relative;width:100%;" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false"><div class="it-img-${uid}" style="${wrapperStyle}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false"><img src="${imageUrl}" alt="${data.title || "Banner"}" style="${imgStyle}"><div style="${gradientStyle}"></div></div><div class="it-box-${uid}" style="${boxStyle}"><h2 style="${titleStyle}">${data.title || "Título"}</h2><div class="it-notch-${uid}" style="${notchStyle}"></div><div class="it-notch-${uid}" style="${topNotchStyle}"></div></div></div><div data-gjs-type="it-responsive-style" data-css-content="${responsiveCss.replace(/"/g, "&quot;")}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false"></div></section>`;
}
