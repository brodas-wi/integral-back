...
function repositionToolbar(editor, toolbarElement = null) {
    ...
    if (physRight > canvasW - margin) {
        newLeft -= physRight - (canvasW - margin);
    }
    // Clamp left overflow (applied after right so left wins on very narrow elements)
    if (physLeft < margin) {
        newLeft += margin - physLeft;
    }

    // Clamp bottom overflow
    if (physBottom > canvasH - margin) {
        newTop -= physBottom - (canvasH - margin);
    }
    // Clamp top overflow
    if (physTop < margin) {
