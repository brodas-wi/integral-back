...
class UpdateScriptRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            ...
            'type.required'        => 'El tipo de script es obligatorio.',
            'type.in'              => 'El tipo debe ser JavaScript o CSS.',
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            // Require content based on type
            if ($this->type === 'js' && empty(trim($this->js_content ?? ''))) {
                $validator->errors()->add('js_content', 'El contenido JavaScript es obligatorio para scripts de tipo JS.');
            }

            if ($this->type === 'css' && empty(trim($this->css_content ?? ''))) {
                $validator->errors()->add('css_content', 'El contenido CSS es obligatorio para scripts de tipo CSS.');
            }
        });
    }
}
