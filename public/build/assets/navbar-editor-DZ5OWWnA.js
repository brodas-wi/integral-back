import{i as G,I as V,o as W,t as X,d as Y,f as K,e as Q,s as Z,g as ee,c as ne,E as te,j as oe,h as ae}from"./editor-commands-Bq37dEX4.js";import"./_commonjsHelpers-CqkleIqs.js";function re(){return G()}const ie=`(function(){
...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function le(r){return r?r.split(" ").filter(o=>o.length>0).map((o,a)=>`<span style="color:${a%2===0?"#E97300":"#003B71"};">${o}</span>`).join(" "):""}function R(r,o){o=o||"nb"+Math.random().toString(36).slice(2,7);const a=r.logo_href||"/",i=r.logo_url?`<img src="${r.logo_url}" alt="${r.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${r.logo_text||"Logo"}</span>`,y=`<a href="${a}" class="nb-logo-link">${i}</a>`,C=(r.top_actions||[]).map(d=>{const B=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-top-action">${B}${d.label||""}</a>`}).join(""),q=r.banking_btn||{},w=q.color==="orange"?"nb-banking-orange":"nb-banking-blue",h=`<a href="${q.href||"#"}" class="nb-banking-btn ${w}">${q.label||"Banca en Línea"}</a>`,m=(r.nav_links||[]).map(d=>{if(d.type==="submenu"&&d.columns?.length){const B=!!(d.cta_column&&(d.cta_column.text||d.cta_column.btn_label)),$=d.columns.slice(0,3),_=$.map(E=>{const z=E.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",n=E.badge?`<span class="nb-mega-badge ${z}">${E.badge}</span>`:"",e=(E.items||[]).map(s=>{const l=s.desc?`<span class="nb-mega-item-desc">${s.desc}</span>`:"";return`<a href="${s.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${s.label||""}</span>${l}</a>`}).join("");return`<div class="nb-mega-col">${n}${e}</div>`}).join("");let A="";if(B){const E=d.cta_column,z=E.btn_color==="blue"?"nb-badge-blue":"nb-badge-orange";A=`<div class="nb-mega-cta-col">
                        <p class="nb-mega-cta-text">${le(E.text||"")}</p>
                        <a href="${E.btn_href||"#"}" class="nb-mega-cta-btn ${z}">${E.btn_label||"Ver más"}</a>
                    </div>`}const N=B?"grid-template-columns:1fr 1fr 1fr minmax(200px,260px);":`grid-template-columns:repeat(${$.length},minmax(0,280px));justify-content:start;`;return`<li class="nb-nav-item nb-has-submenu">
                    <button class="nb-nav-link nb-nav-trigger" type="button">${d.label||"Menú"}</button>
                    <div class="nb-mega"><div class="nb-mega-grid" style="${N}">${_}${A}</div></div>
                </li>`}return`<li class="nb-nav-item"><a href="${d.href||"#"}" class="nb-nav-link">${d.label||""}</a></li>`}).join(""),f=r.bottom_cta||{},k=f.color==="blue"?"nb-cta-blue":"nb-cta-orange",S=f.sublabel?`<span class="nb-bottom-cta-line2">${f.sublabel}</span>`:"",t=f.label?`<a href="${f.href||"#"}" class="nb-bottom-cta ${k}"><span class="nb-bottom-cta-line1">${f.label}</span>${S}</a>`:"",T=(r.top_actions||[]).map(d=>{const B=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-mobile-top-action">${B}${d.label||""}</a>`}).join(""),U=(r.nav_links||[]).map(d=>{if(d.type==="submenu"&&d.columns?.length){const B=d.columns.slice(0,3).map($=>{const _=$.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",A=$.badge?`<span class="nb-mobile-sub-badge ${_}">${$.badge}</span>`:"",N=($.items||[]).map(E=>{const z=E.desc?`<span class="nb-mobile-sub-desc">${E.desc}</span>`:"";return`<a href="${E.href||"#"}" class="nb-mobile-sub-link"><span class="nb-mobile-sub-title">${E.label||""}</span>${z}</a>`}).join("");return`<div class="nb-mobile-sub-col">${A}${N}</div>`}).join("");return`<div class="nb-mobile-item">
                    <button class="nb-mobile-link" type="button">${d.label||"Menú"}<i class="ri-arrow-down-s-line"></i></button>
                    <div class="nb-mobile-submenu">${B}</div>
                </div>`}return`<a href="${d.href||"#"}" class="nb-mobile-link">${d.label||""}</a>`}).join(""),H=[h,f.label?`<a href="${f.href||"#"}" class="nb-bottom-cta ${k}"><span class="nb-bottom-cta-line1">${f.label}</span>${S}</a>`:""].filter(Boolean).join("");return`<div id="nb-root-${o}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false">
        <div class="nb-top">
            <div class="nb-logo">${y}</div>
        ...
        `,document.head.appendChild(n)}const i=(()=>{try{return JSON.parse(o.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),y=i.logo_url||"",C=i.logo_alt||"",q=i.logo_text||"Logo",w=i.logo_href||"/",h=JSON.parse(JSON.stringify(i.top_actions||D.top_actions)),m=JSON.parse(JSON.stringify(i.banking_btn||D.banking_btn)),f=JSON.parse(JSON.stringify(i.nav_links||D.nav_links)),k=JSON.parse(JSON.stringify(i.bottom_cta||D.bottom_cta)),S=document.createElement("div");S.id="navbar-config-modal",S.className="nb-overlay";const t=document.createElement("div");t.className="nb-modal",t.innerHTML=`
                ...
                <div class="nb-card">
                    ...
                    <div class="nb-row" style="align-items:flex-start;flex-wrap:wrap;gap:0.75rem;">
                        <div style="flex:1;min-width:200px;display:flex;flex-direction:column;gap:0.5rem;">
                            ${y?`<img id="nb-logo-preview" src="${y}" style="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;">`:'<div id="nb-logo-preview" style="display:none;"></div>'}
                            <input id="nb-logo-url" type="text" placeholder="URL del logo" value="${y}" class="nb-input">
                        </div>
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,S.appendChild(t),document.body.appendChild(S);const T=new V;t.querySelectorAll(".nb-tab-btn").forEach(n=>{n.addEventListener("click",()=>{t.querySelectorAll(".nb-tab-btn").forEach(e=>e.classList.remove("active")),t.querySelectorAll(".nb-tab-panel").forEach(e=>e.classList.remove("active")),n.classList.add("active"),t.querySelector(`#nb-panel-${n.dataset.tab}`).classList.add("active")})}),t.querySelector("#nb-logo-pick").addEventListener("click",()=>{W({type:"image",title:"Seleccionar logo",onSelect:n=>{t.querySelector("#nb-logo-url").value=n;let e=t.querySelector("#nb-logo-preview");if(!e||e.tagName==="DIV"){const s=document.createElement("img");s.id="nb-logo-preview",s.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;",e?.replaceWith(s)??t.querySelector("#nb-logo-url").before(s),e=s}e.src=n,e.style.display="block"}})});const H=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function d(n){if(n.dataset.autocompleteAttached)return;n.dataset.autocompleteAttached="true";const e=n.parentNode;(!e.style.position||e.style.position==="static")&&(e.style.position="relative");const s=document.createElement("ul");s.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",e.appendChild(s);let l=null;async function u(g){if(g.length<1){s.style.display="none";return}try{const L=await(await fetch(`${H}?q=${encodeURIComponent(g)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();p(L,g)}catch{s.style.display="none"}}function b(g,c){return c?g.replace(new RegExp(`(${c.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):g}function p(g,c){if(s.innerHTML="",!g.length){s.style.display="none";return}g.forEach(L=>{const x=document.createElement("li");x.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",x.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${b(L.title,c)}</span><span style="font-size:0.7rem;color:#64748b;">/${L.slug}</span>`,x.addEventListener("mouseenter",()=>x.style.background="#f1f5f9"),x.addEventListener("mouseleave",()=>x.style.background=""),x.addEventListener("mousedown",j=>{j.preventDefault(),n.value="/"+L.slug,n.dispatchEvent(new Event("input")),s.style.display="none"}),s.appendChild(x)}),s.style.display="block"}n.addEventListener("input",()=>{clearTimeout(l),l=setTimeout(()=>u(n.value.trim()),220)}),n.addEventListener("focus",()=>{n.select(),n.value.trim()&&u(n.value.trim())}),n.addEventListener("blur",()=>{setTimeout(()=>{s.style.display="none"},150)}),n.addEventListener("keydown",g=>{if(s.style.display==="none")return;const c=s.querySelectorAll("li"),L=s.querySelector("li.nb-ac-active");let x=Array.from(c).indexOf(L);if(g.key==="ArrowDown"){g.preventDefault(),L?.classList.remove("nb-ac-active");const j=c[x+1]||c[0];j?.classList.add("nb-ac-active"),j&&(j.style.background="#f1f5f9")}else if(g.key==="ArrowUp"){g.preventDefault(),L?.classList.remove("nb-ac-active");const j=c[x-1]||c[c.length-1];j?.classList.add("nb-ac-active"),j&&(j.style.background="#f1f5f9")}else g.key==="Enter"&&L?(g.preventDefault(),L.dispatchEvent(new MouseEvent("mousedown"))):g.key==="Escape"&&(s.style.display="none")})}function B(n,e,s){let l=null;n.querySelectorAll("[data-drag-idx]").forEach(u=>{u.setAttribute("draggable","true"),u.addEventListener("dragstart",b=>{l=parseInt(u.dataset.dragIdx),setTimeout(()=>u.classList.add("nb-dragging"),0),b.dataTransfer.effectAllowed="move"}),u.addEventListener("dragend",()=>{u.classList.remove("nb-dragging"),n.querySelectorAll(".nb-drag-over").forEach(b=>b.classList.remove("nb-drag-over"))}),u.addEventListener("dragover",b=>{b.preventDefault(),b.dataTransfer.dropEffect="move",parseInt(u.dataset.dragIdx)!==l&&u.classList.add("nb-drag-over")}),u.addEventListener("dragleave",()=>u.classList.remove("nb-drag-over")),u.addEventListener("drop",b=>{b.preventDefault();const p=parseInt(u.dataset.dragIdx);if(l!==null&&p!==l){const[g]=e.splice(l,1);e.splice(p,0,g),s()}l=null})})}function $(){const n=t.querySelector("#nb-top-actions-list");n.innerHTML="",h.forEach((e,s)=>{const l=document.createElement("div");l.className="nb-action-card",l.dataset.dragIdx=s;const u=e.icon||"";l.innerHTML=`
                <div class="nb-row">
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${e.href||"#"}" data-field="href">
                </div>`,l.querySelector(".nb-pick-icon-btn").addEventListener("click",()=>{T.open(p=>{e.icon=p,l.querySelector("[data-field='icon']").value=p,l.querySelector(".nb-icon-preview i").className=p})}),l.querySelector(".nb-remove-action").onclick=()=>{h.splice(s,1),$()},l.querySelectorAll("[data-field]").forEach(p=>{p.addEventListener("input",()=>{e[p.dataset.field]=p.value})});const b=l.querySelector(".nb-url-input");if(b){const p=document.createElement("div");p.style.position="relative",b.parentNode.insertBefore(p,b),p.appendChild(b),d(b)}n.appendChild(l)}),B(n,h,$)}function _(){const n=t.querySelector("#nb-nav-list");n.innerHTML="",f.forEach((e,s)=>{const l=document.createElement("div");l.className="nb-link-card",l.dataset.dragIdx=s;const u=`<span class="nb-type-badge ${e.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${e.type==="submenu"?"Submenú":"Link"} ↕</span>`;if(e.type==="submenu"){let g=function(){p.innerHTML="",(e.columns||[]).forEach((c,L)=>{const x=document.createElement("div");x.className="nb-col-card";const j=(c.items||[]).map((v,I)=>`
                            ...
                            <div class="nb-col-card-header">
                                ...
                                <input class="nb-input-sm" style="flex:1;" placeholder="Badge (ej: Tus ahorros)" value="${c.badge||""}" data-col-field="badge">
                                <div class="nb-color-toggle" style="gap:0.25rem;">
                                    <button class="nb-color-opt nb-color-opt-blue ${(c.badge_color||"blue")==="blue"?"":"nb-color-inactive"}" data-col-color="blue" style="padding:0.2rem 0.5rem;font-size:0.65rem;">Azul</button>
                                    <button class="nb-color-opt nb-color-opt-orange ${(c.badge_color||"blue")==="orange"?"":"nb-color-inactive"}" data-col-color="orange" style="padding:0.2rem 0.5rem;font-size:0.65rem;">Naranja</button>
                                </div>
                            ...
                            </div>`,x.querySelector("[data-col-field='badge']").addEventListener("input",v=>{c.badge=v.target.value}),x.querySelectorAll("[data-col-color]").forEach(v=>{v.addEventListener("click",()=>{c.badge_color=v.dataset.colColor,x.querySelectorAll("[data-col-color]").forEach(I=>{I.classList.toggle("nb-color-inactive",I.dataset.colColor!==v.dataset.colColor)})})}),x.querySelector(".nb-remove-col").onclick=()=>{e.columns.splice(L,1),g();const v=l.querySelector(".nb-add-col-btn");v&&(v.disabled=!1,v.style.opacity="",v.style.cursor="")},x.querySelectorAll("[data-item-field]").forEach(v=>{const I=parseInt(v.closest("[data-item-idx]").dataset.itemIdx);v.addEventListener("input",()=>{c.items[I][v.dataset.itemField]=v.value})}),x.querySelectorAll(".nb-url-input").forEach(v=>{const I=document.createElement("div");I.style.position="relative",v.parentNode.insertBefore(I,v),I.appendChild(v),d(v)}),x.querySelectorAll(".nb-remove-item").forEach(v=>{v.onclick=()=>{const I=parseInt(v.closest("[data-item-idx]").dataset.itemIdx);c.items.splice(I,1),g()}}),x.querySelector(".nb-add-item-btn").onclick=()=>{c.items=c.items||[],c.items.push({label:"Nuevo item",desc:"",href:"#"}),g()},p.appendChild(x)})};l.innerHTML=`
                    ...
                    </div>`,l.querySelector("[data-toggle-type]").onclick=()=>{e.type="link",e.href="#",delete e.columns,delete e.cta_column,_()},l.querySelector(".nb-remove-link").onclick=()=>{f.splice(s,1),_()},l.querySelector("[data-field='label']").addEventListener("input",c=>{e.label=c.target.value}),l.querySelectorAll("[data-cta-field]").forEach(c=>{c.addEventListener("input",()=>{e.cta_column||(e.cta_column={}),e.cta_column[c.dataset.ctaField]=c.value})});const b=l.querySelector("[data-cta-field='btn_href']");if(b){const c=document.createElement("div");c.style.position="relative",b.parentNode.insertBefore(c,b),c.appendChild(b),d(b)}l.querySelectorAll("[data-cta-color]").forEach(c=>{c.addEventListener("click",()=>{e.cta_column||(e.cta_column={}),e.cta_column.btn_color=c.dataset.ctaColor,l.querySelectorAll("[data-cta-color]").forEach(L=>{L.classList.toggle("nb-color-inactive",L.dataset.ctaColor!==c.dataset.ctaColor)})})});const p=l.querySelector(".nb-columns-list");g(),l.querySelector(".nb-add-col-btn").onclick=()=>{if(!((e.columns||[]).length>=3)&&(e.columns=e.columns||[],e.columns.push({badge:"Nueva columna",badge_color:"blue",items:[{label:"Nuevo item",desc:"",href:"#"}]}),g(),e.columns.length>=3)){const c=l.querySelector(".nb-add-col-btn");c.disabled=!0,c.style.opacity="0.4",c.style.cursor="not-allowed"}}}else{l.innerHTML=`
                    <div class="nb-link-card-header">
                        <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                        ${u}
                ...
                </div>`,document.body.appendChild(b),b.querySelector(".nb-confirm-cancel").onclick=()=>{b.remove(),n.target.value=""},b.querySelector(".nb-confirm-ok").onclick=()=>{b.remove(),n.target.value="";const g=o.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);o.addAttributes({"data-navbar-config":JSON.stringify(u)}),o.components(R(u,g)+O),z(),P(r,o)}},s.readAsText(e)};const z=()=>{T.close(),S.remove()};t.querySelector("#nb-modal-close").onclick=z,t.querySelector("#nb-modal-cancel").onclick=z,S.onclick=n=>{n.target===S&&z()},t.querySelector("#nb-modal-save").onclick=()=>{const n={logo_url:t.querySelector("#nb-logo-url").value.trim(),logo_alt:t.querySelector("#nb-logo-alt").value.trim(),logo_text:t.querySelector("#nb-logo-text").value.trim(),logo_href:t.querySelector("#nb-logo-href").value.trim()||"/",top_actions:h,banking_btn:{label:t.querySelector("#nb-banking-label").value.trim(),href:t.querySelector("#nb-banking-href").value.trim()||"#",color:m.color||"blue"},nav_links:f,bottom_cta:{label:t.querySelector("#nb-bcta-label").value.trim(),sublabel:t.querySelector("#nb-bcta-sublabel").value.trim(),href:t.querySelector("#nb-bcta-href").value.trim()||"#",color:k.color||"orange"}},s=o.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);o.addAttributes({"data-navbar-config":JSON.stringify(n)}),o.components(R(n,s)+O),z()}}function se(r){const o="navbar-component";r.DomComponents.addType(o,{isComponent:a=>a.getAttribute?.("data-gjs-type")===o?{type:o}:!1,model:{defaults:{name:"Navbar",tagName:"nav",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":o,class:"nb-wrapper","data-navbar-config":JSON.stringify(D)},components:R(D)+O,script:function(){(function(a){if(!a||typeof a.querySelector!="function"||a.__nbInit)return;a.__nbInit=!0;var i=a.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");if(!i)return;var y=!!window.__gjseditor||document.documentElement.hasAttribute("data-gjs-canvas");function C(){y||(document.body.style.paddingTop=a.offsetHeight+"px")}C(),window.addEventListener("resize",function(){if(C(),window.innerWidth>992){var h=document.getElementById("nb-mobile-"+i);h&&h.classList.contains("nb-open")&&h.classList.remove("nb-open"),a.querySelectorAll(".nb-mobile-item.nb-open").forEach(function(m){m.classList.remove("nb-open")})}});var q=document.getElementById("nb-toggle-"+i),w=document.getElementById("nb-mobile-"+i);q&&w&&q.addEventListener("click",function(){w.classList.toggle("nb-open"),C()}),a.querySelectorAll(".nb-nav-trigger").forEach(function(h){h.addEventListener("click",function(m){m.stopPropagation();var f=h.closest(".nb-nav-item"),k=f.classList.contains("nb-open");a.querySelectorAll(".nb-nav-item.nb-open").forEach(function(S){S.classList.remove("nb-open")}),k||f.classList.add("nb-open"),C()})}),a.querySelectorAll(".nb-mobile-item>.nb-mobile-link").forEach(function(h){h.addEventListener("click",function(){h.closest(".nb-mobile-item").classList.toggle("nb-open"),C()})}),document.addEventListener("click",function(h){a.contains(h.target)||a.querySelectorAll(".nb-nav-item.nb-open").forEach(function(m){m.classList.remove("nb-open")})})})(this)},"script-props":[],toolbar:[],traits:[{type:"button",label:"Navbar",text:"Administrar Navbar",full:!0,command:"open-navbar-config"}]},init(){this.set("type",o),this.addAttributes({"data-gjs-type":o})}}}),r.Commands.add("open-navbar-config",{run(a){const i=a.getSelected();i&&P(a,i)}}),r.BlockManager.add("navbar-block",{label:"Navbar",category:"Navbar",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,activate:!0,content:{type:o,attributes:{"data-gjs-type":o}}}),ce(r,o),de(r)}function ce(r,o){r.on("storage:end:load",()=>{setTimeout(()=>J(r,o),800)}),r.on("component:mount",a=>{const i=a.getEl();i?.getAttribute?.("data-gjs-type")===o&&(a.set("type",o),setTimeout(()=>F(r,i),400))}),r.on("canvas:render",()=>{setTimeout(()=>J(r,o),600)})}function F(r,o){if(o?.isConnected)try{const a=r.Canvas.getFrameEl()?.contentDocument;if(!a)return;o.__nbInit&&delete o.__nbInit;const i=a.createElement("script");i.textContent=`(function(){
            function initNavbar(root){
                if(!root||root.__nbInit)return;
                root.__nbInit=true;
                var id=root.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");
        ...
        })();`,a.head.appendChild(i),i.remove()}catch(a){console.warn("[Navbar] Error inyectando script en canvas:",a)}}function J(r,o){r.getWrapper().find(`[data-gjs-type="${o}"]`).forEach(a=>{a.set("type",o);const i=a.getEl();i?.isConnected&&F(r,i)})}function de(r){r.on("load",()=>{const o=r.Canvas.getFrameEl();if(!o)return;const a=o.contentDocument,i=a?.head;if(i&&(a.documentElement?.setAttribute("data-gjs-canvas","true"),!i.querySelector("#navbar-component-css"))){const y=document.createElement("style");y.id="navbar-component-css",y.textContent=`
                [data-gjs-type="navbar-component"] {
                    position: relative !important;
                    top: auto !important;
                }
                body { padding-top: 0 !important; }
            `,i.appendChild(y)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new te;let o=document.getElementById("navbar-id")?.value||"",a=document.getElementById("navbar-name")?.value||"",i=document.getElementById("navbar-load-url")?.value||"",y=document.getElementById("navbar-store-url")?.value||"";const C=document.getElementById("navbar-is-active")?.value==="1";let q=!!o;const w=re();if(se(w),w.on("load",()=>{X(w),Y(),K(),Q(w),Z(w),ee(w),ne(w),be(w),me(w),setTimeout(()=>{w.runCommand("sw-visibility"),w.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),q&&i)try{await r.loadPageContent(w,i),M("Navbar cargado correctamente","success")}catch{M("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const m=document.getElementById("save-button");m.disabled=!0,m.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!q&&!a?oe({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async f=>{if(!f?.trim()){M("El nombre es obligatorio","error"),m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await h(f)}catch(k){M(k.message,"error")}finally{m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await h(a),m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(f){M(f.message,"error"),m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function h(m){const f=q?"PUT":"POST",k=r.getEditorContent(w);(!k.js_content||k.js_content.trim()==="")&&(k.js_content=ie);const S=await r.savePage(w,{...k,name:m,is_active:C},y,f);if(S.success){if(r.markAsClean(),M(S.message,"success"),!q&&S.navbar){o=S.navbar.id,a=S.navbar.name,q=!0;const t=document.getElementById("navbar-id");t&&(t.value=o);const T=document.getElementById("navbar-name");T&&(T.value=a);const U=document.querySelector('meta[name="app-url"]'),H=U?U.content:"";y=y.endsWith("/navbars")?`${y}/${o}`:`${y.replace(/\/navbars\/?$/,"")}/navbars/${o}`;const B=document.getElementById("navbar-store-url");B&&(B.value=y),i=`${y}/load`;const $=document.getElementById("navbar-load-url");$&&($.value=i);const _=document.getElementById("editor-title");_&&(_.textContent=`Editando Navbar: ${a}`);const A=`/navbars/edit/${o}/edit`,N=H?`${H}${A}`:A;window.history.replaceState({path:N},"",N)}else if(m){a=m;const t=document.getElementById("navbar-name");t&&(t.value=a);const T=document.getElementById("editor-title");T&&(T.textContent=`Editando Navbar: ${a}`)}}}});function be(r){r.Commands.add("canvas-clear",{run:o=>{ae({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{o.DomComponents.clear(),o.CssComposer.clear()}})}})}function me(r){const o=["set-device-desktop","set-device-tablet","set-device-mobile"];function a(i){o.forEach(y=>{r.Panels.getButton("devices-c",y)?.set("active",y===i)})}r.Commands.add("set-device-desktop",{run(i){i.setDevice("desktop"),a("set-device-desktop")}}),r.Commands.add("set-device-tablet",{run(i){i.setDevice("tablet"),a("set-device-tablet")}}),r.Commands.add("set-device-mobile",{run(i){i.setDevice("mobile"),a("set-device-mobile")}}),r.on("device:select",i=>{const y=i.get?i.get("id"):i.id,C={desktop:"set-device-desktop",tablet:"set-device-tablet",mobile:"set-device-mobile"};C[y]&&a(C[y])}),setTimeout(()=>{a("set-device-desktop")},200)}function M(r,o="info"){typeof window.showNotification=="function"&&window.showNotification(r,o)}
