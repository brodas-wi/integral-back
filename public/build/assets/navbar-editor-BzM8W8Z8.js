...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function le(r){return r?r.split(" ").filter(t=>t.length>0).map((t,a)=>`<span style="color:${a%2===0?"#E97300":"#003B71"};">${t}</span>`).join(" "):""}function R(r,t){t=t||"nb"+Math.random().toString(36).slice(2,7);const a=r.logo_href||"/",s=r.logo_url?`<img src="${r.logo_url}" alt="${r.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${r.logo_text||"Logo"}</span>`,k=`<a href="${a}" class="nb-logo-link">${s}</a>`,N=(r.top_actions||[]).map(b=>{const L=b.icon?`<i class="${b.icon}"></i>`:"";return`<a href="${b.href||"#"}" class="nb-top-action">${L}${b.label||""}</a>`}).join(""),C=r.banking_btn||{},x=C.color==="orange"?"nb-banking-orange":"nb-banking-blue",w=`<a href="${C.href||"#"}" class="nb-banking-btn ${x}">${C.label||"Banca en Línea"}</a>`,d=(r.nav_links||[]).map(b=>{if(b.type==="submenu"&&b.columns?.length){const L=Math.min(b.columns.length,3),$=`nb-mega-grid-${L}`,A=!!(b.cta_column&&(b.cta_column.text||b.cta_column.btn_label)),q=b.columns.slice(0,3).map(_=>{const M=_.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",n=_.badge?`<span class="nb-mega-badge ${M}">${_.badge}</span>`:"",e=(_.items||[]).map(c=>{const l=c.desc?`<span class="nb-mega-item-desc">${c.desc}</span>`:"";return`<a href="${c.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${c.label||""}</span>${l}</a>`}).join("");return`<div class="nb-mega-col">${n}${e}</div>`}).join("");let I="";if(A){const _=b.cta_column,M=_.btn_color==="blue"?"nb-badge-blue":"nb-badge-orange";I=`<div class="nb-mega-cta-col">
                        <p class="nb-mega-cta-text">${le(_.text||"")}</p>
                        <a href="${_.btn_href||"#"}" class="nb-mega-cta-btn ${M}">${_.btn_label||"Ver más"}</a>
                    </div>`}const j=A?`<div class="nb-mega-grid ${$}">${q}${I}</div>`:`<div class="nb-mega-grid ${$}" style="grid-template-columns:repeat(${L},1fr);">${q}</div>`;return`<li class="nb-nav-item nb-has-submenu">
                    <button class="nb-nav-link nb-nav-trigger" type="button">${b.label||"Menú"}<i class="ri-arrow-down-s-line nb-chevron"></i></button>
                    <div class="nb-mega">${j}</div>
                </li>`}return`<li class="nb-nav-item"><a href="${b.href||"#"}" class="nb-nav-link">${b.label||""}</a></li>`}).join(""),p=r.bottom_cta||{},h=p.color==="blue"?"nb-cta-blue":"nb-cta-orange",E=p.sublabel?`<span class="nb-bottom-cta-line2">${p.sublabel}</span>`:"",o=p.label?`<a href="${p.href||"#"}" class="nb-bottom-cta ${h}"><span class="nb-bottom-cta-line1">${p.label}</span>${E}</a>`:"",z=(r.nav_links||[]).map(b=>{if(b.type==="submenu"&&b.columns?.length){const L=b.columns.flatMap($=>$.items||[]).map($=>{const A=$.desc?`<span class="nb-mobile-sub-desc">${$.desc}</span>`:"";return`<a href="${$.href||"#"}" class="nb-mobile-sub-link"><span class="nb-mobile-sub-title">${$.label||""}</span>${A}</a>`}).join("");return`<div class="nb-mobile-item">
                ...
                </div>`}return`<a href="${b.href||"#"}" class="nb-mobile-link">${b.label||""}</a>`}).join(""),O=`<div class="nb-mobile-actions">
        ${w}
        ${p.label?`<a href="${p.href||"#"}" class="nb-bottom-cta ${h}" style="max-width:100%;width:100%;border-radius:0.625rem;"><span class="nb-bottom-cta-line1">${p.label}</span>${E}</a>`:""}
        ...
        <div class="nb-top">
            ...
            <div class="nb-top-actions">${N}</div>
            ${w}
        </div>
        <div class="nb-bottom">
        ...
        <div class="nb-mobile-menu" id="nb-mobile-${t}">${z}${O}</div>
        ...
        <div class="nb-modal-body">
            ...
            <div class="nb-tab-panel active" id="nb-panel-logo">
                ...
                <div class="nb-card">
                    ...
                    <div class="nb-row">
                        <input id="nb-logo-alt"  type="text" placeholder="Texto alternativo"      value="${N}"  class="nb-input">
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,E.appendChild(o),document.body.appendChild(E);const z=new V;o.querySelectorAll(".nb-tab-btn").forEach(n=>{n.addEventListener("click",()=>{o.querySelectorAll(".nb-tab-btn").forEach(e=>e.classList.remove("active")),o.querySelectorAll(".nb-tab-panel").forEach(e=>e.classList.remove("active")),n.classList.add("active"),o.querySelector(`#nb-panel-${n.dataset.tab}`).classList.add("active")})}),o.querySelector("#nb-logo-pick").addEventListener("click",()=>{oe({type:"image",title:"Seleccionar logo",onSelect:n=>{o.querySelector("#nb-logo-url").value=n;let e=o.querySelector("#nb-logo-preview");if(!e||e.tagName==="DIV"){const c=document.createElement("img");c.id="nb-logo-preview",c.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;",e?.replaceWith(c)??o.querySelector("#nb-logo-url").before(c),e=c}e.src=n,e.style.display="block"}})});const b=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function L(n){if(n.dataset.autocompleteAttached)return;n.dataset.autocompleteAttached="true";const e=n.parentNode;(!e.style.position||e.style.position==="static")&&(e.style.position="relative");const c=document.createElement("ul");c.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",e.appendChild(c);let l=null;async function y(g){if(g.length<1){c.style.display="none";return}try{const S=await(await fetch(`${b}?q=${encodeURIComponent(g)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();u(S,g)}catch{c.style.display="none"}}function m(g,i){return i?g.replace(new RegExp(`(${i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):g}function u(g,i){if(c.innerHTML="",!g.length){c.style.display="none";return}g.forEach(S=>{const v=document.createElement("li");v.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",v.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${m(S.title,i)}</span><span style="font-size:0.7rem;color:#64748b;">/${S.slug}</span>`,v.addEventListener("mouseenter",()=>v.style.background="#f1f5f9"),v.addEventListener("mouseleave",()=>v.style.background=""),v.addEventListener("mousedown",T=>{T.preventDefault(),n.value="/"+S.slug,n.dispatchEvent(new Event("input")),c.style.display="none"}),c.appendChild(v)}),c.style.display="block"}n.addEventListener("input",()=>{clearTimeout(l),l=setTimeout(()=>y(n.value.trim()),220)}),n.addEventListener("focus",()=>{n.select(),n.value.trim()&&y(n.value.trim())}),n.addEventListener("blur",()=>{setTimeout(()=>{c.style.display="none"},150)}),n.addEventListener("keydown",g=>{if(c.style.display==="none")return;const i=c.querySelectorAll("li"),S=c.querySelector("li.nb-ac-active");let v=Array.from(i).indexOf(S);if(g.key==="ArrowDown"){g.preventDefault(),S?.classList.remove("nb-ac-active");const T=i[v+1]||i[0];T?.classList.add("nb-ac-active"),T&&(T.style.background="#f1f5f9")}else if(g.key==="ArrowUp"){g.preventDefault(),S?.classList.remove("nb-ac-active");const T=i[v-1]||i[i.length-1];T?.classList.add("nb-ac-active"),T&&(T.style.background="#f1f5f9")}else g.key==="Enter"&&S?(g.preventDefault(),S.dispatchEvent(new MouseEvent("mousedown"))):g.key==="Escape"&&(c.style.display="none")})}function $(n,e,c){let l=null;n.querySelectorAll("[data-drag-idx]").forEach(y=>{y.setAttribute("draggable","true"),y.addEventListener("dragstart",m=>{l=parseInt(y.dataset.dragIdx),setTimeout(()=>y.classList.add("nb-dragging"),0),m.dataTransfer.effectAllowed="move"}),y.addEventListener("dragend",()=>{y.classList.remove("nb-dragging"),n.querySelectorAll(".nb-drag-over").forEach(m=>m.classList.remove("nb-drag-over"))}),y.addEventListener("dragover",m=>{m.preventDefault(),m.dataTransfer.dropEffect="move",parseInt(y.dataset.dragIdx)!==l&&y.classList.add("nb-drag-over")}),y.addEventListener("dragleave",()=>y.classList.remove("nb-drag-over")),y.addEventListener("drop",m=>{m.preventDefault();const u=parseInt(y.dataset.dragIdx);if(l!==null&&u!==l){const[g]=e.splice(l,1);e.splice(u,0,g),c()}l=null})})}function A(){const n=o.querySelector("#nb-top-actions-list");n.innerHTML="",w.forEach((e,c)=>{const l=document.createElement("div");l.className="nb-action-card",l.dataset.dragIdx=c;const y=e.icon||"";l.innerHTML=`
                <div class="nb-row">
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${e.href||"#"}" data-field="href">
                </div>`,l.querySelector(".nb-pick-icon-btn").addEventListener("click",()=>{z.open(u=>{e.icon=u,l.querySelector("[data-field='icon']").value=u,l.querySelector(".nb-icon-preview i").className=u})}),l.querySelector(".nb-remove-action").onclick=()=>{w.splice(c,1),A()},l.querySelectorAll("[data-field]").forEach(u=>{u.addEventListener("input",()=>{e[u.dataset.field]=u.value})});const m=l.querySelector(".nb-url-input");if(m){const u=document.createElement("div");u.style.position="relative",m.parentNode.insertBefore(u,m),u.appendChild(m),L(m)}n.appendChild(l)}),$(n,w,A)}function q(){const n=o.querySelector("#nb-nav-list");n.innerHTML="",p.forEach((e,c)=>{const l=document.createElement("div");l.className="nb-link-card",l.dataset.dragIdx=c;const y=`<span class="nb-type-badge ${e.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${e.type==="submenu"?"Submenú":"Link"} ↕</span>`;if(e.type==="submenu"){let g=function(){u.innerHTML="",(e.columns||[]).forEach((i,S)=>{const v=document.createElement("div");v.className="nb-col-card";const T=(i.items||[]).map((f,B)=>`
                            <div class="nb-col-item" data-item-idx="${B}">
                            ...
                            <div class="nb-col-card-header">
                                <span style="font-size:0.75rem;font-weight:700;color:#475569;">Columna ${S+1}</span>
                            ...
                            </div>`,v.querySelector("[data-col-field='badge']").addEventListener("input",f=>{i.badge=f.target.value}),v.querySelectorAll("[data-col-color]").forEach(f=>{f.addEventListener("click",()=>{i.badge_color=f.dataset.colColor,v.querySelectorAll("[data-col-color]").forEach(B=>{B.classList.toggle("nb-color-inactive",B.dataset.colColor!==f.dataset.colColor)})})}),v.querySelector(".nb-remove-col").onclick=()=>{e.columns.splice(S,1),g();const f=l.querySelector(".nb-add-col-btn");f&&(f.disabled=!1,f.style.opacity="",f.style.cursor="")},v.querySelectorAll("[data-item-field]").forEach(f=>{const B=parseInt(f.closest("[data-item-idx]").dataset.itemIdx);f.addEventListener("input",()=>{i.items[B][f.dataset.itemField]=f.value})}),v.querySelectorAll(".nb-url-input").forEach(f=>{const B=document.createElement("div");B.style.position="relative",f.parentNode.insertBefore(B,f),B.appendChild(f),L(f)}),v.querySelectorAll(".nb-remove-item").forEach(f=>{f.onclick=()=>{const B=parseInt(f.closest("[data-item-idx]").dataset.itemIdx);i.items.splice(B,1),g()}}),v.querySelector(".nb-add-item-btn").onclick=()=>{i.items=i.items||[],i.items.push({label:"Nuevo item",desc:"",href:"#"}),g()},u.appendChild(v)})};l.innerHTML=`
                    ...
                    </div>`,l.querySelector("[data-toggle-type]").onclick=()=>{e.type="link",e.href="#",delete e.columns,delete e.cta_column,q()},l.querySelector(".nb-remove-link").onclick=()=>{p.splice(c,1),q()},l.querySelector("[data-field='label']").addEventListener("input",i=>{e.label=i.target.value}),l.querySelectorAll("[data-cta-field]").forEach(i=>{i.addEventListener("input",()=>{e.cta_column||(e.cta_column={}),e.cta_column[i.dataset.ctaField]=i.value})});const m=l.querySelector("[data-cta-field='btn_href']");if(m){const i=document.createElement("div");i.style.position="relative",m.parentNode.insertBefore(i,m),i.appendChild(m),L(m)}l.querySelectorAll("[data-cta-color]").forEach(i=>{i.addEventListener("click",()=>{e.cta_column||(e.cta_column={}),e.cta_column.btn_color=i.dataset.ctaColor,l.querySelectorAll("[data-cta-color]").forEach(S=>{S.classList.toggle("nb-color-inactive",S.dataset.ctaColor!==i.dataset.ctaColor)})})});const u=l.querySelector(".nb-columns-list");g(),l.querySelector(".nb-add-col-btn").onclick=()=>{if(!((e.columns||[]).length>=3)&&(e.columns=e.columns||[],e.columns.push({badge:"Nueva columna",badge_color:"blue",items:[{label:"Nuevo item",desc:"",href:"#"}]}),g(),e.columns.length>=3)){const i=l.querySelector(".nb-add-col-btn");i.disabled=!0,i.style.opacity="0.4",i.style.cursor="not-allowed"}}}else{l.innerHTML=`
                    <div class="nb-link-card-header">
                        <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                        ${y}
            ...
            `,s.appendChild(k)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new ee;let t=document.getElementById("navbar-id")?.value||"",a=document.getElementById("navbar-name")?.value||"",s=document.getElementById("navbar-load-url")?.value||"",k=document.getElementById("navbar-store-url")?.value||"";const N=document.getElementById("navbar-is-active")?.value==="1";let C=!!t;const x=ae();if(se(x),x.on("load",()=>{F(x),W(),K(),X(x),Y(x),Q(x),Z(x),be(x),setTimeout(()=>{x.runCommand("sw-visibility"),x.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),C&&s)try{await r.loadPageContent(x,s),H("Navbar cargado correctamente","success")}catch{H("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const d=document.getElementById("save-button");d.disabled=!0,d.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!C&&!a?ne({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async p=>{if(!p?.trim()){H("El nombre es obligatorio","error"),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await w(p)}catch(h){H(h.message,"error")}finally{d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await w(a),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(p){H(p.message,"error"),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function w(d){const p=C?"PUT":"POST",h=r.getEditorContent(x);(!h.js_content||h.js_content.trim()==="")&&(h.js_content=re);const E=await r.savePage(x,{...h,name:d,is_active:N},k,p);if(E.success){if(r.markAsClean(),H(E.message,"success"),!C&&E.navbar){t=E.navbar.id,a=E.navbar.name,C=!0;const o=document.getElementById("navbar-id");o&&(o.value=t);const z=document.getElementById("navbar-name");z&&(z.value=a);const O=document.querySelector('meta[name="app-url"]'),b=O?O.content:"";k=k.endsWith("/navbars")?`${k}/${t}`:`${k.replace(/\/navbars\/?$/,"")}/navbars/${t}`;const $=document.getElementById("navbar-store-url");$&&($.value=k),s=`${k}/load`;const A=document.getElementById("navbar-load-url");A&&(A.value=s);const q=document.getElementById("editor-title");q&&(q.textContent=`Editando Navbar: ${a}`);const I=`/navbars/edit/${t}/edit`,j=b?`${b}${I}`:I;window.history.replaceState({path:j},"",j)}else if(d){a=d;const o=document.getElementById("navbar-name");o&&(o.value=a);const z=document.getElementById("editor-title");z&&(z.textContent=`Editando Navbar: ${a}`)}}}});function be(r){r.Commands.add("canvas-clear",{run:t=>{te({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{t.DomComponents.clear(),t.CssComposer.clear()}})}})}function H(r,t="info"){typeof window.showNotification=="function"&&window.showNotification(r,t)}
