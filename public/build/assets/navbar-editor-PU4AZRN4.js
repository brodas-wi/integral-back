import{i as Q,I as W,t as Z,d as ee,f as ne,e as te,s as oe,g as ae,c as re,E as ie,j as le,h as se}from"./editor-commands-D6GyM6G7.js";import{o as ce}from"./media-picker-C92Q4am1.js";import"./_commonjsHelpers-CqkleIqs.js";function de(){return Q()}const be="/bancaintegral",me=`(function(){
...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
        ...
        </svg>`,activate:!0,content:{type:o,attributes:{"data-gjs-type":o}}}),fe(r,o),ge(r)}function fe(r,o){r.on("storage:end:load",()=>{setTimeout(()=>X(r,o),800)}),r.on("component:mount",i=>{const a=i.getEl();a?.getAttribute?.("data-gjs-type")===o&&(i.set("type",o),setTimeout(()=>K(r,a),400))}),r.on("canvas:render",()=>{setTimeout(()=>X(r,o),600)})}function K(r,o){if(o?.isConnected)try{const i=r.Canvas.getFrameEl()?.contentDocument;if(!i)return;o.__nbInit&&delete o.__nbInit;const a=i.createElement("script");a.textContent=`(function(){
            function initNavbar(root){
                if(!root||root.__nbInit)return;
                root.__nbInit=true;
                var id=root.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");
        ...
        })();`,i.head.appendChild(a),a.remove()}catch(i){console.warn("[Navbar] Error inyectando script en canvas:",i)}}function X(r,o){r.getWrapper().find(`[data-gjs-type="${o}"]`).forEach(i=>{i.set("type",o);const a=i.getEl();a?.isConnected&&K(r,a)})}function ge(r){r.on("load",()=>{const o=r.Canvas.getFrameEl();if(!o)return;const i=o.contentDocument,a=i?.head;if(a&&(i.documentElement?.setAttribute("data-gjs-canvas","true"),!a.querySelector("#navbar-component-css"))){const h=document.createElement("style");h.id="navbar-component-css",h.textContent=`
                [data-gjs-type="navbar-component"] {
                    position: relative !important;
                    top: auto !important;
                }
                body { padding-top: 0 !important; }
            `,a.appendChild(h)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new ie;let o=document.getElementById("navbar-id")?.value||"",i=document.getElementById("navbar-name")?.value||"",a=document.getElementById("navbar-load-url")?.value||"",h=document.getElementById("navbar-store-url")?.value||"";const C=document.getElementById("navbar-is-active")?.value==="1";let B=!!o;const w=de();if(ue(w),w.on("load",()=>{Z(w),ee(),ne(),te(w),oe(w),ae(w),re(w),ve(w),he(w),setTimeout(()=>{w.runCommand("sw-visibility"),w.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),B&&a)try{await r.loadPageContent(w,a),P("Navbar cargado correctamente","success")}catch{P("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const b=document.getElementById("save-button");b.disabled=!0,b.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!B&&!i?le({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async f=>{if(!f?.trim()){P("El nombre es obligatorio","error"),b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await y(f)}catch(k){P(k.message,"error")}finally{b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await y(i),b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(f){P(f.message,"error"),b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function y(b){const f=B?"PUT":"POST",k=r.getEditorContent(w);(!k.js_content||k.js_content.trim()==="")&&(k.js_content=me);const L=await r.savePage(w,{...k,name:b,is_active:C},h,f);if(L.success){if(r.markAsClean(),P(L.message,"success"),!B&&L.navbar){o=L.navbar.id,i=L.navbar.name,B=!0;const n=document.getElementById("navbar-id");n&&(n.value=o);const N=document.getElementById("navbar-name");N&&(N.value=i);const R=document.querySelector('meta[name="app-url"]'),J=R?R.content:"";h=h.endsWith("/navbars")?`${h}/${o}`:`${h.replace(/\/navbars\/?$/,"")}/navbars/${o}`;const U=document.getElementById("navbar-store-url");U&&(U.value=h),a=`${h}/load`;const m=document.getElementById("navbar-load-url");m&&(m.value=a);const E=document.getElementById("editor-title");E&&(E.textContent=`Editando Navbar: ${i}`);const _=`/navbars/edit/${o}/edit`,A=J?`${J}${_}`:_;window.history.replaceState({path:A},"",A)}else if(b){i=b;const n=document.getElementById("navbar-name");n&&(n.value=i);const N=document.getElementById("editor-title");N&&(N.textContent=`Editando Navbar: ${i}`)}}}});function ve(r){r.Commands.add("canvas-clear",{run:o=>{se({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{o.DomComponents.clear(),o.CssComposer.clear()}})}})}function he(r){const o=["set-device-desktop","set-device-tablet","set-device-mobile"];function i(a){o.forEach(h=>{r.Panels.getButton("devices-c",h)?.set("active",h===a)})}r.Commands.add("set-device-desktop",{run(a){a.setDevice("desktop"),i("set-device-desktop")}}),r.Commands.add("set-device-tablet",{run(a){a.setDevice("tablet"),i("set-device-tablet")}}),r.Commands.add("set-device-mobile",{run(a){a.setDevice("mobile"),i("set-device-mobile")}}),r.on("device:select",a=>{const h=a.get?a.get("id"):a.id,C={desktop:"set-device-desktop",tablet:"set-device-tablet",mobile:"set-device-mobile"};C[h]&&i(C[h])}),setTimeout(()=>{i("set-device-desktop")},200)}function P(r,o="info"){typeof window.showNotification=="function"&&window.showNotification(r,o)}
