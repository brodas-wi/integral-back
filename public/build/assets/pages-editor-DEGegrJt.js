/* empty css                   *//* empty css                    *//* empty css                          *//* empty css                           */import{o as Fe,M as Re,j as Ue,E as Ve,i as We,t as Ye,d as Ge,f as Xe,e as Je,s as Ze,g as Ke,c as Qe,b as et,a as tt,h as it}from"./editor-commands-Bq37dEX4.js";import{a as N}from"./url-DaqOO3yL.js";import"./_commonjsHelpers-CqkleIqs.js";class at{constructor(){this.pageId=this.getElementValue("page-id"),this.pageSlug=this.getElementValue("page-slug"),this.pageTitle=this.getPageTitle(),this.loadUrl=this.getElementValue("page-load-url"),this.storeUrl=this.getElementValue("page-store-url"),this.isPublished=this.getElementValue("page-is-published")==="1",this.isEditMode=!!this.pageId,this.appUrl=(document.querySelector('meta[name="app-url"]')?.content??"").replace(/\/$/,"")}getElementValue(e,t=""){const r=document.getElementById(e);return r?r.value.trim():t}getPageTitle(){const e=document.getElementById("editor-title");if(!e)return"";const r=e.textContent.match(/(?:Editando:|Nueva Página)\s*(.+)?/);return r&&r[1]?r[1].trim():""}updatePageInfo(e){if(e.page)if(this.pageId=e.page.id,this.pageSlug=e.page.slug,this.pageTitle=e.page.title,this.isEditMode=!0,document.getElementById("page-id").value=this.pageId,document.getElementById("page-slug").value=this.pageSlug,e.page.update_url?this.storeUrl=e.page.update_url:this.storeUrl=`${this.appUrl}/pages/${this.pageSlug}`,document.getElementById("page-store-url").value=this.storeUrl,e.page.load_url?this.loadUrl=e.page.load_url:this.loadUrl=`${this.appUrl}/pages/${this.pageSlug}/load`,document.getElementById("page-load-url").value=this.loadUrl,e.page.edit_url)window.history.replaceState(null,"",e.page.edit_url);else try{const t=new URL(window.location.href);t.pathname=t.pathname.replace(/\/pages\/create\/?$/,`/pages/${this.pageSlug}/edit`),window.history.replaceState(null,"",t.toString())}catch(t){console.error("Error updating browser URL:",t)}}updateTitle(e){this.pageTitle=e;const t=document.getElementById("editor-title");t&&(t.textContent=`Editando: ${e}`),document.title=`Editar: ${e} - Editor`}getHttpMethod(){return this.isEditMode?"PUT":"POST"}needsTitle(){return!this.isEditMode}}class rt{constructor(e){this.editorService=e,this.setupBackButton(),this.setupBeforeUnload()}setupBackButton(){const e=document.querySelector("#editor-navbar a[href]");e&&e.addEventListener("click",t=>{this.editorService.shouldPreventUnload()&&(t.preventDefault(),this.showUnsavedChangesModal(()=>{this.editorService.startNavigation(),window.location.href=e.href}))})}setupBeforeUnload(){window.addEventListener("beforeunload",e=>{if(this.editorService.shouldPreventUnload())return e.preventDefault(),e.returnValue="Tienes cambios sin guardar. ¿Estás seguro de que quieres salir?",e.returnValue})}showUnsavedChangesModal(e){const t=document.createElement("div");t.style.cssText=`
        ...
        `;const l=document.createElement("div");l.style.cssText=`
            ...
            margin-top: 1.5rem;
        `;const s=document.createElement("button");s.textContent="Cancelar",s.style.cssText=`
            padding: 0.5rem 1rem;
...
</svg>`,_='<div class="col-cell"></div>',V=`
<style>
...
</style>`,bt=[{id:"columns-1",label:"1 Columna",category:"Columnas",media:st,content:`
...
${V}`},{id:"columns-4",label:"4 Columnas",category:"Columnas",media:gt,content:`
<div class="col-section">
...
${V}`}],ut=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,kt=[{id:"about-section",label:"Nuestra Historia",category:"Contenido",media:ut,content:`
...
${xe}`}],Me=function(){(function(){function i(t){if(!(!t||t.__pcInit)){t.__pcInit=!0;var r=t.querySelector(".pc-carousel-wrap");if(r){var a=!1,l=0,s=0,d=!1;r.querySelectorAll("img").forEach(function(p){p.setAttribute("draggable","false")}),r.addEventListener("mousedown",function(p){p.button===0&&(a=!0,d=!1,l=p.clientX,s=r.scrollLeft,r.style.cursor="grabbing",p.preventDefault())}),document.addEventListener("mousemove",function(p){if(a){var u=l-p.clientX;Math.abs(u)>3&&(d=!0);var b=s+u,n=r.scrollWidth-r.clientWidth;r.scrollLeft=Math.max(0,Math.min(b,n))}}),document.addEventListener("mouseup",function(p){a&&(a=!1,r.style.cursor="grab",d&&p.stopPropagation())}),r.addEventListener("click",function(p){d&&(p.preventDefault(),p.stopPropagation(),d=!1)},!0);var h=0,g=0;r.addEventListener("touchstart",function(p){h=p.touches[0].clientX,g=r.scrollLeft},{passive:!0}),r.addEventListener("touchmove",function(p){var u=h-p.touches[0].clientX,b=g+u,n=r.scrollWidth-r.clientWidth;r.scrollLeft=Math.max(0,Math.min(b,n))},{passive:!0})}}}function e(){document.querySelectorAll(".pc-section").forEach(function(t){delete t.__pcInit,i(t)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e):e()})()},Bt=`(${Me.toString()})();`,Et=`
...
@media(max-width:480px){.pc-card{flex:0 0 80vw;}}`;function $t(i){const e=i.img||N("images/placeholder.svg"),t=i.title||"TÍTULO DEL PRODUCTO",r=i.desc||"Descripción breve del producto financiero.",a=i.href||"#",l=i.btn_label||"Solicitar";return`<div class="pc-card"><div class="pc-card-img-wrap"><img src="${e}" alt="${t}" class="pc-card-img"></div><div class="pc-card-body"><h3 class="pc-card-title">${t}</h3><p class="pc-card-desc">${r}</p></div><a href="${a}" class="pc-btn">${l}</a></div>`}function qe(i){const e=i.heading||"Créditos",t=i.subheading||"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",r=i.more_href||"#",a=i.more_label||"Ver más",s=(i.cards||[]).map($t).join("");return`<section class="pc-section"><style>${Et}</style><div style="text-align:center;margin-bottom:2rem;"><h2 class="pc-section-heading">${e}</h2><p class="pc-section-subheading">${t}</p></div><div class="pc-carousel-wrap"><div class="pc-track">${s}</div></div><div class="pc-more-wrap"><a href="${r}" class="pc-more-btn">${a}</a></div></section>`}const q={heading:"Créditos",subheading:"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",more_href:"#",more_label:"Ver más",cards:[{img:"",title:"CREDINVIERTE",desc:"Adquiere activos fijos",href:"#",btn_label:"Solicitar"},{img:"",title:"SOLUCIONES INTEGRALES",desc:"Financiamiento PYME",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDI-CONFIAMOS",desc:"Rápido y sin fiador",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDILÍNEA",desc:"Línea rotativa",href:"#",btn_label:"Solicitar"}]};function Ct(i,e){const t=document.getElementById("pc-config-modal");if(t&&t.remove(),!document.getElementById("pc-modal-styles")){const g=document.createElement("style");g.id="pc-modal-styles",g.textContent=`
        ...
        `,document.head.appendChild(g)}const r=(()=>{try{return JSON.parse(e.getAttributes()["data-product-cards-config"]||"{}")}catch{return{}}})(),a={heading:r.heading??q.heading,subheading:r.subheading??q.subheading,autoplay:r.autoplay??q.autoplay,more_href:r.more_href??q.more_href,more_label:r.more_label??q.more_label,cards:JSON.parse(JSON.stringify(r.cards??q.cards))},l=document.createElement("div");l.id="pc-config-modal",l.className="pc-overlay";const s=document.createElement("div");s.className="pc-modal",s.innerHTML=`
        ...
        <div class="pc-modal-footer">
            ...
            <button id="pc-modal-save" class="pc-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,l.appendChild(s),document.body.appendChild(l),s.querySelectorAll(".pc-tab-btn").forEach(g=>{g.addEventListener("click",()=>{s.querySelectorAll(".pc-tab-btn").forEach(p=>p.classList.remove("active")),s.querySelectorAll(".pc-tab-panel").forEach(p=>p.classList.remove("active")),g.classList.add("active"),s.querySelector(`#pc-panel-${g.dataset.tab}`).classList.add("active")})});function d(){const g=s.querySelector("#pc-cards-list");g.innerHTML="",a.cards.forEach((p,u)=>{const b=document.createElement("div");b.className="pc-card-config";const n=p.img?`<img class="pc-img-preview" src="${p.img}" alt="">`:'<div class="pc-img-placeholder"><i class="ri-image-line"></i></div>';b.innerHTML=`
                ...
                </div>`,b.querySelectorAll("[data-field]").forEach(f=>{f.addEventListener("input",()=>{if(p[f.dataset.field]=f.value,f.dataset.field==="img"){const w=b.querySelector(`#pc-img-wrap-${u}`);w.innerHTML=f.value?`<img class="pc-img-preview" src="${f.value}" alt="">`:'<div class="pc-img-placeholder"><i class="ri-image-line"></i></div>'}})}),b.querySelector(".pc-pick-img").addEventListener("click",()=>{Fe({type:"image",title:"Seleccionar imagen de tarjeta",onSelect:f=>{p.img=f,b.querySelector("[data-field='img']").value=f,b.querySelector(`#pc-img-wrap-${u}`).innerHTML=`<img class="pc-img-preview" src="${f}" alt="">`}})}),b.querySelector(".pc-remove-card").addEventListener("click",()=>{a.cards.splice(u,1),d()}),g.appendChild(b)})}d(),s.querySelector("#pc-add-card").addEventListener("click",()=>{a.cards.push({img:"",title:"NUEVO PRODUCTO",desc:"Descripción del producto.",href:"#",btn_label:"Solicitar"}),d(),s.querySelector("#pc-cards-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})});const h=()=>l.remove();s.querySelector("#pc-modal-close").addEventListener("click",h),s.querySelector("#pc-modal-cancel").addEventListener("click",h),l.addEventListener("click",g=>{g.target===l&&h()}),s.querySelector("#pc-modal-save").addEventListener("click",()=>{a.heading=s.querySelector("#pc-heading").value.trim()||q.heading,a.subheading=s.querySelector("#pc-subheading").value.trim()||q.subheading,a.more_label=s.querySelector("#pc-more-label").value.trim()||q.more_label,a.more_href=s.querySelector("#pc-more-href").value.trim()||"#",e.addAttributes({"data-product-cards-config":JSON.stringify(a)}),e.components(qe(a)),setTimeout(()=>de(i),300),h()})}function de(i){try{const e=i.Canvas.getFrameEl()?.contentDocument;if(!e)return;const t=e.getElementById("pc-runtime-script");t&&t.remove(),e.querySelectorAll(".pc-section").forEach(a=>{delete a.__pcInit});const r=e.createElement("script");r.id="pc-runtime-script",r.textContent=Bt,e.head.appendChild(r)}catch(e){console.warn("[ProductCards] Error reiniciando carrusel:",e)}}const St=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`;function Lt(i){const e="product-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección de Productos",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!0,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-product-cards-config":JSON.stringify(q)},components:qe(q),script:Me,"script-props":["data-product-cards-config"],traits:[{type:"button",label:"Productos",text:"Administrar Sección",full:!0,command:"open-product-cards-config"}],toolbar:[]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),i.Commands.add("open-product-cards-config",{run(t){const r=t.getSelected();r&&Ct(t,r)}}),i.BlockManager.add("product-cards-block",{label:"Sección de productos",category:"Productos y Servicios",media:St,activate:!0,content:{type:e,attributes:{"data-gjs-type":e}}}),i.on("component:mount",t=>{t.getEl()?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>de(i),400))}),i.on("component:selected",t=>{const r=t.getEl();if(!r)return;const a=r.closest(`[data-gjs-type="${e}"]`);if(a&&!r.hasAttribute("data-gjs-type")){const s=i.getWrapper().find(`[data-gjs-type="${e}"]`).find(d=>d.getEl()===a);s&&setTimeout(()=>i.select(s),0)}}),i.on("canvas:render",()=>{setTimeout(()=>de(i),600)}),i.on("storage:end:load",()=>{setTimeout(()=>de(i),800)})}const jt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,_t=[{id:"dual-card-section",label:"Sección dos tarjetas",category:"Productos y Servicios",media:jt,content:`
...
${pe}`},{id:"dual-card-dark",label:"Tarjeta azul oscuro",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,content:`${ve}${pe}`}],zt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,Tt=[{id:"cta-section",label:"Llamada a la acción",category:"Llamadas a la acción",media:zt,content:`
...
${It}`}],At=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,qt=[{id:"icon-grid-hero",label:"Sección de características",category:"Productos y Servicios",media:At,content:`
...
${Mt}`},{id:"icon-card",label:"Tarjeta con icono",category:"Productos y Servicios",media:Dt,content:`${X}`}],we=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
${me}`},{id:"product-link-card-orange",label:"Tarjeta producto naranja",category:"Productos y Servicios",media:we,media:Nt,content:`${K}${me}`},{id:"product-link-card-blue",label:"Tarjeta producto azul",category:"Productos y Servicios",media:Pt,content:`${Q}${me}`}],ke=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Ht=[{id:"icon-links-strip",label:"Tira de iconos con enlace",category:"Productos y Servicios",media:ke,content:`
...
${Be}`},{id:"icon-link-item",label:"Ítem icono con enlace",category:"Productos y Servicios",media:ke,content:`${ee}${Be}`}],Ft=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Ut=[{id:"promo-cta",label:"Promo con CTA y precio",category:"Llamadas a la acción",media:Ft,content:`
...
${Rt}`}],Vt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<style>${ue}</style>`,Gt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,Xt=[{id:"tabs-cards",label:"Sección con tabs",category:"Contenido",media:Vt,content:{type:"tabs-cards-component"}},{id:"tab-card-single",label:"Tarjeta de tab",category:"Contenido",media:Gt,content:`${D()}`}];function Jt(i){const e="tabs-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección con tabs",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!0,removable:!0,attributes:{"data-gjs-type":e,"data-tab-count":"5"},components:Yt,script:Wt(),traits:[{type:"number",name:"data-tab-count",label:"Número de tabs (2-10)",min:2,max:10,changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",(t,r)=>{const a=parseInt(r["data-tab-count"]);isNaN(a)||this.updateTabCount(a)})},updateTabCount(t){const r=Math.min(10,Math.max(2,t)),a=l=>{const s=Array.from({length:l},()=>'<button class="tabs-btn inline-block px-6 py-2 rounded-full border-2 border-[#003B71] bg-transparent text-[#003B71] text-sm font-medium">Lorem Ipsum</button>').join(`
`),d=Array.from({length:l},(h,g)=>`<div class="tabs-panel${g===0?" active":""} grid-cols-3 gap-5">
                            ${D()}
                            ${D()}
                            ${D()}
                            ${D()}
...
`);return`
                    <div class="max-w-6xl mx-auto px-6 flex flex-col gap-8 py-16">
                        ...
                        <nav class="flex flex-wrap justify-center gap-2">${s}</nav>
                        <div class="tabs-body">${d}</div>
                    </div>
                    <style>${ue}</style>`};this.components(a(r)),setTimeout(()=>{const l=this.get("script"),s=this.getEl();l&&typeof l=="function"&&s&&l.call(s)},200)}}}),Zt(i,e),Kt(i,e)}function Zt(i,e){i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},300))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},300)}}),i.on("storage:end:load",()=>{setTimeout(()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})},800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Kt(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#tabs-section-styles")){const a=document.createElement("style");a.id="tabs-section-styles",a.textContent=ue,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
            ...
            `,r.appendChild(a)}}})}const Qt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</div>`},ai=i=>{const e=i==="light";return`
...
${ci}`,di=[{id:"stats-strip-left",label:"Franja estadísticas - izquierda",category:"Heroes",media:ni,content:$e(!0)},{id:"stats-strip-right",label:"Franja estadísticas - derecha",category:"Heroes",media:si,content:$e(!1)}],gi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,hi=[{id:"service-cards-section",label:"Sección de servicios",category:"Productos y Servicios",media:gi,content:`
...
${Ce}`},{id:"service-card",label:"Tarjeta de servicio",category:"Productos y Servicios",media:fi,content:`${se}${Ce}`}],pi=`<svg viewBox="0 0 32 32" width="32" height="32">
...
</style>`,xi=[{id:"contact-info-row",label:"Información de contacto (fila)",category:"Contacto",media:bi,content:`
...
${ce}`},{id:"contact-info-left",label:"Info de contacto (izquierda)",category:"Contacto",media:pi,content:`
<section class="ci-section">
    <div class="ci-list">
        ${J("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
        ${J("ri-mail-fill","Correo","servicio.cliente@bancointegral.com","mailto:servicio.cliente@bancointegral.com")}
...
${ce}`},{id:"contact-info-right",label:"Info de contacto (derecha)",category:"Contacto",media:mi,content:`
<section class="ci-section">
...
${ce}`}],yi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,wi=[{id:"rich-list",label:"Lista con título y descripción",category:"Contenido",media:yi,content:`
...
${Se}`},{id:"rich-list-item",label:"Ítem de lista con descripción",category:"Contenido",media:vi,content:`
<section class="rl-section">
    <div class="rl-list">
        ${ie}
    </div>
</section>
${Se}`}],ki=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#ffffff" rx="2" stroke="#e5e7eb" stroke-width="1"/>
...
`,Ei=[{id:"contact-form",label:"Formulario de contacto",category:"Formularios",media:ki,content:`
...
<\/script>`}],$i=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,ge={blue:{headerBg:"bg-[#003B71]",headerText:"text-white",subheaderBg:"bg-[#e8f0f8]",subheaderText:"text-[#003B71]",borderColor:"#003B71",rowEvenBg:"bg-[#f4f7fb]",rowOddBg:"bg-white",rowText:"text-[#003B71]",labelBg:"bg-[#e8f0f8]",labelText:"text-[#003B71]"},orange:{headerBg:"bg-[#E97300]",headerText:"text-white",subheaderBg:"bg-[#fef3e8]",subheaderText:"text-[#E97300]",borderColor:"#E97300",rowEvenBg:"bg-[#fff8f2]",rowOddBg:"bg-white",rowText:"text-[#003B71]",labelBg:"bg-[#fef3e8]",labelText:"text-[#E97300]"}};function Ne(i,e){const t=ge[e]||ge.blue;let r='<table class="w-full border-collapse font-[Poppins,sans-serif] table-fixed">';i.title?(r+=`<thead><tr>
            <th colspan="${i.cols}" class="p-3 align-middle text-center text-base font-bold ${t.headerBg} ${t.headerText}">
                ${i.title}
            </th>
        </tr>`,i.headers?.length&&(r+="<tr>",i.headers.forEach((s,d)=>{const h=d<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";r+=`<th class="p-3 align-middle text-sm font-semibold ${t.subheaderBg} ${t.subheaderText} ${h} border-b border-[${t.borderColor}] text-${s.align||"center"}">${s.text||""}</th>`}),r+="</tr>"),r+="</thead>"):i.headers?.length&&(r+="<thead><tr>",i.headers.forEach((s,d)=>{const h=d<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";r+=`<th class="p-3 align-middle text-sm font-semibold ${t.headerBg} ${t.headerText} ${h} border-b border-[${t.borderColor}] text-${s.align||"center"}">${s.text||""}</th>`}),r+="</tr></thead>"),r+="<tbody>";const a=i.rows.length,l={};return i.rows.forEach((s,d)=>{r+="<tr>";let h=0;s.forEach(g=>{for(;l[`${d}-${h}`];)h++;const p=g.colspan||1,u=g.rowspan||1;for(let x=d;x<d+u;x++)for(let L=h;L<h+p;L++)(x!==d||L!==h)&&(l[`${x}-${L}`]=!0);const b=p>1?`colspan="${p}"`:"",n=u>1?`rowspan="${u}"`:"",f=g.isHeader?t.labelBg:d%2===0?t.rowEvenBg:t.rowOddBg,w=g.isHeader?"font-semibold":"font-normal",T=g.isHeader?t.labelText:t.rowText,v=`text-${g.align||"center"}`,$=d+u>=a,C=h+p>=i.cols?"":`border-r border-[${t.borderColor}]`,B=$?"":`border-b border-[${t.borderColor}]`,j=`${C} ${B} p-3 align-middle text-sm ${f} ${w} ${T} ${v}`;g.image?r+=`<td ${b} ${n} class="${j}">
                ...
                </td>`:r+=`<td ${b} ${n} class="${j}">${g.text||""}</td>`,h+=p}),r+="</tr>"}),r+="</tbody></table>",r}function Z(i,e){return{title:"Título de la tabla",cols:i,headers:Array.from({length:i},(t,r)=>({text:`Columna ${r+1}`,align:"center"})),rows:Array.from({length:e},()=>Array.from({length:i},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})))}}function Pe(i,e){return`<div class="w-full overflow-x-auto rounded-2xl border-2 border-[${(ge[e]||ge.blue).borderColor}]">${i}</div>`}function Le(i,e){const t={};return i.forEach((r,a)=>{let l=0;r.forEach(s=>{for(;t[`${a}-${l}`];)l++;const d=Math.min(s.colspan||1,e-l),h=s.rowspan||1;for(let g=a;g<a+h;g++)for(let p=l;p<l+d;p++)(g!==a||p!==l)&&(t[`${g}-${p}`]=`${a}-${l}`);l+=d})}),t}const Ci=`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function Si(){if(document.getElementById("tam-img-modal"))return;const i=document.createElement("div");i.id="tam-img-modal",i.innerHTML=`
        ...
        </div>`,document.body.appendChild(i);let e=null,t=null;async function r(d=""){const h=document.getElementById("tam-img-grid");h.innerHTML='<div class="tam-img-loading"><div class="tam-img-spinner"></div><span>Cargando...</span></div>';try{const g=new URLSearchParams({type:"image",per_page:50});d&&g.append("search",d);const p=document.querySelector('meta[name="media-api-url"]')?.content??"/media/api",n=(await(await fetch(`${p}?${g}`,{headers:{"X-Requested-With":"XMLHttpRequest",Accept:"application/json"}})).json()).items||[];if(!n.length){h.innerHTML='<div class="tam-img-loading"><i class="ri-image-line" style="font-size:2rem;"></i><span>No se encontraron imágenes</span></div>';return}h.innerHTML="",n.forEach(f=>{const w=document.createElement("div");w.className="tam-img-card",w.innerHTML=`<img src="${f.url}" alt="${f.filename}"><p title="${f.filename}">${f.filename}</p>`,w.addEventListener("click",()=>{h.querySelectorAll(".tam-img-card").forEach(T=>T.classList.remove("selected")),w.classList.add("selected"),e=f.url,document.getElementById("tam-img-selected-info").textContent=`Seleccionada: ${f.filename}`,document.getElementById("tam-img-confirm").disabled=!1}),h.appendChild(w)})}catch{h.innerHTML='<div class="tam-img-loading"><i class="ri-error-warning-line" style="font-size:2rem;color:#f87171;"></i><span style="color:#dc2626;">Error al cargar imágenes</span></div>'}}function a(d){t=d,e=null,document.getElementById("tam-img-selected-info").textContent="Ninguna imagen seleccionada",document.getElementById("tam-img-confirm").disabled=!0,document.getElementById("tam-img-search-input").value="",i.classList.add("open"),r()}function l(){i.classList.remove("open"),e=null,t=null}document.getElementById("tam-img-close").addEventListener("click",l),document.getElementById("tam-img-cancel").addEventListener("click",l),document.getElementById("tam-img-confirm").addEventListener("click",()=>{e&&t&&(t(e),l())});let s;document.getElementById("tam-img-search-input").addEventListener("input",d=>{clearTimeout(s),s=setTimeout(()=>r(d.target.value),300)}),i.addEventListener("click",d=>{d.target===i&&l()}),window.__openTableImagePicker=a}function Li(i,e){if(document.getElementById("table-admin-modal"))return;const t=document.createElement("style");t.id="table-admin-modal-styles",t.textContent=Ci,document.head.appendChild(t),Si();const r=document.createElement("div");r.id="table-admin-modal",r.innerHTML=`
        <div class="tam-container">
        ...
        </div>`,document.body.appendChild(r);let a=null,l=null;function s(b){a=b;const n=b.get("tableData");l=n?JSON.parse(JSON.stringify(n)):Z(3,3);const f=l.cols||3;l.rows=l.rows.map((w,T)=>Array.from({length:f},($,z)=>w[z]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})),document.getElementById("tam-title").value=l.title||"",document.getElementById("tam-theme").value=b.get("tableTheme")||"blue",document.getElementById("tam-cols").value=l.cols||3,document.getElementById("tam-rows").value=l.rows.length||3,p(),u(),r.classList.add("open"),document.body.style.overflow="hidden"}function d(){r.classList.remove("open"),document.body.style.overflow="",a=null}function h(){l.title=document.getElementById("tam-title").value.trim(),l.cols=parseInt(document.getElementById("tam-cols").value)||3,l.headers=Array.from(document.querySelectorAll(".tam-header-input")).map(n=>({text:n.value,align:n.closest("th")?.querySelector(".tam-align-select")?.value||"center"})),document.querySelectorAll("#tam-tbody td.tam-cell:not(.is-spanned)").forEach(n=>{const f=parseInt(n.dataset.row),w=parseInt(n.dataset.col);l.rows[f]?.[w]&&(l.rows[f][w].text=n.querySelector(".tam-cell-input")?.value||"",l.rows[f][w].align=n.querySelector(".tam-align-select")?.value||"center",l.rows[f][w].isHeader=n.dataset.isheader==="1",l.rows[f][w].image=n.dataset.image||null)});const b=Le(l.rows,l.cols);l.rows=l.rows.map((n,f)=>n.filter((w,T)=>!b[`${f}-${T}`]))}function g(){if(r.querySelector("#tam-rebuild-notice"))return;const n=document.createElement("div");n.id="tam-rebuild-notice",n.style.cssText="background:#fef9c3;border:1.5px solid #ca8a04;border-radius:0.5rem;padding:0.5rem 1rem;font-size:0.8rem;color:#92400e;display:flex;align-items:center;gap:0.5rem;margin:0 1.5rem 0;flex-shrink:0;",n.innerHTML='<i class="ri-error-warning-line"></i> Has modificado el colspan/rowspan. Haz clic en <strong style="margin:0 4px;">Reconstruir</strong> para aplicar los cambios de fusión.',r.querySelector(".tam-toolbar").after(n)}function p(){r.querySelector("#tam-rebuild-notice")?.remove()}function u(){const b=document.getElementById("tam-thead"),n=document.getElementById("tam-tbody"),f=l.cols,w=l.rows.length,T=Le(l.rows,f);b.innerHTML=`<tr>${l.headers.map((v,$)=>`
            <th>
                ...
                <select class="tam-align-select" style="margin-top:4px;width:100%;font-size:0.7rem;padding:2px;">
            ...
            </th>`).join("")}</tr>`,n.innerHTML=l.rows.map((v,$)=>`<tr>${Array.from({length:f},(C,B)=>{const j=T[`${$}-${B}`];if(j)return`<td class="tam-cell is-spanned" data-row="${$}" data-col="${B}">
                        <div class="tam-spanned-label">Combinada con [${j}]</div>
                    </td>`;const x=v[B]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null},L=x.colspan||1,P=x.rowspan||1,I=L>1||P>1;return`<td class="tam-cell ${x.isHeader?"is-header-cell":""} ${x.image?"has-image":""} ${I?"has-span":""}"
                    data-row="${$}" data-col="${B}"
                    ...
                    <div class="tam-cell-actions">
                        ...
                        <div class="tam-cell-span-group">
                            ...
                            <input type="number" class="tam-cell-span-input" min="1" max="${f-B}"
                            ...
                            <input type="number" class="tam-cell-span-input" min="1" max="${w-$}"
                                value="${P}" data-action="rowspan" data-row="${$}" data-col="${B}">
                        </div>
                    </div>
                </td>`}).join("")}</tr>`).join(""),n.querySelectorAll("input[data-action=colspan], input[data-action=rowspan]").forEach(v=>{v.addEventListener("change",()=>{const $=parseInt(v.dataset.row),z=parseInt(v.dataset.col),C=Math.max(1,parseInt(v.value)||1);l.rows[$]?.[z]&&(v.dataset.action==="colspan"?l.rows[$][z].colspan=Math.min(C,f-z):l.rows[$][z].rowspan=Math.min(C,w-$),g())})}),n.querySelectorAll("button[data-action]").forEach(v=>{v.addEventListener("click",$=>{$.preventDefault(),$.stopPropagation();const z=v.dataset.action,C=parseInt(v.dataset.row),B=parseInt(v.dataset.col);if(!(isNaN(C)||isNaN(B)||!l.rows[C]?.[B])){if(z==="header"){l.rows[C][B].isHeader=!l.rows[C][B].isHeader;const j=n.querySelector(`td[data-row="${C}"][data-col="${B}"]`);j&&(j.dataset.isheader=l.rows[C][B].isHeader?"1":"0",j.classList.toggle("is-header-cell",l.rows[C][B].isHeader)),v.classList.toggle("active",l.rows[C][B].isHeader),v.textContent=l.rows[C][B].isHeader?"✓ Etiqueta":"Etiqueta";return}if(z==="image"){window.__openTableImagePicker&&window.__openTableImagePicker(j=>{l.rows[C][B].image=j;const x=n.querySelector(`td[data-row="${C}"][data-col="${B}"]`);if(x){x.dataset.image=j,x.classList.add("has-image");let L=x.querySelector(".tam-cell-img-preview");L||(L=document.createElement("img"),L.className="tam-cell-img-preview",x.insertBefore(L,x.firstChild)),L.src=j;const P=x.querySelector("[data-action=image]");if(P&&(P.innerHTML='<i class="ri-image-line"></i> Cambiar'),!x.querySelector("[data-action=clear-image]")){const I=document.createElement("button");I.type="button",I.className="tam-cell-btn tam-cell-btn-clear",I.dataset.action="clear-image",I.dataset.row=C,I.dataset.col=B,I.textContent="✕ Quitar",I.addEventListener("click",Y=>{Y.preventDefault(),Y.stopPropagation(),l.rows[C][B].image=null,x.dataset.image="",x.classList.remove("has-image"),L.remove(),I.remove();const H=x.querySelector("[data-action=image]");H&&(H.innerHTML='<i class="ri-image-line"></i> Imagen')}),x.querySelector(".tam-cell-actions").appendChild(I)}}});return}z==="clear-image"&&(l.rows[C][B].image=null,u())}})})}document.getElementById("tam-close").addEventListener("click",d),document.getElementById("tam-cancel").addEventListener("click",d),r.addEventListener("click",b=>{b.target===r&&d()}),document.getElementById("tam-rebuild").addEventListener("click",()=>{const b=parseInt(document.getElementById("tam-cols").value)||3,n=parseInt(document.getElementById("tam-rows").value)||3;for(p(),h();l.headers.length<b;)l.headers.push({text:`Col ${l.headers.length+1}`,align:"center"});for(l.headers=l.headers.slice(0,b),l.cols=b;l.rows.length<n;)l.rows.push(Array.from({length:b},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})));l.rows=l.rows.slice(0,n).map(f=>{for(;f.length<b;)f.push({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null});return f.slice(0,b)}),u()}),document.getElementById("tam-apply").addEventListener("click",()=>{h();const b=document.getElementById("tam-theme").value;a&&(a.set("tableData",JSON.parse(JSON.stringify(l))),a.set("tableTheme",b),a.addAttributes({"data-table-theme":b}),be(a)),d()}),window.__openTableAdminModal=s}function be(i){const e=i.get("tableData"),t=i.get("tableTheme")||"blue";e&&i.components(Pe(Ne(e,t),t))}function ji(){return function(){}}const _i=[{id:"table-blue",label:"Tabla azul",category:"Interactivos",media:$i,content:{type:"table-component",attributes:{"data-table-theme":"blue"}}},{id:"table-orange",label:"Tabla naranja",category:"Interactivos",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
            ...
            `,r.appendChild(a)}})}const Ai=`<svg viewBox="0 0 80 32" width="80" height="32">
...
<a href="#"
   ...
   class="dld-simple-${e}">
...
</svg>`,Xi=[{id:"badge-outline-blue",label:"Badge azul outline",category:"Badges",media:Wi,content:'<span class="inline-block border border-[#003B71] rounded-full px-4 py-1 text-base font-medium bg-transparent text-[#003B71]">Etiqueta</span>'},{id:"badge-outline-orange",label:"Badge naranja outline",category:"Badges",media:Yi,content:'<span class="inline-block border border-[#E97300] rounded-full px-4 py-1 text-base font-medium bg-transparent text-[#E97300]">Etiqueta</span>'},{id:"badge-outline-white",label:"Badge blanco outline",category:"Badges",media:Gi,content:'<span class="inline-block border border-white rounded-full px-4 py-1 text-base font-medium bg-transparent text-white">Etiqueta</span>'}],Ji=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    ...
    <rect x="3" y="25" width="7" height="2.5" fill="#dee2e6" rx="0.5"/>
</svg>`;function Zi(){return function(){const i=this,e="agencies-map-component",t="/api/agencies/active",r="agencies";let a=[],l=[],s={},d=null,h=[];const g=async()=>{try{p(),await b(),await w(),T(),j(),I(),Y(),u()}catch(o){console.error("Error initializing map:",o),ae("Error al cargar las agencias"),u()}};function p(){const o=i.querySelector(`.${e}-list`);o&&(o.innerHTML=`
                ...
                `);const c=i.querySelector(`.${e}-map-container`);c&&(c.style.opacity="0.5")}function u(){const o=i.querySelector(`.${e}-map-container`);o&&(o.style.transition="opacity 0.3s ease",o.style.opacity="1")}async function b(){try{const c=await(await fetch(t)).json(),m=r?c[r]:c;Array.isArray(m)?(a=m.filter(y=>y.latitude&&y.longitude&&!isNaN(y.latitude)&&!isNaN(y.longitude)),l=[...a],n()):(a=[],l=[])}catch(o){console.error("Error loading items:",o),a=[],l=[]}}function n(){const o=[...new Set(a.map(m=>m.zone).filter(Boolean))].sort(),c=[...new Set(a.map(m=>m.department).filter(Boolean))].sort();s={zone:o,department:c},setTimeout(()=>{const m=i.querySelector(`.${e}-filters-container`);m&&!m.hasChildNodes()&&(m.innerHTML=`
                    ...
                    `,f())},100)}function f(){const o=i.querySelector(`.${e}-zone-filter`),c=i.querySelector(`.${e}-department-filter`);o&&s.zone&&s.zone.forEach(m=>{const y=document.createElement("option");y.value=m,y.textContent=m,o.appendChild(y)}),c&&s.department&&s.department.forEach(m=>{const y=document.createElement("option");y.value=m,y.textContent=m,c.appendChild(y)})}async function w(){if(!document.getElementById("leaflet-css")){const o=document.createElement("link");o.id="leaflet-css",o.rel="stylesheet",o.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css",document.head.appendChild(o)}if(typeof window.L>"u"&&await new Promise((o,c)=>{const m=document.createElement("script");m.src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js",m.onload=o,m.onerror=c,document.head.appendChild(m)}),typeof window.L<"u"&&window.L.Icon&&window.L.Icon.Default){const o=window.L.icon({iconUrl:"data:image/svg+xml;base64,"+btoa(`
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 36" width="24" height="36">
                            <path fill="#f0872a" d="M12 0C7.6 0 4 3.6 4 8c0 5.4 8 16 8 16s8-10.6 8-16c0-4.4-3.6-8-8-8z"/>
                            <circle cx="12" cy="8" r="3" fill="#fff"/>
                        </svg>
                    `),iconSize:[30,45],iconAnchor:[15,45],popupAnchor:[0,-45]});window.customOrangeIcon=o}}function T(){const o=i.querySelector(`.${e}-map`);if(!o||!window.L)return;o._leaflet_id&&o._map&&(o._map.remove(),delete o._map),d=window.L.map(o).setView([13.7942,-88.8965],8),window.L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(d),o._map=d,d.whenReady(function(){v(),setTimeout(()=>{d&&d.invalidateSize&&d.invalidateSize()},300)})}function v(){$(),z(),B()}function $(){h.forEach(o=>{o.marker&&d.removeLayer(o.marker)}),h=[]}function z(){l.forEach((o,c)=>{if(o.latitude&&o.longitude){const m=C(o),y=window.L.marker([o.latitude,o.longitude],{icon:window.customOrangeIcon||new window.L.Icon.Default}).addTo(d).bindPopup(m);h.push({marker:y,item:o,index:c})}})}function C(o){let c=`<div style="min-width: 200px;"><h4 style="font-weight: 700; margin-bottom: 12px; color: #0d3f6a; font-size: 16px; border-bottom: 2px solid #f0872a; padding-bottom: 8px;">${o.name}</h4>`;if(o.address&&(c+=`<p style="margin: 8px 0 0 0; font-size: 14px; display: flex; align-items: start; gap: 8px;"><i class="ri-map-pin-line" style="color: #f0872a; font-size: 16px; margin-top: 2px;"></i><span style="color: #4b5563;">${o.address}</span></p>`),o.municipality||o.department){const m=[o.municipality,o.department].filter(Boolean).join(", ");c+=`<p style="margin: 0 0 8px 0; font-size: 13px; color: #6b7280; padding-left: 24px;">${m}</p>`}return o.schedule&&(c+=`<p style="margin: 8px 0; font-size: 14px; display: flex; align-items: start; gap: 8px;"><i class="ri-time-line" style="color: #f0872a; font-size: 16px; margin-top: 2px;"></i><span style="color: #4b5563;">${o.schedule}</span></p>`),c+="</div>",c}function B(){if(l.length>0){const o=l.filter(c=>c.latitude&&c.longitude).map(c=>[c.latitude,c.longitude]);o.length>1?d.once("moveend",function(){setTimeout(()=>{try{d&&d._loaded&&typeof d.fitBounds=="function"&&d.fitBounds(o,{padding:[50,50],maxZoom:12,animate:!1})}catch(c){console.warn("Error fitting bounds:",c)}},100)}):o.length===1&&d.setView(o[0],14)}}function j(){const o=i.querySelector(`.${e}-list`);if(!o)return;if(l.length===0){L(o);return}const c=l.map((m,y)=>x(m,y)).join("");o.innerHTML=c,P()}function x(o,c){const m=o.phones&&o.phones.length>0?o.phones.map(E=>`
                    ...
                    `).join(""):"",y=`https://www.google.com/maps/search/?api=1&query=${o.latitude},${o.longitude}`;return`
                <div class="${e}-item p-5 bg-white/10 border border-white/20 rounded-2xl cursor-pointer transition-all duration-200" data-item-index="${c}">
            ...
            `}function P(){const o=i.querySelectorAll(`.${e}-item`);o.forEach(c=>{c.addEventListener("click",m=>{if(m.target.closest(".agency-maps-btn"))return;m.preventDefault();const y=parseInt(c.dataset.itemIndex),E=l[y];if(!E)return;o.forEach(k=>{k.classList.remove("bg-white","border-secondary","selected-agency"),k.classList.add("bg-white/10","border-white/20");const A=k.querySelector(".agency-title"),re=k.querySelectorAll("i"),le=k.querySelectorAll(".agency-phone-link"),fe=k.querySelectorAll("p:not(:has(.agency-phone-link))"),he=k.querySelector(".agency-maps-btn");A&&(A.classList.remove("text-secondary"),A.classList.add("text-white")),re.forEach(U=>{U.classList.remove("text-secondary","text-gray-300","text-white"),U.classList.add("text-primary")}),le.forEach(U=>{U.classList.remove("text-secondary"),U.classList.add("text-white")}),fe.forEach(U=>{U.classList.remove("text-secondary"),U.classList.add("text-gray-200")}),he&&(he.classList.remove("bg-secondary"),he.classList.add("bg-primary"))}),c.classList.remove("bg-white/10","border-white/20"),c.classList.add("bg-white","border-secondary","selected-agency");const M=c.querySelector(".agency-title"),F=c.querySelectorAll("i"),G=c.querySelectorAll(".agency-phone-link"),R=c.querySelectorAll("p:not(:has(.agency-phone-link))"),O=c.querySelector(".agency-maps-btn");if(M&&(M.classList.remove("text-white"),M.classList.add("text-secondary")),F.forEach(k=>{k.classList.remove("text-secondary","text-gray-300","text-white"),k.classList.add("text-primary")}),G.forEach(k=>{k.classList.remove("text-white"),k.classList.add("text-secondary")}),R.forEach(k=>{k.classList.remove("text-gray-200"),k.classList.add("text-secondary")}),O&&(O.classList.remove("bg-primary"),O.classList.add("bg-secondary")),d&&E&&d._loaded)try{d.flyTo([E.latitude,E.longitude],14,{animate:!0,duration:1});const k=h.find(A=>A.item.id===E.id);k&&k.marker&&k.marker.openPopup()}catch(k){console.warn("Error updating map view:",k)}})})}function I(){const o=i.querySelector(`.${e}-search-input`),c=i.querySelector(`.${e}-zone-filter`),m=i.querySelector(`.${e}-department-filter`),y=i.querySelector(`.${e}-no-results`),E={Occidental:["Ahuachapán","Santa Ana","Sonsonate"],Paracentral:["Chalatenango","Cuscatlán","La Paz","Cabañas","San Vicente"],Central:["La Libertad","San Salvador"],Oriental:["Usulután","San Miguel","Morazán","La Unión"]},M=()=>{if(!c||!m)return;const G=c.value,R=m.value;if(!G)m.innerHTML='<option value="">Todos los Departamentos</option>',[...new Set(a.map(k=>k.department).filter(Boolean))].sort().forEach(k=>{const A=document.createElement("option");A.value=k,A.textContent=k,m.appendChild(A)}),R&&(m.value=R);else{const O=E[G]||[];m.innerHTML='<option value="">Todos los Departamentos</option>',O.forEach(k=>{const A=document.createElement("option");A.value=k,A.textContent=k,m.appendChild(A)}),O.includes(R)&&(m.value=R)}},F=()=>{const G=o?o.value.toLowerCase().trim():"",R=c?c.value:"",O=m?m.value:"";l=a.filter(k=>{let A=!0,re=!0,le=!0;return G&&(A=Object.values(k).some(fe=>String(fe).toLowerCase().includes(G))),R&&(re=k.zone===R),O&&(le=k.department===O),A&&re&&le}),j(),v(),y&&y.classList.toggle("hidden",l.length>0)};o&&o.addEventListener("input",F),c&&c.addEventListener("change",()=>{M(),F()}),m&&m.addEventListener("change",F)}function Y(){const o=i.querySelector("[data-title]");if(o){const y=H("map-title")||"Nuestras Agencias";o.textContent=y}const c=i.querySelector(`.${e}-search-input`);if(c){const y=H("search-placeholder")||"Buscar...";c.setAttribute("placeholder",y)}const m=i.querySelector(`.${e}-no-results`);if(m){const y=H("no-results-text")||"No se encontraron agencias";m.textContent=y}}function H(o){return i.closest(`[data-gjs-type="${e}"]`)?.getAttribute(o)}function ae(o){const c=i.querySelector(`.${e}-list`);c&&(c.innerHTML=`
                    <div class="flex flex-col items-center justify-center p-12 text-center">
                ...
                `)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",g):g()}}const Ki=[{id:"agencies-map",label:"Mapa de Agencias",category:"Interactivos",media:Ji,content:{type:"agencies-map-component"}}];function Qi(i){const e="agencies-map-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute&&t.getAttribute("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Mapa de Agencias",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{class:"py-12 bg-secondary","data-gjs-type":e},components:`
            ...
            `,r.appendChild(a)}})}const ia=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
@media(max-width:480px){
    .banner-btn{flex:1 1 100%;min-width:0;}
            ...
            `;const o=i.ownerDocument??document;if(!o.getElementById("banner-skeleton-styles")){const c=o.createElement("style");c.id="banner-skeleton-styles",c.textContent=`
                ...
                `,o.head.appendChild(c)}}function T(o){return t[o]?t[o]:o==="outline-blue"||o==="outline-orange"?t["outline-white"]:t["fill-white"]}function v(o,c,m,y){const E=T(m),M=c?"a":"span",F=c?`href="${c}"${y?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${M} ${F}
                class="banner-btn"
                data-bg="${E.bg}"
                data-color="${E.color}"
                ...
                ${o}
            </${M}>`}function $(){n.innerHTML=a.map((o,c)=>`
                <div class="banner-slide" data-index="${c}">
                    <div class="banner-slide-inner">
                        <div class="banner-bg">
                            <img src="${o.image_url}"
                        ...
                        <div class="banner-content">
                            ...
                            ${o.btn_primary_text||o.btn_secondary_text?`
                                <div class="banner-buttons">
                                    ${o.btn_primary_text?v(o.btn_primary_text,o.btn_primary_url,o.btn_primary_style,o.btn_primary_external):""}
                                    ${o.btn_secondary_text?v(o.btn_secondary_text,o.btn_secondary_url,o.btn_secondary_style,o.btn_secondary_external):""}
                                </div>`:""}
                        </div>
                    </div>
                </div>
            `).join(""),z(),C(),B()}function z(){i.querySelectorAll(".banner-btn").forEach(o=>{const c=o.style.borderColor;o.addEventListener("mouseenter",()=>{o.style.background=o.dataset.hoverBg,o.style.color=o.dataset.hoverColor,o.style.borderColor=o.dataset.hoverBg}),o.addEventListener("mouseleave",()=>{o.style.background=o.dataset.bg,o.style.color=o.dataset.color,o.style.borderColor=c})})}function C(){n.addEventListener("mousedown",j),n.addEventListener("touchstart",j,{passive:!0}),n.addEventListener("mousemove",x),n.addEventListener("touchmove",x,{passive:!0}),n.addEventListener("mouseup",L),n.addEventListener("touchend",L),n.addEventListener("mouseleave",L)}function B(){a.forEach(o=>{const c=new Image;c.src=o.image_url})}function j(o){d=!0,h=o.touches?o.touches[0].clientX:o.clientX,g=0}function x(o){d&&(g=(o.touches?o.touches[0].clientX:o.clientX)-h)}function L(){d&&(d=!1,Math.abs(g)>=p&&(I(g<0?(l+1)%a.length:(l-1+a.length)%a.length),H()),g=0)}function P(){const o=i.querySelector(".banner-stripe");if(!o||a.length<=1)return;const c=document.createElement("div");c.className="banner-dots",a.forEach((m,y)=>{const E=document.createElement("button");E.className="banner-dot",E.dataset.index=String(y),E.setAttribute("aria-label",`Banner ${y+1}`),E.addEventListener("click",()=>{I(y),H()}),c.appendChild(E)}),o.innerHTML="",o.appendChild(c)}function I(o,c=!0){const m=n.querySelectorAll(".banner-slide"),y=i.querySelectorAll(".banner-dot");m.forEach((E,M)=>{const F=M===o;c||(E.style.transition="none"),E.classList.toggle("banner-slide--active",F),c||requestAnimationFrame(()=>{E.style.transition=""})}),y.forEach((E,M)=>{E.classList.toggle("banner-dot--active",M===o)}),l=o}function Y(){a.length<=1||!u||(s=setInterval(()=>{I((l+1)%a.length)},5e3))}function H(){u&&(clearInterval(s),Y())}function ae(){const o=i.querySelector(".banner-wrapper");o&&(o.innerHTML=`
                    <div style="display:flex;align-items:center;justify-content:center;
                                min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;">
                        Sin contenido.
                    </div>`)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",f):f()}}const la=[{id:"banner-hero",label:"Banner Slider",category:"Banners",media:ia,content:{type:"banner-hero-component"}}];function oa(i){const e="banner-hero-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Slider",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-autoplay":"true","data-category":""},components:`
                ...
                `,script:ra(),traits:[{type:"select",name:"data-autoplay",label:"Avance automático",options:[{id:"true",name:"Activado"},{id:"false",name:"Desactivado"}],changeProp:!1},{type:"select",name:"data-category",label:"Filtrar por categoría",options:[{id:"",name:"Todas las categorías"}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const r=this.get("script");r&&typeof r=="function"&&setTimeout(()=>r.call(t),100)})}}}),sa(i,e),ca(i,e),na(i,e)}async function na(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",a=await(await fetch(t)).json();if(!Array.isArray(a))return;const l=[...new Set(a.map(g=>g.category).filter(Boolean))].sort();if(l.length===0)return;const s=i.DomComponents.getType(e);if(!s)return;const h=s.model.prototype.defaults.traits.find(g=>g.name==="data-category");if(!h)return;h.options=[{id:"",name:"Todas las categorías"},...l.map(g=>({id:g,name:g}))]}catch{}}function sa(i,e){i.on("storage:end:load",()=>{setTimeout(()=>Te(i,e),1e3)}),i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>Te(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Te(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})}function ca(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#banner-hero-styles")){const a=document.createElement("style");a.id="banner-hero-styles",a.textContent=aa,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
            ...
            `,r.appendChild(a)}}})}const da=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    <rect x="1" y="6" width="30" height="20" fill="#003B71" rx="2"/>
    <rect x="1" y="22" width="30" height="4" fill="#E97300" rx="0"/>
    <rect x="3" y="9" width="12" height="2" fill="rgba(255,255,255,0.9)" rx="1"/>
...
@media(max-width:480px){
    .bsingle-btn{flex:1 1 100%;min-width:0;}
                ...
                </div>`,u()}function u(){i.querySelectorAll(".bsingle-btn").forEach(n=>{const f=n.style.borderColor;n.addEventListener("mouseenter",()=>{n.style.background=n.dataset.hoverBg,n.style.color=n.dataset.hoverColor,n.style.borderColor=n.dataset.hoverBg}),n.addEventListener("mouseleave",()=>{n.style.background=n.dataset.bg,n.style.color=n.dataset.color,n.style.borderColor=f})})}function b(){l.innerHTML=`
                <div style="display:flex;align-items:center;justify-content:center;
                            min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;">
                    No hay banner seleccionado o no está activo.
                </div>`}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",s):s()}}const ha=[{id:"banner-single",label:"Banner Individual",category:"Banners",media:da,content:{type:"banner-single-component"}}];function pa(i){const e="banner-single-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Individual",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-banner-id":""},components:`
                ...
                `,script:fa(),traits:[{type:"select",name:"data-banner-id",label:"Banner a mostrar",options:[{id:"",name:"Cargando banners..."}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const r=this.get("script");r&&typeof r=="function"&&setTimeout(()=>r.call(t),100)})}}}),ba(i,e),ua(i,e),ma(i,e)}async function ma(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",a=await(await fetch(t)).json();if(!Array.isArray(a)||a.length===0)return;const l=i.DomComponents.getType(e);if(!l)return;const d=l.model.prototype.defaults.traits.find(h=>h.name==="data-banner-id");if(!d)return;d.options=[{id:"",name:"— Seleccionar banner —"},...a.map(h=>({id:String(h.id),name:h.category?`[${h.category}] ${h.title}`:h.title}))]}catch{}}function ba(i,e){i.on("storage:end:load",()=>{setTimeout(()=>Ae(i,e),1e3)}),i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>Ae(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Ae(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})}function ua(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#banner-single-styles")){const a=document.createElement("style");a.id="banner-single-styles",a.textContent=ga,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
                    outline: 2px dashed rgba(240,135,42,0.6) !important;
            ...
            `,r.appendChild(a)}}})}function xa(i){S.registerBlocks(nt),S.registerBlocks(bt),S.registerBlocks(la),S.registerBlocks(ha),S.registerBlocks(di),S.registerBlocks(Xt),S.registerBlocks(kt),S.registerBlocks(qt),S.registerBlocks(Ot),S.registerBlocks(Ht),S.registerBlocks(_t),S.registerBlocks(oi),S.registerBlocks(Tt),S.registerBlocks(Ut),S.registerBlocks(hi),S.registerBlocks(xi),S.registerBlocks(wi),S.registerBlocks(Ei),S.registerBlocks(_i),S.registerBlocks(Ui),S.registerBlocks(Xi),S.registerBlocks(Ki),S.applyToEditor(i),Qi(i),oa(i),pa(i),Lt(i),Vi(i),Jt(i),zi(i)}function ya(i,e,t){i.on("component:add",()=>e.markAsDirty()),i.on("component:remove",()=>e.markAsDirty()),i.on("component:update",()=>e.markAsDirty()),i.on("style:update",()=>e.markAsDirty());const r=document.getElementById("save-button");r&&r.addEventListener("click",async()=>{await va(i,e,t,r)}),document.addEventListener("keydown",a=>{(a.ctrlKey||a.metaKey)&&a.key==="s"&&(a.preventDefault(),r&&!r.disabled&&r.click())})}async function va(i,e,t,r){r.disabled=!0,r.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{t.needsTitle()?await wa(i,e,t):await Oe(i,e,t)}catch(a){He(a.message,"error")}finally{r.disabled=!1,r.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}async function wa(i,e,t){return new Promise((r,a)=>{Ue({title:"Título de la Página",description:"Ingresa un título descriptivo para identificar esta página",placeholder:"Ej: Acerca de Nosotros",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async l=>{if(!l?.trim()){a(new Error("El título es obligatorio"));return}try{await Oe(i,e,t,l),r()}catch(s){a(s)}},onCancel:()=>{a(new Error("Guardado cancelado"))}})})}async function Oe(i,e,t,r=null){const l={...e.getEditorContent(i),is_published:t.isPublished};r&&(l.title=r);const s=await e.savePage(i,l,t.storeUrl,t.getHttpMethod());s.success&&(e.markAsClean(),He(s.message,"success"),!t.isEditMode&&s.page?(t.updatePageInfo(s),t.updateTitle(s.page.title)):r&&t.updateTitle(r))}function He(i,e){typeof window.showNotification=="function"&&window.showNotification(i,e)}document.addEventListener("DOMContentLoaded",async()=>{const i=new Ve,e=new at;new rt(i);const t=We();if(t.on("load",()=>{xa(t),Ye(t),Ge(),Xe(),Je(t),Ze(t),Ke(t),Qe(t),et(t),tt(t),ka(t),Ba(t),Ea(t),setTimeout(()=>{t.runCommand("sw-visibility"),t.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),e.isEditMode)try{await i.loadPageContent(t,e.loadUrl),De("Contenido cargado correctamente","success")}catch(r){De("Error al cargar el contenido","error"),console.error(r)}ya(t,i,e)});function ka(i){const e=[{cmd:"set-device-desktop",device:"Desktop"},{cmd:"set-device-tablet",device:"Tablet"},{cmd:"set-device-mobile",device:"Mobile"}];e.forEach(({cmd:t,device:r})=>{i.Commands.add(t,{run:a=>{a.setDevice(r),e.forEach(({cmd:l})=>{a.Panels.getButton("devices-c",l)?.set("active",l===t)})}})})}function Ba(i){i.Commands.add("canvas-clear",{run:e=>{it({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{e.DomComponents.clear(),e.CssComposer.clear()}})}})}function Ea(i){const e=i.Canvas.getFrameEl();if(!e?.contentDocument?.head)return;const t=e.contentDocument.createElement("style");t.id="gjs-dashed-fix",t.textContent="body,body.gjs-dashed{margin:0!important;padding:0 0 160px 0!important;}",e.contentDocument.head.appendChild(t)}function De(i,e="info"){typeof window.showNotification=="function"&&window.showNotification(i,e)}
