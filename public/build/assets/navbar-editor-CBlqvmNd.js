import{i as H,t as D,d as U,f as O,e as P,s as R,g as G,c as J,E as V,j as W,h as F}from"./editor-commands-5w86-EOS.js";import{o as X}from"./media-picker-CiKJ1Agb.js";import"./_commonjsHelpers-CqkleIqs.js";function Q(){return H()}const Y=`(function(){
...
@media (max-width: 992px) {
    ...
    .nb-hamburger { display: flex; }
...
</style>`;function z(i,t){t=t||"nb"+Math.random().toString(36).slice(2,7);const a=i.logo_href||"/",l=i.logo_url?`<img src="${i.logo_url}" alt="${i.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${i.logo_text||"Logo"}</span>`,g=`<a href="${a}" class="nb-logo-link">${l}</a>`,S=(i.links||[]).map(n=>{if(n.type==="submenu"&&n.children?.length){const c=n.children.map(o=>{const y=o.icon?`<i class="${o.icon}"></i>`:"";return`<li><a href="${o.href||"#"}" class="nb-submenu-link">${y}${o.label}</a></li>`}).join("");return`<li class="nb-item nb-has-submenu"><button class="nb-link nb-submenu-trigger" type="button">${n.label}<i class="ri-arrow-down-s-line nb-chevron"></i></button><ul class="nb-submenu">${c}</ul></li>`}return`<li class="nb-item"><a href="${n.href||"#"}" class="nb-link">${n.label}</a></li>`}).join(""),x=(i.actions||[]).map(n=>{const c=n.icon?`<i class="${n.icon}"></i>`:"";return`<a href="${n.href||"#"}" class="nb-btn nb-btn-${n.style||"primary"}">${c}${n.label}</a>`}).join(""),m=(i.links||[]).map(n=>{if(n.type==="submenu"&&n.children?.length){const c=n.children.map(o=>{const y=o.icon?`<i class="${o.icon}"></i>`:"";return`<a href="${o.href||"#"}" class="nb-mobile-submenu-link">${y}${o.label}</a>`}).join("");return`<div class="nb-mobile-item"><button class="nb-mobile-link" type="button">${n.label}<i class="ri-arrow-down-s-line"></i></button><div class="nb-mobile-submenu">${c}</div></div>`}return`<a href="${n.href||"#"}" class="nb-mobile-link">${n.label}</a>`}).join(""),p=(i.actions||[]).map(n=>{const c=n.icon?`<i class="${n.icon}"></i>`:"";return`<a href="${n.href||"#"}" class="nb-btn nb-btn-${n.style||"primary"}">${c}${n.label}</a>`}).join("");return`<div id="nb-root-${t}" class="nb-inner" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false"><div class="nb-logo" data-gjs-editable="false" data-gjs-selectable="false">${g}</div><ul class="nb-links" data-gjs-editable="false" data-gjs-selectable="false">${S}</ul><div class="nb-actions" data-gjs-editable="false" data-gjs-selectable="false">${x}</div><button class="nb-hamburger" type="button" id="nb-toggle-${t}" aria-label="Menú" data-gjs-editable="false" data-gjs-selectable="false"><span></span><span></span><span></span></button></div><div class="nb-mobile-menu" id="nb-mobile-${t}" data-gjs-editable="false" data-gjs-selectable="false">${m}${p?`<div class="nb-mobile-actions">${p}</div>`:""}</div>`}function K(i,t){const a=document.getElementById("navbar-config-modal");if(a&&a.remove(),!document.getElementById("nb-modal-styles")){const r=document.createElement("style");r.id="nb-modal-styles",r.textContent=`
        ...
        `,document.head.appendChild(r)}const l=(()=>{try{return JSON.parse(t.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),g=l.logo_url||"",S=l.logo_alt||"",x=l.logo_text||"Logo",m=l.logo_href||"/",p=JSON.parse(JSON.stringify(l.links||[{type:"link",label:"Inicio",href:"/"},{type:"link",label:"Nosotros",href:"#"}])),n=JSON.parse(JSON.stringify(l.actions||[{label:"Ingresar",href:"#",style:"primary",icon:""}])),c=document.createElement("div");c.id="navbar-config-modal",c.className="nb-overlay";const o=document.createElement("div");o.className="nb-modal",o.innerHTML=`
        ...
        </div>`,c.appendChild(o),document.body.appendChild(c),o.querySelectorAll(".nb-tab-btn").forEach(r=>{r.addEventListener("click",()=>{o.querySelectorAll(".nb-tab-btn").forEach(e=>e.classList.remove("active")),o.querySelectorAll(".nb-tab-panel").forEach(e=>e.classList.remove("active")),r.classList.add("active"),o.querySelector(`#nb-panel-${r.dataset.tab}`).classList.add("active")})}),o.querySelector("#nb-logo-pick").addEventListener("click",()=>{X({type:"image",title:"Seleccionar logo",onSelect:r=>{o.querySelector("#nb-logo-url").value=r;let e=o.querySelector("#nb-logo-preview");if(!e||e.tagName==="DIV"){const s=document.createElement("img");s.id="nb-logo-preview",s.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;margin-bottom:0.5rem;",e?.replaceWith(s)??o.querySelector("#nb-logo-url").before(s),e=s}e.src=r,e.style.display="block"}})});function y(r,e,s){let d=null;r.querySelectorAll("[data-drag-idx]").forEach(b=>{b.setAttribute("draggable","true"),b.addEventListener("dragstart",u=>{d=parseInt(b.dataset.dragIdx),setTimeout(()=>b.classList.add("nb-dragging"),0),u.dataTransfer.effectAllowed="move"}),b.addEventListener("dragend",()=>{b.classList.remove("nb-dragging"),r.querySelectorAll(".nb-drag-over").forEach(u=>u.classList.remove("nb-drag-over"))}),b.addEventListener("dragover",u=>{u.preventDefault(),u.dataTransfer.dropEffect="move",parseInt(b.dataset.dragIdx)!==d&&b.classList.add("nb-drag-over")}),b.addEventListener("dragleave",()=>b.classList.remove("nb-drag-over")),b.addEventListener("drop",u=>{u.preventDefault();const f=parseInt(b.dataset.dragIdx);if(d!==null&&f!==d){const[w]=e.splice(d,1);e.splice(f,0,w),s()}d=null})})}const $=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function I(r){if(r.dataset.autocompleteAttached)return;r.dataset.autocompleteAttached="true";const e=document.createElement("div");e.style.cssText="position:relative;flex:1;",r.parentNode.insertBefore(e,r),e.appendChild(r),r.style.width="100%";const s=document.createElement("ul");s.style.cssText=`
        ...
        `,e.appendChild(s);let d=null,b="";async function u(v){if(v.length<1){s.style.display="none";return}try{const h=await(await fetch(`${$}?q=${encodeURIComponent(v)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();f(h,v)}catch{s.style.display="none"}}function f(v,k){if(s.innerHTML="",!v.length){s.style.display="none";return}v.forEach(h=>{const L=document.createElement("li");L.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",L.innerHTML=`
                    <span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${w(h.title,k)}</span>
                    <span style="font-size:0.7rem;color:#64748b;">/${h.slug}</span>`,L.addEventListener("mouseenter",()=>L.style.background="#f1f5f9"),L.addEventListener("mouseleave",()=>L.style.background=""),L.addEventListener("mousedown",C=>{C.preventDefault(),r.value="/"+h.slug,r.dispatchEvent(new Event("input")),s.style.display="none"}),s.appendChild(L)}),s.style.display="block"}function w(v,k){return k?v.replace(new RegExp(`(${k.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):v}r.addEventListener("input",()=>{clearTimeout(d),b=r.value.trim(),d=setTimeout(()=>u(b),220)}),r.addEventListener("focus",()=>{b&&u(b)}),r.addEventListener("blur",()=>{setTimeout(()=>{s.style.display="none"},150)}),r.addEventListener("keydown",v=>{if(s.style.display==="none")return;const k=s.querySelectorAll("li"),h=s.querySelector("li.nb-ac-active");let L=Array.from(k).indexOf(h);if(v.key==="ArrowDown"){v.preventDefault(),h&&h.classList.remove("nb-ac-active");const C=k[L+1]||k[0];C?.classList.add("nb-ac-active"),C.style.background="#f1f5f9"}else if(v.key==="ArrowUp"){v.preventDefault(),h&&h.classList.remove("nb-ac-active");const C=k[L-1]||k[k.length-1];C?.classList.add("nb-ac-active"),C.style.background="#f1f5f9"}else v.key==="Enter"&&h?(v.preventDefault(),h.dispatchEvent(new MouseEvent("mousedown"))):v.key==="Escape"&&(s.style.display="none")})}function E(){const r=o.querySelector("#nb-links-list");r.innerHTML="",p.forEach((e,s)=>{const d=document.createElement("div");d.className="nb-link-card",d.dataset.dragIdx=s;const b=`<span class="nb-type-badge ${e.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${e.type==="submenu"?"Submenú":"Link"}<span class="nb-type-badge-hint">↕</span></span>`;if(e.type==="submenu"){const f=(e.children||[]).map((w,v)=>`
                    <div class="nb-submenu-item" data-child="${v}">
                        ...
                        <input class="nb-input-sm" style="flex:1;" placeholder="URL" value="${w.href||""}" data-field="href">
                    ...
                    <div class="nb-link-card-body">
                        <div class="nb-list nb-children-list" style="gap:0.375rem;">${f}</div>
                    ...
                    </div>`;d.querySelector("[data-toggle-type]").onclick=()=>{e.type==="link"?(e.type="submenu",e.children=e.children?.length?e.children:[{label:"Enlace",href:"#",icon:""}],delete e.href):(e.type="link",e.href="#",delete e.children),E()},d.querySelector(".nb-remove-link").onclick=()=>{p.splice(s,1),E()},d.querySelectorAll("[data-field]").forEach(f=>{f.addEventListener("input",()=>{e[f.dataset.field]=f.value})});const u=d.querySelector(".nb-url-input");u&&I(u),e.type==="submenu"&&(d.querySelector(".nb-add-child").onclick=()=>{e.children=e.children||[],e.children.push({label:"Nuevo enlace",href:"#",icon:""}),E()},d.querySelectorAll(".nb-remove-child").forEach(f=>{f.onclick=()=>{const w=parseInt(f.closest("[data-child]").dataset.child);e.children.splice(w,1),E()}}),d.querySelectorAll(".nb-submenu-item [data-field]").forEach(f=>{const w=parseInt(f.closest("[data-child]").dataset.child);f.addEventListener("input",()=>{e.children[w][f.dataset.field]=f.value}),f.dataset.field==="href"&&I(f)})),r.appendChild(d)}),y(r,p,E)}function j(){const r=o.querySelector("#nb-actions-list");r.innerHTML="",n.forEach((e,s)=>{const d=document.createElement("div");d.className="nb-action-card",d.dataset.dragIdx=s,d.innerHTML=`
                <div class="nb-row">
                    <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                ...
                </div>`,d.querySelector(".nb-remove-action").onclick=()=>{n.splice(s,1),j()},d.querySelectorAll("[data-field]").forEach(u=>{u.addEventListener("input",()=>{e[u.dataset.field]=u.value}),u.addEventListener("change",()=>{e[u.dataset.field]=u.value})});const b=d.querySelector(".nb-url-input");b&&I(b),r.appendChild(d)}),y(r,n,j)}E(),j();const N=o.querySelector("#nb-logo-href");N&&I(N),o.querySelector("#nb-add-link").onclick=()=>{p.push({type:"link",label:"Nuevo enlace",href:"#"}),E(),o.querySelector("#nb-links-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},o.querySelector("#nb-add-submenu").onclick=()=>{p.push({type:"submenu",label:"Menú",children:[{label:"Enlace",href:"#",icon:""}]}),E(),o.querySelector("#nb-links-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},o.querySelector("#nb-add-action").onclick=()=>{n.push({label:"Botón",href:"#",style:"primary",icon:""}),j(),o.querySelector("#nb-actions-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})};const q=()=>c.remove();o.querySelector("#nb-modal-close").onclick=q,o.querySelector("#nb-modal-cancel").onclick=q,c.onclick=r=>{r.target===c&&q()},o.querySelector("#nb-modal-save").onclick=()=>{const r={logo_url:o.querySelector("#nb-logo-url").value.trim(),logo_alt:o.querySelector("#nb-logo-alt").value.trim(),logo_text:o.querySelector("#nb-logo-text").value.trim(),logo_href:o.querySelector("#nb-logo-href").value.trim()||"/",links:p,actions:n},s=t.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);t.addAttributes({"data-navbar-config":JSON.stringify(r)}),t.components(z(r,s)+_),q()}}function Z(i){const t="navbar-component";i.DomComponents.addType(t,{isComponent:a=>a.getAttribute?.("data-gjs-type")===t?{type:t}:!1,model:{defaults:{name:"Navbar",tagName:"nav",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":t,class:"nb-wrapper","data-navbar-config":JSON.stringify({logo_url:"",logo_alt:"Logo",logo_text:"Logo",logo_href:"/",links:[{type:"link",label:"Sobre nosotros",href:"#"},{type:"link",label:"Créditos",href:"#"},{type:"link",label:"Depósitos",href:"#"},{type:"submenu",label:"Otros servicios",children:[{label:"Servicio 1",href:"#",icon:""},{label:"Servicio 2",href:"#",icon:""}]},{type:"link",label:"Contáctanos",href:"#"}],actions:[{label:"Ingresar",href:"#",style:"primary",icon:""}]})},components:z({logo_url:"",logo_alt:"Logo",logo_text:"Logo",links:[{type:"link",label:"Sobre nosotros",href:"#"},{type:"link",label:"Créditos",href:"#"},{type:"link",label:"Depósitos",href:"#"},{type:"submenu",label:"Otros servicios",children:[{label:"Servicio 1",href:"#",icon:""},{label:"Servicio 2",href:"#",icon:""}]},{type:"link",label:"Contáctanos",href:"#"}],actions:[{label:"Ingresar",href:"#",style:"primary",icon:""}]})+_,script:function(){(function(a){if(!a||typeof a.querySelector!="function"||a.__nbInit)return;a.__nbInit=!0;var l=a.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");if(!l)return;var g=!!window.__gjseditor||document.documentElement.hasAttribute("data-gjs-canvas");function S(){g||(document.body.style.paddingTop=a.offsetHeight+"px")}S(),window.addEventListener("resize",S);var x=document.getElementById("nb-toggle-"+l),m=document.getElementById("nb-mobile-"+l);x&&m&&x.addEventListener("click",function(){m.classList.toggle("nb-open"),S()}),a.querySelectorAll(".nb-submenu-trigger").forEach(function(p){p.addEventListener("click",function(n){n.stopPropagation();var c=p.closest(".nb-item"),o=c.classList.contains("nb-open");a.querySelectorAll(".nb-item.nb-open").forEach(function(y){y.classList.remove("nb-open")}),o||c.classList.add("nb-open")})}),a.querySelectorAll(".nb-mobile-item>.nb-mobile-link").forEach(function(p){p.addEventListener("click",function(){p.closest(".nb-mobile-item").classList.toggle("nb-open"),S()})}),document.addEventListener("click",function(p){a.contains(p.target)||a.querySelectorAll(".nb-item.nb-open").forEach(function(n){n.classList.remove("nb-open")})})})(this)},"script-props":[],toolbar:[],traits:[{type:"button",label:"Navbar",text:"Administrar Navbar",full:!0,command:"open-navbar-config"}]},init(){this.set("type",t),this.addAttributes({"data-gjs-type":t})}}}),i.Commands.add("open-navbar-config",{run(a){const l=a.getSelected();l&&K(a,l)}}),i.BlockManager.add("navbar-block",{label:"Navbar",category:"Navbar",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,activate:!0,content:{type:t,attributes:{"data-gjs-type":t}}}),ee(i,t),ne(i)}function ee(i,t){i.on("storage:end:load",()=>{setTimeout(()=>T(i,t),800)}),i.on("component:mount",a=>{const l=a.getEl();l?.getAttribute?.("data-gjs-type")===t&&(a.set("type",t),setTimeout(()=>M(i,l),400))}),i.on("canvas:render",()=>{setTimeout(()=>T(i,t),600)})}function M(i,t){if(t?.isConnected)try{const a=i.Canvas.getFrameEl()?.contentDocument;if(!a)return;t.__nbInit&&delete t.__nbInit;const l=a.createElement("script");l.textContent=`(function(){
            function initNavbar(root){
                if(!root||root.__nbInit)return;
                root.__nbInit=true;
                var id=root.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");
        ...
        })();`,a.head.appendChild(l),l.remove()}catch(a){console.warn("[Navbar] Error inyectando script en canvas:",a)}}function T(i,t){i.getWrapper().find(`[data-gjs-type="${t}"]`).forEach(a=>{a.set("type",t);const l=a.getEl();l?.isConnected&&M(i,l)})}function ne(i){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const a=t.contentDocument,l=a?.head;if(l&&(a.documentElement?.setAttribute("data-gjs-canvas","true"),!l.querySelector("#navbar-component-css"))){const g=document.createElement("style");g.id="navbar-component-css",g.textContent=`
            ...
            `,l.appendChild(g)}})}document.addEventListener("DOMContentLoaded",async()=>{const i=new V;let t=document.getElementById("navbar-id")?.value||"",a=document.getElementById("navbar-name")?.value||"",l=document.getElementById("navbar-load-url")?.value||"",g=document.getElementById("navbar-store-url")?.value||"";const S=document.getElementById("navbar-is-active")?.value==="1";let x=!!t;const m=Q();if(Z(m),m.on("load",()=>{D(m),U(),O(),P(m),R(m),G(m),J(m),te(m),setTimeout(()=>{m.runCommand("sw-visibility"),m.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),x&&l)try{await i.loadPageContent(m,l),B("Navbar cargado correctamente","success")}catch{B("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const n=document.getElementById("save-button");n.disabled=!0,n.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!x&&!a?W({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async c=>{if(!c?.trim()){B("El nombre es obligatorio","error"),n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await p(c)}catch(o){B(o.message,"error")}finally{n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await p(a),n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(c){B(c.message,"error"),n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function p(n){const c=x?"PUT":"POST",o=i.getEditorContent(m);(!o.js_content||o.js_content.trim()==="")&&(o.js_content=Y);const y=await i.savePage(m,{...o,name:n,is_active:S},g,c);if(y.success){if(i.markAsClean(),B(y.message,"success"),!x&&y.navbar){t=y.navbar.id,a=y.navbar.name,x=!0;const A=document.getElementById("navbar-id");A&&(A.value=t);const $=document.getElementById("navbar-name");$&&($.value=a);const I=document.querySelector('meta[name="app-url"]'),E=I?I.content:"";g=g.endsWith("/navbars")?`${g}/${t}`:`${g.replace(/\/navbars\/?$/,"")}/navbars/${t}`;const N=document.getElementById("navbar-store-url");N&&(N.value=g),l=`${g}/load`;const q=document.getElementById("navbar-load-url");q&&(q.value=l);const r=document.getElementById("editor-title");r&&(r.textContent=`Editando Navbar: ${a}`);const e=`/navbars/edit/${t}/edit`,s=E?`${E}${e}`:e;window.history.replaceState({path:s},"",s)}else if(n){a=n;const A=document.getElementById("navbar-name");A&&(A.value=a);const $=document.getElementById("editor-title");$&&($.textContent=`Editando Navbar: ${a}`)}}}});function te(i){i.Commands.add("canvas-clear",{run:t=>{F({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{t.DomComponents.clear(),t.CssComposer.clear()}})}})}function B(i,t="info"){typeof window.showNotification=="function"&&window.showNotification(i,t)}
