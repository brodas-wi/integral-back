import{i as _,t as B,d as I,f as q,e as A,s as T,g as z,c as M,E as P,j as H,h as U}from"./editor-commands-5w86-EOS.js";import{o as R}from"./media-picker-CiKJ1Agb.js";import"./_commonjsHelpers-CqkleIqs.js";const O="modulepreload",D=function(i){return"/adminintegral/public/build/"+i},S={},G=function(t,o,l){let b=Promise.resolve();if(o&&o.length>0){let c=function(n){return Promise.all(n.map(a=>Promise.resolve(a).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};document.getElementsByTagName("link");const m=document.querySelector("meta[property=csp-nonce]"),e=m?.nonce||m?.getAttribute("nonce");b=c(o.map(n=>{if(n=D(n),n in S)return;S[n]=!0;const a=n.endsWith(".css"),u=a?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${n}"]${u}`))return;const f=document.createElement("link");if(f.rel=a?"stylesheet":O,a||(f.as="script"),f.crossOrigin="",f.href=n,e&&f.setAttribute("nonce",e),document.head.appendChild(f),a)return new Promise((g,s)=>{f.addEventListener("load",g),f.addEventListener("error",()=>s(new Error(`Unable to preload CSS for ${n}`)))})}))}function y(m){const e=new Event("vite:preloadError",{cancelable:!0});if(e.payload=m,window.dispatchEvent(e),!e.defaultPrevented)throw m}return b.then(m=>{for(const e of m||[])e.status==="rejected"&&y(e.reason);return t().catch(y)})};function J(){return _()}const V=`(function(){
...
@media (max-width: 992px) {
    ...
    .nb-hamburger { display: flex; }
...
</style>`;function N(i,t){t=t||"nb"+Math.random().toString(36).slice(2,7);const o=i.logo_url?`<img src="${i.logo_url}" alt="${i.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${i.logo_text||"Logo"}</span>`,l=(i.links||[]).map(e=>{if(e.type==="submenu"&&e.children?.length){const c=e.children.map(n=>{const a=n.icon?`<i class="${n.icon}"></i>`:"";return`<li><a href="${n.href||"#"}" class="nb-submenu-link">${a}${n.label}</a></li>`}).join("");return`<li class="nb-item nb-has-submenu"><button class="nb-link nb-submenu-trigger" type="button">${e.label}<i class="ri-arrow-down-s-line nb-chevron"></i></button><ul class="nb-submenu">${c}</ul></li>`}return`<li class="nb-item"><a href="${e.href||"#"}" class="nb-link">${e.label}</a></li>`}).join(""),b=(i.actions||[]).map(e=>{const c=e.icon?`<i class="${e.icon}"></i>`:"";return`<a href="${e.href||"#"}" class="nb-btn nb-btn-${e.style||"primary"}">${c}${e.label}</a>`}).join(""),y=(i.links||[]).map(e=>{if(e.type==="submenu"&&e.children?.length){const c=e.children.map(n=>{const a=n.icon?`<i class="${n.icon}"></i>`:"";return`<a href="${n.href||"#"}" class="nb-mobile-submenu-link">${a}${n.label}</a>`}).join("");return`<div class="nb-mobile-item"><button class="nb-mobile-link" type="button">${e.label}<i class="ri-arrow-down-s-line"></i></button><div class="nb-mobile-submenu">${c}</div></div>`}return`<a href="${e.href||"#"}" class="nb-mobile-link">${e.label}</a>`}).join(""),m=(i.actions||[]).map(e=>{const c=e.icon?`<i class="${e.icon}"></i>`:"";return`<a href="${e.href||"#"}" class="nb-btn nb-btn-${e.style||"primary"}">${c}${e.label}</a>`}).join("");return`<div id="nb-root-${t}" class="nb-inner" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false"><div class="nb-logo" data-gjs-editable="false" data-gjs-selectable="false">${o}</div><ul class="nb-links" data-gjs-editable="false" data-gjs-selectable="false">${l}</ul><div class="nb-actions" data-gjs-editable="false" data-gjs-selectable="false">${b}</div><button class="nb-hamburger" type="button" id="nb-toggle-${t}" aria-label="Menú" data-gjs-editable="false" data-gjs-selectable="false"><span></span><span></span><span></span></button></div><div class="nb-mobile-menu" id="nb-mobile-${t}" data-gjs-editable="false" data-gjs-selectable="false">${y}${m?`<div class="nb-mobile-actions">${m}</div>`:""}</div>`}function W(i,t){const o=document.getElementById("navbar-config-modal");if(o&&o.remove(),!document.getElementById("nb-modal-styles")){const s=document.createElement("style");s.id="nb-modal-styles",s.textContent=`
            .nb-overlay {
        ...
        `,document.head.appendChild(s)}const l=(()=>{try{return JSON.parse(t.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),b=l.logo_url||"",y=l.logo_alt||"",m=l.logo_text||"Logo",e=JSON.parse(JSON.stringify(l.links||[{type:"link",label:"Inicio",href:"/"},{type:"link",label:"Nosotros",href:"#"}])),c=JSON.parse(JSON.stringify(l.actions||[{label:"Ingresar",href:"#",style:"primary",icon:""}])),n=document.createElement("div");n.id="navbar-config-modal",n.className="nb-overlay";const a=document.createElement("div");a.className="nb-modal",a.innerHTML=`
    ...
    `,n.appendChild(a),document.body.appendChild(n),a.querySelectorAll(".nb-tab-btn").forEach(s=>{s.addEventListener("click",()=>{a.querySelectorAll(".nb-tab-btn").forEach(r=>r.classList.remove("active")),a.querySelectorAll(".nb-tab-panel").forEach(r=>r.classList.remove("active")),s.classList.add("active"),a.querySelector(`#nb-panel-${s.dataset.tab}`).classList.add("active")})}),a.querySelector("#nb-logo-pick").addEventListener("click",()=>{R({type:"image",title:"Seleccionar logo",onSelect:s=>{a.querySelector("#nb-logo-url").value=s;let r=a.querySelector("#nb-logo-preview");if(!r||r.tagName==="DIV"){const v=document.createElement("img");v.id="nb-logo-preview",v.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;margin-bottom:0.5rem;",r?.replaceWith(v)??a.querySelector("#nb-logo-url").before(v),r=v}r.src=s,r.style.display="block"}})});function u(){const s=a.querySelector("#nb-links-list");s.innerHTML="",e.forEach((r,v)=>{const p=document.createElement("div");if(p.className="nb-link-card",p.dataset.index=v,r.type==="submenu"){const d=(r.children||[]).map((h,k)=>`
                    ...
                    </div>`;p.querySelector(".nb-remove-link").onclick=()=>{e.splice(v,1),u()},p.querySelectorAll("[data-field]").forEach(d=>{d.addEventListener("input",()=>{r[d.dataset.field]=d.value})}),r.type==="submenu"&&(p.querySelector(".nb-add-child").onclick=()=>{r.children=r.children||[],r.children.push({label:"Nuevo enlace",href:"#",icon:""}),u()},p.querySelectorAll(".nb-remove-child").forEach(d=>{d.onclick=()=>{const h=parseInt(d.closest("[data-child]").dataset.child);r.children.splice(h,1),u()}}),p.querySelectorAll(".nb-submenu-item [data-field]").forEach(d=>{const h=parseInt(d.closest("[data-child]").dataset.child);d.addEventListener("input",()=>{r.children[h][d.dataset.field]=d.value})})),s.appendChild(p)})}function f(){const s=a.querySelector("#nb-actions-list");s.innerHTML="",c.forEach((r,v)=>{const p=document.createElement("div");p.className="nb-action-card",p.innerHTML=`
                <div class="nb-row">
                ...
                </div>`,p.querySelector(".nb-remove-action").onclick=()=>{c.splice(v,1),f()},p.querySelectorAll("[data-field]").forEach(d=>{d.addEventListener("input",()=>{r[d.dataset.field]=d.value}),d.addEventListener("change",()=>{r[d.dataset.field]=d.value})}),s.appendChild(p)})}u(),f(),a.querySelector("#nb-add-link").onclick=()=>{e.push({type:"link",label:"Nuevo enlace",href:"#"}),u()},a.querySelector("#nb-add-submenu").onclick=()=>{e.push({type:"submenu",label:"Menú",children:[{label:"Enlace",href:"#",icon:""}]}),u()},a.querySelector("#nb-add-action").onclick=()=>{c.push({label:"Botón",href:"#",style:"primary",icon:""}),f()};const g=()=>n.remove();a.querySelector("#nb-modal-close").onclick=g,a.querySelector("#nb-modal-cancel").onclick=g,n.onclick=s=>{s.target===n&&g()},a.querySelector("#nb-modal-save").onclick=()=>{const s={logo_url:a.querySelector("#nb-logo-url").value.trim(),logo_alt:a.querySelector("#nb-logo-alt").value.trim(),logo_text:a.querySelector("#nb-logo-text").value.trim(),links:e,actions:c},p=t.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||null||"nb"+Math.random().toString(36).slice(2,7);t.addAttributes({"data-navbar-config":JSON.stringify(s)}),t.components(N(s,p)+$),g()}}function j(i){const t="navbar-component";i.DomComponents.addType(t,{isComponent:o=>o.getAttribute?.("data-gjs-type")===t?{type:t}:!1,model:{defaults:{name:"Navbar",tagName:"nav",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":t,class:"nb-wrapper","data-navbar-config":JSON.stringify({logo_url:"",logo_alt:"Logo",logo_text:"Logo",links:[{type:"link",label:"Sobre nosotros",href:"#"},{type:"link",label:"Créditos",href:"#"},{type:"link",label:"Depósitos",href:"#"},{type:"submenu",label:"Otros servicios",children:[{label:"Servicio 1",href:"#",icon:""},{label:"Servicio 2",href:"#",icon:""}]},{type:"link",label:"Contáctanos",href:"#"}],actions:[{label:"Ingresar",href:"#",style:"primary",icon:""}]})},components:N({logo_url:"",logo_alt:"Logo",logo_text:"Logo",links:[{type:"link",label:"Sobre nosotros",href:"#"},{type:"link",label:"Créditos",href:"#"},{type:"link",label:"Depósitos",href:"#"},{type:"submenu",label:"Otros servicios",children:[{label:"Servicio 1",href:"#",icon:""},{label:"Servicio 2",href:"#",icon:""}]},{type:"link",label:"Contáctanos",href:"#"}],actions:[{label:"Ingresar",href:"#",style:"primary",icon:""}]})+$,script:function(){(function(o){if(!o||typeof o.querySelector!="function"||o.__nbInit)return;o.__nbInit=!0;var l=o.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");if(!l)return;var b=!!window.__gjseditor||document.documentElement.hasAttribute("data-gjs-canvas");function y(){b||(document.body.style.paddingTop=o.offsetHeight+"px")}y(),window.addEventListener("resize",y);var m=document.getElementById("nb-toggle-"+l),e=document.getElementById("nb-mobile-"+l);m&&e&&m.addEventListener("click",function(){e.classList.toggle("nb-open"),y()}),o.querySelectorAll(".nb-submenu-trigger").forEach(function(c){c.addEventListener("click",function(n){n.stopPropagation();var a=c.closest(".nb-item"),u=a.classList.contains("nb-open");o.querySelectorAll(".nb-item.nb-open").forEach(function(f){f.classList.remove("nb-open")}),u||a.classList.add("nb-open")})}),o.querySelectorAll(".nb-mobile-item>.nb-mobile-link").forEach(function(c){c.addEventListener("click",function(){c.closest(".nb-mobile-item").classList.toggle("nb-open"),y()})}),document.addEventListener("click",function(c){o.contains(c.target)||o.querySelectorAll(".nb-item.nb-open").forEach(function(n){n.classList.remove("nb-open")})})})(this)},"script-props":[],toolbar:[],traits:[{type:"button",label:"Navbar",text:"Administrar Navbar",full:!0,command:"open-navbar-config"}]},init(){this.set("type",t),this.addAttributes({"data-gjs-type":t})}}}),i.Commands.add("open-navbar-config",{run(o){const l=o.getSelected();l&&W(o,l)}}),i.BlockManager.add("navbar-block",{label:"Navbar",category:"Navbar",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,activate:!0,content:{type:t,attributes:{"data-gjs-type":t}}}),F(i,t),Y(i)}function F(i,t){i.on("storage:end:load",()=>{setTimeout(()=>L(i,t),800)}),i.on("component:mount",o=>{const l=o.getEl();l?.getAttribute?.("data-gjs-type")===t&&(o.set("type",t),setTimeout(()=>C(i,l),400))}),i.on("canvas:render",()=>{setTimeout(()=>L(i,t),600)})}function C(i,t){if(t?.isConnected)try{const o=i.Canvas.getFrameEl()?.contentDocument;if(!o)return;t.__nbInit&&delete t.__nbInit;const l=o.createElement("script");l.textContent=`(function(){
            function initNavbar(root){
                if(!root||root.__nbInit)return;
                root.__nbInit=true;
                var id=root.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");
        ...
        })();`,o.head.appendChild(l),l.remove()}catch(o){console.warn("[Navbar] Error inyectando script en canvas:",o)}}function L(i,t){i.getWrapper().find(`[data-gjs-type="${t}"]`).forEach(o=>{o.set("type",t);const l=o.getEl();l?.isConnected&&C(i,l)})}function Y(i){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const o=t.contentDocument,l=o?.head;if(l&&(o.documentElement?.setAttribute("data-gjs-canvas","true"),!l.querySelector("#navbar-component-css"))){const b=document.createElement("style");b.id="navbar-component-css",b.textContent=`
            ...
            `,l.appendChild(b)}})}const K=Object.freeze(Object.defineProperty({__proto__:null,NAVBAR_RUNTIME_SCRIPT:V,initializeNavbarBlock:j},Symbol.toStringTag,{value:"Module"}));document.addEventListener("DOMContentLoaded",async()=>{const i=new P;let t=document.getElementById("navbar-id")?.value||"",o=document.getElementById("navbar-name")?.value||"",l=document.getElementById("navbar-load-url")?.value||"",b=document.getElementById("navbar-store-url")?.value||"";const y=document.getElementById("navbar-is-active")?.value==="1";let m=!!t;const e=J();if(j(e),e.on("load",()=>{B(e),I(),q(),A(e),T(e),z(e),M(e),Q(e),setTimeout(()=>{e.runCommand("sw-visibility"),e.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),m&&l)try{await i.loadPageContent(e,l),x("Navbar cargado correctamente","success")}catch{x("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const n=document.getElementById("save-button");n.disabled=!0,n.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!m&&!o?H({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async a=>{if(!a?.trim()){x("El nombre es obligatorio","error"),n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await c(a)}catch(u){x(u.message,"error")}finally{n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await c(o),n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(a){x(a.message,"error"),n.disabled=!1,n.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function c(n){const a=m?"PUT":"POST",u=i.getEditorContent(e);if(!u.js_content||u.js_content.trim()===""){const{NAVBAR_RUNTIME_SCRIPT:g}=await G(async()=>{const{NAVBAR_RUNTIME_SCRIPT:s}=await Promise.resolve().then(()=>K);return{NAVBAR_RUNTIME_SCRIPT:s}},void 0);u.js_content=g}const f=await i.savePage(e,{...u,name:n,is_active:y},b,a);if(f.success){if(i.markAsClean(),x(f.message,"success"),!m&&f.navbar){t=f.navbar.id,o=f.navbar.name,m=!0;const g=document.getElementById("navbar-id");g&&(g.value=t);const s=document.getElementById("navbar-name");s&&(s.value=o);const r=document.querySelector('meta[name="app-url"]'),v=r?r.content:"";b=b.endsWith("/navbars")?`${b}/${t}`:`${b.replace(/\/navbars\/?$/,"")}/navbars/${t}`;const d=document.getElementById("navbar-store-url");d&&(d.value=b),l=`${b}/load`;const h=document.getElementById("navbar-load-url");h&&(h.value=l);const k=document.getElementById("editor-title");k&&(k.textContent=`Editando Navbar: ${o}`);const E=`/navbars/edit/${t}/edit`,w=v?`${v}${E}`:E;window.history.replaceState({path:w},"",w)}else if(n){o=n;const g=document.getElementById("navbar-name");g&&(g.value=o);const s=document.getElementById("editor-title");s&&(s.textContent=`Editando Navbar: ${o}`)}}}});function Q(i){i.Commands.add("canvas-clear",{run:t=>{U({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{t.DomComponents.clear(),t.CssComposer.clear()}})}})}function x(i,t="info"){typeof window.showNotification=="function"&&window.showNotification(i,t)}
