...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function le(r){return r?r.split(" ").filter(o=>o.length>0).map((o,a)=>`<span style="color:${a%2===0?"#E97300":"#003B71"};">${o}</span>`).join(" "):""}function R(r,o){o=o||"nb"+Math.random().toString(36).slice(2,7);const a=r.logo_href||"/",c=r.logo_url?`<img src="${r.logo_url}" alt="${r.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${r.logo_text||"Logo"}</span>`,k=`<a href="${a}" class="nb-logo-link">${c}</a>`,A=(r.top_actions||[]).map(d=>{const C=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-top-action">${C}${d.label||""}</a>`}).join(""),q=r.banking_btn||{},w=q.color==="orange"?"nb-banking-orange":"nb-banking-blue",y=`<a href="${q.href||"#"}" class="nb-banking-btn ${w}">${q.label||"Banca en Línea"}</a>`,m=(r.nav_links||[]).map(d=>{if(d.type==="submenu"&&d.columns?.length){const C=!!(d.cta_column&&(d.cta_column.text||d.cta_column.btn_label)),S=d.columns.slice(0,3),$=S.map(_=>{const T=_.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",n=_.badge?`<span class="nb-mega-badge ${T}">${_.badge}</span>`:"",e=(_.items||[]).map(l=>{const i=l.desc?`<span class="nb-mega-item-desc">${l.desc}</span>`:"";return`<a href="${l.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${l.label||""}</span>${i}</a>`}).join("");return`<div class="nb-mega-col">${n}${e}</div>`}).join("");let N="";if(C){const _=d.cta_column,T=_.btn_color==="blue"?"nb-badge-blue":"nb-badge-orange";N=`<div class="nb-mega-cta-col">
                        <p class="nb-mega-cta-text">${le(_.text||"")}</p>
                        <a href="${_.btn_href||"#"}" class="nb-mega-cta-btn ${T}">${_.btn_label||"Ver más"}</a>
                    </div>`}const j=C?"grid-template-columns:1fr 1fr 1fr minmax(200px,260px);":`grid-template-columns:repeat(${S.length},minmax(0,280px));justify-content:start;`;return`<li class="nb-nav-item nb-has-submenu">
                    <button class="nb-nav-link nb-nav-trigger" type="button">${d.label||"Menú"}</button>
                    <div class="nb-mega"><div class="nb-mega-grid" style="${j}">${$}${N}</div></div>
                </li>`}return`<li class="nb-nav-item"><a href="${d.href||"#"}" class="nb-nav-link">${d.label||""}</a></li>`}).join(""),f=r.bottom_cta||{},x=f.color==="blue"?"nb-cta-blue":"nb-cta-orange",E=f.sublabel?`<span class="nb-bottom-cta-line2">${f.sublabel}</span>`:"",t=f.label?`<a href="${f.href||"#"}" class="nb-bottom-cta ${x}"><span class="nb-bottom-cta-line1">${f.label}</span>${E}</a>`:"",z=(r.top_actions||[]).map(d=>{const C=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-mobile-top-action">${C}${d.label||""}</a>`}).join(""),O=(r.nav_links||[]).map(d=>{if(d.type==="submenu"&&d.columns?.length){const C=d.columns.flatMap(S=>S.items||[]).map(S=>{const $=S.desc?`<span class="nb-mobile-sub-desc">${S.desc}</span>`:"";return`<a href="${S.href||"#"}" class="nb-mobile-sub-link"><span class="nb-mobile-sub-title">${S.label||""}</span>${$}</a>`}).join("");return`<div class="nb-mobile-item">
                ...
                </div>`}return`<a href="${d.href||"#"}" class="nb-mobile-link">${d.label||""}</a>`}).join(""),H=[y,f.label?`<a href="${f.href||"#"}" class="nb-bottom-cta ${x}"><span class="nb-bottom-cta-line1">${f.label}</span>${E}</a>`:""].filter(Boolean).join("");return`<div id="nb-root-${o}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false">
        <div class="nb-top">
            <div class="nb-logo">${k}</div>
            <div class="nb-top-actions">${A}</div>
            ${y}
        </div>
        <div class="nb-bottom">
        ...
        `,document.head.appendChild(n)}const c=(()=>{try{return JSON.parse(o.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),k=c.logo_url||"",A=c.logo_alt||"",q=c.logo_text||"Logo",w=c.logo_href||"/",y=JSON.parse(JSON.stringify(c.top_actions||U.top_actions)),m=JSON.parse(JSON.stringify(c.banking_btn||U.banking_btn)),f=JSON.parse(JSON.stringify(c.nav_links||U.nav_links)),x=JSON.parse(JSON.stringify(c.bottom_cta||U.bottom_cta)),E=document.createElement("div");E.id="navbar-config-modal",E.className="nb-overlay";const t=document.createElement("div");t.className="nb-modal",t.innerHTML=`
        ...
        <div class="nb-modal-body">
            <div class="nb-tab-panel active" id="nb-panel-logo">
                ...
                <div class="nb-card">
                    ...
                    <div class="nb-row">
                        <input id="nb-logo-alt"  type="text" placeholder="Texto alternativo"      value="${A}"  class="nb-input">
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${e.href||"#"}" data-field="href">
                            ...
                            </div>`,h.querySelector("[data-col-field='badge']").addEventListener("input",v=>{s.badge=v.target.value}),h.querySelectorAll("[data-col-color]").forEach(v=>{v.addEventListener("click",()=>{s.badge_color=v.dataset.colColor,h.querySelectorAll("[data-col-color]").forEach(B=>{B.classList.toggle("nb-color-inactive",B.dataset.colColor!==v.dataset.colColor)})})}),h.querySelector(".nb-remove-col").onclick=()=>{e.columns.splice(L,1),g();const v=i.querySelector(".nb-add-col-btn");v&&(v.disabled=!1,v.style.opacity="",v.style.cursor="")},h.querySelectorAll("[data-item-field]").forEach(v=>{const B=parseInt(v.closest("[data-item-idx]").dataset.itemIdx);v.addEventListener("input",()=>{s.items[B][v.dataset.itemField]=v.value})}),h.querySelectorAll(".nb-url-input").forEach(v=>{const B=document.createElement("div");B.style.position="relative",v.parentNode.insertBefore(B,v),B.appendChild(v),d(v)}),h.querySelectorAll(".nb-remove-item").forEach(v=>{v.onclick=()=>{const B=parseInt(v.closest("[data-item-idx]").dataset.itemIdx);s.items.splice(B,1),g()}}),h.querySelector(".nb-add-item-btn").onclick=()=>{s.items=s.items||[],s.items.push({label:"Nuevo item",desc:"",href:"#"}),g()},p.appendChild(h)})};i.innerHTML=`
            ...
            `,c.appendChild(k)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new ne;let o=document.getElementById("navbar-id")?.value||"",a=document.getElementById("navbar-name")?.value||"",c=document.getElementById("navbar-load-url")?.value||"",k=document.getElementById("navbar-store-url")?.value||"";const A=document.getElementById("navbar-is-active")?.value==="1";let q=!!o;const w=re();if(se(w),w.on("load",()=>{W(w),X(),Y(),K(w),Q(w),Z(w),ee(w),be(w),setTimeout(()=>{w.runCommand("sw-visibility"),w.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),q&&c)try{await r.loadPageContent(w,c),M("Navbar cargado correctamente","success")}catch{M("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const m=document.getElementById("save-button");m.disabled=!0,m.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!q&&!a?te({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async f=>{if(!f?.trim()){M("El nombre es obligatorio","error"),m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await y(f)}catch(x){M(x.message,"error")}finally{m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await y(a),m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(f){M(f.message,"error"),m.disabled=!1,m.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function y(m){const f=q?"PUT":"POST",x=r.getEditorContent(w);(!x.js_content||x.js_content.trim()==="")&&(x.js_content=ie);const E=await r.savePage(w,{...x,name:m,is_active:A},k,f);if(E.success){if(r.markAsClean(),M(E.message,"success"),!q&&E.navbar){o=E.navbar.id,a=E.navbar.name,q=!0;const t=document.getElementById("navbar-id");t&&(t.value=o);const z=document.getElementById("navbar-name");z&&(z.value=a);const O=document.querySelector('meta[name="app-url"]'),H=O?O.content:"";k=k.endsWith("/navbars")?`${k}/${o}`:`${k.replace(/\/navbars\/?$/,"")}/navbars/${o}`;const C=document.getElementById("navbar-store-url");C&&(C.value=k),c=`${k}/load`;const S=document.getElementById("navbar-load-url");S&&(S.value=c);const $=document.getElementById("editor-title");$&&($.textContent=`Editando Navbar: ${a}`);const N=`/navbars/edit/${o}/edit`,j=H?`${H}${N}`:N;window.history.replaceState({path:j},"",j)}else if(m){a=m;const t=document.getElementById("navbar-name");t&&(t.value=a);const z=document.getElementById("editor-title");z&&(z.textContent=`Editando Navbar: ${a}`)}}}});function be(r){r.Commands.add("canvas-clear",{run:o=>{oe({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{o.DomComponents.clear(),o.CssComposer.clear()}})}})}function M(r,o="info"){typeof window.showNotification=="function"&&window.showNotification(r,o)}
