...
`);return`<section class="pc-section" data-autoplay="${r}">
    ...
    <rect x="27.5" y="20" width="2.5" height="2.5" rx="1" fill="#003B71" fill-opacity="0.5"/>
</svg>`;function de(i){try{const e=i.Canvas.getFrameEl()?.contentDocument;if(!e)return;const t=e.getElementById("pc-runtime-script");t&&t.remove();const r=e.createElement("script");r.id="pc-runtime-script",r.textContent=kt,e.head.appendChild(r)}catch(e){console.warn("[ProductCards] Error inyectando script en canvas:",e)}}function Ct(i){i.on("load",()=>{const e=i.Canvas.getFrameEl();if(!e)return;const t=e.contentDocument,r=t?.head;if(r&&!r.querySelector("#pc-canvas-styles")){const a=t.createElement("style");a.id="pc-canvas-styles",a.textContent=`
            ...
            `,r.appendChild(a)}})}function Lt(i){const e="product-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección de Productos",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!0,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,propagate:["selectable","hoverable","editable","highlightable","draggable","droppable"],attributes:{"data-gjs-type":e,"data-product-cards-config":JSON.stringify(M)},components:qe(M),traits:[{type:"button",label:"Productos",text:"Administrar Sección",full:!0,command:"open-product-cards-config"}],toolbar:[]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),i.Commands.add("open-product-cards-config",{run(t){const r=t.getSelected();r&&Et(t,r)}}),i.BlockManager.add("product-cards-block",{label:"Sección de productos",category:"Productos y Servicios",media:$t,activate:!0,content:{type:e,attributes:{"data-gjs-type":e}}}),i.on("component:mount",t=>{t.getEl()?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>de(i),400))}),i.on("component:selected",t=>{const r=t.getEl();if(!r)return;const a=r.closest(`[data-gjs-type="${e}"]`);if(a&&!r.hasAttribute("data-gjs-type")){const l=i.getWrapper().find(`[data-gjs-type="${e}"]`).find(c=>c.getEl()===a);l&&setTimeout(()=>i.select(l),0)}}),i.on("canvas:render",()=>{setTimeout(()=>de(i),600)}),i.on("storage:end:load",()=>{setTimeout(()=>de(i),800)}),Ct(i)}const St=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,zt=[{id:"cta-section",label:"Llamada a la acción",category:"Llamadas a la acción",media:_t,content:`
...
${It}`}],Tt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Ot=[{id:"icon-links-strip",label:"Tira de iconos con enlace",category:"Productos y Servicios",media:ke,content:`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function Ci(){if(document.getElementById("tam-img-modal"))return;const i=document.createElement("div");i.id="tam-img-modal",i.innerHTML=`
        ...
        <div class="tam-container">
        ...
        </div>`,document.body.appendChild(r);let a=null,l=null;function c(m){a=m;const n=m.get("tableData");l=n?JSON.parse(JSON.stringify(n)):Z(3,3);const g=l.cols||3;l.rows=l.rows.map((u,j)=>Array.from({length:g},($,z)=>u[z]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})),document.getElementById("tam-title").value=l.title||"",document.getElementById("tam-theme").value=m.get("tableTheme")||"blue",document.getElementById("tam-cols").value=l.cols||3,document.getElementById("tam-rows").value=l.rows.length||3,b(),w(),r.classList.add("open"),document.body.style.overflow="hidden"}function d(){r.classList.remove("open"),document.body.style.overflow="",a=null}function h(){l.title=document.getElementById("tam-title").value.trim(),l.cols=parseInt(document.getElementById("tam-cols").value)||3,l.headers=Array.from(document.querySelectorAll(".tam-header-input")).map(n=>({text:n.value,align:n.closest("th")?.querySelector(".tam-align-select")?.value||"center"})),document.querySelectorAll("#tam-tbody td.tam-cell:not(.is-spanned)").forEach(n=>{const g=parseInt(n.dataset.row),u=parseInt(n.dataset.col);l.rows[g]?.[u]&&(l.rows[g][u].text=n.querySelector(".tam-cell-input")?.value||"",l.rows[g][u].align=n.querySelector(".tam-align-select")?.value||"center",l.rows[g][u].isHeader=n.dataset.isheader==="1",l.rows[g][u].image=n.dataset.image||null)});const m=Se(l.rows,l.cols);l.rows=l.rows.map((n,g)=>n.filter((u,j)=>!m[`${g}-${j}`]))}function f(){if(r.querySelector("#tam-rebuild-notice"))return;const n=document.createElement("div");n.id="tam-rebuild-notice",n.style.cssText="background:#fef9c3;border:1.5px solid #ca8a04;border-radius:0.5rem;padding:0.5rem 1rem;font-size:0.8rem;color:#92400e;display:flex;align-items:center;gap:0.5rem;margin:0 1.5rem 0;flex-shrink:0;",n.innerHTML='<i class="ri-error-warning-line"></i> Has modificado el colspan/rowspan. Haz clic en <strong style="margin:0 4px;">Reconstruir</strong> para aplicar los cambios de fusión.',r.querySelector(".tam-toolbar").after(n)}function b(){r.querySelector("#tam-rebuild-notice")?.remove()}function w(){const m=document.getElementById("tam-thead"),n=document.getElementById("tam-tbody"),g=l.cols,u=l.rows.length,j=Se(l.rows,g);m.innerHTML=`<tr>${l.headers.map((v,$)=>`
            <th>
            ...
            </th>`).join("")}</tr>`,n.innerHTML=l.rows.map((v,$)=>`<tr>${Array.from({length:g},(C,B)=>{const _=j[`${$}-${B}`];if(_)return`<td class="tam-cell is-spanned" data-row="${$}" data-col="${B}">
                        <div class="tam-spanned-label">Combinada con [${_}]</div>
                    </td>`;const x=v[B]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null},S=x.colspan||1,P=x.rowspan||1,T=S>1||P>1;return`<td class="tam-cell ${x.isHeader?"is-header-cell":""} ${x.image?"has-image":""} ${T?"has-span":""}"
                    data-row="${$}" data-col="${B}"
...
</svg>`;function Ji(){return function(){const i=this,e="agencies-map-component",t="/api/agencies/active",r="agencies";let a=[],l=[],c={},d=null,h=[];const f=async()=>{try{b(),await m(),await u(),j(),_(),T(),Y(),w()}catch(o){console.error("Error initializing map:",o),ae("Error al cargar las agencias"),w()}};function b(){const o=i.querySelector(`.${e}-list`);o&&(o.innerHTML=`
...
@media(max-width:480px){
    .banner-btn{flex:1 1 100%;min-width:0;}
            ...
            `;const o=i.ownerDocument??document;if(!o.getElementById("banner-skeleton-styles")){const s=o.createElement("style");s.id="banner-skeleton-styles",s.textContent=`
                ...
                `,o.head.appendChild(s)}}function j(o){return t[o]?t[o]:o==="outline-blue"||o==="outline-orange"?t["outline-white"]:t["fill-white"]}function v(o,s,p,y){const E=j(p),q=s?"a":"span",F=s?`href="${s}"${y?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${q} ${F}
                class="banner-btn"
                data-bg="${E.bg}"
                data-color="${E.color}"
            ...
            `).join(""),z(),C(),B()}function z(){i.querySelectorAll(".banner-btn").forEach(o=>{const s=o.style.borderColor;o.addEventListener("mouseenter",()=>{o.style.background=o.dataset.hoverBg,o.style.color=o.dataset.hoverColor,o.style.borderColor=o.dataset.hoverBg}),o.addEventListener("mouseleave",()=>{o.style.background=o.dataset.bg,o.style.color=o.dataset.color,o.style.borderColor=s})})}function C(){n.addEventListener("mousedown",_),n.addEventListener("touchstart",_,{passive:!0}),n.addEventListener("mousemove",x),n.addEventListener("touchmove",x,{passive:!0}),n.addEventListener("mouseup",S),n.addEventListener("touchend",S),n.addEventListener("mouseleave",S)}function B(){a.forEach(o=>{const s=new Image;s.src=o.image_url})}function _(o){d=!0,h=o.touches?o.touches[0].clientX:o.clientX,f=0}function x(o){d&&(f=(o.touches?o.touches[0].clientX:o.clientX)-h)}function S(){d&&(d=!1,Math.abs(f)>=b&&(T(f<0?(l+1)%a.length:(l-1+a.length)%a.length),H()),f=0)}function P(){const o=i.querySelector(".banner-stripe");if(!o||a.length<=1)return;const s=document.createElement("div");s.className="banner-dots",a.forEach((p,y)=>{const E=document.createElement("button");E.className="banner-dot",E.dataset.index=String(y),E.setAttribute("aria-label",`Banner ${y+1}`),E.addEventListener("click",()=>{T(y),H()}),s.appendChild(E)}),o.innerHTML="",o.appendChild(s)}function T(o,s=!0){const p=n.querySelectorAll(".banner-slide"),y=i.querySelectorAll(".banner-dot");p.forEach((E,q)=>{const F=q===o;s||(E.style.transition="none"),E.classList.toggle("banner-slide--active",F),s||requestAnimationFrame(()=>{E.style.transition=""})}),y.forEach((E,q)=>{E.classList.toggle("banner-dot--active",q===o)}),l=o}function Y(){a.length<=1||!w||(c=setInterval(()=>{T((l+1)%a.length)},5e3))}function H(){w&&(clearInterval(c),Y())}function ae(){const o=i.querySelector(".banner-wrapper");o&&(o.innerHTML=`
