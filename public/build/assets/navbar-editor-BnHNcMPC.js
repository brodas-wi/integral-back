...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function le(r){return r?r.split(" ").filter(t=>t.length>0).map((t,o)=>`<span style="color:${o%2===0?"#E97300":"#003B71"};">${t}</span>`).join(" "):""}function O(r,t){t=t||"nb"+Math.random().toString(36).slice(2,7);const o=r.logo_href||"/",s=r.logo_url?`<img src="${r.logo_url}" alt="${r.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${r.logo_text||"Logo"}</span>`,k=`<a href="${o}" class="nb-logo-link">${s}</a>`,B=(r.top_actions||[]).map(c=>{const q=c.icon?`<i class="${c.icon}"></i>`:"";return`<a href="${c.href||"#"}" class="nb-top-action">${q}${c.label||""}</a>`}).join(""),C=r.banking_btn||{},w=C.color==="orange"?"nb-banking-orange":"nb-banking-blue",v=`<a href="${C.href||"#"}" class="nb-banking-btn ${w}">${C.label||"Banca en Línea"}</a>`,b=(r.nav_links||[]).map(c=>{if(c.type==="submenu"&&c.columns?.length){const q=!!(c.cta_column&&(c.cta_column.text||c.cta_column.btn_label)),S=c.columns.slice(0,3),$=S.map(_=>{const j=_.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",n=_.badge?`<span class="nb-mega-badge ${j}">${_.badge}</span>`:"",e=(_.items||[]).map(d=>{const l=d.desc?`<span class="nb-mega-item-desc">${d.desc}</span>`:"";return`<a href="${d.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${d.label||""}</span>${l}</a>`}).join("");return`<div class="nb-mega-col">${n}${e}</div>`}).join("");let I="";if(q){const _=c.cta_column,j=_.btn_color==="blue"?"nb-badge-blue":"nb-badge-orange";I=`<div class="nb-mega-cta-col">
                ...
                </div>`}return`<a href="${c.href||"#"}" class="nb-mobile-link">${c.label||""}</a>`}).join(""),H=[v,p.label?`<a href="${p.href||"#"}" class="nb-bottom-cta ${x}"><span class="nb-bottom-cta-line1">${p.label}</span>${E}</a>`:""].filter(Boolean).join("");return`<div id="nb-root-${t}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false">
        <div class="nb-top">
        ...
        `,document.head.appendChild(n)}const s=(()=>{try{return JSON.parse(t.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),k=s.logo_url||"",B=s.logo_alt||"",C=s.logo_text||"Logo",w=s.logo_href||"/",v=JSON.parse(JSON.stringify(s.top_actions||U.top_actions)),b=JSON.parse(JSON.stringify(s.banking_btn||U.banking_btn)),p=JSON.parse(JSON.stringify(s.nav_links||U.nav_links)),x=JSON.parse(JSON.stringify(s.bottom_cta||U.bottom_cta)),E=document.createElement("div");E.id="navbar-config-modal",E.className="nb-overlay";const a=document.createElement("div");a.className="nb-modal",a.innerHTML=`
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,E.appendChild(a),document.body.appendChild(E);const z=new G;a.querySelectorAll(".nb-tab-btn").forEach(n=>{n.addEventListener("click",()=>{a.querySelectorAll(".nb-tab-btn").forEach(e=>e.classList.remove("active")),a.querySelectorAll(".nb-tab-panel").forEach(e=>e.classList.remove("active")),n.classList.add("active"),a.querySelector(`#nb-panel-${n.dataset.tab}`).classList.add("active")})}),a.querySelector("#nb-logo-pick").addEventListener("click",()=>{oe({type:"image",title:"Seleccionar logo",onSelect:n=>{a.querySelector("#nb-logo-url").value=n;let e=a.querySelector("#nb-logo-preview");if(!e||e.tagName==="DIV"){const d=document.createElement("img");d.id="nb-logo-preview",d.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;",e?.replaceWith(d)??a.querySelector("#nb-logo-url").before(d),e=d}e.src=n,e.style.display="block"}})});const H=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function c(n){if(n.dataset.autocompleteAttached)return;n.dataset.autocompleteAttached="true";const e=n.parentNode;(!e.style.position||e.style.position==="static")&&(e.style.position="relative");const d=document.createElement("ul");d.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",e.appendChild(d);let l=null;async function h(g){if(g.length<1){d.style.display="none";return}try{const L=await(await fetch(`${H}?q=${encodeURIComponent(g)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();u(L,g)}catch{d.style.display="none"}}function m(g,i){return i?g.replace(new RegExp(`(${i.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):g}function u(g,i){if(d.innerHTML="",!g.length){d.style.display="none";return}g.forEach(L=>{const y=document.createElement("li");y.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",y.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${m(L.title,i)}</span><span style="font-size:0.7rem;color:#64748b;">/${L.slug}</span>`,y.addEventListener("mouseenter",()=>y.style.background="#f1f5f9"),y.addEventListener("mouseleave",()=>y.style.background=""),y.addEventListener("mousedown",N=>{N.preventDefault(),n.value="/"+L.slug,n.dispatchEvent(new Event("input")),d.style.display="none"}),d.appendChild(y)}),d.style.display="block"}n.addEventListener("input",()=>{clearTimeout(l),l=setTimeout(()=>h(n.value.trim()),220)}),n.addEventListener("focus",()=>{n.select(),n.value.trim()&&h(n.value.trim())}),n.addEventListener("blur",()=>{setTimeout(()=>{d.style.display="none"},150)}),n.addEventListener("keydown",g=>{if(d.style.display==="none")return;const i=d.querySelectorAll("li"),L=d.querySelector("li.nb-ac-active");let y=Array.from(i).indexOf(L);if(g.key==="ArrowDown"){g.preventDefault(),L?.classList.remove("nb-ac-active");const N=i[y+1]||i[0];N?.classList.add("nb-ac-active"),N&&(N.style.background="#f1f5f9")}else if(g.key==="ArrowUp"){g.preventDefault(),L?.classList.remove("nb-ac-active");const N=i[y-1]||i[i.length-1];N?.classList.add("nb-ac-active"),N&&(N.style.background="#f1f5f9")}else g.key==="Enter"&&L?(g.preventDefault(),L.dispatchEvent(new MouseEvent("mousedown"))):g.key==="Escape"&&(d.style.display="none")})}function q(n,e,d){let l=null;n.querySelectorAll("[data-drag-idx]").forEach(h=>{h.setAttribute("draggable","true"),h.addEventListener("dragstart",m=>{l=parseInt(h.dataset.dragIdx),setTimeout(()=>h.classList.add("nb-dragging"),0),m.dataTransfer.effectAllowed="move"}),h.addEventListener("dragend",()=>{h.classList.remove("nb-dragging"),n.querySelectorAll(".nb-drag-over").forEach(m=>m.classList.remove("nb-drag-over"))}),h.addEventListener("dragover",m=>{m.preventDefault(),m.dataTransfer.dropEffect="move",parseInt(h.dataset.dragIdx)!==l&&h.classList.add("nb-drag-over")}),h.addEventListener("dragleave",()=>h.classList.remove("nb-drag-over")),h.addEventListener("drop",m=>{m.preventDefault();const u=parseInt(h.dataset.dragIdx);if(l!==null&&u!==l){const[g]=e.splice(l,1);e.splice(u,0,g),d()}l=null})})}function S(){const n=a.querySelector("#nb-top-actions-list");n.innerHTML="",v.forEach((e,d)=>{const l=document.createElement("div");l.className="nb-action-card",l.dataset.dragIdx=d;const h=e.icon||"";l.innerHTML=`
                <div class="nb-row">
                    <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                    <div class="nb-icon-preview"><i class="${h||"ri-star-line"}"></i></div>
                    <input class="nb-input-sm" style="width:130px;flex-shrink:0;" placeholder="Clase del icono" value="${h}" data-field="icon" readonly>
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${e.href||"#"}" data-field="href">
                </div>`,l.querySelector(".nb-pick-icon-btn").addEventListener("click",()=>{z.open(u=>{e.icon=u,l.querySelector("[data-field='icon']").value=u,l.querySelector(".nb-icon-preview i").className=u})}),l.querySelector(".nb-remove-action").onclick=()=>{v.splice(d,1),S()},l.querySelectorAll("[data-field]").forEach(u=>{u.addEventListener("input",()=>{e[u.dataset.field]=u.value})});const m=l.querySelector(".nb-url-input");if(m){const u=document.createElement("div");u.style.position="relative",m.parentNode.insertBefore(u,m),u.appendChild(m),c(m)}n.appendChild(l)}),q(n,v,S)}function $(){const n=a.querySelector("#nb-nav-list");n.innerHTML="",p.forEach((e,d)=>{const l=document.createElement("div");l.className="nb-link-card",l.dataset.dragIdx=d;const h=`<span class="nb-type-badge ${e.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${e.type==="submenu"?"Submenú":"Link"} ↕</span>`;if(e.type==="submenu"){let g=function(){u.innerHTML="",(e.columns||[]).forEach((i,L)=>{const y=document.createElement("div");y.className="nb-col-card";const N=(i.items||[]).map((f,A)=>`
                            ...
                            </div>`,y.querySelector("[data-col-field='badge']").addEventListener("input",f=>{i.badge=f.target.value}),y.querySelectorAll("[data-col-color]").forEach(f=>{f.addEventListener("click",()=>{i.badge_color=f.dataset.colColor,y.querySelectorAll("[data-col-color]").forEach(A=>{A.classList.toggle("nb-color-inactive",A.dataset.colColor!==f.dataset.colColor)})})}),y.querySelector(".nb-remove-col").onclick=()=>{e.columns.splice(L,1),g();const f=l.querySelector(".nb-add-col-btn");f&&(f.disabled=!1,f.style.opacity="",f.style.cursor="")},y.querySelectorAll("[data-item-field]").forEach(f=>{const A=parseInt(f.closest("[data-item-idx]").dataset.itemIdx);f.addEventListener("input",()=>{i.items[A][f.dataset.itemField]=f.value})}),y.querySelectorAll(".nb-url-input").forEach(f=>{const A=document.createElement("div");A.style.position="relative",f.parentNode.insertBefore(A,f),A.appendChild(f),c(f)}),y.querySelectorAll(".nb-remove-item").forEach(f=>{f.onclick=()=>{const A=parseInt(f.closest("[data-item-idx]").dataset.itemIdx);i.items.splice(A,1),g()}}),y.querySelector(".nb-add-item-btn").onclick=()=>{i.items=i.items||[],i.items.push({label:"Nuevo item",desc:"",href:"#"}),g()},u.appendChild(y)})};l.innerHTML=`
                    ...
                    </div>`,l.querySelector("[data-toggle-type]").onclick=()=>{e.type="submenu",e.columns=[{badge:"Columna 1",badge_color:"blue",items:[{label:"Enlace",desc:"",href:"#"}]}],e.cta_column={text:"",btn_label:"Ver más",btn_href:"#",btn_color:"orange"},delete e.href,$()},l.querySelector(".nb-remove-link").onclick=()=>{p.splice(d,1),$()},l.querySelectorAll("[data-field]").forEach(u=>{u.addEventListener("input",()=>{e[u.dataset.field]=u.value})});const m=l.querySelector(".nb-url-input");if(m){const u=document.createElement("div");u.style.position="relative",m.parentNode.insertBefore(u,m),u.appendChild(m),c(m)}}n.appendChild(l)}),q(n,p,$)}a.querySelectorAll("#nb-banking-blue, #nb-banking-orange").forEach(n=>{n.addEventListener("click",()=>{b.color=n.dataset.color,a.querySelector("#nb-banking-blue").classList.toggle("nb-color-inactive",b.color!=="blue"),a.querySelector("#nb-banking-orange").classList.toggle("nb-color-inactive",b.color!=="orange")})}),a.querySelectorAll("#nb-bcta-blue, #nb-bcta-orange").forEach(n=>{n.addEventListener("click",()=>{x.color=n.dataset.color,a.querySelector("#nb-bcta-blue").classList.toggle("nb-color-inactive",x.color!=="blue"),a.querySelector("#nb-bcta-orange").classList.toggle("nb-color-inactive",x.color!=="orange")})});const I=a.querySelector("#nb-banking-href");if(I){const n=document.createElement("div");n.style.position="relative",I.parentNode.insertBefore(n,I),n.appendChild(I),c(I)}const T=a.querySelector("#nb-bcta-href");if(T){const n=document.createElement("div");n.style.position="relative",T.parentNode.insertBefore(n,T),n.appendChild(T),c(T)}const _=a.querySelector("#nb-logo-href");if(_){const n=document.createElement("div");n.style.position="relative",_.parentNode.insertBefore(n,_),n.appendChild(_),c(_)}a.querySelector("#nb-add-top-action").onclick=()=>{v.push({label:"Nueva acción",href:"#",icon:"ri-star-line"}),S(),a.querySelector("#nb-top-actions-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},a.querySelector("#nb-add-nav-link").onclick=()=>{p.push({type:"link",label:"Nuevo enlace",href:"#"}),$(),a.querySelector("#nb-nav-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},a.querySelector("#nb-add-nav-submenu").onclick=()=>{p.push({type:"submenu",label:"Nuevo menú",columns:[{badge:"Columna 1",badge_color:"blue",items:[{label:"Enlace",desc:"",href:"#"}]}],cta_column:{text:"",btn_label:"Ver más",btn_href:"#",btn_color:"orange"}}),$(),a.querySelector("#nb-nav-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},S(),$();const j=()=>{z.close(),E.remove()};a.querySelector("#nb-modal-close").onclick=j,a.querySelector("#nb-modal-cancel").onclick=j,E.onclick=n=>{n.target===E&&j()},a.querySelector("#nb-modal-save").onclick=()=>{const n={logo_url:a.querySelector("#nb-logo-url").value.trim(),logo_alt:a.querySelector("#nb-logo-alt").value.trim(),logo_text:a.querySelector("#nb-logo-text").value.trim(),logo_href:a.querySelector("#nb-logo-href").value.trim()||"/",top_actions:v,banking_btn:{label:a.querySelector("#nb-banking-label").value.trim(),href:a.querySelector("#nb-banking-href").value.trim()||"#",color:b.color||"blue"},nav_links:p,bottom_cta:{label:a.querySelector("#nb-bcta-label").value.trim(),sublabel:a.querySelector("#nb-bcta-sublabel").value.trim(),href:a.querySelector("#nb-bcta-href").value.trim()||"#",color:x.color||"orange"}},d=t.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);t.addAttributes({"data-navbar-config":JSON.stringify(n)}),t.components(O(n,d)+R),j()}}function se(r){const t="navbar-component";r.DomComponents.addType(t,{isComponent:o=>o.getAttribute?.("data-gjs-type")===t?{type:t}:!1,model:{defaults:{name:"Navbar",tagName:"nav",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":t,class:"nb-wrapper","data-navbar-config":JSON.stringify(U)},components:O(U)+R,script:function(){(function(o){if(!o||typeof o.querySelector!="function"||o.__nbInit)return;o.__nbInit=!0;var s=o.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");if(!s)return;var k=!!window.__gjseditor||document.documentElement.hasAttribute("data-gjs-canvas");function B(){k||(document.body.style.paddingTop=o.offsetHeight+"px")}B(),window.addEventListener("resize",function(){if(B(),window.innerWidth>992){var v=document.getElementById("nb-mobile-"+s);v&&v.classList.contains("nb-open")&&v.classList.remove("nb-open"),o.querySelectorAll(".nb-mobile-item.nb-open").forEach(function(b){b.classList.remove("nb-open")})}});var C=document.getElementById("nb-toggle-"+s),w=document.getElementById("nb-mobile-"+s);C&&w&&C.addEventListener("click",function(){w.classList.toggle("nb-open"),B()}),o.querySelectorAll(".nb-nav-trigger").forEach(function(v){v.addEventListener("click",function(b){b.stopPropagation();var p=v.closest(".nb-nav-item"),x=p.classList.contains("nb-open");o.querySelectorAll(".nb-nav-item.nb-open").forEach(function(E){E.classList.remove("nb-open")}),x||p.classList.add("nb-open"),B()})}),o.querySelectorAll(".nb-mobile-item>.nb-mobile-link").forEach(function(v){v.addEventListener("click",function(){v.closest(".nb-mobile-item").classList.toggle("nb-open"),B()})}),document.addEventListener("click",function(v){o.contains(v.target)||o.querySelectorAll(".nb-nav-item.nb-open").forEach(function(b){b.classList.remove("nb-open")})})})(this)},"script-props":[],toolbar:[],traits:[{type:"button",label:"Navbar",text:"Administrar Navbar",full:!0,command:"open-navbar-config"}]},init(){this.set("type",t),this.addAttributes({"data-gjs-type":t})}}}),r.Commands.add("open-navbar-config",{run(o){const s=o.getSelected();s&&ie(o,s)}}),r.BlockManager.add("navbar-block",{label:"Navbar",category:"Navbar",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,activate:!0,content:{type:t,attributes:{"data-gjs-type":t}}}),ce(r,t),de(r)}function ce(r,t){r.on("storage:end:load",()=>{setTimeout(()=>P(r,t),800)}),r.on("component:mount",o=>{const s=o.getEl();s?.getAttribute?.("data-gjs-type")===t&&(o.set("type",t),setTimeout(()=>J(r,s),400))}),r.on("canvas:render",()=>{setTimeout(()=>P(r,t),600)})}function J(r,t){if(t?.isConnected)try{const o=r.Canvas.getFrameEl()?.contentDocument;if(!o)return;t.__nbInit&&delete t.__nbInit;const s=o.createElement("script");s.textContent=`(function(){
            function initNavbar(root){
                if(!root||root.__nbInit)return;
                root.__nbInit=true;
                var id=root.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");
        ...
        })();`,o.head.appendChild(s),s.remove()}catch(o){console.warn("[Navbar] Error inyectando script en canvas:",o)}}function P(r,t){r.getWrapper().find(`[data-gjs-type="${t}"]`).forEach(o=>{o.set("type",t);const s=o.getEl();s?.isConnected&&J(r,s)})}function de(r){r.on("load",()=>{const t=r.Canvas.getFrameEl();if(!t)return;const o=t.contentDocument,s=o?.head;if(s&&(o.documentElement?.setAttribute("data-gjs-canvas","true"),!s.querySelector("#navbar-component-css"))){const k=document.createElement("style");k.id="navbar-component-css",k.textContent=`
            ...
            `,s.appendChild(k)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new ee;let t=document.getElementById("navbar-id")?.value||"",o=document.getElementById("navbar-name")?.value||"",s=document.getElementById("navbar-load-url")?.value||"",k=document.getElementById("navbar-store-url")?.value||"";const B=document.getElementById("navbar-is-active")?.value==="1";let C=!!t;const w=ae();if(se(w),w.on("load",()=>{V(w),W(),X(),Y(w),K(w),Q(w),Z(w),be(w),setTimeout(()=>{w.runCommand("sw-visibility"),w.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),C&&s)try{await r.loadPageContent(w,s),M("Navbar cargado correctamente","success")}catch{M("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const b=document.getElementById("save-button");b.disabled=!0,b.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!C&&!o?ne({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async p=>{if(!p?.trim()){M("El nombre es obligatorio","error"),b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await v(p)}catch(x){M(x.message,"error")}finally{b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await v(o),b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(p){M(p.message,"error"),b.disabled=!1,b.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function v(b){const p=C?"PUT":"POST",x=r.getEditorContent(w);(!x.js_content||x.js_content.trim()==="")&&(x.js_content=re);const E=await r.savePage(w,{...x,name:b,is_active:B},k,p);if(E.success){if(r.markAsClean(),M(E.message,"success"),!C&&E.navbar){t=E.navbar.id,o=E.navbar.name,C=!0;const a=document.getElementById("navbar-id");a&&(a.value=t);const z=document.getElementById("navbar-name");z&&(z.value=o);const D=document.querySelector('meta[name="app-url"]'),H=D?D.content:"";k=k.endsWith("/navbars")?`${k}/${t}`:`${k.replace(/\/navbars\/?$/,"")}/navbars/${t}`;const q=document.getElementById("navbar-store-url");q&&(q.value=k),s=`${k}/load`;const S=document.getElementById("navbar-load-url");S&&(S.value=s);const $=document.getElementById("editor-title");$&&($.textContent=`Editando Navbar: ${o}`);const I=`/navbars/edit/${t}/edit`,T=H?`${H}${I}`:I;window.history.replaceState({path:T},"",T)}else if(b){o=b;const a=document.getElementById("navbar-name");a&&(a.value=o);const z=document.getElementById("editor-title");z&&(z.textContent=`Editando Navbar: ${o}`)}}}});function be(r){r.Commands.add("canvas-clear",{run:t=>{te({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{t.DomComponents.clear(),t.CssComposer.clear()}})}})}function M(r,t="info"){typeof window.showNotification=="function"&&window.showNotification(r,t)}
