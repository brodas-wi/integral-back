...
@media(max-width:480px){.pc-card{flex:0 0 80vw;}}`;function li(t){const e=t.img||P("images/placeholder.svg"),i=t.title||"TÍTULO DEL PRODUCTO",a=t.desc||"Descripción breve del producto financiero.",r=t.href||"#",l=t.btn_label||"Solicitar";return`<div class="pc-card"><div class="pc-card-img-wrap"><img src="${e}" alt="${i}" class="pc-card-img"></div><div class="pc-card-body"><h3 class="pc-card-title">${i}</h3><p class="pc-card-desc">${a}</p></div><a href="${r}" class="pc-btn">${l}</a></div>`}function ct(t){const e=t.heading||"Créditos",i=t.subheading||"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",a=t.more_href||"#",r=t.more_label||"Ver más",l=t.show_more!==!1,n=(t.cards||[]).map(li).join(""),f=l?`<div class="pc-more-wrap"><a href="${a}" class="pc-more-btn">${r}</a></div>`:"";return`<section class="pc-section"><style>${ri}</style><div style="text-align:center;margin-bottom:2rem;"><h2 class="pc-section-heading">${e}</h2><p class="pc-section-subheading">${i}</p></div><div class="pc-carousel-wrap"><div class="pc-track">${n}</div></div>${f}</section>`}const Y={heading:"Créditos",subheading:"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",more_href:"#",more_label:"Ver más",cards:[{img:"",title:"CREDINVIERTE",desc:"Adquiere activos fijos",href:"#",btn_label:"Solicitar"},{img:"",title:"SOLUCIONES INTEGRALES",desc:"Financiamiento PYME",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDI-CONFIAMOS",desc:"Rápido y sin fiador",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDILÍNEA",desc:"Línea rotativa",href:"#",btn_label:"Solicitar"}]};function oi(t,e){const i=document.getElementById("pc-config-modal");if(i&&i.remove(),!document.getElementById("pc-modal-styles")){const s=document.createElement("style");s.id="pc-modal-styles",s.textContent=`
        ...
        </div>`,l.appendChild(o),document.body.appendChild(l),o.querySelectorAll(".pc-tab-btn").forEach(s=>{s.addEventListener("click",()=>{o.querySelectorAll(".pc-tab-btn").forEach(c=>c.classList.remove("active")),o.querySelectorAll(".pc-tab-panel").forEach(c=>c.classList.remove("active")),s.classList.add("active"),o.querySelector(`#pc-panel-${s.dataset.tab}`).classList.add("active")})}),o.querySelector("#pc-show-more").addEventListener("change",function(){r.show_more=this.checked,o.querySelector("#pc-more-fields").style.display=this.checked?"flex":"none"});function n(){const s=o.querySelector("#pc-cards-list");s.innerHTML="",r.cards.forEach((c,b)=>{const g=document.createElement("div");g.className="pc-card-config";const x=c.img?`<img class="pc-img-preview" src="${c.img}" alt="">`:'<div class="pc-img-placeholder"><i class="ri-image-line"></i></div>';g.innerHTML=`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function Na(){if(document.getElementById("tam-img-modal"))return;const t=document.createElement("div");t.id="tam-img-modal",t.innerHTML=`
        ...
        <div class="tam-container">
            ...
            <th>
                ...
                </td>`}).join("")}</tr>`).join(""),x.querySelectorAll("input[data-action=colspan], input[data-action=rowspan]").forEach(B=>{B.addEventListener("change",()=>{const y=parseInt(B.dataset.row),h=parseInt(B.dataset.col),d=Math.max(1,parseInt(B.value)||1);l.rows[y]?.[h]&&(B.dataset.action==="colspan"?l.rows[y][h].colspan=Math.min(d,m-h):l.rows[y][h].rowspan=Math.min(d,E-y),s())})}),x.querySelectorAll("button[data-action]").forEach(B=>{B.addEventListener("click",y=>{y.preventDefault(),y.stopPropagation();const h=B.dataset.action,d=parseInt(B.dataset.row),u=parseInt(B.dataset.col);if(!(isNaN(d)||isNaN(u)||!l.rows[d]?.[u])){if(h==="header"){l.rows[d][u].isHeader=!l.rows[d][u].isHeader;const S=x.querySelector(`td[data-row="${d}"][data-col="${u}"]`);S&&(S.dataset.isheader=l.rows[d][u].isHeader?"1":"0",S.classList.toggle("is-header-cell",l.rows[d][u].isHeader)),B.classList.toggle("active",l.rows[d][u].isHeader),B.textContent=l.rows[d][u].isHeader?"✓ Etiqueta":"Etiqueta";return}if(h==="image"){window.__openTableImagePicker&&window.__openTableImagePicker(S=>{l.rows[d][u].image=S;const v=x.querySelector(`td[data-row="${d}"][data-col="${u}"]`);if(v){v.dataset.image=S,v.classList.add("has-image");let j=v.querySelector(".tam-cell-img-preview");j||(j=document.createElement("img"),j.className="tam-cell-img-preview",v.insertBefore(j,v.firstChild)),j.src=S;const q=v.querySelector("[data-action=image]");if(q&&(q.innerHTML='<i class="ri-image-line"></i> Cambiar'),!v.querySelector("[data-action=clear-image]")){const _=document.createElement("button");_.type="button",_.className="tam-cell-btn tam-cell-btn-clear",_.dataset.action="clear-image",_.dataset.row=d,_.dataset.col=u,_.textContent="✕ Quitar",_.addEventListener("click",D=>{D.preventDefault(),D.stopPropagation(),l.rows[d][u].image=null,v.dataset.image="",v.classList.remove("has-image"),j.remove(),_.remove();const M=v.querySelector("[data-action=image]");M&&(M.innerHTML='<i class="ri-image-line"></i> Imagen')}),v.querySelector(".tam-cell-actions").appendChild(_)}}});return}h==="clear-image"&&(l.rows[d][u].image=null,b())}})})}document.getElementById("tam-close").addEventListener("click",n),document.getElementById("tam-cancel").addEventListener("click",n),a.addEventListener("click",g=>{g.target===a&&n()}),document.getElementById("tam-rebuild").addEventListener("click",()=>{const g=parseInt(document.getElementById("tam-cols").value)||3,x=parseInt(document.getElementById("tam-rows").value)||3;for(c(),f();l.headers.length<g;)l.headers.push({text:`Col ${l.headers.length+1}`,align:"center"});for(l.headers=l.headers.slice(0,g),l.cols=g;l.rows.length<x;)l.rows.push(Array.from({length:g},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})));l.rows=l.rows.slice(0,x).map(m=>{for(;m.length<g;)m.push({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null});return m.slice(0,g)}),b()}),document.getElementById("tam-apply").addEventListener("click",()=>{f();const g=document.getElementById("tam-theme").value;r&&(r.set("tableData",JSON.parse(JSON.stringify(l))),r.set("tableTheme",g),r.addAttributes({"data-table-theme":g}),ze(r)),n()}),window.__openTableAdminModal=o}function ze(t){const e=t.get("tableData"),i=t.get("tableTheme")||"blue";e&&t.components(gt(pt(e,i),i))}function Pa(){return function(){}}const Ra=[{id:"table-blue",label:"Tabla azul",category:"Interactivos",media:Ma,content:{type:"table-component",attributes:{"data-table-theme":"blue"}}},{id:"table-orange",label:"Tabla naranja",category:"Interactivos",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<a href="#"
   ...
   class="dld-simple-${e}">
    ...
    <rect x="3" y="25" width="7" height="2.5" fill="#dee2e6" rx="0.5"/>
</svg>`;function sr(){return function(){const t=this,e="agencies-map-component",i="/api/agencies/active",a="agencies";let r=[],l=[],o={},n=null,f=[];const s=async()=>{try{c(),await g(),await E(),A(),S(),_(),D(),b()}catch(p){console.error("Error initializing map:",p),V("Error al cargar las agencias"),b()}};function c(){const p=t.querySelector(`.${e}-list`);p&&(p.innerHTML=`
                ...
                `);const w=t.querySelector(`.${e}-map-container`);w&&(w.style.opacity="0.5")}function b(){const p=t.querySelector(`.${e}-map-container`);p&&(p.style.transition="opacity 0.3s ease",p.style.opacity="1")}async function g(){try{const w=await(await fetch(i)).json(),$=a?w[a]:w;Array.isArray($)?(r=$.filter(T=>T.latitude&&T.longitude&&!isNaN(T.latitude)&&!isNaN(T.longitude)),l=[...r],x()):(r=[],l=[])}catch(p){console.error("Error loading items:",p),r=[],l=[]}}function x(){const p=[...new Set(r.map($=>$.zone).filter(Boolean))].sort(),w=[...new Set(r.map($=>$.department).filter(Boolean))].sort();o={zone:p,department:w},setTimeout(()=>{const $=t.querySelector(`.${e}-filters-container`);$&&!$.hasChildNodes()&&($.innerHTML=`
...
.mp-map-wrapper{height:320px;}
        ...
        </div>`,n.appendChild(f),document.body.appendChild(n),f.querySelectorAll(".mp-tab-btn").forEach(g=>{g.addEventListener("click",()=>{f.querySelectorAll(".mp-tab-btn").forEach(x=>x.classList.remove("active")),f.querySelectorAll(".mp-tab-panel").forEach(x=>x.classList.remove("active")),g.classList.add("active"),f.querySelector(`#mp-panel-${g.dataset.tab}`).classList.add("active")})});const s=f.querySelector("#mp-filters-list");function c(){s.innerHTML="",o.forEach((g,x)=>{const m=document.createElement("div");m.className="mp-filter-card",m.innerHTML=`
                <div class="mp-filter-card-header">
                    <input class="mp-input" style="flex:1;" placeholder="Etiqueta del filtro" value="${g.label||""}" data-filter-field="label">
                    <button type="button" class="mp-btn-remove" data-remove-filter><i class="ri-delete-bin-line"></i></button>
                </div>
                <textarea class="mp-textarea" placeholder="Una opción por línea" style="min-height:70px;" data-filter-field="options">${(g.options||[]).join(`
`)}</textarea>
            `,m.querySelector("[data-filter-field='label']").addEventListener("input",E=>{g.label=E.target.value}),m.querySelector("[data-filter-field='options']").addEventListener("input",E=>{g.options=E.target.value.split(`
`).map(A=>A.trim()).filter(Boolean)}),m.querySelector("[data-remove-filter]").addEventListener("click",()=>{o.splice(x,1),c()}),s.appendChild(m)})}c(),f.querySelector("#mp-add-filter").addEventListener("click",()=>{o.push({label:"Nuevo filtro",options:["Opción 1","Opción 2"]}),c()});const b=()=>n.remove();f.querySelector("#mp-modal-close").onclick=b,f.querySelector("#mp-modal-cancel").onclick=b,n.onclick=g=>{g.target===n&&b()},f.querySelector("#mp-modal-save").onclick=()=>{const g={title:f.querySelector("#mp-title").value.trim(),stats_text:f.querySelector("#mp-stats").value.trim(),filters:o.filter(E=>E.label)},m=e.getEl()?.querySelector("[id^='mp-root-']")?.id?.replace("mp-root-","")||"mp"+Math.random().toString(36).slice(2,7);e.addAttributes({"data-map-config":JSON.stringify(g)}),e.components(ht(g,m)+`<style>${Ie}</style>`),b()}}const ur=[{id:"map-filter-block",label:"Mapa con Filtros",category:"Interactivos",media:gr,content:{type:"map-filter-component"}}];function xr(t){const e="map-filter-component";t.DomComponents.addType(e,{isComponent:i=>i.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Mapa con Filtros",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-map-config":JSON.stringify(fe)},components:ht(fe)+`<style>${Ie}</style>`,script:mr(),toolbar:[],traits:[{type:"button",label:"Mapa con Filtros",text:"Administrar Mapa y Filtros",full:!0,command:"open-map-filter-config"}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),t.Commands.add("open-map-filter-config",{run(i){const a=i.getSelected();a&&br(i,a)}}),yr(t,e),vr(t,e)}function yr(t,e){t.on("storage:end:load",()=>{setTimeout(()=>rt(t,e),1e3)}),t.on("component:mount",i=>{const a=i.getEl();a?.getAttribute?.("data-gjs-type")===e&&(i.set("type",e),setTimeout(()=>{const r=i.get("script");r&&typeof r=="function"&&r.call(a)},500))}),t.on("component:clone",i=>{if(i.get("type")===e){const a=i.getEl();if(a){const r=a.querySelector("[data-mp-map]");r&&r._map&&(r._map.remove(),delete r._map,delete r.__mpMapInit),setTimeout(()=>{const l=i.get("script");l&&typeof l=="function"&&l.call(a)},500)}}}),t.on("canvas:render",()=>{setTimeout(()=>rt(t,e),800)}),t.on("storage:start:store",()=>{t.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(i=>{i.set("type",e),i.addAttributes({"data-gjs-type":e})})})}function rt(t,e){t.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(i=>{i.set("type",e);const a=i.getEl();if(a?.isConnected){const r=i.get("script");r&&typeof r=="function"&&r.call(a)}})}function vr(t,e){t.on("load",()=>{const i=t.Canvas.getFrameEl();if(!i)return;const a=i.contentDocument?.head;if(a){if(!a.querySelector("#leaflet-css")){const r=document.createElement("link");r.id="leaflet-css",r.rel="stylesheet",r.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css",a.appendChild(r)}if(!a.querySelector("#mp-filter-styles")){const r=document.createElement("style");r.id="mp-filter-styles",r.textContent=Ie,a.appendChild(r)}if(!a.querySelector(`#${e}-editor-css`)){const r=document.createElement("style");r.id=`${e}-editor-css`,r.textContent=".leaflet-container{height:100%;width:100%;border-radius:inherit;z-index:0;}",a.appendChild(r)}}})}const wr=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
@media(max-width:768px){.bsk-buttons{flex-direction:column;}}`;function Er(){return function(){const t=this,e=t.ownerDocument??document,i="/adminintegral",a=(e.defaultView??window).location.origin,r=e.querySelector('meta[name="api-banners-url"]')?.content||`${a}${i}/api/banners/active`,l=["fill-blue","outline-blue","fill-orange","outline-orange","fill-white","outline-white"],o=".banner-wrapper{position:relative;width:100%;background:#003B71;overflow:hidden;}.banner-slide-container{position:relative;width:100%;user-select:none;cursor:grab;display:grid;}.banner-slide-container:active{cursor:grabbing;}.banner-slide{grid-area:1/1;opacity:0;pointer-events:none;transition:opacity 0.7s ease;visibility:hidden;display:flex;flex-direction:column;}.banner-slide.banner-slide--active{opacity:1;pointer-events:auto;visibility:visible;}.banner-slide-inner{position:relative;width:100%;flex:1;min-height:480px;display:flex;align-items:center;}.banner-bg{position:absolute;inset:0;z-index:0;}.banner-bg img{width:100%;height:100%;object-fit:cover;object-position:center;display:block;}.banner-overlay{position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,59,113,0.85) 0%,rgba(0,59,113,0.65) 50%,rgba(0,59,113,0.2) 100%);z-index:1;}.banner-content{position:relative;z-index:10;display:flex;flex-direction:column;justify-content:center;padding:64px 64px 96px;max-width:60%;}.banner-category-badge{display:inline-block;margin-bottom:18px;padding:7px 20px;border-radius:999px;border:2px solid rgba(255,255,255,0.65);color:#fff;font-size:13px;font-weight:500;letter-spacing:0.04em;align-self:flex-start;}.banner-title{font-size:clamp(1.8rem,3.2vw,3rem);font-weight:800;color:#fff;line-height:1.15;margin:0 0 20px;}.banner-description{color:rgba(255,255,255,0.9);font-size:clamp(0.95rem,1.4vw,1.1rem);line-height:1.7;margin:0 0 36px;max-width:520px;}.banner-buttons{display:flex;flex-wrap:wrap;gap:16px;align-items:center;}.banner-btn{display:inline-block;padding:12px 32px;border-radius:999px;font-size:15px;font-weight:600;cursor:pointer;transition:background .2s,color .2s,border-color .2s;text-decoration:none;white-space:nowrap;line-height:1.4;box-sizing:border-box;}.banner-btn--fill-blue{background:#003B71;color:#ffffff;border:2px solid #003B71;}.banner-btn--fill-blue:hover{background:#002a52;border-color:#002a52;color:#ffffff;}.banner-btn--outline-blue{background:transparent;color:#003B71;border:2px solid #003B71;}.banner-btn--outline-blue:hover{background:#003B71;border-color:#003B71;color:#ffffff;}.banner-btn--fill-orange{background:#E97300;color:#ffffff;border:2px solid #E97300;}.banner-btn--fill-orange:hover{background:#c96200;border-color:#c96200;color:#ffffff;}.banner-btn--outline-orange{background:transparent;color:#E97300;border:2px solid #E97300;}.banner-btn--outline-orange:hover{background:#E97300;border-color:#E97300;color:#ffffff;}.banner-btn--fill-white{background:#ffffff;color:#003B71;border:2px solid #ffffff;}.banner-btn--fill-white:hover{background:#dce8f5;border-color:#dce8f5;color:#003B71;}.banner-btn--outline-white{background:transparent;color:#ffffff;border:2px solid #ffffff;}.banner-btn--outline-white:hover{background:#ffffff;border-color:#ffffff;color:#003B71;}.banner-dots-wrapper{display:none;}.banner-stripe{position:relative;width:100%;height:26px;background:#E97300;z-index:20;display:flex;align-items:center;justify-content:center;}.banner-dots{display:flex;gap:8px;align-items:center;}.banner-dot{width:10px;height:10px;border-radius:999px;border:none;cursor:pointer;background:rgba(255,255,255,0.5);transition:all 0.3s ease;padding:0;flex-shrink:0;}.banner-dot--active{width:28px;background:#ffffff;}.banner-empty{display:flex;align-items:center;justify-content:center;min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;}@media(max-width:768px){.banner-slide-inner{min-height:340px;}.banner-bg img{object-position:right center;}.banner-content{max-width:100%;width:100%;padding:32px 24px 80px;}.banner-buttons{flex-wrap:wrap;gap:12px;}.banner-btn{flex:1 1 45%;min-width:140px;text-align:center;white-space:normal;word-break:break-word;}}@media(max-width:480px){.banner-btn{flex:1 1 100%;min-width:0;}}",n="@keyframes bsk-shimmer{0%{background-position:-600px 0}100%{background-position:600px 0}}.bsk-badge{width:120px;height:32px;margin-bottom:20px;border-radius:999px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-title{height:40px;margin-bottom:12px;border-radius:8px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-title--short{width:60%;}.bsk-line{height:16px;margin-bottom:10px;border-radius:6px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-line--short{width:55%;}.bsk-buttons{display:flex;gap:16px;margin-top:36px;}.bsk-btn{height:48px;flex:1;border-radius:999px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-bg-img{background:linear-gradient(90deg,rgba(255,255,255,0.04) 25%,rgba(255,255,255,0.09) 50%,rgba(255,255,255,0.04) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}@media(max-width:768px){.bsk-buttons{flex-direction:column;}}";(function(){if(!e.getElementById("banner-hero-styles")){const z=e.createElement("style");z.id="banner-hero-styles",z.textContent=o,e.head.appendChild(z)}if(!e.getElementById("banner-skeleton-styles")){const z=e.createElement("style");z.id="banner-skeleton-styles",z.textContent=n,e.head.appendChild(z)}})();let f=[],s=0,c=null,b=!1,g=0,x=0;const m=50,E=t.dataset.autoplay!=="false",A=t.dataset.category??"",B=t.querySelector(".banner-slide-container"),y=t.querySelector(".banner-stripe");if(!B||!y)return;function h(k){return l.indexOf(k)!==-1?k:k==="outline-blue"||k==="outline-orange"?"outline-white":"fill-white"}function d(k,z,X,O){const R=h(X),H=z?"a":"span",L=z?`href="${z}"${O?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${H} ${L} class="banner-btn banner-btn--${R}">${k}</${H}>`}async function u(){if(!t.__bannerLoading){t.__bannerLoading=!0,S();try{const k=await fetch(r,{headers:{Accept:"application/json"}});if(!k.ok){T();return}const z=await k.json();if(f=Array.isArray(z)?A?z.filter(X=>X.category===A):z:[],f.length===0){T();return}v(),V(),p(0,!1),E&&w()}catch{T()}finally{t.__bannerLoading=!1}}}function S(){B.innerHTML=`
        ...
        </div>`,o.appendChild(n),document.body.appendChild(o);const f=n.querySelector("#bnr-autoplay"),s=n.querySelector("#bnr-autoplay-slider"),c=n.querySelector("#bnr-autoplay-knob"),b=()=>{s.style.background=f.checked?"#003B71":"#cbd5e1",c.style.left=f.checked?"21px":"3px"};b(),f.addEventListener("change",b);const g=n.querySelector("#bnr-category");(async()=>{try{const E=document.querySelector('meta[name="api-banners-url"]')?.content||`${window.location.origin}/adminintegral/api/banners/active`,A=await fetch(E,{headers:{Accept:"application/json"}});if(!A.ok)return;const B=await A.json();if(!Array.isArray(B))return;[...new Set(B.map(h=>h.category).filter(Boolean))].sort().forEach(h=>{const d=document.createElement("option");d.value=h,d.textContent=h,g.appendChild(d)}),g.value=l}catch{}})();const x=()=>o.remove();n.querySelector("#bnr-modal-close").onclick=x,n.querySelector("#bnr-modal-cancel").onclick=x,o.onclick=m=>{m.target===o&&x()},n.querySelector("#bnr-modal-save").onclick=()=>{e.addAttributes({"data-autoplay":f.checked?"true":"false","data-category":g.value||""}),x()}}const Sr=[{id:"banner-hero",label:"Banner Slider",category:"Banners",media:wr,content:{type:"banner-hero-component"}}];function Lr(t){const e="banner-hero-component";t.DomComponents.addType(e,{isComponent:i=>i.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Slider",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-autoplay":"true","data-category":""},components:`
...
.hb-btn{width:100%;text-align:center;}
        ...
        </div>`,s.appendChild(c),document.body.appendChild(s);const g=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function x(h){if(h.dataset.autocompleteAttached)return;h.dataset.autocompleteAttached="true";const d=h.parentNode;(!d.style.position||d.style.position==="static")&&(d.style.position="relative");const u=document.createElement("ul");u.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",d.appendChild(u);let S=null;async function v(_){if(_.length<1){u.style.display="none";return}try{const M=await(await fetch(`${g}?q=${encodeURIComponent(_)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();q(M,_)}catch{u.style.display="none"}}function j(_,D){return D?_.replace(new RegExp(`(${D.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):_}function q(_,D){if(u.innerHTML="",!_.length){u.style.display="none";return}_.forEach(M=>{const V=document.createElement("li");V.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",V.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${j(M.title,D)}</span><span style="font-size:0.7rem;color:#64748b;">/${M.slug}</span>`,V.addEventListener("mouseenter",()=>V.style.background="#f1f5f9"),V.addEventListener("mouseleave",()=>V.style.background=""),V.addEventListener("mousedown",p=>{p.preventDefault(),h.value=`${Dr}/${M.slug}`,h.dispatchEvent(new Event("input")),u.style.display="none"}),u.appendChild(V)}),u.style.display="block"}h.addEventListener("input",()=>{clearTimeout(S),S=setTimeout(()=>v(h.value.trim()),220)}),h.addEventListener("focus",()=>{h.select(),h.value.trim()&&v(h.value.trim())}),h.addEventListener("blur",()=>{setTimeout(()=>{u.style.display="none"},150)}),h.addEventListener("keydown",_=>{if(u.style.display==="none")return;const D=u.querySelectorAll("li"),M=u.querySelector("li.hb-ac-active");let V=Array.from(D).indexOf(M);if(_.key==="ArrowDown"){_.preventDefault(),M?.classList.remove("hb-ac-active");const p=D[V+1]||D[0];p?.classList.add("hb-ac-active"),p&&(p.style.background="#f1f5f9")}else if(_.key==="ArrowUp"){_.preventDefault(),M?.classList.remove("hb-ac-active");const p=D[V-1]||D[D.length-1];p?.classList.add("hb-ac-active"),p&&(p.style.background="#f1f5f9")}else _.key==="Enter"&&M?(_.preventDefault(),M.dispatchEvent(new MouseEvent("mousedown"))):_.key==="Escape"&&(u.style.display="none")})}x(c.querySelector("#hb-btn1-href")),x(c.querySelector("#hb-btn2-href"));let m=n.color||"white",E=f.color||"white";function A(h,d,u){const S=c.querySelector(`#${h}`);S.querySelectorAll("[data-color]").forEach(v=>{v.classList.toggle("hb-color-inactive",v.dataset.color!==d),v.addEventListener("click",()=>{S.querySelectorAll("[data-color]").forEach(j=>j.classList.toggle("hb-color-inactive",j.dataset.color!==v.dataset.color)),u(v.dataset.color)})})}A("hb-btn1-colors",m,h=>m=h),A("hb-btn2-colors",E,h=>E=h);function B(h,d,u){const S=c.querySelector(`#${h}`),v=c.querySelector(`#${d}`),j=c.querySelector(`#${u}`),q=()=>{v.style.background=S.checked?"#003B71":"#cbd5e1",j.style.left=S.checked?"21px":"3px"};q(),S.addEventListener("change",q)}B("hb-btn1-enabled","hb-btn1-slider","hb-btn1-knob"),B("hb-btn2-enabled","hb-btn2-slider","hb-btn2-knob"),c.querySelectorAll(".hb-tab-btn").forEach(h=>{h.addEventListener("click",()=>{c.querySelectorAll(".hb-tab-btn").forEach(d=>d.classList.remove("active")),c.querySelectorAll(".hb-tab-panel").forEach(d=>d.classList.remove("active")),h.classList.add("active"),c.querySelector(`#hb-panel-${h.dataset.tab}`).classList.add("active")})}),c.querySelector("#hb-bg-pick").addEventListener("click",()=>{$e({type:"image",title:"Seleccionar imagen de fondo",onSelect:h=>{c.querySelector("#hb-bg-url").value=h,c.querySelector("#hb-bg-preview").src=h}})}),c.querySelector("#hb-bg-url").addEventListener("input",h=>{c.querySelector("#hb-bg-preview").src=h.target.value});const y=()=>s.remove();c.querySelector("#hb-modal-close").onclick=y,c.querySelector("#hb-modal-cancel").onclick=y,s.onclick=h=>{h.target===s&&y()},c.querySelector("#hb-modal-save").onclick=()=>{const h={bg_image:c.querySelector("#hb-bg-url").value.trim()||Q.bg_image,title:c.querySelector("#hb-title").value.trim(),subtitle:c.querySelector("#hb-subtitle").value.trim(),btn_primary:{enabled:c.querySelector("#hb-btn1-enabled").checked,label:c.querySelector("#hb-btn1-label").value.trim(),href:c.querySelector("#hb-btn1-href").value.trim()||"#",color:m},btn_secondary:{enabled:c.querySelector("#hb-btn2-enabled").checked,label:c.querySelector("#hb-btn2-label").value.trim(),href:c.querySelector("#hb-btn2-href").value.trim()||"#",color:E}},u=e.getEl()?.querySelector("[id^='hb-root-']")?.id?.replace("hb-root-","")||"hb"+Math.random().toString(36).slice(2,7);e.addAttributes({"data-hero-banner-config":JSON.stringify(h)}),e.components(bt(h,u)+mt),y()}}function Or(t){const e="hero-banner-component";t.DomComponents.addType(e,{isComponent:i=>i.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-hero-banner-config":JSON.stringify(Q)},components:bt(Q)+mt,toolbar:[],traits:[{type:"button",label:"Banner",text:"Administrar Banner",full:!0,command:"open-hero-banner-config"}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),t.Commands.add("open-hero-banner-config",{run(i){const a=i.getSelected();a&&Nr(i,a)}}),t.BlockManager.add("hero-banner-block",{label:"Banner",category:"Banners",media:`<svg viewBox="0 0 32 32" width="32" height="32">
