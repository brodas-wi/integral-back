/* empty css                   *//* empty css                    *//* empty css                          *//* empty css                           */import{o as Oe,M as Ye,j as Ge,E as Je,i as Ze,t as Ke,d as Qe,f as et,e as tt,s as it,g as at,c as rt,b as lt,a as ot,h as nt}from"./editor-commands-Bq37dEX4.js";import{a as M}from"./url-DaqOO3yL.js";import"./_commonjsHelpers-CqkleIqs.js";class st{constructor(){this.pageId=this.getElementValue("page-id"),this.pageSlug=this.getElementValue("page-slug"),this.pageTitle=this.getPageTitle(),this.loadUrl=this.getElementValue("page-load-url"),this.storeUrl=this.getElementValue("page-store-url"),this.isPublished=this.getElementValue("page-is-published")==="1",this.isEditMode=!!this.pageId,this.appUrl=(document.querySelector('meta[name="app-url"]')?.content??"").replace(/\/$/,"")}getElementValue(e,t=""){const a=document.getElementById(e);return a?a.value.trim():t}getPageTitle(){const e=document.getElementById("editor-title");if(!e)return"";const a=e.textContent.match(/(?:Editando:|Nueva Página)\s*(.+)?/);return a&&a[1]?a[1].trim():""}updatePageInfo(e){if(e.page)if(this.pageId=e.page.id,this.pageSlug=e.page.slug,this.pageTitle=e.page.title,this.isEditMode=!0,document.getElementById("page-id").value=this.pageId,document.getElementById("page-slug").value=this.pageSlug,e.page.update_url?this.storeUrl=e.page.update_url:this.storeUrl=`${this.appUrl}/pages/${this.pageSlug}`,document.getElementById("page-store-url").value=this.storeUrl,e.page.load_url?this.loadUrl=e.page.load_url:this.loadUrl=`${this.appUrl}/pages/${this.pageSlug}/load`,document.getElementById("page-load-url").value=this.loadUrl,e.page.edit_url)window.history.replaceState(null,"",e.page.edit_url);else try{const t=new URL(window.location.href);t.pathname=t.pathname.replace(/\/pages\/create\/?$/,`/pages/${this.pageSlug}/edit`),window.history.replaceState(null,"",t.toString())}catch(t){console.error("Error updating browser URL:",t)}}updateTitle(e){this.pageTitle=e;const t=document.getElementById("editor-title");t&&(t.textContent=`Editando: ${e}`),document.title=`Editar: ${e} - Editor`}getHttpMethod(){return this.isEditMode?"PUT":"POST"}needsTitle(){return!this.isEditMode}}class ct{constructor(e){this.editorService=e,this.setupBackButton(),this.setupBeforeUnload()}setupBackButton(){const e=document.querySelector("#editor-navbar a[href]");e&&e.addEventListener("click",t=>{this.editorService.shouldPreventUnload()&&(t.preventDefault(),this.showUnsavedChangesModal(()=>{this.editorService.startNavigation(),window.location.href=e.href}))})}setupBeforeUnload(){window.addEventListener("beforeunload",e=>{if(this.editorService.shouldPreventUnload())return e.preventDefault(),e.returnValue="Tienes cambios sin guardar. ¿Estás seguro de que quieres salir?",e.returnValue})}showUnsavedChangesModal(e){const t=document.createElement("div");t.style.cssText=`
            position: fixed;
            inset: 0;
            z-index: 9999;
            display: flex;
        ...
        `;const c=document.createElement("button");c.textContent="Salir sin guardar",c.style.cssText=`
            ...
            font-family: inherit;
        `,[n,c].forEach(d=>{d.addEventListener("mouseenter",()=>{d.style.opacity="0.85"}),d.addEventListener("mouseleave",()=>{d.style.opacity="1"})});const h=()=>t.remove();n.addEventListener("click",h),c.addEventListener("click",()=>{h(),e()}),t.addEventListener("click",d=>{d.target===t&&h()}),l.appendChild(n),l.appendChild(c),a.appendChild(r),a.appendChild(l),t.appendChild(a),document.body.appendChild(t)}}const dt="Básico";class ft{constructor(){this.blocks=new Map}registerBlock(e,t){this.blocks.has(t.category)||this.blocks.set(t.category,[]),this.blocks.get(t.category).push({id:e,...t})}registerBlocks(e){e.forEach(t=>{this.registerBlock(t.id,t)})}applyToEditor(e){this.blocks.forEach(t=>{t.forEach(a=>{const{id:r,...l}=a;e.BlockManager.add(r,l)})}),this.applyCollapseSettings(e),this.hideDefaultCategories(e)}applyCollapseSettings(e){setTimeout(()=>{e.BlockManager.getCategories().each(t=>{t.set("open",t.get("label")===dt)})},500)}hideDefaultCategories(e){setTimeout(()=>{const t=["Basic","Extra"];document.querySelectorAll(".gjs-block-category").forEach(a=>{const r=a.querySelector(".gjs-title");r&&t.includes(r.textContent.trim())&&(a.style.display="none")})},100)}}const T=new ft,gt=[{id:"heading1",label:"Título H1",category:"Básico",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,content:'<div class="h-12 w-full"></div>'}],ht=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,wt=[{id:"columns-1",label:"1 Columna",category:"Columnas",media:ht,content:`
<div class="col-section">
    <div class="col-cell"></div>
</div>
${X}`},{id:"columns-2",label:"2 Columnas",category:"Columnas",media:pt,content:`
<div class="col-section">
...
${X}`},{id:"columns-3",label:"3 Columnas",category:"Columnas",media:mt,content:`
<div class="col-section">
...
${X}`},{id:"columns-1-3",label:"1/3 — 2/3",category:"Columnas",media:ut,content:`
<div class="col-section">
...
${X}`},{id:"columns-3-1",label:"2/3 — 1/3",category:"Columnas",media:xt,content:`
<div class="col-section">
...
${X}`},{id:"columns-2-1",label:"1/2 — 1/4 — 1/4",category:"Columnas",media:yt,content:`
<div class="col-section">
...
${X}`}],kt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,St=[{id:"about-section",label:"Nuestra Historia",category:"Contenido",media:kt,content:`
...
${ye}`},{id:"about-mvv-section",label:"Misión, Visión y Valores",category:"Contenido",media:Bt,content:`
...
${ye}`}],Pe=function(){(function(){function i(t){if(!t||t.__pcInit)return;t.__pcInit=!0;var a=t.querySelector(".pc-carousel-wrap");if(!a)return;var r=!1,l=0,n=0,c=!1,h=0,d=0,m=0,b=null;a.querySelectorAll("img").forEach(function(p){p.setAttribute("draggable","false")}),setTimeout(function(){var p=a.scrollWidth-a.clientWidth;if(p<=0)return;var y=Math.min(60,p),$=null;function v(S){$||($=S);var z=(S-$)/400;if(z<.5)a.scrollLeft=y*(z*2);else if(z<1)a.scrollLeft=y*(1-(z-.5)*2);else{a.scrollLeft=0;return}requestAnimationFrame(v)}requestAnimationFrame(v)},400),a.scrollLeft=0;function u(){return a.scrollWidth-a.clientWidth}function s(p){return Math.max(0,Math.min(p,u()))}function g(){Math.abs(h)<.5||(h*=.92,a.scrollLeft=s(a.scrollLeft+h),b=requestAnimationFrame(g))}a.addEventListener("mousedown",function(p){p.button===0&&(b&&(cancelAnimationFrame(b),b=null),r=!0,c=!1,h=0,l=p.clientX,d=p.clientX,m=Date.now(),n=a.scrollLeft,a.style.cursor="grabbing",p.preventDefault())}),document.addEventListener("mousemove",function(p){if(r){var y=l-p.clientX;Math.abs(y)>3&&(c=!0);var $=Date.now(),v=$-m||1;h=(p.clientX-d)/v*16*-1,d=p.clientX,m=$,a.scrollLeft=s(n+y)}}),document.addEventListener("mouseup",function(p){r&&(r=!1,a.style.cursor="grab",c&&(p.stopPropagation(),b=requestAnimationFrame(g)))}),a.addEventListener("click",function(p){c&&(p.preventDefault(),p.stopPropagation(),c=!1)},!0);var k=0,j=0,w=0,E=0,L=0;a.addEventListener("touchstart",function(p){b&&(cancelAnimationFrame(b),b=null),k=p.touches[0].clientX,w=p.touches[0].clientX,E=Date.now(),j=a.scrollLeft,L=0},{passive:!0}),a.addEventListener("touchmove",function(p){var y=Date.now(),$=y-E||1,v=p.touches[0].clientX;L=(v-w)/$*16*-1,w=v,E=y;var S=k-v;a.scrollLeft=s(j+S)},{passive:!0}),a.addEventListener("touchend",function(){b=requestAnimationFrame(function p(){Math.abs(L)<.5||(L*=.92,a.scrollLeft=s(a.scrollLeft+L),b=requestAnimationFrame(p))})},{passive:!0})}function e(){document.querySelectorAll(".pc-section").forEach(function(t){delete t.__pcInit,i(t)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e):e()})()},Lt=`(${Pe.toString()})();`,_t=`
...
@media(max-width:480px){.pc-card{flex:0 0 80vw;}}`;function jt(i){const e=i.img||M("images/placeholder.svg"),t=i.title||"TÍTULO DEL PRODUCTO",a=i.desc||"Descripción breve del producto financiero.",r=i.href||"#",l=i.btn_label||"Solicitar";return`<div class="pc-card"><div class="pc-card-img-wrap"><img src="${e}" alt="${t}" class="pc-card-img"></div><div class="pc-card-body"><h3 class="pc-card-title">${t}</h3><p class="pc-card-desc">${a}</p></div><a href="${r}" class="pc-btn">${l}</a></div>`}function He(i){const e=i.heading||"Créditos",t=i.subheading||"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",a=i.more_href||"#",r=i.more_label||"Ver más",l=i.show_more!==!1,c=(i.cards||[]).map(jt).join(""),h=l?`<div class="pc-more-wrap"><a href="${a}" class="pc-more-btn">${r}</a></div>`:"";return`<section class="pc-section"><style>${_t}</style><div style="text-align:center;margin-bottom:2rem;"><h2 class="pc-section-heading">${e}</h2><p class="pc-section-subheading">${t}</p></div><div class="pc-carousel-wrap"><div class="pc-track">${c}</div></div>${h}</section>`}const P={heading:"Créditos",subheading:"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",more_href:"#",more_label:"Ver más",cards:[{img:"",title:"CREDINVIERTE",desc:"Adquiere activos fijos",href:"#",btn_label:"Solicitar"},{img:"",title:"SOLUCIONES INTEGRALES",desc:"Financiamiento PYME",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDI-CONFIAMOS",desc:"Rápido y sin fiador",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDILÍNEA",desc:"Línea rotativa",href:"#",btn_label:"Solicitar"}]};function Tt(i,e){const t=document.getElementById("pc-config-modal");if(t&&t.remove(),!document.getElementById("pc-modal-styles")){const d=document.createElement("style");d.id="pc-modal-styles",d.textContent=`
        ...
        </div>`,l.appendChild(n),document.body.appendChild(l),n.querySelectorAll(".pc-tab-btn").forEach(d=>{d.addEventListener("click",()=>{n.querySelectorAll(".pc-tab-btn").forEach(m=>m.classList.remove("active")),n.querySelectorAll(".pc-tab-panel").forEach(m=>m.classList.remove("active")),d.classList.add("active"),n.querySelector(`#pc-panel-${d.dataset.tab}`).classList.add("active")})}),n.querySelector("#pc-show-more").addEventListener("change",function(){r.show_more=this.checked,n.querySelector("#pc-more-fields").style.display=this.checked?"flex":"none"});function c(){const d=n.querySelector("#pc-cards-list");d.innerHTML="",r.cards.forEach((m,b)=>{const u=document.createElement("div");u.className="pc-card-config";const s=m.img?`<img class="pc-img-preview" src="${m.img}" alt="">`:'<div class="pc-img-placeholder"><i class="ri-image-line"></i></div>';u.innerHTML=`
...
</svg>`;function It(i){const e="product-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección de Productos",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!0,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-product-cards-config":JSON.stringify(P)},components:He(P),script:Pe,"script-props":["data-product-cards-config"],traits:[{type:"button",label:"Productos",text:"Administrar Sección",full:!0,command:"open-product-cards-config"}],toolbar:[]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),i.Commands.add("open-product-cards-config",{run(t){const a=t.getSelected();a&&Tt(t,a)}}),i.BlockManager.add("product-cards-block",{label:"Sección de productos",category:"Productos y Servicios",media:zt,activate:!0,content:{type:e,attributes:{"data-gjs-type":e}}}),i.on("component:mount",t=>{t.getEl()?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>fe(i),400))}),i.on("component:selected",t=>{const a=t.getEl();if(!a)return;const r=a.closest(`[data-gjs-type="${e}"]`);if(r&&!a.hasAttribute("data-gjs-type")){const n=i.getWrapper().find(`[data-gjs-type="${e}"]`).find(c=>c.getEl()===r);n&&setTimeout(()=>i.select(n),0)}}),i.on("canvas:render",()=>{setTimeout(()=>fe(i),600)}),i.on("storage:end:load",()=>{setTimeout(()=>fe(i),800)})}const At=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,qt=[{id:"dual-card-section",label:"Sección dos tarjetas",category:"Productos y Servicios",media:At,content:`
...
${be}`},{id:"dual-card-dark",label:"Tarjeta azul oscuro",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,content:`${we}${be}`},{id:"dual-card-light",label:"Tarjeta blanca",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
        ...
        </svg>`,content:`${ke}${be}`}],Dt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,Nt=[{id:"cta-section",label:"Llamada a la acción",category:"Llamadas a la acción",media:Dt,content:`
<section class="relative overflow-hidden w-full bg-white py-16 px-6 md:px-16">
    <div class="cta-watermark-left">
        <img src="${M("images/brand-logo.png")}" alt="">
    </div>
...
${Mt}`}],Fe=function(){(function(){function i(t){if(!t||t.__savInit)return;t.__savInit=!0;var a=t.querySelector(".sav-carousel-wrap");if(!a)return;a.scrollLeft=0;var r=!1,l=0,n=0,c=!1,h=0,d=0,m=0,b=null;a.querySelectorAll("img").forEach(function(p){p.setAttribute("draggable","false")}),setTimeout(function(){var p=a.scrollWidth-a.clientWidth;if(p<=0)return;var y=Math.min(60,p),$=null;function v(S){$||($=S);var z=(S-$)/400;if(z<.5)a.scrollLeft=y*(z*2);else if(z<1)a.scrollLeft=y*(1-(z-.5)*2);else{a.scrollLeft=0;return}requestAnimationFrame(v)}requestAnimationFrame(v)},400);function u(){return a.scrollWidth-a.clientWidth}function s(p){return Math.max(0,Math.min(p,u()))}function g(){Math.abs(h)<.5||(h*=.92,a.scrollLeft=s(a.scrollLeft+h),b=requestAnimationFrame(g))}a.addEventListener("mousedown",function(p){p.button===0&&(b&&(cancelAnimationFrame(b),b=null),r=!0,c=!1,h=0,l=p.clientX,d=p.clientX,m=Date.now(),n=a.scrollLeft,a.style.cursor="grabbing",p.preventDefault())}),document.addEventListener("mousemove",function(p){if(r){var y=l-p.clientX;Math.abs(y)>3&&(c=!0);var $=Date.now(),v=$-m||1;h=(p.clientX-d)/v*16*-1,d=p.clientX,m=$,a.scrollLeft=s(n+y)}}),document.addEventListener("mouseup",function(p){r&&(r=!1,a.style.cursor="grab",c&&(p.stopPropagation(),b=requestAnimationFrame(g)))}),a.addEventListener("click",function(p){c&&(p.preventDefault(),p.stopPropagation(),c=!1)},!0);var k=0,j=0,w=0,E=0,L=0;a.addEventListener("touchstart",function(p){b&&(cancelAnimationFrame(b),b=null),k=p.touches[0].clientX,w=p.touches[0].clientX,E=Date.now(),j=a.scrollLeft,L=0},{passive:!0}),a.addEventListener("touchmove",function(p){var y=Date.now(),$=y-E||1,v=p.touches[0].clientX;L=(v-w)/$*16*-1,w=v,E=y;var S=k-v;a.scrollLeft=s(j+S)},{passive:!0}),a.addEventListener("touchend",function(){b=requestAnimationFrame(function p(){Math.abs(L)<.5||(L*=.92,a.scrollLeft=s(a.scrollLeft+L),b=requestAnimationFrame(p))})},{passive:!0})}function e(){document.querySelectorAll(".sav-section").forEach(function(t){delete t.__savInit,i(t)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e):e()})()},Ot=`(${Fe.toString()})();`,Pt=`
...
</svg>`;function Ut(i){const e="savings-section-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección Fondo Azul",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!0,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-savings-config":JSON.stringify(H)},components:Re(H),script:Fe,"script-props":["data-savings-config"],traits:[{type:"button",label:"Sección",text:"Administrar Sección",full:!0,command:"open-savings-config"}],toolbar:[]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),i.Commands.add("open-savings-config",{run(t){const a=t.getSelected();a&&Ft(t,a)}}),i.BlockManager.add("savings-section-block",{label:"Sección Fondo Azul",category:"Productos y Servicios",media:Rt,activate:!0,content:{type:e,attributes:{"data-gjs-type":e}}}),i.on("component:mount",t=>{t.getEl()?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>ge(i),400))}),i.on("component:selected",t=>{const a=t.getEl();if(!a)return;const r=a.closest(`[data-gjs-type="${e}"]`);if(r&&!a.hasAttribute("data-gjs-type")){const l=i.getWrapper().find(`[data-gjs-type="${e}"]`).find(n=>n.getEl()===r);l&&setTimeout(()=>i.select(l),0)}}),i.on("canvas:render",()=>setTimeout(()=>ge(i),600)),i.on("storage:end:load",()=>setTimeout(()=>ge(i),800))}const Be=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Wt=[{id:"product-link-cards",label:"Tarjetas de productos con enlace",category:"Productos y Servicios",media:Be,content:`
...
${ue}`},{id:"product-link-card-orange",label:"Tarjeta producto naranja",category:"Productos y Servicios",media:Be,media:Vt,content:`${K}${ue}`},{id:"product-link-card-blue",label:"Tarjeta producto azul",category:"Productos y Servicios",media:Xt,content:`${Q}${ue}`}],Ee=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Yt=[{id:"icon-links-strip",label:"Tira de iconos con enlace",category:"Productos y Servicios",media:Ee,content:`
...
${$e}`},{id:"icon-link-item",label:"Ítem icono con enlace",category:"Productos y Servicios",media:Ee,content:`${ee}${$e}`}],Gt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Zt=[{id:"promo-cta",label:"Promo con CTA y precio",category:"Llamadas a la acción",media:Gt,content:`
...
${Jt}`}],Kt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<style>${ve}</style>`,ti=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,ii=[{id:"tabs-cards",label:"Sección con tabs",category:"Contenido",media:Kt,content:{type:"tabs-cards-component"}},{id:"tab-card-single",label:"Tarjeta de tab",category:"Contenido",media:ti,content:`${D()}`}];function ai(i){const e="tabs-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección con tabs",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!0,removable:!0,attributes:{"data-gjs-type":e,"data-tab-count":"5"},components:ei,script:Qt(),traits:[{type:"number",name:"data-tab-count",label:"Número de tabs (2-10)",min:2,max:10,changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",(t,a)=>{const r=parseInt(a["data-tab-count"]);isNaN(r)||this.updateTabCount(r)})},updateTabCount(t){const a=Math.min(10,Math.max(2,t)),r=l=>{const n=Array.from({length:l},()=>'<button class="tabs-btn inline-block px-6 py-2 rounded-full border-2 border-[#003B71] bg-transparent text-[#003B71] text-sm font-medium">Lorem Ipsum</button>').join(`
...
`);return`
            ...
            `,a.appendChild(r)}}})}const oi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,Ce=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</div>`,di=i=>{const e=i==="light";return`
...
${bi}`,ui=[{id:"stats-strip-left",label:"Franja estadísticas - izquierda",category:"Heroes",media:pi,content:Se(!0)},{id:"stats-strip-right",label:"Franja estadísticas - derecha",category:"Heroes",media:mi,content:Se(!1)}],xi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,yi=[{id:"service-cards-section",label:"Sección de servicios",category:"Productos y Servicios",media:xi,content:`
...
${Le}`},{id:"service-card",label:"Tarjeta de servicio",category:"Productos y Servicios",media:vi,content:`${ce}${Le}`}],wi=`<svg viewBox="0 0 32 32" width="32" height="32">
...
</style>`,$i=[{id:"contact-info-row",label:"Información de contacto (fila)",category:"Contacto",media:Bi,content:`
...
${de}`},{id:"contact-info-left",label:"Info de contacto (izquierda)",category:"Contacto",media:wi,content:`
<section class="ci-section">
    <div class="ci-list">
        ${J("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
        ${J("ri-mail-fill","Correo","servicio.cliente@bancointegral.com","mailto:servicio.cliente@bancointegral.com")}
...
${de}`},{id:"contact-info-right",label:"Info de contacto (derecha)",category:"Contacto",media:ki,content:`
<section class="ci-section">
...
${de}`},{id:"contact-info-item",label:"Ítem de contacto",category:"Contacto",media:Ei,content:`
<section class="ci-section">
    <div class="ci-list">
        ${J("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
    </div>
</section>
${de}`}],Ci=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Li=[{id:"rich-list",label:"Lista con título y descripción",category:"Contenido",media:Ci,content:`
...
${_e}`},{id:"rich-list-item",label:"Ítem de lista con descripción",category:"Contenido",media:Si,content:`
<section class="rl-section">
    <div class="rl-list">
        ${ie}
    </div>
</section>
${_e}`}],_i=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</div>`,Ti=[{id:"news-grid-section",label:"Noticias y Publicaciones",category:"Contenido",media:_i,content:`
...
${je}`},{id:"news-card",label:"Tarjeta de noticia",category:"Contenido",media:ji,content:`${ae}${je}`}],zi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#ffffff" rx="2" stroke="#e5e7eb" stroke-width="1"/>
...
`,Ai=[{id:"contact-form",label:"Formulario de contacto",category:"Formularios",media:zi,content:`
...
<\/script>`}],qi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,he={blue:{headerBg:"bg-[#003B71]",headerText:"text-white",subheaderBg:"bg-[#e8f0f8]",subheaderText:"text-[#003B71]",borderColor:"#003B71",rowEvenBg:"bg-[#f4f7fb]",rowOddBg:"bg-white",rowText:"text-[#003B71]",labelBg:"bg-[#e8f0f8]",labelText:"text-[#003B71]"},orange:{headerBg:"bg-[#E97300]",headerText:"text-white",subheaderBg:"bg-[#fef3e8]",subheaderText:"text-[#E97300]",borderColor:"#E97300",rowEvenBg:"bg-[#fff8f2]",rowOddBg:"bg-white",rowText:"text-[#003B71]",labelBg:"bg-[#fef3e8]",labelText:"text-[#E97300]"}};function Ue(i,e){const t=he[e]||he.blue;let a='<table class="w-full border-collapse font-[Poppins,sans-serif] table-fixed">';i.title?(a+=`<thead><tr>
            <th colspan="${i.cols}" class="p-3 align-middle text-center text-base font-bold ${t.headerBg} ${t.headerText}">
                ${i.title}
            </th>
        </tr>`,i.headers?.length&&(a+="<tr>",i.headers.forEach((n,c)=>{const h=c<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";a+=`<th class="p-3 align-middle text-sm font-semibold ${t.subheaderBg} ${t.subheaderText} ${h} border-b border-[${t.borderColor}] text-${n.align||"center"}">${n.text||""}</th>`}),a+="</tr>"),a+="</thead>"):i.headers?.length&&(a+="<thead><tr>",i.headers.forEach((n,c)=>{const h=c<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";a+=`<th class="p-3 align-middle text-sm font-semibold ${t.headerBg} ${t.headerText} ${h} border-b border-[${t.borderColor}] text-${n.align||"center"}">${n.text||""}</th>`}),a+="</tr></thead>"),a+="<tbody>";const r=i.rows.length,l={};return i.rows.forEach((n,c)=>{a+="<tr>";let h=0;n.forEach(d=>{for(;l[`${c}-${h}`];)h++;const m=d.colspan||1,b=d.rowspan||1;for(let v=c;v<c+b;v++)for(let S=h;S<h+m;S++)(v!==c||S!==h)&&(l[`${v}-${S}`]=!0);const u=m>1?`colspan="${m}"`:"",s=b>1?`rowspan="${b}"`:"",g=d.isHeader?t.labelBg:c%2===0?t.rowEvenBg:t.rowOddBg,k=d.isHeader?"font-semibold":"font-normal",j=d.isHeader?t.labelText:t.rowText,w=`text-${d.align||"center"}`,E=c+b>=r,p=h+m>=i.cols?"":`border-r border-[${t.borderColor}]`,y=E?"":`border-b border-[${t.borderColor}]`,$=`${p} ${y} p-3 align-middle text-sm ${g} ${k} ${j} ${w}`;d.image?a+=`<td ${u} ${s} class="${$}">
                    <img src="${d.image}" alt="${d.text||""}" class="max-w-full max-h-20 h-auto object-contain block mx-auto">
                    ${d.text?`<span class="block mt-1 text-xs ${j}">${d.text}</span>`:""}
                </td>`:a+=`<td ${u} ${s} class="${$}">${d.text||""}</td>`,h+=m}),a+="</tr>"}),a+="</tbody></table>",a}function Z(i,e){return{title:"Título de la tabla",cols:i,headers:Array.from({length:i},(t,a)=>({text:`Columna ${a+1}`,align:"center"})),rows:Array.from({length:e},()=>Array.from({length:i},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})))}}function Ve(i,e){return`<div class="w-full overflow-x-auto rounded-2xl border-2 border-[${(he[e]||he.blue).borderColor}]">${i}</div>`}function Te(i,e){const t={};return i.forEach((a,r)=>{let l=0;a.forEach(n=>{for(;t[`${r}-${l}`];)l++;const c=Math.min(n.colspan||1,e-l),h=n.rowspan||1;for(let d=r;d<r+h;d++)for(let m=l;m<l+c;m++)(d!==r||m!==l)&&(t[`${d}-${m}`]=`${r}-${l}`);l+=c})}),t}const Di=`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function Mi(){if(document.getElementById("tam-img-modal"))return;const i=document.createElement("div");i.id="tam-img-modal",i.innerHTML=`
        ...
        <div class="tam-container">
            ...
            <th>
            ...
            </th>`).join("")}</tr>`,s.innerHTML=l.rows.map((w,E)=>`<tr>${Array.from({length:g},(p,y)=>{const $=j[`${E}-${y}`];if($)return`<td class="tam-cell is-spanned" data-row="${E}" data-col="${y}">
                ...
                </td>`}).join("")}</tr>`).join(""),s.querySelectorAll("input[data-action=colspan], input[data-action=rowspan]").forEach(w=>{w.addEventListener("change",()=>{const E=parseInt(w.dataset.row),L=parseInt(w.dataset.col),p=Math.max(1,parseInt(w.value)||1);l.rows[E]?.[L]&&(w.dataset.action==="colspan"?l.rows[E][L].colspan=Math.min(p,g-L):l.rows[E][L].rowspan=Math.min(p,k-E),d())})}),s.querySelectorAll("button[data-action]").forEach(w=>{w.addEventListener("click",E=>{E.preventDefault(),E.stopPropagation();const L=w.dataset.action,p=parseInt(w.dataset.row),y=parseInt(w.dataset.col);if(!(isNaN(p)||isNaN(y)||!l.rows[p]?.[y])){if(L==="header"){l.rows[p][y].isHeader=!l.rows[p][y].isHeader;const $=s.querySelector(`td[data-row="${p}"][data-col="${y}"]`);$&&($.dataset.isheader=l.rows[p][y].isHeader?"1":"0",$.classList.toggle("is-header-cell",l.rows[p][y].isHeader)),w.classList.toggle("active",l.rows[p][y].isHeader),w.textContent=l.rows[p][y].isHeader?"✓ Etiqueta":"Etiqueta";return}if(L==="image"){window.__openTableImagePicker&&window.__openTableImagePicker($=>{l.rows[p][y].image=$;const v=s.querySelector(`td[data-row="${p}"][data-col="${y}"]`);if(v){v.dataset.image=$,v.classList.add("has-image");let S=v.querySelector(".tam-cell-img-preview");S||(S=document.createElement("img"),S.className="tam-cell-img-preview",v.insertBefore(S,v.firstChild)),S.src=$;const z=v.querySelector("[data-action=image]");if(z&&(z.innerHTML='<i class="ri-image-line"></i> Cambiar'),!v.querySelector("[data-action=clear-image]")){const A=document.createElement("button");A.type="button",A.className="tam-cell-btn tam-cell-btn-clear",A.dataset.action="clear-image",A.dataset.row=p,A.dataset.col=y,A.textContent="✕ Quitar",A.addEventListener("click",Y=>{Y.preventDefault(),Y.stopPropagation(),l.rows[p][y].image=null,v.dataset.image="",v.classList.remove("has-image"),S.remove(),A.remove();const F=v.querySelector("[data-action=image]");F&&(F.innerHTML='<i class="ri-image-line"></i> Imagen')}),v.querySelector(".tam-cell-actions").appendChild(A)}}});return}L==="clear-image"&&(l.rows[p][y].image=null,b())}})})}document.getElementById("tam-close").addEventListener("click",c),document.getElementById("tam-cancel").addEventListener("click",c),a.addEventListener("click",u=>{u.target===a&&c()}),document.getElementById("tam-rebuild").addEventListener("click",()=>{const u=parseInt(document.getElementById("tam-cols").value)||3,s=parseInt(document.getElementById("tam-rows").value)||3;for(m(),h();l.headers.length<u;)l.headers.push({text:`Col ${l.headers.length+1}`,align:"center"});for(l.headers=l.headers.slice(0,u),l.cols=u;l.rows.length<s;)l.rows.push(Array.from({length:u},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})));l.rows=l.rows.slice(0,s).map(g=>{for(;g.length<u;)g.push({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null});return g.slice(0,u)}),b()}),document.getElementById("tam-apply").addEventListener("click",()=>{h();const u=document.getElementById("tam-theme").value;r&&(r.set("tableData",JSON.parse(JSON.stringify(l))),r.set("tableTheme",u),r.addAttributes({"data-table-theme":u}),xe(r)),c()}),window.__openTableAdminModal=n}function xe(i){const e=i.get("tableData"),t=i.get("tableTheme")||"blue";e&&i.components(Ve(Ue(e,t),t))}function Oi(){return function(){}}const Pi=[{id:"table-blue",label:"Tabla azul",category:"Interactivos",media:qi,content:{type:"table-component",attributes:{"data-table-theme":"blue"}}},{id:"table-orange",label:"Tabla naranja",category:"Interactivos",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
            ...
            `,a.appendChild(r)}})}const Ui=`<svg viewBox="0 0 80 32" width="80" height="32">
...
</svg>`,ze=i=>{const e=i==="#003B71"?"blue":"orange";return`
...
</div>`},Ie=i=>{const e=i==="#003B71"?"blue":"orange";return`
...
<a href="#"
   ...
   class="dld-simple-${e}">
...
</div>`},Ae={"button-fill-blue":"inline-block px-8 py-3 rounded-full text-base font-semibold leading-snug no-underline whitespace-nowrap border-2 border-[#003B71] bg-[#003B71] text-white transition-all duration-200 hover:bg-[#002a52] hover:border-[#002a52]","button-fill-orange":"inline-block px-8 py-3 rounded-full text-base font-semibold leading-snug no-underline whitespace-nowrap border-2 border-[#E97300] bg-[#E97300] text-white transition-all duration-200 hover:bg-[#c96200] hover:border-[#c96200]","button-outline-blue":"inline-block px-8 py-3 rounded-full text-base font-semibold leading-snug no-underline whitespace-nowrap border-2 border-[#003B71] bg-transparent text-[#003B71] transition-all duration-200 hover:bg-[#003B71] hover:text-white","button-outline-orange":"inline-block px-8 py-3 rounded-full text-base font-semibold leading-snug no-underline whitespace-nowrap border-2 border-[#E97300] bg-transparent text-[#E97300] transition-all duration-200 hover:bg-[#E97300] hover:text-white","button-fill-white":"inline-block px-8 py-3 rounded-full text-base font-semibold leading-snug no-underline whitespace-nowrap border-2 border-white bg-white text-[#003B71] transition-all duration-200 hover:bg-[#dce8f5] hover:border-[#dce8f5]","button-outline-white":"inline-block px-8 py-3 rounded-full text-base font-semibold leading-snug no-underline whitespace-nowrap border-2 border-white bg-transparent text-white transition-all duration-200 hover:bg-white hover:text-[#003B71]"},ea=[{id:"button-fill-blue",label:"Botón azul sólido",category:"Botones",media:Yi,content:{type:"integral-button",attributes:{"data-btn-variant":"button-fill-blue"}}},{id:"button-fill-orange",label:"Botón naranja sólido",category:"Botones",media:Gi,content:{type:"integral-button",attributes:{"data-btn-variant":"button-fill-orange"}}},{id:"button-outline-blue",label:"Botón azul outline",category:"Botones",media:Ji,content:{type:"integral-button",attributes:{"data-btn-variant":"button-outline-blue"}}},{id:"button-outline-orange",label:"Botón naranja outline",category:"Botones",media:Zi,content:{type:"integral-button",attributes:{"data-btn-variant":"button-outline-orange"}}},{id:"button-fill-white",label:"Botón blanco sólido",category:"Botones",media:Ki,content:{type:"integral-button",attributes:{"data-btn-variant":"button-fill-white"}}},{id:"button-outline-white",label:"Botón blanco outline",category:"Botones",media:Qi,content:{type:"integral-button",attributes:{"data-btn-variant":"button-outline-white"}}},{id:"button-download-full-blue",label:"Descarga completa azul",category:"Botones",media:Ui,content:ze("#003B71")},{id:"button-download-full-orange",label:"Descarga completa naranja",category:"Botones",media:Vi,content:ze("#E97300")},{id:"button-download-simple-blue",label:"Descarga simple azul",category:"Botones",media:Xi,content:Ie("#003B71")},{id:"button-download-simple-orange",label:"Descarga simple naranja",category:"Botones",media:Wi,content:Ie("#E97300")}];function ta(i){const e=[{type:"button",label:"Documento",name:"select-document",text:"Seleccionar documento",full:!0,command:"open-document-picker"},{type:"text",name:"href",label:"URL / Enlace",placeholder:"https://..."},{type:"select",name:"target",label:"Abrir en",options:[{id:"_self",name:"Misma ventana"},{id:"_blank",name:"Nueva ventana"}]}];i.DomComponents.addType("link",{model:{defaults:{traits:e}}}),i.DomComponents.addType("integral-button",{isComponent:r=>r.tagName==="A"&&r.hasAttribute("data-btn-variant"),model:{defaults:{tagName:"a",draggable:!0,droppable:!1,editable:!0,attributes:{href:"#",target:"_self","data-btn-variant":"button-fill-blue"},components:"Texto del botón",traits:e},init(){const r=this.getAttributes()["data-btn-variant"]??"button-fill-blue",l=Ae[r]??Ae["button-fill-blue"];this.setClass(l.split(" "))}}});function t(r,l){if(r.getEl()?.matches?.(l))return r;let c=null;const h=r.components?.();return h?(h.each(d=>{c||(c=t(d,l))}),c):null}function a(r,l){const c={pdf:"ri-file-pdf-line",xlsx:"ri-file-excel-line",xls:"ri-file-excel-line",doc:"ri-file-word-line",docx:"ri-file-word-line"}[l]??"ri-file-line";function h(m){const b=m.getEl?.();if(b?.tagName==="I"){const g=b.parentElement;if(g&&[...g.classList].some(k=>k.includes("-icon")))return m}let u=null;const s=m.components?.();return s?(s.each(g=>{u||(u=h(g))}),u):null}const d=h(r);if(d){const m=d.getClasses().find(b=>b.startsWith("ri-"));m&&d.removeClass(m),d.addClass(c)}else{const b=r.getEl()?.querySelector("[class*='-icon'] i");if(b){const u=[...b.classList].filter(s=>!s.startsWith("ri-"));b.className=[...u,c].join(" ")}}}i.Commands.add("open-document-picker",{run(r){const l=r.getSelected();if(l){if(r._documentPicker)try{r._documentPicker.destroy()}catch{}r._documentPicker=new Ye,r._documentPicker.open(n=>{const c=n.filename.split(".").pop().toLowerCase();l.addAttributes({href:n.url});const h=l.getTrait("href");h&&h.set("value",n.url);const d=t(l,"[class*='-filename']");d&&d.components(n.filename),a(l,c)},{filters:{type:"document"}})}}})}const ia=`<svg viewBox="0 0 80 32" width="80" height="32">
    <rect width="80" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,la=[{id:"badge-outline-blue",label:"Badge azul outline",category:"Badges",media:ia,content:'<span class="inline-block border border-[#003B71] rounded-full px-4 py-1 text-base font-medium bg-transparent text-[#003B71]">Etiqueta</span>'},{id:"badge-outline-orange",label:"Badge naranja outline",category:"Badges",media:aa,content:'<span class="inline-block border border-[#E97300] rounded-full px-4 py-1 text-base font-medium bg-transparent text-[#E97300]">Etiqueta</span>'},{id:"badge-outline-white",label:"Badge blanco outline",category:"Badges",media:ra,content:'<span class="inline-block border border-white rounded-full px-4 py-1 text-base font-medium bg-transparent text-white">Etiqueta</span>'}],oa=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    ...
    <rect x="3" y="25" width="7" height="2.5" fill="#dee2e6" rx="0.5"/>
</svg>`;function na(){return function(){const i=this,e="agencies-map-component",t="/api/agencies/active",a="agencies";let r=[],l=[],n={},c=null,h=[];const d=async()=>{try{m(),await u(),await k(),j(),$(),A(),Y(),b()}catch(o){console.error("Error initializing map:",o),re("Error al cargar las agencias"),b()}};function m(){const o=i.querySelector(`.${e}-list`);o&&(o.innerHTML=`
            ...
            `}function z(){const o=i.querySelectorAll(`.${e}-item`);o.forEach(f=>{f.addEventListener("click",x=>{if(x.target.closest(".agency-maps-btn"))return;x.preventDefault();const B=parseInt(f.dataset.itemIndex),_=l[B];if(!_)return;o.forEach(C=>{C.classList.remove("bg-white","border-secondary","selected-agency"),C.classList.add("bg-white/10","border-white/20");const q=C.querySelector(".agency-title"),le=C.querySelectorAll("i"),oe=C.querySelectorAll(".agency-phone-link"),pe=C.querySelectorAll("p:not(:has(.agency-phone-link))"),me=C.querySelector(".agency-maps-btn");q&&(q.classList.remove("text-secondary"),q.classList.add("text-white")),le.forEach(V=>{V.classList.remove("text-secondary","text-gray-300","text-white"),V.classList.add("text-primary")}),oe.forEach(V=>{V.classList.remove("text-secondary"),V.classList.add("text-white")}),pe.forEach(V=>{V.classList.remove("text-secondary"),V.classList.add("text-gray-200")}),me&&(me.classList.remove("bg-secondary"),me.classList.add("bg-primary"))}),f.classList.remove("bg-white/10","border-white/20"),f.classList.add("bg-white","border-secondary","selected-agency");const N=f.querySelector(".agency-title"),R=f.querySelectorAll("i"),G=f.querySelectorAll(".agency-phone-link"),U=f.querySelectorAll("p:not(:has(.agency-phone-link))"),O=f.querySelector(".agency-maps-btn");if(N&&(N.classList.remove("text-white"),N.classList.add("text-secondary")),R.forEach(C=>{C.classList.remove("text-secondary","text-gray-300","text-white"),C.classList.add("text-primary")}),G.forEach(C=>{C.classList.remove("text-white"),C.classList.add("text-secondary")}),U.forEach(C=>{C.classList.remove("text-gray-200"),C.classList.add("text-secondary")}),O&&(O.classList.remove("bg-primary"),O.classList.add("bg-secondary")),c&&_&&c._loaded)try{c.flyTo([_.latitude,_.longitude],14,{animate:!0,duration:1});const C=h.find(q=>q.item.id===_.id);C&&C.marker&&C.marker.openPopup()}catch(C){console.warn("Error updating map view:",C)}})})}function A(){const o=i.querySelector(`.${e}-search-input`),f=i.querySelector(`.${e}-zone-filter`),x=i.querySelector(`.${e}-department-filter`),B=i.querySelector(`.${e}-no-results`),_={Occidental:["Ahuachapán","Santa Ana","Sonsonate"],Paracentral:["Chalatenango","Cuscatlán","La Paz","Cabañas","San Vicente"],Central:["La Libertad","San Salvador"],Oriental:["Usulután","San Miguel","Morazán","La Unión"]},N=()=>{if(!f||!x)return;const G=f.value,U=x.value;if(!G)x.innerHTML='<option value="">Todos los Departamentos</option>',[...new Set(r.map(C=>C.department).filter(Boolean))].sort().forEach(C=>{const q=document.createElement("option");q.value=C,q.textContent=C,x.appendChild(q)}),U&&(x.value=U);else{const O=_[G]||[];x.innerHTML='<option value="">Todos los Departamentos</option>',O.forEach(C=>{const q=document.createElement("option");q.value=C,q.textContent=C,x.appendChild(q)}),O.includes(U)&&(x.value=U)}},R=()=>{const G=o?o.value.toLowerCase().trim():"",U=f?f.value:"",O=x?x.value:"";l=r.filter(C=>{let q=!0,le=!0,oe=!0;return G&&(q=Object.values(C).some(pe=>String(pe).toLowerCase().includes(G))),U&&(le=C.zone===U),O&&(oe=C.department===O),q&&le&&oe}),$(),w(),B&&B.classList.toggle("hidden",l.length>0)};o&&o.addEventListener("input",R),f&&f.addEventListener("change",()=>{N(),R()}),x&&x.addEventListener("change",R)}function Y(){const o=i.querySelector("[data-title]");if(o){const B=F("map-title")||"Nuestras Agencias";o.textContent=B}const f=i.querySelector(`.${e}-search-input`);if(f){const B=F("search-placeholder")||"Buscar...";f.setAttribute("placeholder",B)}const x=i.querySelector(`.${e}-no-results`);if(x){const B=F("no-results-text")||"No se encontraron agencias";x.textContent=B}}function F(o){return i.closest(`[data-gjs-type="${e}"]`)?.getAttribute(o)}function re(o){const f=i.querySelector(`.${e}-list`);f&&(f.innerHTML=`
                    <div class="flex flex-col items-center justify-center p-12 text-center">
                ...
                `)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",d):d()}}const sa=[{id:"agencies-map",label:"Mapa de Agencias",category:"Interactivos",media:oa,content:{type:"agencies-map-component"}}];function ca(i){const e="agencies-map-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute&&t.getAttribute("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Mapa de Agencias",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{class:"py-12 bg-secondary","data-gjs-type":e},components:`
                ...
                `,script:na()},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),da(i,e),fa(i,e)}function da(i,e){i.on("component:selected",t=>{if(t.get("type")===e){const a=t.getEl();if(a){const r=a.querySelector(`.${e}-map`);r&&r._map&&setTimeout(()=>{r._map.invalidateSize()},100)}}}),i.on("storage:end:load",()=>{setTimeout(()=>{qe(i,e)},1e3)}),i.on("component:mount",t=>{const a=t.getEl();a&&a.getAttribute&&a.getAttribute("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const r=t.get("script");r&&typeof r=="function"&&a&&r.call(a)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const a=t.getEl();if(a){const r=a.querySelector(`.${e}-map`);r&&r._map&&(r._map.remove(),delete r._map),setTimeout(()=>{const l=t.get("script");l&&typeof l=="function"&&l.call(a)},500)}}}),i.on("canvas:render",()=>{setTimeout(()=>{qe(i,e)},800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(r=>{r.set("type",e),r.addAttributes({"data-gjs-type":e})})})}function qe(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(r=>{r.set("type",e);const l=r.getEl();if(l&&l.isConnected){const n=r.get("script");n&&typeof n=="function"&&n.call(l)}})}function fa(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const a=t.contentDocument.head;if(!a.querySelector("#leaflet-css")){const r=document.createElement("link");r.id="leaflet-css",r.rel="stylesheet",r.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css",a.appendChild(r)}if(!a.querySelector(`#${e}-css`)){const r=document.createElement("style");r.id=`${e}-css`,r.innerHTML=`
                .leaflet-container {
                    height: 100%;
                    width: 100%;
...
@media(max-width:480px){
    .banner-btn{flex:1 1 100%;min-width:0;}
            ...
            `;const o=i.ownerDocument??document;if(!o.getElementById("banner-skeleton-styles")){const f=o.createElement("style");f.id="banner-skeleton-styles",f.textContent=`
                ...
                `,o.head.appendChild(f)}}function j(o){return t[o]?t[o]:o==="outline-blue"||o==="outline-orange"?t["outline-white"]:t["fill-white"]}function w(o,f,x,B){const _=j(x),N=f?"a":"span",R=f?`href="${f}"${B?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${N} ${R}
                class="banner-btn"
                data-bg="${_.bg}"
                data-color="${_.color}"
            ...
            `).join(""),L(),p(),y()}function L(){i.querySelectorAll(".banner-btn").forEach(o=>{const f=o.style.borderColor;o.addEventListener("mouseenter",()=>{o.style.background=o.dataset.hoverBg,o.style.color=o.dataset.hoverColor,o.style.borderColor=o.dataset.hoverBg}),o.addEventListener("mouseleave",()=>{o.style.background=o.dataset.bg,o.style.color=o.dataset.color,o.style.borderColor=f})})}function p(){s.addEventListener("mousedown",$),s.addEventListener("touchstart",$,{passive:!0}),s.addEventListener("mousemove",v),s.addEventListener("touchmove",v,{passive:!0}),s.addEventListener("mouseup",S),s.addEventListener("touchend",S),s.addEventListener("mouseleave",S)}function y(){r.forEach(o=>{const f=new Image;f.src=o.image_url})}function $(o){c=!0,h=o.touches?o.touches[0].clientX:o.clientX,d=0}function v(o){c&&(d=(o.touches?o.touches[0].clientX:o.clientX)-h)}function S(){c&&(c=!1,Math.abs(d)>=m&&(A(d<0?(l+1)%r.length:(l-1+r.length)%r.length),F()),d=0)}function z(){const o=i.querySelector(".banner-stripe");if(!o||r.length<=1)return;const f=document.createElement("div");f.className="banner-dots",r.forEach((x,B)=>{const _=document.createElement("button");_.className="banner-dot",_.dataset.index=String(B),_.setAttribute("aria-label",`Banner ${B+1}`),_.addEventListener("click",()=>{A(B),F()}),f.appendChild(_)}),o.innerHTML="",o.appendChild(f)}function A(o,f=!0){const x=s.querySelectorAll(".banner-slide"),B=i.querySelectorAll(".banner-dot");x.forEach((_,N)=>{const R=N===o;f||(_.style.transition="none"),_.classList.toggle("banner-slide--active",R),f||requestAnimationFrame(()=>{_.style.transition=""})}),B.forEach((_,N)=>{_.classList.toggle("banner-dot--active",N===o)}),l=o}function Y(){r.length<=1||!b||(n=setInterval(()=>{A((l+1)%r.length)},5e3))}function F(){b&&(clearInterval(n),Y())}function re(){const o=i.querySelector(".banner-wrapper");o&&(o.innerHTML=`
                    <div style="display:flex;align-items:center;justify-content:center;
                                min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;">
                        Sin contenido.
                    </div>`)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",g):g()}}const ma=[{id:"banner-hero",label:"Banner Slider",category:"Banners",media:ga,content:{type:"banner-hero-component"}}];function ba(i){const e="banner-hero-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Slider",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-autoplay":"true","data-category":""},components:`
...
@media(max-width:480px){
    .bsingle-btn{flex:1 1 100%;min-width:0;}
                ...
                </div>`}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",n):n()}}const Ba=[{id:"banner-single",label:"Banner Individual",category:"Banners",media:ya,content:{type:"banner-single-component"}}];function Ea(i){const e="banner-single-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Individual",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-banner-id":""},components:`
                ...
                `,script:ka(),traits:[{type:"select",name:"data-banner-id",label:"Banner a mostrar",options:[{id:"",name:"Cargando banners..."}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const a=this.get("script");a&&typeof a=="function"&&setTimeout(()=>a.call(t),100)})}}}),Ca(i,e),Sa(i,e),$a(i,e)}async function $a(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",r=await(await fetch(t)).json();if(!Array.isArray(r)||r.length===0)return;const l=i.DomComponents.getType(e);if(!l)return;const c=l.model.prototype.defaults.traits.find(h=>h.name==="data-banner-id");if(!c)return;c.options=[{id:"",name:"— Seleccionar banner —"},...r.map(h=>({id:String(h.id),name:h.category?`[${h.category}] ${h.title}`:h.title}))]}catch{}}function Ca(i,e){i.on("storage:end:load",()=>{setTimeout(()=>Me(i,e),1e3)}),i.on("component:mount",t=>{const a=t.getEl();a?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const r=t.get("script");r&&typeof r=="function"&&r.call(a)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const a=t.getEl();a&&setTimeout(()=>{const r=t.get("script");r&&typeof r=="function"&&r.call(a)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>Me(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Me(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const a=t.getEl();if(a?.isConnected){const r=t.get("script");r&&typeof r=="function"&&r.call(a)}})}function Sa(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const a=t.contentDocument?.head;if(a){if(!a.querySelector("#banner-single-styles")){const r=document.createElement("style");r.id="banner-single-styles",r.textContent=wa,a.appendChild(r)}if(!a.querySelector(`#${e}-editor-css`)){const r=document.createElement("style");r.id=`${e}-editor-css`,r.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
                    outline: 2px dashed rgba(240,135,42,0.6) !important;
            ...
            `,a.appendChild(r)}}})}function La(i){T.registerBlocks(gt),T.registerBlocks(wt),T.registerBlocks(ma),T.registerBlocks(Ba),T.registerBlocks(ui),T.registerBlocks(ii),T.registerBlocks(St),T.registerBlocks(Wt),T.registerBlocks(Yt),T.registerBlocks(qt),T.registerBlocks(hi),T.registerBlocks(Nt),T.registerBlocks(Zt),T.registerBlocks(yi),T.registerBlocks($i),T.registerBlocks(Li),T.registerBlocks(Ti),T.registerBlocks(Ai),T.registerBlocks(Pi),T.registerBlocks(ea),T.registerBlocks(la),T.registerBlocks(sa),T.applyToEditor(i),ca(i),ba(i),Ea(i),Ut(i),It(i),ta(i),ai(i),Hi(i)}function _a(i,e,t){i.on("component:add",()=>e.markAsDirty()),i.on("component:remove",()=>e.markAsDirty()),i.on("component:update",()=>e.markAsDirty()),i.on("style:update",()=>e.markAsDirty());const a=document.getElementById("save-button");a&&a.addEventListener("click",async()=>{await ja(i,e,t,a)}),document.addEventListener("keydown",r=>{(r.ctrlKey||r.metaKey)&&r.key==="s"&&(r.preventDefault(),a&&!a.disabled&&a.click())})}async function ja(i,e,t,a){a.disabled=!0,a.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{t.needsTitle()?await Ta(i,e,t):await Xe(i,e,t)}catch(r){We(r.message,"error")}finally{a.disabled=!1,a.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}async function Ta(i,e,t){return new Promise((a,r)=>{Ge({title:"Título de la Página",description:"Ingresa un título descriptivo para identificar esta página",placeholder:"Ej: Acerca de Nosotros",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async l=>{if(!l?.trim()){r(new Error("El título es obligatorio"));return}try{await Xe(i,e,t,l),a()}catch(n){r(n)}},onCancel:()=>{r(new Error("Guardado cancelado"))}})})}async function Xe(i,e,t,a=null){const l={...e.getEditorContent(i),is_published:t.isPublished};a&&(l.title=a);const n=await e.savePage(i,l,t.storeUrl,t.getHttpMethod());n.success&&(e.markAsClean(),We(n.message,"success"),!t.isEditMode&&n.page?(t.updatePageInfo(n),t.updateTitle(n.page.title)):a&&t.updateTitle(a))}function We(i,e){typeof window.showNotification=="function"&&window.showNotification(i,e)}document.addEventListener("DOMContentLoaded",async()=>{const i=new Je,e=new st;new ct(i);const t=Ze();if(t.on("load",()=>{La(t),Ke(t),Qe(),et(),tt(t),it(t),at(t),rt(t),lt(t),ot(t),za(t),Ia(t),Aa(t),setTimeout(()=>{t.runCommand("sw-visibility"),t.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),e.isEditMode)try{await i.loadPageContent(t,e.loadUrl),Ne("Contenido cargado correctamente","success")}catch(a){Ne("Error al cargar el contenido","error"),console.error(a)}_a(t,i,e)});function za(i){const e=[{cmd:"set-device-desktop",device:"Desktop"},{cmd:"set-device-tablet",device:"Tablet"},{cmd:"set-device-mobile",device:"Mobile"}];e.forEach(({cmd:t,device:a})=>{i.Commands.add(t,{run:r=>{r.setDevice(a),e.forEach(({cmd:l})=>{r.Panels.getButton("devices-c",l)?.set("active",l===t)})}})})}function Ia(i){i.Commands.add("canvas-clear",{run:e=>{nt({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{e.DomComponents.clear(),e.CssComposer.clear()}})}})}function Aa(i){const e=i.Canvas.getFrameEl();if(!e?.contentDocument?.head)return;const t=e.contentDocument.createElement("style");t.id="gjs-dashed-fix",t.textContent="body,body.gjs-dashed{margin:0!important;padding:0 0 160px 0!important;}",e.contentDocument.head.appendChild(t)}function Ne(i,e="info"){typeof window.showNotification=="function"&&window.showNotification(i,e)}
