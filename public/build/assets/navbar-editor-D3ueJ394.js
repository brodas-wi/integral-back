import{i as Y,I as K,o as Q,t as Z,d as ee,f as ne,e as te,s as oe,g as ae,c as re,E as ie,j as le,h as se}from"./editor-commands-Bq37dEX4.js";import"./_commonjsHelpers-CqkleIqs.js";function ce(){return Y()}const be=`(function(){
...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function de(r){return r?r.split(" ").filter(o=>o.length>0).map((o,i)=>`<span style="color:${i%2===0?"#E97300":"#003B71"};">${o}</span>`).join(" "):""}function G(r,o){o=o||"nb"+Math.random().toString(36).slice(2,7);const i=r.logo_href||"/",a=r.logo_url?`<img src="${r.logo_url}" alt="${r.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${r.logo_text||"Logo"}</span>`,y=`<a href="${i}" class="nb-logo-link">${a}</a>`,C=(r.top_actions||[]).map(m=>{const E=m.icon?`<i class="${m.icon}"></i>`:"";return`<a href="${m.href||"#"}" class="nb-top-action">${E}${m.label||""}</a>`}).join(""),B=r.banking_btn||{},w=B.color==="orange"?"nb-banking-orange":"nb-banking-blue",h=`<a href="${B.href||"#"}" class="nb-banking-btn ${w}">${B.label||"Banca en Línea"}</a>`,d=(r.nav_links||[]).map(m=>{if(m.type==="submenu"&&m.columns?.length){const E=!!(m.cta_column&&(m.cta_column.text||m.cta_column.btn_label)),q=m.columns.slice(0,3),A=q.map(_=>{const H=_.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",M=_.badge?`<span class="nb-mega-badge ${H}">${_.badge}</span>`:"",U=(_.items||[]).map(e=>{const t=e.desc?`<span class="nb-mega-item-desc">${e.desc}</span>`:"";return`<a href="${e.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${e.label||""}</span>${t}</a>`}).join("");return`<div class="nb-mega-col">${M}${U}</div>`}).join("");let T="";if(E){const _=m.cta_column,H=_.btn_color==="blue"?"nb-badge-blue":"nb-badge-orange";T=`<div class="nb-mega-cta-col">
                        <p class="nb-mega-cta-text">${de(_.text||"")}</p>
                        <a href="${_.btn_href||"#"}" class="nb-mega-cta-btn ${H}">${_.btn_label||"Ver más"}</a>
                    </div>`}const L=E?"grid-template-columns:1fr 1fr 1fr minmax(200px,260px);":`grid-template-columns:repeat(${q.length},minmax(0,280px));justify-content:start;`;return`<li class="nb-nav-item nb-has-submenu">
                    <button class="nb-nav-link nb-nav-trigger" type="button">${m.label||"Menú"}</button>
                    <div class="nb-mega"><div class="nb-mega-grid" style="${L}">${A}${T}</div></div>
                </li>`}return`<li class="nb-nav-item"><a href="${m.href||"#"}" class="nb-nav-link">${m.label||""}</a></li>`}).join(""),f=r.bottom_cta||{},k=f.color==="blue"?"nb-cta-blue":"nb-cta-orange",S=f.sublabel?`<span class="nb-bottom-cta-line2">${f.sublabel}</span>`:"",n=f.label?`<a href="${f.href||"#"}" class="nb-bottom-cta ${k}"><span class="nb-bottom-cta-line1">${f.label}</span>${S}</a>`:"",N=r.icon_btn||{},R=N.enabled?`<a href="${N.href||"#"}" class="nb-icon-btn" title="${N.label||""}"><i class="${N.icon||"ri-search-line"}"></i></a>`:"",J=(r.top_actions||[]).map(m=>{const E=m.icon?`<i class="${m.icon}"></i>`:"";return`<a href="${m.href||"#"}" class="nb-mobile-top-action">${E}${m.label||""}</a>`}).join(""),z=(r.nav_links||[]).map(m=>{if(m.type==="submenu"&&m.columns?.length){const E=m.columns.slice(0,3).map(q=>{const A=q.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",T=q.badge?`<span class="nb-mobile-sub-badge ${A}">${q.badge}</span>`:"",L=(q.items||[]).map(_=>{const H=_.desc?`<span class="nb-mobile-sub-desc">${_.desc}</span>`:"";return`<a href="${_.href||"#"}" class="nb-mobile-sub-link"><span class="nb-mobile-sub-title">${_.label||""}</span>${H}</a>`}).join("");return`<div class="nb-mobile-sub-col">${T}${L}</div>`}).join("");return`<div class="nb-mobile-item">
                ...
                </div>`}return`<a href="${m.href||"#"}" class="nb-mobile-link">${m.label||""}</a>`}).join(""),D=[h,f.label?`<a href="${f.href||"#"}" class="nb-bottom-cta ${k}"><span class="nb-bottom-cta-line1">${f.label}</span>${S}</a>`:""].filter(Boolean).join("");return`<div id="nb-root-${o}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false">
        <div class="nb-top">
        ...
        `,document.head.appendChild(e)}const a=(()=>{try{return JSON.parse(o.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),y=a.logo_url||"",C=a.logo_alt||"",B=a.logo_text||"Logo",w=a.logo_href||"/",h=JSON.parse(JSON.stringify(a.top_actions||O.top_actions)),d=JSON.parse(JSON.stringify(a.banking_btn||O.banking_btn)),f=JSON.parse(JSON.stringify(a.nav_links||O.nav_links)),k=JSON.parse(JSON.stringify(a.bottom_cta||O.bottom_cta)),S=document.createElement("div");S.id="navbar-config-modal",S.className="nb-overlay";const n=document.createElement("div");n.className="nb-modal",n.innerHTML=`
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,S.appendChild(n),document.body.appendChild(S);const N=new K;n.querySelectorAll(".nb-tab-btn").forEach(e=>{e.addEventListener("click",()=>{n.querySelectorAll(".nb-tab-btn").forEach(t=>t.classList.remove("active")),n.querySelectorAll(".nb-tab-panel").forEach(t=>t.classList.remove("active")),e.classList.add("active"),n.querySelector(`#nb-panel-${e.dataset.tab}`).classList.add("active")})}),n.querySelector("#nb-logo-pick").addEventListener("click",()=>{Q({type:"image",title:"Seleccionar logo",onSelect:e=>{n.querySelector("#nb-logo-url").value=e;let t=n.querySelector("#nb-logo-preview");if(!t||t.tagName==="DIV"){const c=document.createElement("img");c.id="nb-logo-preview",c.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;",t?.replaceWith(c)??n.querySelector("#nb-logo-url").before(c),t=c}t.src=e,t.style.display="block"}})});const J=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function z(e){if(e.dataset.autocompleteAttached)return;e.dataset.autocompleteAttached="true";const t=e.parentNode;(!t.style.position||t.style.position==="static")&&(t.style.position="relative");const c=document.createElement("ul");c.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",t.appendChild(c);let l=null;async function p(g){if(g.length<1){c.style.display="none";return}try{const $=await(await fetch(`${J}?q=${encodeURIComponent(g)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();u($,g)}catch{c.style.display="none"}}function b(g,s){return s?g.replace(new RegExp(`(${s.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):g}function u(g,s){if(c.innerHTML="",!g.length){c.style.display="none";return}g.forEach($=>{const x=document.createElement("li");x.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",x.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${b($.title,s)}</span><span style="font-size:0.7rem;color:#64748b;">/${$.slug}</span>`,x.addEventListener("mouseenter",()=>x.style.background="#f1f5f9"),x.addEventListener("mouseleave",()=>x.style.background=""),x.addEventListener("mousedown",j=>{j.preventDefault(),e.value="/"+$.slug,e.dispatchEvent(new Event("input")),c.style.display="none"}),c.appendChild(x)}),c.style.display="block"}e.addEventListener("input",()=>{clearTimeout(l),l=setTimeout(()=>p(e.value.trim()),220)}),e.addEventListener("focus",()=>{e.select(),e.value.trim()&&p(e.value.trim())}),e.addEventListener("blur",()=>{setTimeout(()=>{c.style.display="none"},150)}),e.addEventListener("keydown",g=>{if(c.style.display==="none")return;const s=c.querySelectorAll("li"),$=c.querySelector("li.nb-ac-active");let x=Array.from(s).indexOf($);if(g.key==="ArrowDown"){g.preventDefault(),$?.classList.remove("nb-ac-active");const j=s[x+1]||s[0];j?.classList.add("nb-ac-active"),j&&(j.style.background="#f1f5f9")}else if(g.key==="ArrowUp"){g.preventDefault(),$?.classList.remove("nb-ac-active");const j=s[x-1]||s[s.length-1];j?.classList.add("nb-ac-active"),j&&(j.style.background="#f1f5f9")}else g.key==="Enter"&&$?(g.preventDefault(),$.dispatchEvent(new MouseEvent("mousedown"))):g.key==="Escape"&&(c.style.display="none")})}function D(e,t,c){let l=null;e.querySelectorAll("[data-drag-idx]").forEach(p=>{p.setAttribute("draggable","true"),p.addEventListener("dragstart",b=>{l=parseInt(p.dataset.dragIdx),setTimeout(()=>p.classList.add("nb-dragging"),0),b.dataTransfer.effectAllowed="move"}),p.addEventListener("dragend",()=>{p.classList.remove("nb-dragging"),e.querySelectorAll(".nb-drag-over").forEach(b=>b.classList.remove("nb-drag-over"))}),p.addEventListener("dragover",b=>{b.preventDefault(),b.dataTransfer.dropEffect="move",parseInt(p.dataset.dragIdx)!==l&&p.classList.add("nb-drag-over")}),p.addEventListener("dragleave",()=>p.classList.remove("nb-drag-over")),p.addEventListener("drop",b=>{b.preventDefault();const u=parseInt(p.dataset.dragIdx);if(l!==null&&u!==l){const[g]=t.splice(l,1);t.splice(u,0,g),c()}l=null})})}function m(){const e=n.querySelector("#nb-top-actions-list");e.innerHTML="",h.forEach((t,c)=>{const l=document.createElement("div");l.className="nb-action-card",l.dataset.dragIdx=c;const p=t.icon||"";l.innerHTML=`
                <div class="nb-row">
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${t.href||"#"}" data-field="href">
                </div>`,l.querySelector(".nb-pick-icon-btn").addEventListener("click",()=>{N.open(u=>{t.icon=u,l.querySelector("[data-field='icon']").value=u,l.querySelector(".nb-icon-preview i").className=u})}),l.querySelector(".nb-remove-action").onclick=()=>{h.splice(c,1),m()},l.querySelectorAll("[data-field]").forEach(u=>{u.addEventListener("input",()=>{t[u.dataset.field]=u.value})});const b=l.querySelector(".nb-url-input");if(b){const u=document.createElement("div");u.style.position="relative",b.parentNode.insertBefore(u,b),u.appendChild(b),z(b)}e.appendChild(l)}),D(e,h,m)}function E(){const e=n.querySelector("#nb-nav-list");e.innerHTML="",f.forEach((t,c)=>{const l=document.createElement("div");l.className="nb-link-card",l.dataset.dragIdx=c;const p=`<span class="nb-type-badge ${t.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${t.type==="submenu"?"Submenú":"Link"} ↕</span>`;if(t.type==="submenu"){let g=function(){u.innerHTML="",(t.columns||[]).forEach((s,$)=>{const x=document.createElement("div");x.className="nb-col-card";const j=(s.items||[]).map((v,I)=>`
                            ...
                            <div class="nb-col-card-header">
                                <span style="font-size:0.75rem;font-weight:700;color:#475569;">Columna ${$+1}</span>
                    ...
                    </div>`,l.querySelector("[data-toggle-type]").onclick=()=>{t.type="submenu",t.columns=[{badge:"Columna 1",badge_color:"blue",items:[{label:"Enlace",desc:"",href:"#"}]}],t.cta_column={text:"",btn_label:"Ver más",btn_href:"#",btn_color:"orange"},delete t.href,E()},l.querySelector(".nb-remove-link").onclick=()=>{f.splice(c,1),E()},l.querySelectorAll("[data-field]").forEach(u=>{u.addEventListener("input",()=>{t[u.dataset.field]=u.value})});const b=l.querySelector(".nb-url-input");if(b){const u=document.createElement("div");u.style.position="relative",b.parentNode.insertBefore(u,b),u.appendChild(b),z(b)}}e.appendChild(l)}),D(e,f,E)}n.querySelectorAll("#nb-banking-blue, #nb-banking-orange").forEach(e=>{e.addEventListener("click",()=>{d.color=e.dataset.color,n.querySelector("#nb-banking-blue").classList.toggle("nb-color-inactive",d.color!=="blue"),n.querySelector("#nb-banking-orange").classList.toggle("nb-color-inactive",d.color!=="orange")})}),n.querySelectorAll("#nb-bcta-blue, #nb-bcta-orange").forEach(e=>{e.addEventListener("click",()=>{k.color=e.dataset.color,n.querySelector("#nb-bcta-blue").classList.toggle("nb-color-inactive",k.color!=="blue"),n.querySelector("#nb-bcta-orange").classList.toggle("nb-color-inactive",k.color!=="orange")})});const q=n.querySelector("#nb-banking-href");if(q){const e=document.createElement("div");e.style.position="relative",q.parentNode.insertBefore(e,q),e.appendChild(q),z(q)}const A=n.querySelector("#nb-bcta-href");if(A){const e=document.createElement("div");e.style.position="relative",A.parentNode.insertBefore(e,A),e.appendChild(A),z(A)}const T=n.querySelector("#nb-logo-href");if(T){const e=document.createElement("div");e.style.position="relative",T.parentNode.insertBefore(e,T),e.appendChild(T),z(T)}n.querySelector("#nb-add-top-action").onclick=()=>{h.push({label:"Nueva acción",href:"#",icon:"ri-star-line"}),m(),n.querySelector("#nb-top-actions-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},n.querySelector("#nb-add-nav-link").onclick=()=>{f.push({type:"link",label:"Nuevo enlace",href:"#"}),E(),n.querySelector("#nb-nav-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},n.querySelector("#nb-add-nav-submenu").onclick=()=>{f.push({type:"submenu",label:"Nuevo menú",columns:[{badge:"Columna 1",badge_color:"blue",items:[{label:"Enlace",desc:"",href:"#"}]}],cta_column:{text:"",btn_label:"Ver más",btn_href:"#",btn_color:"orange"}}),E(),n.querySelector("#nb-nav-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},m(),E();const L=JSON.parse(JSON.stringify(a.icon_btn||O.icon_btn)),_=n.querySelector("#nb-iconbtn-toggle-wrap");function H(){_.innerHTML=`
            <button class="nb-color-opt nb-color-opt-blue ${L.enabled!==!1?"":"nb-color-inactive"}" id="nb-iconbtn-on">Activado</button>
            <button class="nb-color-opt" style="background:#94a3b8;color:#fff;${L.enabled===!1?"":"opacity:0.35;"}" id="nb-iconbtn-off">Desactivado</button>`,n.querySelector("#nb-iconbtn-on").addEventListener("click",()=>{L.enabled=!0,H()}),n.querySelector("#nb-iconbtn-off").addEventListener("click",()=>{L.enabled=!1,H()})}H(),n.querySelector("#nb-iconbtn-icon").value=L.icon||"ri-search-line",n.querySelector("#nb-iconbtn-preview").className=L.icon||"ri-search-line",n.querySelector("#nb-iconbtn-label").value=L.label||"Buscar",n.querySelector("#nb-iconbtn-href").value=L.href||"#",n.querySelector("#nb-iconbtn-icon").addEventListener("input",e=>{L.icon=e.target.value,n.querySelector("#nb-iconbtn-preview").className=e.target.value}),n.querySelector("#nb-iconbtn-label").addEventListener("input",e=>{L.label=e.target.value}),n.querySelector("#nb-iconbtn-pick").addEventListener("click",()=>{N.open(e=>{L.icon=e,n.querySelector("#nb-iconbtn-icon").value=e,n.querySelector("#nb-iconbtn-preview").className=e})});const M=n.querySelector("#nb-iconbtn-href");if(M){const e=document.createElement("div");e.style.position="relative",M.parentNode.insertBefore(e,M),e.appendChild(M),z(M)}n.querySelector("#nb-modal-backup").onclick=()=>{const e={icon_btn:{enabled:L.enabled!==!1,icon:n.querySelector("#nb-iconbtn-icon").value.trim()||"ri-search-line",label:n.querySelector("#nb-iconbtn-label").value.trim()||"Buscar",href:n.querySelector("#nb-iconbtn-href").value.trim()||"#"},logo_url:n.querySelector("#nb-logo-url").value.trim(),logo_alt:n.querySelector("#nb-logo-alt").value.trim(),logo_text:n.querySelector("#nb-logo-text").value.trim(),logo_href:n.querySelector("#nb-logo-href").value.trim()||"/",top_actions:JSON.parse(JSON.stringify(h)),banking_btn:{label:n.querySelector("#nb-banking-label").value.trim(),href:n.querySelector("#nb-banking-href").value.trim()||"#",color:d.color||"blue"},nav_links:JSON.parse(JSON.stringify(f)),bottom_cta:{label:n.querySelector("#nb-bcta-label").value.trim(),sublabel:n.querySelector("#nb-bcta-sublabel").value.trim(),href:n.querySelector("#nb-bcta-href").value.trim()||"#",color:k.color||"orange"}},t=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),c=URL.createObjectURL(t),l=new Date().toISOString().replace(/[:.]/g,"-").slice(0,19),p=document.createElement("a");p.href=c,p.download=`navbar-backup-${l}.json`,document.body.appendChild(p),p.click(),p.remove(),URL.revokeObjectURL(c)},n.querySelector("#nb-modal-restore-input").onchange=e=>{const t=e.target.files?.[0];if(!t)return;const c=new FileReader;c.onload=l=>{let p;try{p=JSON.parse(l.target.result)}catch{const u=document.createElement("div");u.className="nb-confirm-overlay",u.innerHTML='<div class="nb-confirm-modal"><div class="nb-confirm-header"><i class="ri-error-warning-line" style="color:#ef4444;"></i><h3>Archivo inválido</h3></div><div class="nb-confirm-body"><p>El archivo seleccionado no es un JSON válido.</p></div><div class="nb-confirm-footer"><button class="nb-confirm-ok" style="background:#ef4444;">Cerrar</button></div></div>',document.body.appendChild(u),u.querySelector(".nb-confirm-ok").onclick=()=>u.remove(),e.target.value="";return}const b=document.createElement("div");b.className="nb-confirm-overlay",b.innerHTML=`
                ...
                </div>`,document.body.appendChild(b),b.querySelector(".nb-confirm-cancel").onclick=()=>{b.remove(),e.target.value=""},b.querySelector(".nb-confirm-ok").onclick=()=>{b.remove(),e.target.value="";const g=o.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);o.addAttributes({"data-navbar-config":JSON.stringify(p)}),o.components(G(p,g)+F),U(),W(r,o)}},c.readAsText(t)};const U=()=>{N.close(),S.remove()};n.querySelector("#nb-modal-close").onclick=U,n.querySelector("#nb-modal-cancel").onclick=U,S.onclick=e=>{e.target===S&&U()},n.querySelector("#nb-modal-save").onclick=()=>{const e={icon_btn:{enabled:L.enabled!==!1,icon:n.querySelector("#nb-iconbtn-icon").value.trim()||"ri-search-line",label:n.querySelector("#nb-iconbtn-label").value.trim()||"Buscar",href:n.querySelector("#nb-iconbtn-href").value.trim()||"#"},logo_url:n.querySelector("#nb-logo-url").value.trim(),logo_alt:n.querySelector("#nb-logo-alt").value.trim(),logo_text:n.querySelector("#nb-logo-text").value.trim(),logo_href:n.querySelector("#nb-logo-href").value.trim()||"/",top_actions:h,banking_btn:{label:n.querySelector("#nb-banking-label").value.trim(),href:n.querySelector("#nb-banking-href").value.trim()||"#",color:d.color||"blue"},nav_links:f,bottom_cta:{label:n.querySelector("#nb-bcta-label").value.trim(),sublabel:n.querySelector("#nb-bcta-sublabel").value.trim(),href:n.querySelector("#nb-bcta-href").value.trim()||"#",color:k.color||"orange"}},c=o.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);o.addAttributes({"data-navbar-config":JSON.stringify(e)}),o.components(G(e,c)+F),U()}}function me(r){const o="navbar-component";r.DomComponents.addType(o,{isComponent:i=>i.getAttribute?.("data-gjs-type")===o?{type:o}:!1,model:{defaults:{name:"Navbar",tagName:"nav",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":o,class:"nb-wrapper","data-navbar-config":JSON.stringify(O)},components:G(O)+F,script:function(){(function(i){if(!i||typeof i.querySelector!="function"||i.__nbInit)return;i.__nbInit=!0;var a=i.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");if(!a)return;var y=!!window.__gjseditor||document.documentElement.hasAttribute("data-gjs-canvas");function C(){y||(document.body.style.paddingTop=i.offsetHeight+"px")}C(),window.addEventListener("resize",function(){if(C(),window.innerWidth>992){var h=document.getElementById("nb-mobile-"+a);h&&h.classList.contains("nb-open")&&h.classList.remove("nb-open"),i.querySelectorAll(".nb-mobile-item.nb-open").forEach(function(d){d.classList.remove("nb-open")})}});var B=document.getElementById("nb-toggle-"+a),w=document.getElementById("nb-mobile-"+a);B&&w&&B.addEventListener("click",function(){w.classList.toggle("nb-open"),C()}),i.querySelectorAll(".nb-nav-trigger").forEach(function(h){h.addEventListener("click",function(d){d.stopPropagation();var f=h.closest(".nb-nav-item"),k=f.classList.contains("nb-open");i.querySelectorAll(".nb-nav-item.nb-open").forEach(function(S){S.classList.remove("nb-open")}),k||f.classList.add("nb-open"),C()})}),i.querySelectorAll(".nb-mobile-item>.nb-mobile-link").forEach(function(h){h.addEventListener("click",function(){h.closest(".nb-mobile-item").classList.toggle("nb-open"),C()})}),document.addEventListener("click",function(h){i.contains(h.target)||i.querySelectorAll(".nb-nav-item.nb-open").forEach(function(d){d.classList.remove("nb-open")})})})(this)},"script-props":[],toolbar:[],traits:[{type:"button",label:"Navbar",text:"Administrar Navbar",full:!0,command:"open-navbar-config"}]},init(){this.set("type",o),this.addAttributes({"data-gjs-type":o})}}}),r.Commands.add("open-navbar-config",{run(i){const a=i.getSelected();a&&W(i,a)}}),r.BlockManager.add("navbar-block",{label:"Navbar",category:"Navbar",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,activate:!0,content:{type:o,attributes:{"data-gjs-type":o}}}),ue(r,o),pe(r)}function ue(r,o){r.on("storage:end:load",()=>{setTimeout(()=>V(r,o),800)}),r.on("component:mount",i=>{const a=i.getEl();a?.getAttribute?.("data-gjs-type")===o&&(i.set("type",o),setTimeout(()=>X(r,a),400))}),r.on("canvas:render",()=>{setTimeout(()=>V(r,o),600)})}function X(r,o){if(o?.isConnected)try{const i=r.Canvas.getFrameEl()?.contentDocument;if(!i)return;o.__nbInit&&delete o.__nbInit;const a=i.createElement("script");a.textContent=`(function(){
            function initNavbar(root){
                if(!root||root.__nbInit)return;
                root.__nbInit=true;
                var id=root.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");
        ...
        })();`,i.head.appendChild(a),a.remove()}catch(i){console.warn("[Navbar] Error inyectando script en canvas:",i)}}function V(r,o){r.getWrapper().find(`[data-gjs-type="${o}"]`).forEach(i=>{i.set("type",o);const a=i.getEl();a?.isConnected&&X(r,a)})}function pe(r){r.on("load",()=>{const o=r.Canvas.getFrameEl();if(!o)return;const i=o.contentDocument,a=i?.head;if(a&&(i.documentElement?.setAttribute("data-gjs-canvas","true"),!a.querySelector("#navbar-component-css"))){const y=document.createElement("style");y.id="navbar-component-css",y.textContent=`
                [data-gjs-type="navbar-component"] {
                    position: relative !important;
                    top: auto !important;
                }
                body { padding-top: 0 !important; }
            `,a.appendChild(y)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new ie;let o=document.getElementById("navbar-id")?.value||"",i=document.getElementById("navbar-name")?.value||"",a=document.getElementById("navbar-load-url")?.value||"",y=document.getElementById("navbar-store-url")?.value||"";const C=document.getElementById("navbar-is-active")?.value==="1";let B=!!o;const w=ce();if(me(w),w.on("load",()=>{Z(w),ee(),ne(),te(w),oe(w),ae(w),re(w),fe(w),ge(w),setTimeout(()=>{w.runCommand("sw-visibility"),w.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),B&&a)try{await r.loadPageContent(w,a),P("Navbar cargado correctamente","success")}catch{P("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const d=document.getElementById("save-button");d.disabled=!0,d.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!B&&!i?le({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async f=>{if(!f?.trim()){P("El nombre es obligatorio","error"),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await h(f)}catch(k){P(k.message,"error")}finally{d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await h(i),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(f){P(f.message,"error"),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function h(d){const f=B?"PUT":"POST",k=r.getEditorContent(w);(!k.js_content||k.js_content.trim()==="")&&(k.js_content=be);const S=await r.savePage(w,{...k,name:d,is_active:C},y,f);if(S.success){if(r.markAsClean(),P(S.message,"success"),!B&&S.navbar){o=S.navbar.id,i=S.navbar.name,B=!0;const n=document.getElementById("navbar-id");n&&(n.value=o);const N=document.getElementById("navbar-name");N&&(N.value=i);const R=document.querySelector('meta[name="app-url"]'),J=R?R.content:"";y=y.endsWith("/navbars")?`${y}/${o}`:`${y.replace(/\/navbars\/?$/,"")}/navbars/${o}`;const D=document.getElementById("navbar-store-url");D&&(D.value=y),a=`${y}/load`;const m=document.getElementById("navbar-load-url");m&&(m.value=a);const E=document.getElementById("editor-title");E&&(E.textContent=`Editando Navbar: ${i}`);const q=`/navbars/edit/${o}/edit`,A=J?`${J}${q}`:q;window.history.replaceState({path:A},"",A)}else if(d){i=d;const n=document.getElementById("navbar-name");n&&(n.value=i);const N=document.getElementById("editor-title");N&&(N.textContent=`Editando Navbar: ${i}`)}}}});function fe(r){r.Commands.add("canvas-clear",{run:o=>{se({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{o.DomComponents.clear(),o.CssComposer.clear()}})}})}function ge(r){const o=["set-device-desktop","set-device-tablet","set-device-mobile"];function i(a){o.forEach(y=>{r.Panels.getButton("devices-c",y)?.set("active",y===a)})}r.Commands.add("set-device-desktop",{run(a){a.setDevice("desktop"),i("set-device-desktop")}}),r.Commands.add("set-device-tablet",{run(a){a.setDevice("tablet"),i("set-device-tablet")}}),r.Commands.add("set-device-mobile",{run(a){a.setDevice("mobile"),i("set-device-mobile")}}),r.on("device:select",a=>{const y=a.get?a.get("id"):a.id,C={desktop:"set-device-desktop",tablet:"set-device-tablet",mobile:"set-device-mobile"};C[y]&&i(C[y])}),setTimeout(()=>{i("set-device-desktop")},200)}function P(r,o="info"){typeof window.showNotification=="function"&&window.showNotification(r,o)}
