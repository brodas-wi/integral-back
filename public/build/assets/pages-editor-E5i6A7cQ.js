...
${Qt}`}],st=function(){(function(){function t(i){if(!i||i.__pcInit)return;i.__pcInit=!0;var a=i.querySelector(".pc-carousel-wrap");if(!a)return;var r=!1,l=0,o=0,n=!1,f=0,s=0,c=0,b=null;a.querySelectorAll("img").forEach(function(d){d.setAttribute("draggable","false")}),setTimeout(function(){var d=a.scrollWidth-a.clientWidth;if(d<=0)return;var u=Math.min(60,d),S=null;function v(j){S||(S=j);var q=(j-S)/400;if(q<.5)a.scrollLeft=u*(q*2);else if(q<1)a.scrollLeft=u*(1-(q-.5)*2);else{a.scrollLeft=0;return}requestAnimationFrame(v)}requestAnimationFrame(v)},400),a.scrollLeft=0;function h(){return a.scrollWidth-a.clientWidth}function x(d){return Math.max(0,Math.min(d,h()))}function m(){Math.abs(f)<.5||(f*=.92,a.scrollLeft=x(a.scrollLeft+f),b=requestAnimationFrame(m))}a.addEventListener("mousedown",function(d){d.button===0&&(b&&(cancelAnimationFrame(b),b=null),r=!0,n=!1,f=0,l=d.clientX,s=d.clientX,c=Date.now(),o=a.scrollLeft,a.style.cursor="grabbing",d.preventDefault())}),document.addEventListener("mousemove",function(d){if(r){var u=l-d.clientX;Math.abs(u)>3&&(n=!0);var S=Date.now(),v=S-c||1;f=(d.clientX-s)/v*16*-1,s=d.clientX,c=S,a.scrollLeft=x(o+u)}}),document.addEventListener("mouseup",function(d){r&&(r=!1,a.style.cursor="grab",n&&(d.stopPropagation(),b=requestAnimationFrame(m)))}),a.addEventListener("click",function(d){n&&(d.preventDefault(),d.stopPropagation(),n=!1)},!0);var E=0,A=0,B=0,y=0,g=0;a.addEventListener("touchstart",function(d){b&&(cancelAnimationFrame(b),b=null),E=d.touches[0].clientX,B=d.touches[0].clientX,y=Date.now(),A=a.scrollLeft,g=0},{passive:!0}),a.addEventListener("touchmove",function(d){var u=Date.now(),S=u-y||1,v=d.touches[0].clientX;g=(v-B)/S*16*-1,B=v,y=u;var j=E-v;a.scrollLeft=x(A+j)},{passive:!0}),a.addEventListener("touchend",function(){b=requestAnimationFrame(function d(){Math.abs(g)<.5||(g*=.92,a.scrollLeft=x(a.scrollLeft+g),b=requestAnimationFrame(d))})},{passive:!0})}function e(){document.querySelectorAll(".pc-section").forEach(function(i){delete i.__pcInit,t(i)})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",e):e()})()},ai=`(${st.toString()})();`,ri=`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function Na(){if(document.getElementById("tam-img-modal"))return;const t=document.createElement("div");t.id="tam-img-modal",t.innerHTML=`
        ...
        <div class="tam-container">
        ...
        </div>`,document.body.appendChild(a);let r=null,l=null;function o(h){r=h;const x=h.get("tableData");l=x?JSON.parse(JSON.stringify(x)):te(3,3);const m=l.cols||3;l.rows=l.rows.map((E,A)=>Array.from({length:m},(y,g)=>E[g]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})),document.getElementById("tam-title").value=l.title||"",document.getElementById("tam-theme").value=h.get("tableTheme")||"blue",document.getElementById("tam-cols").value=l.cols||3,document.getElementById("tam-rows").value=l.rows.length||3,c(),b(),a.classList.add("open"),document.body.style.overflow="hidden"}function n(){a.classList.remove("open"),document.body.style.overflow="",r=null}function f(){l.title=document.getElementById("tam-title").value.trim(),l.cols=parseInt(document.getElementById("tam-cols").value)||3,l.headers=Array.from(document.querySelectorAll(".tam-header-input")).map(x=>({text:x.value,align:x.closest("th")?.querySelector(".tam-align-select")?.value||"center"})),document.querySelectorAll("#tam-tbody td.tam-cell:not(.is-spanned)").forEach(x=>{const m=parseInt(x.dataset.row),E=parseInt(x.dataset.col);l.rows[m]?.[E]&&(l.rows[m][E].text=x.querySelector(".tam-cell-input")?.value||"",l.rows[m][E].align=x.querySelector(".tam-align-select")?.value||"center",l.rows[m][E].isHeader=x.dataset.isheader==="1",l.rows[m][E].image=x.dataset.image||null)});const h=Qe(l.rows,l.cols);l.rows=l.rows.map((x,m)=>x.filter((E,A)=>!h[`${m}-${A}`]))}function s(){if(a.querySelector("#tam-rebuild-notice"))return;const x=document.createElement("div");x.id="tam-rebuild-notice",x.style.cssText="background:#fef9c3;border:1.5px solid #ca8a04;border-radius:0.5rem;padding:0.5rem 1rem;font-size:0.8rem;color:#92400e;display:flex;align-items:center;gap:0.5rem;margin:0 1.5rem 0;flex-shrink:0;",x.innerHTML='<i class="ri-error-warning-line"></i> Has modificado el colspan/rowspan. Haz clic en <strong style="margin:0 4px;">Reconstruir</strong> para aplicar los cambios de fusión.',a.querySelector(".tam-toolbar").after(x)}function c(){a.querySelector("#tam-rebuild-notice")?.remove()}function b(){const h=document.getElementById("tam-thead"),x=document.getElementById("tam-tbody"),m=l.cols,E=l.rows.length,A=Qe(l.rows,m);h.innerHTML=`<tr>${l.headers.map((B,y)=>`
            <th>
                ...
                <select class="tam-align-select" style="margin-top:4px;width:100%;font-size:0.7rem;padding:2px;">
            ...
            </th>`).join("")}</tr>`,x.innerHTML=l.rows.map((B,y)=>`<tr>${Array.from({length:m},(d,u)=>{const S=A[`${y}-${u}`];if(S)return`<td class="tam-cell is-spanned" data-row="${y}" data-col="${u}">
                        <div class="tam-spanned-label">Combinada con [${S}]</div>
                    </td>`;const v=B[u]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null},j=v.colspan||1,q=v.rowspan||1,_=j>1||q>1;return`<td class="tam-cell ${v.isHeader?"is-header-cell":""} ${v.image?"has-image":""} ${_?"has-span":""}"
                    data-row="${y}" data-col="${u}"
                    ...
                    <div class="tam-cell-actions">
                        <button type="button" class="tam-cell-btn tam-cell-btn-header ${v.isHeader?"active":""}"
                            data-action="header" data-row="${y}" data-col="${u}">
                            ${v.isHeader?"✓ Etiqueta":"Etiqueta"}
                        </button>
                        <button type="button" class="tam-cell-btn tam-cell-btn-img"
                            data-action="image" data-row="${y}" data-col="${u}">
                            <i class="ri-image-line"></i> ${v.image?"Cambiar":"Imagen"}
                        </button>
                        ${v.image?`<button type="button" class="tam-cell-btn tam-cell-btn-clear" data-action="clear-image" data-row="${y}" data-col="${u}">✕ Quitar</button>`:""}
                        <div class="tam-cell-span-group">
                            ...
                            <input type="number" class="tam-cell-span-input" min="1" max="${m-u}"
                                value="${j}" data-action="colspan" data-row="${y}" data-col="${u}">
                            <label title="Filas que ocupa">RS</label>
                            <input type="number" class="tam-cell-span-input" min="1" max="${E-y}"
                                value="${q}" data-action="rowspan" data-row="${y}" data-col="${u}">
                        </div>
                    </div>
                </td>`}).join("")}</tr>`).join(""),x.querySelectorAll("input[data-action=colspan], input[data-action=rowspan]").forEach(B=>{B.addEventListener("change",()=>{const y=parseInt(B.dataset.row),g=parseInt(B.dataset.col),d=Math.max(1,parseInt(B.value)||1);l.rows[y]?.[g]&&(B.dataset.action==="colspan"?l.rows[y][g].colspan=Math.min(d,m-g):l.rows[y][g].rowspan=Math.min(d,E-y),s())})}),x.querySelectorAll("button[data-action]").forEach(B=>{B.addEventListener("click",y=>{y.preventDefault(),y.stopPropagation();const g=B.dataset.action,d=parseInt(B.dataset.row),u=parseInt(B.dataset.col);if(!(isNaN(d)||isNaN(u)||!l.rows[d]?.[u])){if(g==="header"){l.rows[d][u].isHeader=!l.rows[d][u].isHeader;const S=x.querySelector(`td[data-row="${d}"][data-col="${u}"]`);S&&(S.dataset.isheader=l.rows[d][u].isHeader?"1":"0",S.classList.toggle("is-header-cell",l.rows[d][u].isHeader)),B.classList.toggle("active",l.rows[d][u].isHeader),B.textContent=l.rows[d][u].isHeader?"✓ Etiqueta":"Etiqueta";return}if(g==="image"){window.__openTableImagePicker&&window.__openTableImagePicker(S=>{l.rows[d][u].image=S;const v=x.querySelector(`td[data-row="${d}"][data-col="${u}"]`);if(v){v.dataset.image=S,v.classList.add("has-image");let j=v.querySelector(".tam-cell-img-preview");j||(j=document.createElement("img"),j.className="tam-cell-img-preview",v.insertBefore(j,v.firstChild)),j.src=S;const q=v.querySelector("[data-action=image]");if(q&&(q.innerHTML='<i class="ri-image-line"></i> Cambiar'),!v.querySelector("[data-action=clear-image]")){const _=document.createElement("button");_.type="button",_.className="tam-cell-btn tam-cell-btn-clear",_.dataset.action="clear-image",_.dataset.row=d,_.dataset.col=u,_.textContent="✕ Quitar",_.addEventListener("click",D=>{D.preventDefault(),D.stopPropagation(),l.rows[d][u].image=null,v.dataset.image="",v.classList.remove("has-image"),j.remove(),_.remove();const M=v.querySelector("[data-action=image]");M&&(M.innerHTML='<i class="ri-image-line"></i> Imagen')}),v.querySelector(".tam-cell-actions").appendChild(_)}}});return}g==="clear-image"&&(l.rows[d][u].image=null,b())}})})}document.getElementById("tam-close").addEventListener("click",n),document.getElementById("tam-cancel").addEventListener("click",n),a.addEventListener("click",h=>{h.target===a&&n()}),document.getElementById("tam-rebuild").addEventListener("click",()=>{const h=parseInt(document.getElementById("tam-cols").value)||3,x=parseInt(document.getElementById("tam-rows").value)||3;for(c(),f();l.headers.length<h;)l.headers.push({text:`Col ${l.headers.length+1}`,align:"center"});for(l.headers=l.headers.slice(0,h),l.cols=h;l.rows.length<x;)l.rows.push(Array.from({length:h},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})));l.rows=l.rows.slice(0,x).map(m=>{for(;m.length<h;)m.push({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null});return m.slice(0,h)}),b()}),document.getElementById("tam-apply").addEventListener("click",()=>{f();const h=document.getElementById("tam-theme").value;r&&(r.set("tableData",JSON.parse(JSON.stringify(l))),r.set("tableTheme",h),r.addAttributes({"data-table-theme":h}),ze(r)),n()}),window.__openTableAdminModal=o}function ze(t){const e=t.get("tableData"),i=t.get("tableTheme")||"blue";e&&t.components(ht(pt(e,i),i))}function Pa(){return function(){}}const Ra=[{id:"table-blue",label:"Tabla azul",category:"Interactivos",media:Ma,content:{type:"table-component",attributes:{"data-table-theme":"blue"}}},{id:"table-orange",label:"Tabla naranja",category:"Interactivos",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<a href="#"
   ...
   class="dld-simple-${e}">
    ...
    <rect x="3" y="25" width="7" height="2.5" fill="#dee2e6" rx="0.5"/>
                ...
                `);const w=t.querySelector(`.${e}-map-container`);w&&(w.style.opacity="0.5")}function b(){const p=t.querySelector(`.${e}-map-container`);p&&(p.style.transition="opacity 0.3s ease",p.style.opacity="1")}async function h(){try{const w=await(await fetch(i)).json(),$=a?w[a]:w;Array.isArray($)?(r=$.filter(T=>T.latitude&&T.longitude&&!isNaN(T.latitude)&&!isNaN(T.longitude)),l=[...r],x()):(r=[],l=[])}catch(p){console.error("Error loading items:",p),r=[],l=[]}}function x(){const p=[...new Set(r.map($=>$.zone).filter(Boolean))].sort(),w=[...new Set(r.map($=>$.department).filter(Boolean))].sort();o={zone:p,department:w},setTimeout(()=>{const $=t.querySelector(`.${e}-filters-container`);$&&!$.hasChildNodes()&&($.innerHTML=`
...
.mp-map-wrapper{height:320px;}
        ...
        </div>`,n.appendChild(f),document.body.appendChild(n),f.querySelectorAll(".mp-tab-btn").forEach(h=>{h.addEventListener("click",()=>{f.querySelectorAll(".mp-tab-btn").forEach(x=>x.classList.remove("active")),f.querySelectorAll(".mp-tab-panel").forEach(x=>x.classList.remove("active")),h.classList.add("active"),f.querySelector(`#mp-panel-${h.dataset.tab}`).classList.add("active")})});const s=f.querySelector("#mp-filters-list");function c(){s.innerHTML="",o.forEach((h,x)=>{const m=document.createElement("div");m.className="mp-filter-card",m.innerHTML=`
...
@media(max-width:768px){.bsk-buttons{flex-direction:column;}}`;function Er(){return function(){const t=this,e=t.ownerDocument??document,i="/adminintegral",a=(e.defaultView??window).location.origin,r=e.querySelector('meta[name="api-banners-url"]')?.content||`${a}${i}/api/banners/active`,l=["fill-blue","outline-blue","fill-orange","outline-orange","fill-white","outline-white"],o=".banner-wrapper{position:relative;width:100%;background:#003B71;overflow:hidden;}.banner-slide-container{position:relative;width:100%;user-select:none;cursor:grab;display:grid;}.banner-slide-container:active{cursor:grabbing;}.banner-slide{grid-area:1/1;opacity:0;pointer-events:none;transition:opacity 0.7s ease;visibility:hidden;display:flex;flex-direction:column;}.banner-slide.banner-slide--active{opacity:1;pointer-events:auto;visibility:visible;}.banner-slide-inner{position:relative;width:100%;flex:1;min-height:480px;display:flex;align-items:center;}.banner-bg{position:absolute;inset:0;z-index:0;}.banner-bg img{width:100%;height:100%;object-fit:cover;object-position:center;display:block;}.banner-overlay{position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,59,113,0.85) 0%,rgba(0,59,113,0.65) 50%,rgba(0,59,113,0.2) 100%);z-index:1;}.banner-content{position:relative;z-index:10;display:flex;flex-direction:column;justify-content:center;padding:64px 64px 96px;max-width:60%;}.banner-category-badge{display:inline-block;margin-bottom:18px;padding:7px 20px;border-radius:999px;border:2px solid rgba(255,255,255,0.65);color:#fff;font-size:13px;font-weight:500;letter-spacing:0.04em;align-self:flex-start;}.banner-title{font-size:clamp(1.8rem,3.2vw,3rem);font-weight:800;color:#fff;line-height:1.15;margin:0 0 20px;}.banner-description{color:rgba(255,255,255,0.9);font-size:clamp(0.95rem,1.4vw,1.1rem);line-height:1.7;margin:0 0 36px;max-width:520px;}.banner-buttons{display:flex;flex-wrap:wrap;gap:16px;align-items:center;}.banner-btn{display:inline-block;padding:12px 32px;border-radius:999px;font-size:15px;font-weight:600;cursor:pointer;transition:background .2s,color .2s,border-color .2s;text-decoration:none;white-space:nowrap;line-height:1.4;box-sizing:border-box;}.banner-btn--fill-blue{background:#003B71;color:#ffffff;border:2px solid #003B71;}.banner-btn--fill-blue:hover{background:#002a52;border-color:#002a52;color:#ffffff;}.banner-btn--outline-blue{background:transparent;color:#003B71;border:2px solid #003B71;}.banner-btn--outline-blue:hover{background:#003B71;border-color:#003B71;color:#ffffff;}.banner-btn--fill-orange{background:#E97300;color:#ffffff;border:2px solid #E97300;}.banner-btn--fill-orange:hover{background:#c96200;border-color:#c96200;color:#ffffff;}.banner-btn--outline-orange{background:transparent;color:#E97300;border:2px solid #E97300;}.banner-btn--outline-orange:hover{background:#E97300;border-color:#E97300;color:#ffffff;}.banner-btn--fill-white{background:#ffffff;color:#003B71;border:2px solid #ffffff;}.banner-btn--fill-white:hover{background:#dce8f5;border-color:#dce8f5;color:#003B71;}.banner-btn--outline-white{background:transparent;color:#ffffff;border:2px solid #ffffff;}.banner-btn--outline-white:hover{background:#ffffff;border-color:#ffffff;color:#003B71;}.banner-dots-wrapper{display:none;}.banner-stripe{position:relative;width:100%;height:26px;background:#E97300;z-index:20;display:flex;align-items:center;justify-content:center;}.banner-dots{display:flex;gap:8px;align-items:center;}.banner-dot{width:10px;height:10px;border-radius:999px;border:none;cursor:pointer;background:rgba(255,255,255,0.5);transition:all 0.3s ease;padding:0;flex-shrink:0;}.banner-dot--active{width:28px;background:#ffffff;}.banner-empty{display:flex;align-items:center;justify-content:center;min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;}@media(max-width:768px){.banner-slide-inner{min-height:340px;}.banner-bg img{object-position:right center;}.banner-content{max-width:100%;width:100%;padding:32px 24px 80px;}.banner-buttons{flex-wrap:wrap;gap:12px;}.banner-btn{flex:1 1 45%;min-width:140px;text-align:center;white-space:normal;word-break:break-word;}}@media(max-width:480px){.banner-btn{flex:1 1 100%;min-width:0;}}",n="@keyframes bsk-shimmer{0%{background-position:-600px 0}100%{background-position:600px 0}}.bsk-badge{width:120px;height:32px;margin-bottom:20px;border-radius:999px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-title{height:40px;margin-bottom:12px;border-radius:8px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-title--short{width:60%;}.bsk-line{height:16px;margin-bottom:10px;border-radius:6px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-line--short{width:55%;}.bsk-buttons{display:flex;gap:16px;margin-top:36px;}.bsk-btn{height:48px;flex:1;border-radius:999px;background:linear-gradient(90deg,rgba(255,255,255,0.06) 25%,rgba(255,255,255,0.14) 50%,rgba(255,255,255,0.06) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}.bsk-bg-img{background:linear-gradient(90deg,rgba(255,255,255,0.04) 25%,rgba(255,255,255,0.09) 50%,rgba(255,255,255,0.04) 75%);background-size:600px 100%;animation:bsk-shimmer 1.6s infinite linear;}@media(max-width:768px){.bsk-buttons{flex-direction:column;}}";(function(){if(!e.getElementById("banner-hero-styles")){const z=e.createElement("style");z.id="banner-hero-styles",z.textContent=o,e.head.appendChild(z)}if(!e.getElementById("banner-skeleton-styles")){const z=e.createElement("style");z.id="banner-skeleton-styles",z.textContent=n,e.head.appendChild(z)}})();let f=[],s=0,c=null,b=!1,h=0,x=0;const m=50,E=t.dataset.autoplay!=="false",A=t.dataset.category??"",B=t.querySelector(".banner-slide-container"),y=t.querySelector(".banner-stripe");if(!B||!y)return;function g(k){return l.indexOf(k)!==-1?k:k==="outline-blue"||k==="outline-orange"?"outline-white":"fill-white"}function d(k,z,X,O){const R=g(X),H=z?"a":"span",L=z?`href="${z}"${O?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${H} ${L} class="banner-btn banner-btn--${R}">${k}</${H}>`}async function u(){if(!t.__bannerLoading){t.__bannerLoading=!0,S();try{const k=await fetch(r,{headers:{Accept:"application/json"}});if(!k.ok){T();return}const z=await k.json();if(f=Array.isArray(z)?A?z.filter(X=>X.category===A):z:[],f.length===0){T();return}v(),V(),p(0,!1),E&&w()}catch{T()}finally{t.__bannerLoading=!1}}}function S(){B.innerHTML=`
                ...
                </div>`).join(""),j(),q()}function j(){B.addEventListener("mousedown",_),B.addEventListener("touchstart",_,{passive:!0}),B.addEventListener("mousemove",D),B.addEventListener("touchmove",D,{passive:!0}),B.addEventListener("mouseup",M),B.addEventListener("touchend",M),B.addEventListener("mouseleave",M)}function q(){f.forEach(k=>{const z=new Image;z.src=k.image_url})}function _(k){b=!0,h=k.touches?k.touches[0].clientX:k.clientX,x=0}function D(k){b&&(x=(k.touches?k.touches[0].clientX:k.clientX)-h)}function M(){b&&(b=!1,Math.abs(x)>=m&&(p(x<0?(s+1)%f.length:(s-1+f.length)%f.length),$()),x=0)}function V(){if(f.length<=1){y.innerHTML="";return}const k=e.createElement("div");k.className="banner-dots",f.forEach((z,X)=>{const O=e.createElement("button");O.className="banner-dot",O.type="button",O.dataset.index=String(X),O.setAttribute("aria-label",`Banner ${X+1}`),O.addEventListener("click",()=>{p(X),$()}),k.appendChild(O)}),y.innerHTML="",y.appendChild(k)}function p(k,z=!0){const X=B.querySelectorAll(".banner-slide"),O=t.querySelectorAll(".banner-dot");X.forEach((R,H)=>{const L=H===k;z||(R.style.transition="none"),R.classList.toggle("banner-slide--active",L),z||requestAnimationFrame(()=>{R.style.transition=""})}),O.forEach((R,H)=>R.classList.toggle("banner-dot--active",H===k)),s=k}function w(){f.length<=1||!E||(c=setInterval(()=>p((s+1)%f.length),5e3))}function $(){E&&(clearInterval(c),w())}function T(){clearInterval(c),B.innerHTML=`
                <div class="banner-slide banner-slide--active">
                    <div class="banner-empty">Sin contenido.</div>
                </div>`,y.innerHTML=""}e.readyState==="loading"?e.addEventListener("DOMContentLoaded",u):u()}}function $r(t,e){const i=document.getElementById("banner-hero-config-modal");if(i&&i.remove(),!document.getElementById("bnr-modal-styles")){const m=document.createElement("style");m.id="bnr-modal-styles",m.textContent=`
...
.hb-btn{width:100%;text-align:center;}
        ...
        </div>`,s.appendChild(c),document.body.appendChild(s);const h=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function x(g){if(g.dataset.autocompleteAttached)return;g.dataset.autocompleteAttached="true";const d=g.parentNode;(!d.style.position||d.style.position==="static")&&(d.style.position="relative");const u=document.createElement("ul");u.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",d.appendChild(u);let S=null;async function v(_){if(_.length<1){u.style.display="none";return}try{const M=await(await fetch(`${h}?q=${encodeURIComponent(_)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();q(M,_)}catch{u.style.display="none"}}function j(_,D){return D?_.replace(new RegExp(`(${D.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):_}function q(_,D){if(u.innerHTML="",!_.length){u.style.display="none";return}_.forEach(M=>{const V=document.createElement("li");V.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",V.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${j(M.title,D)}</span><span style="font-size:0.7rem;color:#64748b;">/${M.slug}</span>`,V.addEventListener("mouseenter",()=>V.style.background="#f1f5f9"),V.addEventListener("mouseleave",()=>V.style.background=""),V.addEventListener("mousedown",p=>{p.preventDefault(),g.value=`${Dr}/${M.slug}`,g.dispatchEvent(new Event("input")),u.style.display="none"}),u.appendChild(V)}),u.style.display="block"}g.addEventListener("input",()=>{clearTimeout(S),S=setTimeout(()=>v(g.value.trim()),220)}),g.addEventListener("focus",()=>{g.select(),g.value.trim()&&v(g.value.trim())}),g.addEventListener("blur",()=>{setTimeout(()=>{u.style.display="none"},150)}),g.addEventListener("keydown",_=>{if(u.style.display==="none")return;const D=u.querySelectorAll("li"),M=u.querySelector("li.hb-ac-active");let V=Array.from(D).indexOf(M);if(_.key==="ArrowDown"){_.preventDefault(),M?.classList.remove("hb-ac-active");const p=D[V+1]||D[0];p?.classList.add("hb-ac-active"),p&&(p.style.background="#f1f5f9")}else if(_.key==="ArrowUp"){_.preventDefault(),M?.classList.remove("hb-ac-active");const p=D[V-1]||D[D.length-1];p?.classList.add("hb-ac-active"),p&&(p.style.background="#f1f5f9")}else _.key==="Enter"&&M?(_.preventDefault(),M.dispatchEvent(new MouseEvent("mousedown"))):_.key==="Escape"&&(u.style.display="none")})}x(c.querySelector("#hb-btn1-href")),x(c.querySelector("#hb-btn2-href"));let m=n.color||"white",E=f.color||"white";function A(g,d,u){const S=c.querySelector(`#${g}`);S.querySelectorAll("[data-color]").forEach(v=>{v.classList.toggle("hb-color-inactive",v.dataset.color!==d),v.addEventListener("click",()=>{S.querySelectorAll("[data-color]").forEach(j=>j.classList.toggle("hb-color-inactive",j.dataset.color!==v.dataset.color)),u(v.dataset.color)})})}A("hb-btn1-colors",m,g=>m=g),A("hb-btn2-colors",E,g=>E=g);function B(g,d,u){const S=c.querySelector(`#${g}`),v=c.querySelector(`#${d}`),j=c.querySelector(`#${u}`),q=()=>{v.style.background=S.checked?"#003B71":"#cbd5e1",j.style.left=S.checked?"21px":"3px"};q(),S.addEventListener("change",q)}B("hb-btn1-enabled","hb-btn1-slider","hb-btn1-knob"),B("hb-btn2-enabled","hb-btn2-slider","hb-btn2-knob"),c.querySelectorAll(".hb-tab-btn").forEach(g=>{g.addEventListener("click",()=>{c.querySelectorAll(".hb-tab-btn").forEach(d=>d.classList.remove("active")),c.querySelectorAll(".hb-tab-panel").forEach(d=>d.classList.remove("active")),g.classList.add("active"),c.querySelector(`#hb-panel-${g.dataset.tab}`).classList.add("active")})}),c.querySelector("#hb-bg-pick").addEventListener("click",()=>{$e({type:"image",title:"Seleccionar imagen de fondo",onSelect:g=>{c.querySelector("#hb-bg-url").value=g,c.querySelector("#hb-bg-preview").src=g}})}),c.querySelector("#hb-bg-url").addEventListener("input",g=>{c.querySelector("#hb-bg-preview").src=g.target.value});const y=()=>s.remove();c.querySelector("#hb-modal-close").onclick=y,c.querySelector("#hb-modal-cancel").onclick=y,s.onclick=g=>{g.target===s&&y()},c.querySelector("#hb-modal-save").onclick=()=>{const g={bg_image:c.querySelector("#hb-bg-url").value.trim()||Q.bg_image,title:c.querySelector("#hb-title").value.trim(),subtitle:c.querySelector("#hb-subtitle").value.trim(),btn_primary:{enabled:c.querySelector("#hb-btn1-enabled").checked,label:c.querySelector("#hb-btn1-label").value.trim(),href:c.querySelector("#hb-btn1-href").value.trim()||"#",color:m},btn_secondary:{enabled:c.querySelector("#hb-btn2-enabled").checked,label:c.querySelector("#hb-btn2-label").value.trim(),href:c.querySelector("#hb-btn2-href").value.trim()||"#",color:E}},u=e.getEl()?.querySelector("[id^='hb-root-']")?.id?.replace("hb-root-","")||"hb"+Math.random().toString(36).slice(2,7);e.addAttributes({"data-hero-banner-config":JSON.stringify(g)}),e.components(bt(g,u)+mt),y()}}function Or(t){const e="hero-banner-component";t.DomComponents.addType(e,{isComponent:i=>i.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-hero-banner-config":JSON.stringify(Q)},components:bt(Q)+mt,toolbar:[],traits:[{type:"button",label:"Banner",text:"Administrar Banner",full:!0,command:"open-hero-banner-config"}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),t.Commands.add("open-hero-banner-config",{run(i){const a=i.getSelected();a&&Nr(i,a)}}),t.BlockManager.add("hero-banner-block",{label:"Banner",category:"Banners",media:`<svg viewBox="0 0 32 32" width="32" height="32">
