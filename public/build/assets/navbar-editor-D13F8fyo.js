...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function le(r){return r?r.split(" ").filter(t=>t.length>0).map((t,a)=>`<span style="color:${a%2===0?"#E97300":"#003B71"};">${t}</span>`).join(" "):""}function O(r,t){t=t||"nb"+Math.random().toString(36).slice(2,7);const a=r.logo_href||"/",s=r.logo_url?`<img src="${r.logo_url}" alt="${r.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${r.logo_text||"Logo"}</span>`,h=`<a href="${a}" class="nb-logo-link">${s}</a>`,B=(r.top_actions||[]).map(b=>{const L=b.icon?`<i class="${b.icon}"></i>`:"";return`<a href="${b.href||"#"}" class="nb-top-action">${L}${b.label||""}</a>`}).join(""),C=r.banking_btn||{},x=C.color==="orange"?"nb-banking-orange":"nb-banking-blue",w=`<a href="${C.href||"#"}" class="nb-banking-btn ${x}">${C.label||"Banca en Línea"}</a>`,d=(r.nav_links||[]).map(b=>{if(b.type==="submenu"&&b.columns?.length){const L=!!(b.cta_column&&(b.cta_column.text||b.cta_column.btn_label)),_=b.columns.slice(0,3),I=_.map(E=>{const j=E.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",H=E.badge?`<span class="nb-mega-badge ${j}">${E.badge}</span>`:"",n=(E.items||[]).map(e=>{const c=e.desc?`<span class="nb-mega-item-desc">${e.desc}</span>`:"";return`<a href="${e.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${e.label||""}</span>${c}</a>`}).join("");return`<div class="nb-mega-col">${H}${n}</div>`}).join("");let q="";if(L){const E=b.cta_column,j=E.btn_color==="blue"?"nb-badge-blue":"nb-badge-orange";q=`<div class="nb-mega-cta-col">
                        <p class="nb-mega-cta-text">${le(E.text||"")}</p>
                        <a href="${E.btn_href||"#"}" class="nb-mega-cta-btn ${j}">${E.btn_label||"Ver más"}</a>
                    </div>`}const T=L?"grid-template-columns:1fr 1fr 1fr minmax(200px,260px);":`grid-template-columns:repeat(${_.length},minmax(0,280px));justify-content:start;`;return`<li class="nb-nav-item nb-has-submenu">
                    <button class="nb-nav-link nb-nav-trigger" type="button">${b.label||"Menú"}</button>
                    <div class="nb-mega"><div class="nb-mega-grid" style="${T}">${I}${q}</div></div>
                </li>`}return`<li class="nb-nav-item"><a href="${b.href||"#"}" class="nb-nav-link">${b.label||""}</a></li>`}).join(""),g=r.bottom_cta||{},k=g.color==="blue"?"nb-cta-blue":"nb-cta-orange",S=g.sublabel?`<span class="nb-bottom-cta-line2">${g.sublabel}</span>`:"",o=g.label?`<a href="${g.href||"#"}" class="nb-bottom-cta ${k}"><span class="nb-bottom-cta-line1">${g.label}</span>${S}</a>`:"",z=(r.top_actions||[]).map(b=>{const L=b.icon?`<i class="${b.icon}"></i>`:"";return`<a href="${b.href||"#"}" class="nb-mobile-top-action">${L}${b.label||""}</a>`}).join(""),D=(r.nav_links||[]).map(b=>{if(b.type==="submenu"&&b.columns?.length){const L=b.columns.flatMap(_=>_.items||[]).map(_=>{const I=_.desc?`<span class="nb-mobile-sub-desc">${_.desc}</span>`:"";return`<a href="${_.href||"#"}" class="nb-mobile-sub-link"><span class="nb-mobile-sub-title">${_.label||""}</span>${I}</a>`}).join("");return`<div class="nb-mobile-item">
                    <button class="nb-mobile-link" type="button">${b.label||"Menú"}<i class="ri-arrow-down-s-line"></i></button>
                    <div class="nb-mobile-submenu">${L}</div>
                </div>`}return`<a href="${b.href||"#"}" class="nb-mobile-link">${b.label||""}</a>`}).join("");return`<div id="nb-root-${t}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false">
        <div class="nb-top">
        ...
        <div class="nb-mobile-menu" id="nb-mobile-${t}">
            ...
            <div class="nb-mobile-banking">${w}</div>
            ${D}
        </div>
        ...
        `,document.head.appendChild(n)}const s=(()=>{try{return JSON.parse(t.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),h=s.logo_url||"",B=s.logo_alt||"",C=s.logo_text||"Logo",x=s.logo_href||"/",w=JSON.parse(JSON.stringify(s.top_actions||U.top_actions)),d=JSON.parse(JSON.stringify(s.banking_btn||U.banking_btn)),g=JSON.parse(JSON.stringify(s.nav_links||U.nav_links)),k=JSON.parse(JSON.stringify(s.bottom_cta||U.bottom_cta)),S=document.createElement("div");S.id="navbar-config-modal",S.className="nb-overlay";const o=document.createElement("div");o.className="nb-modal",o.innerHTML=`
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,S.appendChild(o),document.body.appendChild(S);const z=new G;o.querySelectorAll(".nb-tab-btn").forEach(n=>{n.addEventListener("click",()=>{o.querySelectorAll(".nb-tab-btn").forEach(e=>e.classList.remove("active")),o.querySelectorAll(".nb-tab-panel").forEach(e=>e.classList.remove("active")),n.classList.add("active"),o.querySelector(`#nb-panel-${n.dataset.tab}`).classList.add("active")})}),o.querySelector("#nb-logo-pick").addEventListener("click",()=>{oe({type:"image",title:"Seleccionar logo",onSelect:n=>{o.querySelector("#nb-logo-url").value=n;let e=o.querySelector("#nb-logo-preview");if(!e||e.tagName==="DIV"){const c=document.createElement("img");c.id="nb-logo-preview",c.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;",e?.replaceWith(c)??o.querySelector("#nb-logo-url").before(c),e=c}e.src=n,e.style.display="block"}})});const b=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function L(n){if(n.dataset.autocompleteAttached)return;n.dataset.autocompleteAttached="true";const e=n.parentNode;(!e.style.position||e.style.position==="static")&&(e.style.position="relative");const c=document.createElement("ul");c.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",e.appendChild(c);let i=null;async function y(f){if(f.length<1){c.style.display="none";return}try{const $=await(await fetch(`${b}?q=${encodeURIComponent(f)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();p($,f)}catch{c.style.display="none"}}function m(f,l){return l?f.replace(new RegExp(`(${l.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):f}function p(f,l){if(c.innerHTML="",!f.length){c.style.display="none";return}f.forEach($=>{const v=document.createElement("li");v.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",v.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${m($.title,l)}</span><span style="font-size:0.7rem;color:#64748b;">/${$.slug}</span>`,v.addEventListener("mouseenter",()=>v.style.background="#f1f5f9"),v.addEventListener("mouseleave",()=>v.style.background=""),v.addEventListener("mousedown",N=>{N.preventDefault(),n.value="/"+$.slug,n.dispatchEvent(new Event("input")),c.style.display="none"}),c.appendChild(v)}),c.style.display="block"}n.addEventListener("input",()=>{clearTimeout(i),i=setTimeout(()=>y(n.value.trim()),220)}),n.addEventListener("focus",()=>{n.select(),n.value.trim()&&y(n.value.trim())}),n.addEventListener("blur",()=>{setTimeout(()=>{c.style.display="none"},150)}),n.addEventListener("keydown",f=>{if(c.style.display==="none")return;const l=c.querySelectorAll("li"),$=c.querySelector("li.nb-ac-active");let v=Array.from(l).indexOf($);if(f.key==="ArrowDown"){f.preventDefault(),$?.classList.remove("nb-ac-active");const N=l[v+1]||l[0];N?.classList.add("nb-ac-active"),N&&(N.style.background="#f1f5f9")}else if(f.key==="ArrowUp"){f.preventDefault(),$?.classList.remove("nb-ac-active");const N=l[v-1]||l[l.length-1];N?.classList.add("nb-ac-active"),N&&(N.style.background="#f1f5f9")}else f.key==="Enter"&&$?(f.preventDefault(),$.dispatchEvent(new MouseEvent("mousedown"))):f.key==="Escape"&&(c.style.display="none")})}function _(n,e,c){let i=null;n.querySelectorAll("[data-drag-idx]").forEach(y=>{y.setAttribute("draggable","true"),y.addEventListener("dragstart",m=>{i=parseInt(y.dataset.dragIdx),setTimeout(()=>y.classList.add("nb-dragging"),0),m.dataTransfer.effectAllowed="move"}),y.addEventListener("dragend",()=>{y.classList.remove("nb-dragging"),n.querySelectorAll(".nb-drag-over").forEach(m=>m.classList.remove("nb-drag-over"))}),y.addEventListener("dragover",m=>{m.preventDefault(),m.dataTransfer.dropEffect="move",parseInt(y.dataset.dragIdx)!==i&&y.classList.add("nb-drag-over")}),y.addEventListener("dragleave",()=>y.classList.remove("nb-drag-over")),y.addEventListener("drop",m=>{m.preventDefault();const p=parseInt(y.dataset.dragIdx);if(i!==null&&p!==i){const[f]=e.splice(i,1);e.splice(p,0,f),c()}i=null})})}function I(){const n=o.querySelector("#nb-top-actions-list");n.innerHTML="",w.forEach((e,c)=>{const i=document.createElement("div");i.className="nb-action-card",i.dataset.dragIdx=c;const y=e.icon||"";i.innerHTML=`
                <div class="nb-row">
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${e.href||"#"}" data-field="href">
                </div>`,i.querySelector(".nb-pick-icon-btn").addEventListener("click",()=>{z.open(p=>{e.icon=p,i.querySelector("[data-field='icon']").value=p,i.querySelector(".nb-icon-preview i").className=p})}),i.querySelector(".nb-remove-action").onclick=()=>{w.splice(c,1),I()},i.querySelectorAll("[data-field]").forEach(p=>{p.addEventListener("input",()=>{e[p.dataset.field]=p.value})});const m=i.querySelector(".nb-url-input");if(m){const p=document.createElement("div");p.style.position="relative",m.parentNode.insertBefore(p,m),p.appendChild(m),L(m)}n.appendChild(i)}),_(n,w,I)}function q(){const n=o.querySelector("#nb-nav-list");n.innerHTML="",g.forEach((e,c)=>{const i=document.createElement("div");i.className="nb-link-card",i.dataset.dragIdx=c;const y=`<span class="nb-type-badge ${e.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${e.type==="submenu"?"Submenú":"Link"} ↕</span>`;if(e.type==="submenu"){let f=function(){p.innerHTML="",(e.columns||[]).forEach((l,$)=>{const v=document.createElement("div");v.className="nb-col-card";const N=(l.items||[]).map((u,A)=>`
                            <div class="nb-col-item" data-item-idx="${A}">
                                <div class="nb-col-item-row">
                                    <input class="nb-input-sm" style="flex:1;" placeholder="Título del item" value="${u.label||""}" data-item-field="label">
                                    <button class="nb-btn-remove nb-remove-item" style="padding:0.2rem;"><i class="ri-delete-bin-line"></i></button>
                                </div>
                                <input class="nb-input-sm" style="width:100%;box-sizing:border-box;" placeholder="Descripción (opcional)" value="${u.desc||""}" data-item-field="desc">
                                <div style="position:relative;">
                                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL" value="${u.href||"#"}" data-item-field="href">
                                </div>
                            ...
                            </div>`,v.querySelector("[data-col-field='badge']").addEventListener("input",u=>{l.badge=u.target.value}),v.querySelectorAll("[data-col-color]").forEach(u=>{u.addEventListener("click",()=>{l.badge_color=u.dataset.colColor,v.querySelectorAll("[data-col-color]").forEach(A=>{A.classList.toggle("nb-color-inactive",A.dataset.colColor!==u.dataset.colColor)})})}),v.querySelector(".nb-remove-col").onclick=()=>{e.columns.splice($,1),f();const u=i.querySelector(".nb-add-col-btn");u&&(u.disabled=!1,u.style.opacity="",u.style.cursor="")},v.querySelectorAll("[data-item-field]").forEach(u=>{const A=parseInt(u.closest("[data-item-idx]").dataset.itemIdx);u.addEventListener("input",()=>{l.items[A][u.dataset.itemField]=u.value})}),v.querySelectorAll(".nb-url-input").forEach(u=>{const A=document.createElement("div");A.style.position="relative",u.parentNode.insertBefore(A,u),A.appendChild(u),L(u)}),v.querySelectorAll(".nb-remove-item").forEach(u=>{u.onclick=()=>{const A=parseInt(u.closest("[data-item-idx]").dataset.itemIdx);l.items.splice(A,1),f()}}),v.querySelector(".nb-add-item-btn").onclick=()=>{l.items=l.items||[],l.items.push({label:"Nuevo item",desc:"",href:"#"}),f()},p.appendChild(v)})};i.innerHTML=`
                    ...
                    </div>`,i.querySelector("[data-toggle-type]").onclick=()=>{e.type="link",e.href="#",delete e.columns,delete e.cta_column,q()},i.querySelector(".nb-remove-link").onclick=()=>{g.splice(c,1),q()},i.querySelector("[data-field='label']").addEventListener("input",l=>{e.label=l.target.value}),i.querySelectorAll("[data-cta-field]").forEach(l=>{l.addEventListener("input",()=>{e.cta_column||(e.cta_column={}),e.cta_column[l.dataset.ctaField]=l.value})});const m=i.querySelector("[data-cta-field='btn_href']");if(m){const l=document.createElement("div");l.style.position="relative",m.parentNode.insertBefore(l,m),l.appendChild(m),L(m)}i.querySelectorAll("[data-cta-color]").forEach(l=>{l.addEventListener("click",()=>{e.cta_column||(e.cta_column={}),e.cta_column.btn_color=l.dataset.ctaColor,i.querySelectorAll("[data-cta-color]").forEach($=>{$.classList.toggle("nb-color-inactive",$.dataset.ctaColor!==l.dataset.ctaColor)})})});const p=i.querySelector(".nb-columns-list");f(),i.querySelector(".nb-add-col-btn").onclick=()=>{if(!((e.columns||[]).length>=3)&&(e.columns=e.columns||[],e.columns.push({badge:"Nueva columna",badge_color:"blue",items:[{label:"Nuevo item",desc:"",href:"#"}]}),f(),e.columns.length>=3)){const l=i.querySelector(".nb-add-col-btn");l.disabled=!0,l.style.opacity="0.4",l.style.cursor="not-allowed"}}}else{i.innerHTML=`
                    <div class="nb-link-card-header">
                        <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                        ${y}
            ...
            `,s.appendChild(h)}})}document.addEventListener("DOMContentLoaded",async()=>{const r=new ee;let t=document.getElementById("navbar-id")?.value||"",a=document.getElementById("navbar-name")?.value||"",s=document.getElementById("navbar-load-url")?.value||"",h=document.getElementById("navbar-store-url")?.value||"";const B=document.getElementById("navbar-is-active")?.value==="1";let C=!!t;const x=ae();if(se(x),x.on("load",()=>{V(x),W(),X(),Y(x),K(x),Q(x),Z(x),be(x),setTimeout(()=>{x.runCommand("sw-visibility"),x.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),C&&s)try{await r.loadPageContent(x,s),M("Navbar cargado correctamente","success")}catch{M("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const d=document.getElementById("save-button");d.disabled=!0,d.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!C&&!a?ne({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async g=>{if(!g?.trim()){M("El nombre es obligatorio","error"),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await w(g)}catch(k){M(k.message,"error")}finally{d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await w(a),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(g){M(g.message,"error"),d.disabled=!1,d.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function w(d){const g=C?"PUT":"POST",k=r.getEditorContent(x);(!k.js_content||k.js_content.trim()==="")&&(k.js_content=re);const S=await r.savePage(x,{...k,name:d,is_active:B},h,g);if(S.success){if(r.markAsClean(),M(S.message,"success"),!C&&S.navbar){t=S.navbar.id,a=S.navbar.name,C=!0;const o=document.getElementById("navbar-id");o&&(o.value=t);const z=document.getElementById("navbar-name");z&&(z.value=a);const D=document.querySelector('meta[name="app-url"]'),b=D?D.content:"";h=h.endsWith("/navbars")?`${h}/${t}`:`${h.replace(/\/navbars\/?$/,"")}/navbars/${t}`;const _=document.getElementById("navbar-store-url");_&&(_.value=h),s=`${h}/load`;const I=document.getElementById("navbar-load-url");I&&(I.value=s);const q=document.getElementById("editor-title");q&&(q.textContent=`Editando Navbar: ${a}`);const T=`/navbars/edit/${t}/edit`,E=b?`${b}${T}`:T;window.history.replaceState({path:E},"",E)}else if(d){a=d;const o=document.getElementById("navbar-name");o&&(o.value=a);const z=document.getElementById("editor-title");z&&(z.textContent=`Editando Navbar: ${a}`)}}}});function be(r){r.Commands.add("canvas-clear",{run:t=>{te({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{t.DomComponents.clear(),t.CssComposer.clear()}})}})}function M(r,t="info"){typeof window.showNotification=="function"&&window.showNotification(r,t)}
