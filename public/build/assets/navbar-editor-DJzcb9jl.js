import{i as te,I as Q,o as oe,t as ae,d as ie,f as re,e as le,s as se,g as ce,c as de,E as be,j as ue,h as me}from"./editor-commands-DULPa6UC.js";import"./_commonjsHelpers-CqkleIqs.js";function pe(){return te()}const fe="/bancaintegral",ge=`(function(){
...
@media(max-width:992px){
    ...
    .nb-hamburger{display:flex;}
...
</style>`;function ve(o){return o?o.split(" ").filter(a=>a.length>0).map((a,r)=>`<span style="color:${r%2===0?"#E97300":"#003B71"};">${a}</span>`).join(" "):""}function Y(o){return o?Array.isArray(o.buttons)?{text:o.text||"",buttons:o.buttons.slice(0,2).map(a=>({label:a.label||"",href:a.href||"#",color:a.color==="blue"?"blue":"orange",icon:a.icon||""}))}:o.btn_label||o.btn_href||o.btn_color?{text:o.text||"",buttons:[{label:o.btn_label||"",href:o.btn_href||"#",color:o.btn_color==="blue"?"blue":"orange",icon:""}]}:{text:o.text||"",buttons:[]}:{text:"",buttons:[]}}function ye(o){return(o||[]).map(a=>a.type==="submenu"&&a.cta_column?{...a,cta_column:Y(a.cta_column)}:a)}function X(o,a){a=a||"nb"+Math.random().toString(36).slice(2,7);const r=o.logo_href||"/",i=o.logo_url?`<img src="${o.logo_url}" alt="${o.logo_alt||"Logo"}">`:`<span class="nb-logo-text">${o.logo_text||"Logo"}</span>`,h=`<a href="${r}" class="nb-logo-link">${i}</a>`,I=(o.top_actions||[]).map(d=>{const $=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-top-action">${$}${d.label||""}</a>`}).join(""),A=o.banking_btn||{},S=A.color==="orange"?"nb-banking-orange":"nb-banking-blue",x=`<a href="${A.href||"#"}" class="nb-banking-btn ${S}">${A.label||"Banca en Línea"}</a>`,u=(o.nav_links||[]).map(d=>{if(d.type==="submenu"&&d.columns?.length){const C=d.cta_column?Y(d.cta_column):null,B=!!(C&&(C.text||C.buttons.length)),M=d.columns.slice(0,3),_=M.map(O=>{const R=O.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",j=O.badge?`<span class="nb-mega-badge ${R}">${O.badge}</span>`:"",n=(O.items||[]).map(e=>{const s=e.desc?`<span class="nb-mega-item-desc">${e.desc}</span>`:"";return`<a href="${e.href||"#"}" class="nb-mega-item"><span class="nb-mega-item-title">${e.label||""}</span>${s}</a>`}).join("");return`<div class="nb-mega-col">${j}${n}</div>`}).join("");let U="";if(B){const O=ve(C.text||""),R=C.buttons.slice(0,2).map(j=>{const n=j.color==="blue"?"nb-badge-blue":"nb-badge-orange",e=j.icon?`<i class="${j.icon}"></i>`:"";return`<a href="${j.href||"#"}" class="nb-mega-cta-btn ${n}">${e}${j.label||"Ver más"}</a>`}).join("");U=`<div class="nb-mega-cta-col">
                ...
                </li>`}const $=d.icon?`<i class="${d.icon}"></i>`:"";return`<li class="nb-nav-item"><a href="${d.href||"#"}" class="nb-nav-link">${$}${d.label||""}</a></li>`}).join(""),g=o.bottom_cta||{},w=g.color==="blue"?"nb-cta-blue":"nb-cta-orange",q=g.sublabel?`<span class="nb-bottom-cta-line2">${g.sublabel}</span>`:"",t=g.label?`<a href="${g.href||"#"}" class="nb-bottom-cta ${w}"><span class="nb-bottom-cta-line1">${g.label}</span>${q}</a>`:"",N=o.icon_btn||{},F=N.enabled?`<a href="${N.href||"#"}" class="nb-icon-btn" title="${N.label||""}"><i class="${N.icon||"ri-search-line"}"></i></a>`:"",G=(o.top_actions||[]).map(d=>{const $=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-mobile-top-action">${$}${d.label||""}</a>`}).join(""),T=(o.nav_links||[]).map(d=>{if(d.type==="submenu"&&d.columns?.length){const C=d.columns.slice(0,3).map(B=>{const M=B.badge_color==="orange"?"nb-badge-orange":"nb-badge-blue",_=B.badge?`<span class="nb-mobile-sub-badge ${M}">${B.badge}</span>`:"",U=(B.items||[]).map(D=>{const O=D.desc?`<span class="nb-mobile-sub-desc">${D.desc}</span>`:"";return`<a href="${D.href||"#"}" class="nb-mobile-sub-link"><span class="nb-mobile-sub-title">${D.label||""}</span>${O}</a>`}).join("");return`<div class="nb-mobile-sub-col">${_}${U}</div>`}).join("");return`<div class="nb-mobile-item">
                ...
                </div>`}const $=d.icon?`<i class="${d.icon}"></i>`:"";return`<a href="${d.href||"#"}" class="nb-mobile-link"><span class="nb-mobile-link-content">${$}${d.label||""}</span></a>`}).join(""),J=[x,g.label?`<a href="${g.href||"#"}" class="nb-bottom-cta ${w}"><span class="nb-bottom-cta-line1">${g.label}</span>${q}</a>`:""].filter(Boolean).join("");return`<div id="nb-root-${a}" data-gjs-editable="false" data-gjs-selectable="false" data-gjs-hoverable="false">
        <div class="nb-top">
        ...
        `,document.head.appendChild(n)}const i=(()=>{try{return JSON.parse(a.getAttributes()["data-navbar-config"]||"{}")}catch{return{}}})(),h=i.logo_url||"",I=i.logo_alt||"",A=i.logo_text||"Logo",S=i.logo_href||"/",x=JSON.parse(JSON.stringify(i.top_actions||P.top_actions)),u=JSON.parse(JSON.stringify(i.banking_btn||P.banking_btn)),g=ye(JSON.parse(JSON.stringify(i.nav_links||P.nav_links))),w=JSON.parse(JSON.stringify(i.bottom_cta||P.bottom_cta)),q=document.createElement("div");q.id="navbar-config-modal",q.className="nb-overlay";const t=document.createElement("div");t.className="nb-modal",t.innerHTML=`
        ...
        <div class="nb-modal-footer">
            ...
            <button id="nb-modal-save"   class="nb-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,q.appendChild(t),document.body.appendChild(q),t.addEventListener("focusin",n=>{const e=n.target;e.tagName==="INPUT"&&e.type==="text"&&!e.readOnly&&!e.classList.contains("nb-url-input")&&e.select()});const N=new Q;t.querySelectorAll(".nb-tab-btn").forEach(n=>{n.addEventListener("click",()=>{t.querySelectorAll(".nb-tab-btn").forEach(e=>e.classList.remove("active")),t.querySelectorAll(".nb-tab-panel").forEach(e=>e.classList.remove("active")),n.classList.add("active"),t.querySelector(`#nb-panel-${n.dataset.tab}`).classList.add("active")})}),t.querySelector("#nb-logo-pick").addEventListener("click",()=>{oe({type:"image",title:"Seleccionar logo",onSelect:n=>{t.querySelector("#nb-logo-url").value=n;let e=t.querySelector("#nb-logo-preview");if(!e||e.tagName==="DIV"){const s=document.createElement("img");s.id="nb-logo-preview",s.style.cssText="height:48px;max-width:160px;object-fit:contain;border-radius:0.375rem;border:1px solid #e2e8f0;padding:4px;background:#f8fafc;display:block;",e?.replaceWith(s)??t.querySelector("#nb-logo-url").before(s),e=s}e.src=n,e.style.display="block"}})});const G=`${document.querySelector('meta[name="app-url"]')?.content?.replace(/\/$/,"")??""}/api/pages/search`;function T(n){if(n.dataset.autocompleteAttached)return;n.dataset.autocompleteAttached="true";const e=n.parentNode;(!e.style.position||e.style.position==="static")&&(e.style.position="relative");const s=document.createElement("ul");s.style.cssText="position:absolute;top:calc(100% + 2px);left:0;right:0;z-index:999999;background:#fff;border:1px solid #e2e8f0;border-radius:0.5rem;box-shadow:0 8px 24px rgba(0,0,0,0.1);list-style:none;margin:0;padding:0.25rem;max-height:200px;overflow-y:auto;display:none;",e.appendChild(s);let l=null;async function m(c){if(c.length<1){s.style.display="none";return}try{const L=await(await fetch(`${G}?q=${encodeURIComponent(c)}`,{headers:{Accept:"application/json","X-Requested-With":"XMLHttpRequest"}})).json();p(L,c)}catch{s.style.display="none"}}function v(c,y){return y?c.replace(new RegExp(`(${y.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")})`,"gi"),'<mark style="background:#fef3c7;color:#92400e;border-radius:2px;padding:0 1px;">$1</mark>'):c}function p(c,y){if(s.innerHTML="",!c.length){s.style.display="none";return}c.forEach(L=>{const b=document.createElement("li");b.style.cssText="padding:0.375rem 0.625rem;border-radius:0.375rem;cursor:pointer;display:flex;flex-direction:column;gap:0.125rem;",b.innerHTML=`<span style="font-size:0.8rem;font-weight:600;color:#1e293b;">${v(L.title,y)}</span><span style="font-size:0.7rem;color:#64748b;">/${L.slug}</span>`,b.addEventListener("mouseenter",()=>b.style.background="#f1f5f9"),b.addEventListener("mouseleave",()=>b.style.background=""),b.addEventListener("mousedown",E=>{E.preventDefault(),n.value=`${fe}/${L.slug}`,n.dispatchEvent(new Event("input")),s.style.display="none"}),s.appendChild(b)}),s.style.display="block"}n.addEventListener("input",()=>{clearTimeout(l),l=setTimeout(()=>m(n.value.trim()),220)}),n.addEventListener("focus",()=>{n.select(),n.value.trim()&&m(n.value.trim())}),n.addEventListener("blur",()=>{setTimeout(()=>{s.style.display="none"},150)}),n.addEventListener("keydown",c=>{if(s.style.display==="none")return;const y=s.querySelectorAll("li"),L=s.querySelector("li.nb-ac-active");let b=Array.from(y).indexOf(L);if(c.key==="ArrowDown"){c.preventDefault(),L?.classList.remove("nb-ac-active");const E=y[b+1]||y[0];E?.classList.add("nb-ac-active"),E&&(E.style.background="#f1f5f9")}else if(c.key==="ArrowUp"){c.preventDefault(),L?.classList.remove("nb-ac-active");const E=y[b-1]||y[y.length-1];E?.classList.add("nb-ac-active"),E&&(E.style.background="#f1f5f9")}else c.key==="Enter"&&L?(c.preventDefault(),L.dispatchEvent(new MouseEvent("mousedown"))):c.key==="Escape"&&(s.style.display="none")})}function J(n,e,s){let l=null;n.querySelectorAll("[data-drag-idx]").forEach(m=>{m.draggable=!1;const v=m.querySelector(".nb-drag-handle"),p=()=>{m.draggable=!0},c=()=>{m.draggable=!1};v&&v.addEventListener("mousedown",p),document.addEventListener("mouseup",c),m.addEventListener("dragstart",y=>{l=parseInt(m.dataset.dragIdx),setTimeout(()=>m.classList.add("nb-dragging"),0),y.dataTransfer.effectAllowed="move"}),m.addEventListener("dragend",()=>{m.classList.remove("nb-dragging"),n.querySelectorAll(".nb-drag-over").forEach(y=>y.classList.remove("nb-drag-over")),c()}),m.addEventListener("dragover",y=>{y.preventDefault(),y.dataTransfer.dropEffect="move",parseInt(m.dataset.dragIdx)!==l&&m.classList.add("nb-drag-over")}),m.addEventListener("dragleave",()=>m.classList.remove("nb-drag-over")),m.addEventListener("drop",y=>{y.preventDefault();const L=parseInt(m.dataset.dragIdx);if(l!==null&&L!==l){const[b]=e.splice(l,1);e.splice(L,0,b),s()}l=null})})}function d(){const n=t.querySelector("#nb-top-actions-list");n.innerHTML="",x.forEach((e,s)=>{const l=document.createElement("div");l.className="nb-action-card",l.dataset.dragIdx=s;const m=e.icon||"";l.innerHTML=`
                <div class="nb-row">
                    <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                    <div class="nb-icon-preview"><i class="${m||"ri-star-line"}"></i></div>
                    <input class="nb-input-sm" style="width:130px;flex-shrink:0;" placeholder="Clase del icono" value="${m}" data-field="icon" readonly>
                ...
                <div style="position:relative;">
                    <input class="nb-input-sm nb-url-input" style="width:100%;box-sizing:border-box;" placeholder="URL o buscar página..." value="${e.href||"#"}" data-field="href">
                </div>`,l.querySelector(".nb-pick-icon-btn").addEventListener("click",()=>{N.open(p=>{e.icon=p,l.querySelector("[data-field='icon']").value=p,l.querySelector(".nb-icon-preview i").className=p})}),l.querySelector(".nb-remove-action").onclick=()=>{x.splice(s,1),d()},l.querySelectorAll("[data-field]").forEach(p=>{p.addEventListener("input",()=>{e[p.dataset.field]=p.value})});const v=l.querySelector(".nb-url-input");if(v){const p=document.createElement("div");p.style.position="relative",v.parentNode.insertBefore(p,v),p.appendChild(v),T(v)}n.appendChild(l)}),J(n,x,d)}function $(){const n=t.querySelector("#nb-nav-list");n.innerHTML="",g.forEach((e,s)=>{const l=document.createElement("div");l.className="nb-link-card",l.dataset.dragIdx=s;const m=`<span class="nb-type-badge ${e.type==="submenu"?"nb-type-submenu":"nb-type-link"}" data-toggle-type title="Clic para cambiar tipo">${e.type==="submenu"?"Submenú":"Link"} ↕</span>`;if(e.type==="submenu"){let c=function(){v.innerHTML="",e.cta_column.buttons.forEach((E,z)=>{const H=document.createElement("div");H.className="nb-action-card",H.innerHTML=`
                            <div class="nb-row">
                            ...
                            </div>`,H.querySelectorAll("[data-btn-field]").forEach(k=>{k.readOnly||k.addEventListener("input",()=>{E[k.dataset.btnField]=k.value})});const f=H.querySelector(".nb-url-input");f&&T(f),H.querySelectorAll("[data-btn-color]").forEach(k=>{k.addEventListener("click",()=>{E.color=k.dataset.btnColor,H.querySelectorAll("[data-btn-color]").forEach(K=>{K.classList.toggle("nb-color-inactive",K.dataset.btnColor!==k.dataset.btnColor)})})}),H.querySelector(".nb-pick-cta-icon").addEventListener("click",()=>{N.open(k=>{E.icon=k,c()})}),H.querySelector(".nb-remove-cta-btn").addEventListener("click",()=>{e.cta_column.buttons.splice(z,1),c()}),v.appendChild(H)});const b=e.cta_column.buttons.length>=2;p.disabled=b,p.style.opacity=b?"0.4":"",p.style.cursor=b?"not-allowed":""},L=function(){y.innerHTML="",(e.columns||[]).forEach((b,E)=>{const z=document.createElement("div");z.className="nb-col-card";const H=(b.items||[]).map((f,k)=>`
                            ...
                            </div>`,z.querySelector("[data-col-field='badge']").addEventListener("input",f=>{b.badge=f.target.value}),z.querySelectorAll("[data-col-color]").forEach(f=>{f.addEventListener("click",()=>{b.badge_color=f.dataset.colColor,z.querySelectorAll("[data-col-color]").forEach(k=>{k.classList.toggle("nb-color-inactive",k.dataset.colColor!==f.dataset.colColor)})})}),z.querySelector(".nb-remove-col").onclick=()=>{e.columns.splice(E,1),L();const f=l.querySelector(".nb-add-col-btn");f&&(f.disabled=!1,f.style.opacity="",f.style.cursor="")},z.querySelectorAll("[data-item-field]").forEach(f=>{const k=parseInt(f.closest("[data-item-idx]").dataset.itemIdx);f.addEventListener("input",()=>{b.items[k][f.dataset.itemField]=f.value})}),z.querySelectorAll(".nb-url-input").forEach(f=>{const k=document.createElement("div");k.style.position="relative",f.parentNode.insertBefore(k,f),k.appendChild(f),T(f)}),z.querySelectorAll(".nb-remove-item").forEach(f=>{f.onclick=()=>{const k=parseInt(f.closest("[data-item-idx]").dataset.itemIdx);b.items.splice(k,1),L()}}),z.querySelector(".nb-add-item-btn").onclick=()=>{b.items=b.items||[],b.items.push({label:"Nuevo item",desc:"",href:"#"}),L()},y.appendChild(z)})};e.cta_column=Y(e.cta_column),l.innerHTML=`
                    ...
                    <div class="nb-link-card-body">
                        ...
                        </div>`,l.querySelector("[data-toggle-type]").onclick=()=>{e.type="link",e.href="#",delete e.columns,delete e.cta_column,$()},l.querySelector(".nb-remove-link").onclick=()=>{g.splice(s,1),$()},l.querySelector("[data-field='label']").addEventListener("input",b=>{e.label=b.target.value}),l.querySelector("[data-cta-field='text']").addEventListener("input",b=>{e.cta_column.text=b.target.value});const v=l.querySelector(".nb-cta-buttons-list"),p=l.querySelector(".nb-add-cta-btn");c(),p.addEventListener("click",()=>{e.cta_column.buttons.length>=2||(e.cta_column.buttons.push({label:"Nuevo botón",href:"#",color:"orange",icon:""}),c())});const y=l.querySelector(".nb-columns-list");L(),l.querySelector(".nb-add-col-btn").onclick=()=>{if(!((e.columns||[]).length>=3)&&(e.columns=e.columns||[],e.columns.push({badge:"Nueva columna",badge_color:"blue",items:[{label:"Nuevo item",desc:"",href:"#"}]}),L(),e.columns.length>=3)){const b=l.querySelector(".nb-add-col-btn");b.disabled=!0,b.style.opacity="0.4",b.style.cursor="not-allowed"}}}else{l.innerHTML=`
                    <div class="nb-link-card-header">
                        <span class="nb-drag-handle"><i class="ri-draggable"></i></span>
                        ${m}
                    ...
                    </div>`,l.querySelector("[data-toggle-type]").onclick=()=>{e.type="submenu",e.columns=[{badge:"Columna 1",badge_color:"blue",items:[{label:"Enlace",desc:"",href:"#"}]}],e.cta_column={text:"",buttons:[]},delete e.href,$()},l.querySelector(".nb-remove-link").onclick=()=>{g.splice(s,1),$()},l.querySelectorAll("[data-field]").forEach(c=>{c.addEventListener("input",()=>{e[c.dataset.field]=c.value})});const v=l.querySelector("[data-field='icon']");v&&v.addEventListener("input",()=>{const c=l.querySelector(".nb-icon-preview i");c&&(c.className=v.value.trim()||"ri-star-line")}),l.querySelector(".nb-pick-link-icon").addEventListener("click",()=>{N.open(c=>{e.icon=c,v&&(v.value=c);const y=l.querySelector(".nb-icon-preview i");y&&(y.className=c)})});const p=l.querySelector(".nb-url-input");if(p){const c=document.createElement("div");c.style.position="relative",p.parentNode.insertBefore(c,p),c.appendChild(p),T(p)}}n.appendChild(l)}),J(n,g,$)}t.querySelectorAll("#nb-banking-blue, #nb-banking-orange").forEach(n=>{n.addEventListener("click",()=>{u.color=n.dataset.color,t.querySelector("#nb-banking-blue").classList.toggle("nb-color-inactive",u.color!=="blue"),t.querySelector("#nb-banking-orange").classList.toggle("nb-color-inactive",u.color!=="orange")})}),t.querySelectorAll("#nb-bcta-blue, #nb-bcta-orange").forEach(n=>{n.addEventListener("click",()=>{w.color=n.dataset.color,t.querySelector("#nb-bcta-blue").classList.toggle("nb-color-inactive",w.color!=="blue"),t.querySelector("#nb-bcta-orange").classList.toggle("nb-color-inactive",w.color!=="orange")})});const C=t.querySelector("#nb-banking-href");if(C){const n=document.createElement("div");n.style.position="relative",C.parentNode.insertBefore(n,C),n.appendChild(C),T(C)}const B=t.querySelector("#nb-bcta-href");if(B){const n=document.createElement("div");n.style.position="relative",B.parentNode.insertBefore(n,B),n.appendChild(B),T(B)}const M=t.querySelector("#nb-logo-href");if(M){const n=document.createElement("div");n.style.position="relative",M.parentNode.insertBefore(n,M),n.appendChild(M),T(M)}t.querySelector("#nb-add-top-action").onclick=()=>{x.push({label:"Nueva acción",href:"#",icon:"ri-star-line"}),d(),t.querySelector("#nb-top-actions-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},t.querySelector("#nb-add-nav-link").onclick=()=>{g.push({type:"link",label:"Nuevo enlace",href:"#"}),$(),t.querySelector("#nb-nav-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},t.querySelector("#nb-add-nav-submenu").onclick=()=>{g.push({type:"submenu",label:"Nuevo menú",columns:[{badge:"Columna 1",badge_color:"blue",items:[{label:"Enlace",desc:"",href:"#"}]}],cta_column:{text:"",buttons:[]}}),$(),t.querySelector("#nb-nav-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})},d(),$();const _=JSON.parse(JSON.stringify(i.icon_btn||P.icon_btn)),U=t.querySelector("#nb-iconbtn-toggle-wrap"),D="nb-iconbtn-switch-"+Math.random().toString(36).slice(2,6);U.innerHTML=`
        ...
        <span style="font-size:0.875rem;color:#475569;">${_.enabled!==!1?"Botón activo":"Botón desactivado"}</span>`,U.querySelector(`#${D}`).addEventListener("change",function(){_.enabled=this.checked;const n=U.querySelector("span:nth-child(2)"),e=U.querySelector("span:nth-child(3)"),s=U.querySelector("span:last-child");n&&(n.style.background=this.checked?"#003B71":"#cbd5e1"),e&&(e.style.left=this.checked?"21px":"3px"),s&&(s.textContent=this.checked?"Botón activo":"Botón desactivado")}),t.querySelector("#nb-iconbtn-icon").value=_.icon||"ri-search-line",t.querySelector("#nb-iconbtn-preview").className=_.icon||"ri-search-line",t.querySelector("#nb-iconbtn-label").value=_.label||"Buscar",t.querySelector("#nb-iconbtn-href").value=_.href||"#",t.querySelector("#nb-iconbtn-icon").addEventListener("input",n=>{_.icon=n.target.value,t.querySelector("#nb-iconbtn-preview").className=n.target.value}),t.querySelector("#nb-iconbtn-label").addEventListener("input",n=>{_.label=n.target.value}),t.querySelector("#nb-iconbtn-pick").addEventListener("click",()=>{new Q().open(e=>{_.icon=e,t.querySelector("#nb-iconbtn-icon").value=e,t.querySelector("#nb-iconbtn-preview").className=e})});const R=t.querySelector("#nb-iconbtn-href");if(R){const n=document.createElement("div");n.style.position="relative",R.parentNode.insertBefore(n,R),n.appendChild(R),T(R)}t.querySelector("#nb-modal-backup").onclick=()=>{const n={icon_btn:{enabled:_.enabled!==!1,icon:t.querySelector("#nb-iconbtn-icon").value.trim()||"ri-search-line",label:t.querySelector("#nb-iconbtn-label").value.trim()||"Buscar",href:t.querySelector("#nb-iconbtn-href").value.trim()||"#"},logo_url:t.querySelector("#nb-logo-url").value.trim(),logo_alt:t.querySelector("#nb-logo-alt").value.trim(),logo_text:t.querySelector("#nb-logo-text").value.trim(),logo_href:t.querySelector("#nb-logo-href").value.trim()||"/",top_actions:JSON.parse(JSON.stringify(x)),banking_btn:{label:t.querySelector("#nb-banking-label").value.trim(),href:t.querySelector("#nb-banking-href").value.trim()||"#",color:u.color||"blue"},nav_links:JSON.parse(JSON.stringify(g)),bottom_cta:{label:t.querySelector("#nb-bcta-label").value.trim(),sublabel:t.querySelector("#nb-bcta-sublabel").value.trim(),href:t.querySelector("#nb-bcta-href").value.trim()||"#",color:w.color||"orange"}},e=new Blob([JSON.stringify(n,null,2)],{type:"application/json"}),s=URL.createObjectURL(e),l=new Date().toISOString().replace(/[:.]/g,"-").slice(0,19),m=document.createElement("a");m.href=s,m.download=`navbar-backup-${l}.json`,document.body.appendChild(m),m.click(),m.remove(),URL.revokeObjectURL(s)},t.querySelector("#nb-modal-restore-input").onchange=n=>{const e=n.target.files?.[0];if(!e)return;const s=new FileReader;s.onload=l=>{let m;try{m=JSON.parse(l.target.result)}catch{const p=document.createElement("div");p.className="nb-confirm-overlay",p.innerHTML='<div class="nb-confirm-modal"><div class="nb-confirm-header"><i class="ri-error-warning-line" style="color:#ef4444;"></i><h3>Archivo inválido</h3></div><div class="nb-confirm-body"><p>El archivo seleccionado no es un JSON válido.</p></div><div class="nb-confirm-footer"><button class="nb-confirm-ok" style="background:#ef4444;">Cerrar</button></div></div>',document.body.appendChild(p),p.querySelector(".nb-confirm-ok").onclick=()=>p.remove(),n.target.value="";return}const v=document.createElement("div");v.className="nb-confirm-overlay",v.innerHTML=`
                ...
                </div>`,document.body.appendChild(v),v.querySelector(".nb-confirm-cancel").onclick=()=>{v.remove(),n.target.value=""},v.querySelector(".nb-confirm-ok").onclick=()=>{v.remove(),n.target.value="";const c=a.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);a.addAttributes({"data-navbar-config":JSON.stringify(m)}),a.components(X(m,c)+W),j(),ee(o,a)}},s.readAsText(e)};const j=()=>{N.close(),q.remove()};t.querySelector("#nb-modal-close").onclick=j,t.querySelector("#nb-modal-cancel").onclick=j,q.onclick=n=>{n.target===q&&j()},t.querySelector("#nb-modal-save").onclick=()=>{const n={icon_btn:{enabled:_.enabled!==!1,icon:t.querySelector("#nb-iconbtn-icon").value.trim()||"ri-search-line",label:t.querySelector("#nb-iconbtn-label").value.trim()||"Buscar",href:t.querySelector("#nb-iconbtn-href").value.trim()||"#"},logo_url:t.querySelector("#nb-logo-url").value.trim(),logo_alt:t.querySelector("#nb-logo-alt").value.trim(),logo_text:t.querySelector("#nb-logo-text").value.trim(),logo_href:t.querySelector("#nb-logo-href").value.trim()||"/",top_actions:x,banking_btn:{label:t.querySelector("#nb-banking-label").value.trim(),href:t.querySelector("#nb-banking-href").value.trim()||"#",color:u.color||"blue"},nav_links:g,bottom_cta:{label:t.querySelector("#nb-bcta-label").value.trim(),sublabel:t.querySelector("#nb-bcta-sublabel").value.trim(),href:t.querySelector("#nb-bcta-href").value.trim()||"#",color:w.color||"orange"}},s=a.getEl()?.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","")||"nb"+Math.random().toString(36).slice(2,7);a.addAttributes({"data-navbar-config":JSON.stringify(n)}),a.components(X(n,s)+W),j()}}function he(o){const a="navbar-component";o.DomComponents.addType(a,{isComponent:r=>r.getAttribute?.("data-gjs-type")===a?{type:a}:!1,model:{defaults:{name:"Navbar",tagName:"nav",draggable:!0,droppable:!1,removable:!0,copyable:!1,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":a,class:"nb-wrapper","data-navbar-config":JSON.stringify(P)},components:X(P)+W,script:function(){(function(r){if(!r||typeof r.querySelector!="function"||r.__nbInit)return;r.__nbInit=!0;var i=r.querySelector("[id^='nb-root-']")?.id?.replace("nb-root-","");if(!i)return;var h=!!window.__gjseditor||document.documentElement.hasAttribute("data-gjs-canvas");function I(){h||(document.body.style.paddingTop=r.offsetHeight+"px")}I(),window.addEventListener("resize",function(){if(I(),window.innerWidth>992){var x=document.getElementById("nb-mobile-"+i);x&&x.classList.contains("nb-open")&&x.classList.remove("nb-open"),r.querySelectorAll(".nb-mobile-item.nb-open").forEach(function(u){u.classList.remove("nb-open")})}});var A=document.getElementById("nb-toggle-"+i),S=document.getElementById("nb-mobile-"+i);A&&S&&A.addEventListener("click",function(){S.classList.toggle("nb-open"),I()}),r.querySelectorAll(".nb-nav-trigger").forEach(function(x){x.addEventListener("click",function(u){u.stopPropagation();var g=x.closest(".nb-nav-item"),w=g.classList.contains("nb-open");r.querySelectorAll(".nb-nav-item.nb-open").forEach(function(q){q.classList.remove("nb-open")}),w||g.classList.add("nb-open"),I()})}),r.querySelectorAll(".nb-mobile-item>.nb-mobile-link").forEach(function(x){x.addEventListener("click",function(){x.closest(".nb-mobile-item").classList.toggle("nb-open"),I()})}),document.addEventListener("click",function(x){r.contains(x.target)||r.querySelectorAll(".nb-nav-item.nb-open").forEach(function(u){u.classList.remove("nb-open")})})})(this)},"script-props":[],traits:[{type:"button",label:"Navbar",text:"Administrar Navbar",full:!0,command:"open-navbar-config"}]},init(){this.set("type",a),this.addAttributes({"data-gjs-type":a})}}}),o.Commands.add("open-navbar-config",{run(r){const i=r.getSelected();i&&ee(r,i)}}),o.BlockManager.add("navbar-block",{label:"Navbar",category:"Navbar",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        })();`,r.head.appendChild(i),i.remove()}catch(r){console.warn("[Navbar] Error inyectando script en canvas:",r)}}function Z(o,a){o.getWrapper().find(`[data-gjs-type="${a}"]`).forEach(r=>{r.set("type",a);const i=r.getEl();i?.isConnected&&ne(o,i)})}function ke(o){o.on("load",()=>{const a=o.Canvas.getFrameEl();if(!a)return;const r=a.contentDocument,i=r?.head;if(i&&(r.documentElement?.setAttribute("data-gjs-canvas","true"),!i.querySelector("#navbar-component-css"))){const h=document.createElement("style");h.id="navbar-component-css",h.textContent=`
                [data-gjs-type="navbar-component"] {
                    position: relative !important;
                    top: auto !important;
                }
                body { padding-top: 0 !important; }
            `,i.appendChild(h)}})}document.addEventListener("DOMContentLoaded",async()=>{const o=new be;let a=document.getElementById("navbar-id")?.value||"",r=document.getElementById("navbar-name")?.value||"",i=document.getElementById("navbar-load-url")?.value||"",h=document.getElementById("navbar-store-url")?.value||"";const I=document.getElementById("navbar-is-active")?.value==="1";let A=!!a;const S=pe();if(he(S),S.on("load",()=>{ae(S),ie(),re(),le(S),se(S),ce(S),de(S),we(S),Ee(S),setTimeout(()=>{S.runCommand("sw-visibility"),S.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),A&&i)try{await o.loadPageContent(S,i),V("Navbar cargado correctamente","success")}catch{V("Error al cargar el navbar","error")}document.getElementById("save-button")?.addEventListener("click",async()=>{const u=document.getElementById("save-button");u.disabled=!0,u.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{!A&&!r?ue({title:"Nombre del Navbar",description:"Ingresa un nombre descriptivo para identificar este navbar.",placeholder:"Ej: Navbar Principal",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async g=>{if(!g?.trim()){V("El nombre es obligatorio","error"),u.disabled=!1,u.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>';return}try{await x(g)}catch(w){V(w.message,"error")}finally{u.disabled=!1,u.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}},onCancel:()=>{u.disabled=!1,u.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}):(await x(r),u.disabled=!1,u.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>')}catch(g){V(g.message,"error"),u.disabled=!1,u.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}});async function x(u){const g=A?"PUT":"POST",w=o.getEditorContent(S);(!w.js_content||w.js_content.trim()==="")&&(w.js_content=ge);const q=await o.savePage(S,{...w,name:u,is_active:I},h,g);if(q.success){if(o.markAsClean(),V(q.message,"success"),!A&&q.navbar){a=q.navbar.id,r=q.navbar.name,A=!0;const t=document.getElementById("navbar-id");t&&(t.value=a);const N=document.getElementById("navbar-name");N&&(N.value=r);const F=document.querySelector('meta[name="app-url"]'),G=F?F.content:"";h=h.endsWith("/navbars")?`${h}/${a}`:`${h.replace(/\/navbars\/?$/,"")}/navbars/${a}`;const J=document.getElementById("navbar-store-url");J&&(J.value=h),i=`${h}/load`;const d=document.getElementById("navbar-load-url");d&&(d.value=i);const $=document.getElementById("editor-title");$&&($.textContent=`Editando Navbar: ${r}`);const C=`/navbars/edit/${a}/edit`,B=G?`${G}${C}`:C;window.history.replaceState({path:B},"",B)}else if(u){r=u;const t=document.getElementById("navbar-name");t&&(t.value=r);const N=document.getElementById("editor-title");N&&(N.textContent=`Editando Navbar: ${r}`)}}}});function we(o){o.Commands.add("canvas-clear",{run:a=>{me({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{a.DomComponents.clear(),a.CssComposer.clear()}})}})}function Ee(o){const a=["set-device-desktop","set-device-tablet","set-device-mobile"];function r(i){a.forEach(h=>{o.Panels.getButton("devices-c",h)?.set("active",h===i)})}o.Commands.add("set-device-desktop",{run(i){i.setDevice("desktop"),r("set-device-desktop")}}),o.Commands.add("set-device-tablet",{run(i){i.setDevice("tablet"),r("set-device-tablet")}}),o.Commands.add("set-device-mobile",{run(i){i.setDevice("mobile"),r("set-device-mobile")}}),o.on("device:select",i=>{const h=i.get?i.get("id"):i.id,I={desktop:"set-device-desktop",tablet:"set-device-tablet",mobile:"set-device-mobile"};I[h]&&r(I[h])}),setTimeout(()=>{r("set-device-desktop")},200)}function V(o,a="info"){typeof window.showNotification=="function"&&window.showNotification(o,a)}
