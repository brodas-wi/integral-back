/* empty css                   *//* empty css                    *//* empty css                          *//* empty css                           */import{o as Fe,M as Re,j as Ue,E as Ve,i as We,t as Ye,d as Xe,f as Ge,e as Je,s as Ze,g as Ke,c as Qe,b as et,a as tt,h as it}from"./editor-commands-Bq37dEX4.js";import{a as N}from"./url-DaqOO3yL.js";import"./_commonjsHelpers-CqkleIqs.js";class at{constructor(){this.pageId=this.getElementValue("page-id"),this.pageSlug=this.getElementValue("page-slug"),this.pageTitle=this.getPageTitle(),this.loadUrl=this.getElementValue("page-load-url"),this.storeUrl=this.getElementValue("page-store-url"),this.isPublished=this.getElementValue("page-is-published")==="1",this.isEditMode=!!this.pageId,this.appUrl=(document.querySelector('meta[name="app-url"]')?.content??"").replace(/\/$/,"")}getElementValue(e,t=""){const r=document.getElementById(e);return r?r.value.trim():t}getPageTitle(){const e=document.getElementById("editor-title");if(!e)return"";const r=e.textContent.match(/(?:Editando:|Nueva Página)\s*(.+)?/);return r&&r[1]?r[1].trim():""}updatePageInfo(e){if(e.page)if(this.pageId=e.page.id,this.pageSlug=e.page.slug,this.pageTitle=e.page.title,this.isEditMode=!0,document.getElementById("page-id").value=this.pageId,document.getElementById("page-slug").value=this.pageSlug,e.page.update_url?this.storeUrl=e.page.update_url:this.storeUrl=`${this.appUrl}/pages/${this.pageSlug}`,document.getElementById("page-store-url").value=this.storeUrl,e.page.load_url?this.loadUrl=e.page.load_url:this.loadUrl=`${this.appUrl}/pages/${this.pageSlug}/load`,document.getElementById("page-load-url").value=this.loadUrl,e.page.edit_url)window.history.replaceState(null,"",e.page.edit_url);else try{const t=new URL(window.location.href);t.pathname=t.pathname.replace(/\/pages\/create\/?$/,`/pages/${this.pageSlug}/edit`),window.history.replaceState(null,"",t.toString())}catch(t){console.error("Error updating browser URL:",t)}}updateTitle(e){this.pageTitle=e;const t=document.getElementById("editor-title");t&&(t.textContent=`Editando: ${e}`),document.title=`Editar: ${e} - Editor`}getHttpMethod(){return this.isEditMode?"PUT":"POST"}needsTitle(){return!this.isEditMode}}class rt{constructor(e){this.editorService=e,this.setupBackButton(),this.setupBeforeUnload()}setupBackButton(){const e=document.querySelector("#editor-navbar a[href]");e&&e.addEventListener("click",t=>{this.editorService.shouldPreventUnload()&&(t.preventDefault(),this.showUnsavedChangesModal(()=>{this.editorService.startNavigation(),window.location.href=e.href}))})}setupBeforeUnload(){window.addEventListener("beforeunload",e=>{if(this.editorService.shouldPreventUnload())return e.preventDefault(),e.returnValue="Tienes cambios sin guardar. ¿Estás seguro de que quieres salir?",e.returnValue})}showUnsavedChangesModal(e){const t=document.createElement("div");t.style.cssText=`
            position: fixed;
            inset: 0;
            z-index: 9999;
            display: flex;
        ...
        `;const n=document.createElement("button");n.textContent="Salir sin guardar",n.style.cssText=`
            ...
            font-family: inherit;
        `,[c,n].forEach(f=>{f.addEventListener("mouseenter",()=>{f.style.opacity="0.85"}),f.addEventListener("mouseleave",()=>{f.style.opacity="1"})});const h=()=>t.remove();c.addEventListener("click",h),n.addEventListener("click",()=>{h(),e()}),t.addEventListener("click",f=>{f.target===t&&h()}),l.appendChild(c),l.appendChild(n),r.appendChild(a),r.appendChild(l),t.appendChild(r),document.body.appendChild(t)}}const lt="Básico";class ot{constructor(){this.blocks=new Map}registerBlock(e,t){this.blocks.has(t.category)||this.blocks.set(t.category,[]),this.blocks.get(t.category).push({id:e,...t})}registerBlocks(e){e.forEach(t=>{this.registerBlock(t.id,t)})}applyToEditor(e){this.blocks.forEach(t=>{t.forEach(r=>{const{id:a,...l}=r;e.BlockManager.add(a,l)})}),this.applyCollapseSettings(e),this.hideDefaultCategories(e)}applyCollapseSettings(e){setTimeout(()=>{e.BlockManager.getCategories().each(t=>{t.set("open",t.get("label")===lt)})},500)}hideDefaultCategories(e){setTimeout(()=>{const t=["Basic","Extra"];document.querySelectorAll(".gjs-block-category").forEach(r=>{const a=r.querySelector(".gjs-title");a&&t.includes(a.textContent.trim())&&(r.style.display="none")})},100)}}const S=new ot,nt=[{id:"heading1",label:"Título H1",category:"Básico",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,content:'<div class="h-12 w-full"></div>'}],st=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,ut=[{id:"columns-1",label:"1 Columna",category:"Columnas",media:st,content:`
<div class="col-section">
    <div class="col-cell"></div>
</div>
${V}`},{id:"columns-2",label:"2 Columnas",category:"Columnas",media:ct,content:`
<div class="col-section">
...
${V}`},{id:"columns-3",label:"3 Columnas",category:"Columnas",media:dt,content:`
<div class="col-section">
...
${V}`},{id:"columns-4",label:"4 Columnas",category:"Columnas",media:ft,content:`
<div class="col-section">
...
${V}`},{id:"columns-1-3",label:"1/3 — 2/3",category:"Columnas",media:gt,content:`
<div class="col-section">
...
${V}`},{id:"columns-3-1",label:"2/3 — 1/3",category:"Columnas",media:ht,content:`
<div class="col-section">
...
${V}`},{id:"columns-1-2",label:"1/4 — 1/4 — 1/2",category:"Columnas",media:pt,content:`
<div class="col-section">
...
${V}`},{id:"columns-2-1",label:"1/2 — 1/4 — 1/4",category:"Columnas",media:mt,content:`
<div class="col-section">
...
</style>`,kt=[{id:"about-section",label:"Nuestra Historia",category:"Contenido",media:bt,content:`
...
${xe}`},{id:"about-mvv-section",label:"Misión, Visión y Valores",category:"Contenido",media:xt,content:`
...
@media(max-width:480px){.pc-card{flex:0 0 80vw;}}`;function Et(i){const e=i.img||N("images/placeholder.svg"),t=i.title||"TÍTULO DEL PRODUCTO",r=i.desc||"Descripción breve del producto financiero.",a=i.href||"#",l=i.btn_label||"Solicitar";return`<div class="pc-card"><div class="pc-card-img-wrap"><img src="${e}" alt="${t}" class="pc-card-img"></div><div class="pc-card-body"><h3 class="pc-card-title">${t}</h3><p class="pc-card-desc">${r}</p></div><a href="${a}" class="pc-btn">${l}</a></div>`}function Me(i){const e=i.heading||"Créditos",t=i.subheading||"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",r=i.autoplay?"true":"false",a=i.more_href||"#",l=i.more_label||"Ver más",n=(i.cards||[]).map(Et).join("");return`<section class="pc-section" data-autoplay="${r}"><style>${Bt}</style><div style="text-align:center;margin-bottom:2rem;"><h2 class="pc-section-heading">${e}</h2><p class="pc-section-subheading">${t}</p></div><div class="pc-carousel-wrap"><div class="pc-track">${n}</div></div><div class="pc-more-wrap"><a href="${a}" class="pc-more-btn">${l}</a></div></section><script>${qe}<\/script>`}const M={heading:"Créditos",subheading:"Opciones de financiamiento diseñadas para hacer realidad tus proyectos.",autoplay:!1,more_href:"#",more_label:"Ver más",cards:[{img:"",title:"CREDINVIERTE",desc:"Adquiere activos fijos",href:"#",btn_label:"Solicitar"},{img:"",title:"SOLUCIONES INTEGRALES",desc:"Financiamiento PYME",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDI-CONFIAMOS",desc:"Rápido y sin fiador",href:"#",btn_label:"Solicitar"},{img:"",title:"CREDILÍNEA",desc:"Línea rotativa",href:"#",btn_label:"Solicitar"}]};function $t(i,e){const t=document.getElementById("pc-config-modal");if(t&&t.remove(),!document.getElementById("pc-modal-styles")){const f=document.createElement("style");f.id="pc-modal-styles",f.textContent=`
        ...
        <div class="pc-modal-footer">
            ...
            <button id="pc-modal-save" class="pc-btn-save"><i class="ri-check-line"></i> Aplicar cambios</button>
        </div>`,l.appendChild(c),document.body.appendChild(l),c.querySelectorAll(".pc-tab-btn").forEach(f=>{f.addEventListener("click",()=>{c.querySelectorAll(".pc-tab-btn").forEach(u=>u.classList.remove("active")),c.querySelectorAll(".pc-tab-panel").forEach(u=>u.classList.remove("active")),f.classList.add("active"),c.querySelector(`#pc-panel-${f.dataset.tab}`).classList.add("active")})});function n(){const f=c.querySelector("#pc-cards-list");f.innerHTML="",a.cards.forEach((u,x)=>{const m=document.createElement("div");m.className="pc-card-config";const s=u.img?`<img class="pc-img-preview" src="${u.img}" alt="">`:'<div class="pc-img-placeholder"><i class="ri-image-line"></i></div>';m.innerHTML=`
                ...
                <div class="pc-row">
                    <div id="pc-img-wrap-${x}">${s}</div>
                    <div style="flex:1;display:flex;flex-direction:column;gap:0.5rem;">
                        <input class="pc-input-sm" placeholder="URL de la imagen" value="${u.img||""}" data-field="img">
                        <button class="pc-pick-btn pc-pick-img"><i class="ri-image-line"></i> Seleccionar imagen</button>
                    </div>
                </div>
                <div>
                    <label class="pc-label">Título</label>
                    <input class="pc-input" placeholder="TÍTULO DEL PRODUCTO" value="${u.title||""}" data-field="title">
                </div>
                <div>
                    <label class="pc-label">Descripción</label>
                    <input class="pc-input" placeholder="Descripción breve" value="${u.desc||""}" data-field="desc">
                </div>
                <div class="pc-row">
                    <div style="flex:1;">
                        <label class="pc-label">URL del botón</label>
                        <input class="pc-input" placeholder="#" value="${u.href||"#"}" data-field="href">
                    </div>
                    <div style="flex:1;">
                        <label class="pc-label">Texto del botón</label>
                        <input class="pc-input" placeholder="Solicitar" value="${u.btn_label||"Solicitar"}" data-field="btn_label">
                    </div>
                </div>`,m.querySelectorAll("[data-field]").forEach(g=>{g.addEventListener("input",()=>{if(u[g.dataset.field]=g.value,g.dataset.field==="img"){const b=m.querySelector(`#pc-img-wrap-${x}`);b.innerHTML=g.value?`<img class="pc-img-preview" src="${g.value}" alt="">`:'<div class="pc-img-placeholder"><i class="ri-image-line"></i></div>'}})}),m.querySelector(".pc-pick-img").addEventListener("click",()=>{Fe({type:"image",title:"Seleccionar imagen de tarjeta",onSelect:g=>{u.img=g,m.querySelector("[data-field='img']").value=g,m.querySelector(`#pc-img-wrap-${x}`).innerHTML=`<img class="pc-img-preview" src="${g}" alt="">`}})}),m.querySelector(".pc-remove-card").addEventListener("click",()=>{a.cards.splice(x,1),n()}),f.appendChild(m)})}n(),c.querySelector("#pc-add-card").addEventListener("click",()=>{a.cards.push({img:"",title:"NUEVO PRODUCTO",desc:"Descripción del producto.",href:"#",btn_label:"Solicitar"}),n(),c.querySelector("#pc-cards-list").lastElementChild?.scrollIntoView({behavior:"smooth",block:"nearest"})});const h=()=>l.remove();c.querySelector("#pc-modal-close").addEventListener("click",h),c.querySelector("#pc-modal-cancel").addEventListener("click",h),l.addEventListener("click",f=>{f.target===l&&h()}),c.querySelector("#pc-modal-save").addEventListener("click",()=>{a.heading=c.querySelector("#pc-heading").value.trim()||M.heading,a.subheading=c.querySelector("#pc-subheading").value.trim()||M.subheading,a.autoplay=c.querySelector("#pc-autoplay").checked,a.more_label=c.querySelector("#pc-more-label").value.trim()||M.more_label,a.more_href=c.querySelector("#pc-more-href").value.trim()||"#",e.addAttributes({"data-product-cards-config":JSON.stringify(a)}),e.components(Me(a)),setTimeout(()=>de(i),300),h()})}function de(i){try{const e=i.Canvas.getFrameEl()?.contentDocument;if(!e)return;const t=e.getElementById("pc-runtime-script");t&&t.remove(),e.querySelectorAll(".pc-section").forEach(a=>{delete a.__pcInit});const r=e.createElement("script");r.id="pc-runtime-script",r.textContent=qe,e.head.appendChild(r)}catch(e){console.warn("[ProductCards] Error reiniciando carrusel:",e)}}const Ct=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`;function Lt(i){const e="product-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección de Productos",tagName:"div",draggable:!0,droppable:!1,removable:!0,copyable:!0,selectable:!0,hoverable:!0,editable:!1,highlightable:!1,attributes:{"data-gjs-type":e,"data-product-cards-config":JSON.stringify(M)},components:Me(M),script:function(){(function(){function t(l){if(!l||l.__pcInit)return;l.__pcInit=!0;var c=l.querySelector(".pc-track"),n=l.querySelector(".pc-carousel-wrap");if(!c||!n)return;var h=l.dataset.autoplay==="true",f=!1,u=0,x=0,m=null;Array.from(c.children).forEach(function(b){var C=b.cloneNode(!0);C.setAttribute("aria-hidden","true"),C.classList.add("pc-clone"),c.appendChild(C)});function s(){return c.scrollWidth/2}function g(){n.scrollLeft>=s()?n.scrollLeft-=s():n.scrollLeft<=0&&(n.scrollLeft=s()-n.offsetWidth)}h&&(m=setInterval(function(){n.scrollLeft+=1,g()},16)),n.addEventListener("scroll",g,{passive:!0}),n.addEventListener("mousedown",function(b){f=!0,c.classList.add("is-dragging"),u=b.pageX-n.offsetLeft,x=n.scrollLeft,m&&clearInterval(m)}),document.addEventListener("mouseup",function(){f&&(f=!1,c.classList.remove("is-dragging"),h&&(m=setInterval(function(){n.scrollLeft+=1,g()},16)))}),document.addEventListener("mousemove",function(b){if(f){b.preventDefault();var C=b.pageX-n.offsetLeft;n.scrollLeft=x-(C-u)*1.5,g()}}),n.addEventListener("touchstart",function(b){u=b.touches[0].pageX-n.offsetLeft,x=n.scrollLeft,m&&clearInterval(m)},{passive:!0}),n.addEventListener("touchend",function(){h&&(m=setInterval(function(){n.scrollLeft+=1,g()},16))},{passive:!0}),n.addEventListener("touchmove",function(b){var C=b.touches[0].pageX-n.offsetLeft;n.scrollLeft=x-(C-u)*1.5,g()},{passive:!0})}var r=this,a=r.querySelector(".pc-section")||r;delete a.__pcInit,t(a)}).call(this)},"script-props":["data-product-cards-config"],traits:[{type:"button",label:"Productos",text:"Administrar Sección",full:!0,command:"open-product-cards-config"}],toolbar:[]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),i.Commands.add("open-product-cards-config",{run(t){const r=t.getSelected();r&&$t(t,r)}}),i.BlockManager.add("product-cards-block",{label:"Sección de productos",category:"Productos y Servicios",media:Ct,activate:!0,content:{type:e,attributes:{"data-gjs-type":e}}}),i.on("component:mount",t=>{t.getEl()?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>de(i),400))}),i.on("component:selected",t=>{const r=t.getEl();if(!r)return;const a=r.closest(`[data-gjs-type="${e}"]`);if(a&&!r.hasAttribute("data-gjs-type")){const c=i.getWrapper().find(`[data-gjs-type="${e}"]`).find(n=>n.getEl()===a);c&&setTimeout(()=>i.select(c),0)}}),i.on("canvas:render",()=>{setTimeout(()=>de(i),600)}),i.on("storage:end:load",()=>{setTimeout(()=>de(i),800)})}const St=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,jt=[{id:"dual-card-section",label:"Sección dos tarjetas",category:"Productos y Servicios",media:St,content:`
...
${pe}`},{id:"dual-card-dark",label:"Tarjeta azul oscuro",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
        ...
        </svg>`,content:`${ye}${pe}`},{id:"dual-card-light",label:"Tarjeta blanca",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
        ...
        </svg>`,content:`${ve}${pe}`}],_t=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,Tt=[{id:"cta-section",label:"Llamada a la acción",category:"Llamadas a la acción",media:_t,content:`
...
${It}`}],zt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,qt=[{id:"icon-grid-hero",label:"Sección de características",category:"Productos y Servicios",media:zt,content:`
...
${Dt}`},{id:"icon-card",label:"Tarjeta con icono",category:"Productos y Servicios",media:At,content:`${G}`}],we=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Ot=[{id:"product-link-cards",label:"Tarjetas de productos con enlace",category:"Productos y Servicios",media:we,content:`
...
${me}`},{id:"product-link-card-orange",label:"Tarjeta producto naranja",category:"Productos y Servicios",media:we,media:Mt,content:`${K}${me}`},{id:"product-link-card-blue",label:"Tarjeta producto azul",category:"Productos y Servicios",media:Nt,content:`${Q}${me}`}],ke=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Pt=[{id:"icon-links-strip",label:"Tira de iconos con enlace",category:"Productos y Servicios",media:ke,content:`
...
${Be}`},{id:"icon-link-item",label:"Ítem icono con enlace",category:"Productos y Servicios",media:ke,content:`${ee}${Be}`}],Ht=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Rt=[{id:"promo-cta",label:"Promo con CTA y precio",category:"Llamadas a la acción",media:Ht,content:`
...
${Ft}`}],Ut=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
.tab-card:hover .tab-card-text{color:#ffffff;}`;function Vt(){return function(){const i=this,e=".tabs-btn{transition:background 0.2s,color 0.2s;}.tabs-btn.active,.tabs-btn:hover{background:#003B71 !important;color:#ffffff !important;}.tabs-panel{display:none;}.tabs-panel.active{display:grid;}.tab-card:hover{background:#003B71;}.tab-card:hover .tab-card-icon{color:#ffffff;border-color:rgba(255,255,255,0.6);}.tab-card:hover .tab-card-title{color:#ffffff;}.tab-card:hover .tab-card-text{color:#ffffff;}";(function(){const a=i.ownerDocument??document;if(!a.getElementById("tabs-section-styles")){const l=a.createElement("style");l.id="tabs-section-styles",l.textContent=e,a.head.appendChild(l)}})();function t(r){i.querySelectorAll(".tabs-btn").forEach((a,l)=>{a.classList.toggle("active",l===r)}),i.querySelectorAll(".tabs-panel").forEach((a,l)=>{a.classList.toggle("active",l===r)})}i.querySelectorAll(".tabs-btn").forEach((r,a)=>{r.addEventListener("click",()=>t(a))}),t(0)}}const D=()=>`
...
<style>${be}</style>`,Yt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,Xt=[{id:"tabs-cards",label:"Sección con tabs",category:"Contenido",media:Ut,content:{type:"tabs-cards-component"}},{id:"tab-card-single",label:"Tarjeta de tab",category:"Contenido",media:Yt,content:`${D()}`}];function Gt(i){const e="tabs-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección con tabs",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!0,removable:!0,attributes:{"data-gjs-type":e,"data-tab-count":"5"},components:Wt,script:Vt(),traits:[{type:"number",name:"data-tab-count",label:"Número de tabs (2-10)",min:2,max:10,changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",(t,r)=>{const a=parseInt(r["data-tab-count"]);isNaN(a)||this.updateTabCount(a)})},updateTabCount(t){const r=Math.min(10,Math.max(2,t)),a=l=>{const c=Array.from({length:l},()=>'<button class="tabs-btn inline-block px-6 py-2 rounded-full border-2 border-[#003B71] bg-transparent text-[#003B71] text-sm font-medium">Lorem Ipsum</button>').join(`
`),n=Array.from({length:l},(h,f)=>`<div class="tabs-panel${f===0?" active":""} grid-cols-3 gap-5">
                            ${D()}
                            ${D()}
                            ${D()}
                            ${D()}
...
`);return`
                    ...
                    <style>${be}</style>`};this.components(a(r)),setTimeout(()=>{const l=this.get("script"),c=this.getEl();l&&typeof l=="function"&&c&&l.call(c)},200)}}}),Jt(i,e),Zt(i,e)}function Jt(i,e){i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},300))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},300)}}),i.on("storage:end:load",()=>{setTimeout(()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})},800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Zt(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#tabs-section-styles")){const a=document.createElement("style");a.id="tabs-section-styles",a.textContent=be,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
            ...
            `,r.appendChild(a)}}})}const Kt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,Ee=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</div>`},ii=i=>{const e=i==="light";return`
...
${si}`,ci=[{id:"stats-strip-left",label:"Franja estadísticas - izquierda",category:"Heroes",media:oi,content:$e(!0)},{id:"stats-strip-right",label:"Franja estadísticas - derecha",category:"Heroes",media:ni,content:$e(!1)}],di=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,gi=[{id:"service-cards-section",label:"Sección de servicios",category:"Productos y Servicios",media:di,content:`
...
${Ce}`},{id:"service-card",label:"Tarjeta de servicio",category:"Productos y Servicios",media:fi,content:`${se}${Ce}`}],hi=`<svg viewBox="0 0 32 32" width="32" height="32">
...
</style>`,bi=[{id:"contact-info-row",label:"Información de contacto (fila)",category:"Contacto",media:mi,content:`
...
${ce}`},{id:"contact-info-left",label:"Info de contacto (izquierda)",category:"Contacto",media:hi,content:`
<section class="ci-section">
    <div class="ci-list">
        ${J("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
        ${J("ri-mail-fill","Correo","servicio.cliente@bancointegral.com","mailto:servicio.cliente@bancointegral.com")}
...
${ce}`},{id:"contact-info-right",label:"Info de contacto (derecha)",category:"Contacto",media:pi,content:`
<section class="ci-section">
...
${ce}`},{id:"contact-info-item",label:"Ítem de contacto",category:"Contacto",media:ui,content:`
<section class="ci-section">
    <div class="ci-list">
        ${J("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
    </div>
</section>
${ce}`}],xi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,vi=[{id:"rich-list",label:"Lista con título y descripción",category:"Contenido",media:xi,content:`
...
${Le}`},{id:"rich-list-item",label:"Ítem de lista con descripción",category:"Contenido",media:yi,content:`
<section class="rl-section">
    <div class="rl-list">
        ${ie}
    </div>
</section>
${Le}`}],wi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#ffffff" rx="2" stroke="#e5e7eb" stroke-width="1"/>
...
`,Bi=[{id:"contact-form",label:"Formulario de contacto",category:"Formularios",media:wi,content:`
...
<\/script>`}],Ei=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,fe={blue:{headerBg:"bg-[#003B71]",headerText:"text-white",subheaderBg:"bg-[#e8f0f8]",subheaderText:"text-[#003B71]",borderColor:"#003B71",rowEvenBg:"bg-[#f4f7fb]",rowOddBg:"bg-white",rowText:"text-[#003B71]",labelBg:"bg-[#e8f0f8]",labelText:"text-[#003B71]"},orange:{headerBg:"bg-[#E97300]",headerText:"text-white",subheaderBg:"bg-[#fef3e8]",subheaderText:"text-[#E97300]",borderColor:"#E97300",rowEvenBg:"bg-[#fff8f2]",rowOddBg:"bg-white",rowText:"text-[#003B71]",labelBg:"bg-[#fef3e8]",labelText:"text-[#E97300]"}};function Ne(i,e){const t=fe[e]||fe.blue;let r='<table class="w-full border-collapse font-[Poppins,sans-serif] table-fixed">';i.title?(r+=`<thead><tr>
            <th colspan="${i.cols}" class="p-3 align-middle text-center text-base font-bold ${t.headerBg} ${t.headerText}">
                ${i.title}
            </th>
        </tr>`,i.headers?.length&&(r+="<tr>",i.headers.forEach((c,n)=>{const h=n<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";r+=`<th class="p-3 align-middle text-sm font-semibold ${t.subheaderBg} ${t.subheaderText} ${h} border-b border-[${t.borderColor}] text-${c.align||"center"}">${c.text||""}</th>`}),r+="</tr>"),r+="</thead>"):i.headers?.length&&(r+="<thead><tr>",i.headers.forEach((c,n)=>{const h=n<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";r+=`<th class="p-3 align-middle text-sm font-semibold ${t.headerBg} ${t.headerText} ${h} border-b border-[${t.borderColor}] text-${c.align||"center"}">${c.text||""}</th>`}),r+="</tr></thead>"),r+="<tbody>";const a=i.rows.length,l={};return i.rows.forEach((c,n)=>{r+="<tr>";let h=0;c.forEach(f=>{for(;l[`${n}-${h}`];)h++;const u=f.colspan||1,x=f.rowspan||1;for(let y=n;y<n+x;y++)for(let j=h;j<h+u;j++)(y!==n||j!==h)&&(l[`${y}-${j}`]=!0);const m=u>1?`colspan="${u}"`:"",s=x>1?`rowspan="${x}"`:"",g=f.isHeader?t.labelBg:n%2===0?t.rowEvenBg:t.rowOddBg,b=f.isHeader?"font-semibold":"font-normal",C=f.isHeader?t.labelText:t.rowText,w=`text-${f.align||"center"}`,$=n+x>=a,L=h+u>=i.cols?"":`border-r border-[${t.borderColor}]`,B=$?"":`border-b border-[${t.borderColor}]`,_=`${L} ${B} p-3 align-middle text-sm ${g} ${b} ${C} ${w}`;f.image?r+=`<td ${m} ${s} class="${_}">
                    <img src="${f.image}" alt="${f.text||""}" class="max-w-full max-h-20 h-auto object-contain block mx-auto">
                    ${f.text?`<span class="block mt-1 text-xs ${C}">${f.text}</span>`:""}
                </td>`:r+=`<td ${m} ${s} class="${_}">${f.text||""}</td>`,h+=u}),r+="</tr>"}),r+="</tbody></table>",r}function Z(i,e){return{title:"Título de la tabla",cols:i,headers:Array.from({length:i},(t,r)=>({text:`Columna ${r+1}`,align:"center"})),rows:Array.from({length:e},()=>Array.from({length:i},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})))}}function Oe(i,e){return`<div class="w-full overflow-x-auto rounded-2xl border-2 border-[${(fe[e]||fe.blue).borderColor}]">${i}</div>`}function Se(i,e){const t={};return i.forEach((r,a)=>{let l=0;r.forEach(c=>{for(;t[`${a}-${l}`];)l++;const n=Math.min(c.colspan||1,e-l),h=c.rowspan||1;for(let f=a;f<a+h;f++)for(let u=l;u<l+n;u++)(f!==a||u!==l)&&(t[`${f}-${u}`]=`${a}-${l}`);l+=n})}),t}const $i=`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function Ci(){if(document.getElementById("tam-img-modal"))return;const i=document.createElement("div");i.id="tam-img-modal",i.innerHTML=`
        ...
        </div>`,document.body.appendChild(i);let e=null,t=null;async function r(n=""){const h=document.getElementById("tam-img-grid");h.innerHTML='<div class="tam-img-loading"><div class="tam-img-spinner"></div><span>Cargando...</span></div>';try{const f=new URLSearchParams({type:"image",per_page:50});n&&f.append("search",n);const u=document.querySelector('meta[name="media-api-url"]')?.content??"/media/api",s=(await(await fetch(`${u}?${f}`,{headers:{"X-Requested-With":"XMLHttpRequest",Accept:"application/json"}})).json()).items||[];if(!s.length){h.innerHTML='<div class="tam-img-loading"><i class="ri-image-line" style="font-size:2rem;"></i><span>No se encontraron imágenes</span></div>';return}h.innerHTML="",s.forEach(g=>{const b=document.createElement("div");b.className="tam-img-card",b.innerHTML=`<img src="${g.url}" alt="${g.filename}"><p title="${g.filename}">${g.filename}</p>`,b.addEventListener("click",()=>{h.querySelectorAll(".tam-img-card").forEach(C=>C.classList.remove("selected")),b.classList.add("selected"),e=g.url,document.getElementById("tam-img-selected-info").textContent=`Seleccionada: ${g.filename}`,document.getElementById("tam-img-confirm").disabled=!1}),h.appendChild(b)})}catch{h.innerHTML='<div class="tam-img-loading"><i class="ri-error-warning-line" style="font-size:2rem;color:#f87171;"></i><span style="color:#dc2626;">Error al cargar imágenes</span></div>'}}function a(n){t=n,e=null,document.getElementById("tam-img-selected-info").textContent="Ninguna imagen seleccionada",document.getElementById("tam-img-confirm").disabled=!0,document.getElementById("tam-img-search-input").value="",i.classList.add("open"),r()}function l(){i.classList.remove("open"),e=null,t=null}document.getElementById("tam-img-close").addEventListener("click",l),document.getElementById("tam-img-cancel").addEventListener("click",l),document.getElementById("tam-img-confirm").addEventListener("click",()=>{e&&t&&(t(e),l())});let c;document.getElementById("tam-img-search-input").addEventListener("input",n=>{clearTimeout(c),c=setTimeout(()=>r(n.target.value),300)}),i.addEventListener("click",n=>{n.target===i&&l()}),window.__openTableImagePicker=a}function Li(i,e){if(document.getElementById("table-admin-modal"))return;const t=document.createElement("style");t.id="table-admin-modal-styles",t.textContent=$i,document.head.appendChild(t),Ci();const r=document.createElement("div");r.id="table-admin-modal",r.innerHTML=`
        <div class="tam-container">
        ...
        </div>`,document.body.appendChild(r);let a=null,l=null;function c(m){a=m;const s=m.get("tableData");l=s?JSON.parse(JSON.stringify(s)):Z(3,3);const g=l.cols||3;l.rows=l.rows.map((b,C)=>Array.from({length:g},($,T)=>b[T]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})),document.getElementById("tam-title").value=l.title||"",document.getElementById("tam-theme").value=m.get("tableTheme")||"blue",document.getElementById("tam-cols").value=l.cols||3,document.getElementById("tam-rows").value=l.rows.length||3,u(),x(),r.classList.add("open"),document.body.style.overflow="hidden"}function n(){r.classList.remove("open"),document.body.style.overflow="",a=null}function h(){l.title=document.getElementById("tam-title").value.trim(),l.cols=parseInt(document.getElementById("tam-cols").value)||3,l.headers=Array.from(document.querySelectorAll(".tam-header-input")).map(s=>({text:s.value,align:s.closest("th")?.querySelector(".tam-align-select")?.value||"center"})),document.querySelectorAll("#tam-tbody td.tam-cell:not(.is-spanned)").forEach(s=>{const g=parseInt(s.dataset.row),b=parseInt(s.dataset.col);l.rows[g]?.[b]&&(l.rows[g][b].text=s.querySelector(".tam-cell-input")?.value||"",l.rows[g][b].align=s.querySelector(".tam-align-select")?.value||"center",l.rows[g][b].isHeader=s.dataset.isheader==="1",l.rows[g][b].image=s.dataset.image||null)});const m=Se(l.rows,l.cols);l.rows=l.rows.map((s,g)=>s.filter((b,C)=>!m[`${g}-${C}`]))}function f(){if(r.querySelector("#tam-rebuild-notice"))return;const s=document.createElement("div");s.id="tam-rebuild-notice",s.style.cssText="background:#fef9c3;border:1.5px solid #ca8a04;border-radius:0.5rem;padding:0.5rem 1rem;font-size:0.8rem;color:#92400e;display:flex;align-items:center;gap:0.5rem;margin:0 1.5rem 0;flex-shrink:0;",s.innerHTML='<i class="ri-error-warning-line"></i> Has modificado el colspan/rowspan. Haz clic en <strong style="margin:0 4px;">Reconstruir</strong> para aplicar los cambios de fusión.',r.querySelector(".tam-toolbar").after(s)}function u(){r.querySelector("#tam-rebuild-notice")?.remove()}function x(){const m=document.getElementById("tam-thead"),s=document.getElementById("tam-tbody"),g=l.cols,b=l.rows.length,C=Se(l.rows,g);m.innerHTML=`<tr>${l.headers.map((w,$)=>`
            <th>
                <input class="tam-cell-input tam-header-input" value="${w.text||""}" placeholder="Col ${$+1}" style="font-weight:600;">
            ...
            </th>`).join("")}</tr>`,s.innerHTML=l.rows.map((w,$)=>`<tr>${Array.from({length:g},(L,B)=>{const _=C[`${$}-${B}`];if(_)return`<td class="tam-cell is-spanned" data-row="${$}" data-col="${B}">
                        <div class="tam-spanned-label">Combinada con [${_}]</div>
                    </td>`;const y=w[B]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null},j=y.colspan||1,O=y.rowspan||1,z=j>1||O>1;return`<td class="tam-cell ${y.isHeader?"is-header-cell":""} ${y.image?"has-image":""} ${z?"has-span":""}"
                    data-row="${$}" data-col="${B}"
                    ...
                    <div class="tam-cell-actions">
                        ...
                        <div class="tam-cell-span-group">
                            ...
                            <input type="number" class="tam-cell-span-input" min="1" max="${g-B}"
                                value="${j}" data-action="colspan" data-row="${$}" data-col="${B}">
                ...
                </td>`}).join("")}</tr>`).join(""),s.querySelectorAll("input[data-action=colspan], input[data-action=rowspan]").forEach(w=>{w.addEventListener("change",()=>{const $=parseInt(w.dataset.row),T=parseInt(w.dataset.col),L=Math.max(1,parseInt(w.value)||1);l.rows[$]?.[T]&&(w.dataset.action==="colspan"?l.rows[$][T].colspan=Math.min(L,g-T):l.rows[$][T].rowspan=Math.min(L,b-$),f())})}),s.querySelectorAll("button[data-action]").forEach(w=>{w.addEventListener("click",$=>{$.preventDefault(),$.stopPropagation();const T=w.dataset.action,L=parseInt(w.dataset.row),B=parseInt(w.dataset.col);if(!(isNaN(L)||isNaN(B)||!l.rows[L]?.[B])){if(T==="header"){l.rows[L][B].isHeader=!l.rows[L][B].isHeader;const _=s.querySelector(`td[data-row="${L}"][data-col="${B}"]`);_&&(_.dataset.isheader=l.rows[L][B].isHeader?"1":"0",_.classList.toggle("is-header-cell",l.rows[L][B].isHeader)),w.classList.toggle("active",l.rows[L][B].isHeader),w.textContent=l.rows[L][B].isHeader?"✓ Etiqueta":"Etiqueta";return}if(T==="image"){window.__openTableImagePicker&&window.__openTableImagePicker(_=>{l.rows[L][B].image=_;const y=s.querySelector(`td[data-row="${L}"][data-col="${B}"]`);if(y){y.dataset.image=_,y.classList.add("has-image");let j=y.querySelector(".tam-cell-img-preview");j||(j=document.createElement("img"),j.className="tam-cell-img-preview",y.insertBefore(j,y.firstChild)),j.src=_;const O=y.querySelector("[data-action=image]");if(O&&(O.innerHTML='<i class="ri-image-line"></i> Cambiar'),!y.querySelector("[data-action=clear-image]")){const z=document.createElement("button");z.type="button",z.className="tam-cell-btn tam-cell-btn-clear",z.dataset.action="clear-image",z.dataset.row=L,z.dataset.col=B,z.textContent="✕ Quitar",z.addEventListener("click",Y=>{Y.preventDefault(),Y.stopPropagation(),l.rows[L][B].image=null,y.dataset.image="",y.classList.remove("has-image"),j.remove(),z.remove();const H=y.querySelector("[data-action=image]");H&&(H.innerHTML='<i class="ri-image-line"></i> Imagen')}),y.querySelector(".tam-cell-actions").appendChild(z)}}});return}T==="clear-image"&&(l.rows[L][B].image=null,x())}})})}document.getElementById("tam-close").addEventListener("click",n),document.getElementById("tam-cancel").addEventListener("click",n),r.addEventListener("click",m=>{m.target===r&&n()}),document.getElementById("tam-rebuild").addEventListener("click",()=>{const m=parseInt(document.getElementById("tam-cols").value)||3,s=parseInt(document.getElementById("tam-rows").value)||3;for(u(),h();l.headers.length<m;)l.headers.push({text:`Col ${l.headers.length+1}`,align:"center"});for(l.headers=l.headers.slice(0,m),l.cols=m;l.rows.length<s;)l.rows.push(Array.from({length:m},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})));l.rows=l.rows.slice(0,s).map(g=>{for(;g.length<m;)g.push({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null});return g.slice(0,m)}),x()}),document.getElementById("tam-apply").addEventListener("click",()=>{h();const m=document.getElementById("tam-theme").value;a&&(a.set("tableData",JSON.parse(JSON.stringify(l))),a.set("tableTheme",m),a.addAttributes({"data-table-theme":m}),ue(a)),n()}),window.__openTableAdminModal=c}function ue(i){const e=i.get("tableData"),t=i.get("tableTheme")||"blue";e&&i.components(Oe(Ne(e,t),t))}function Si(){return function(){}}const ji=[{id:"table-blue",label:"Tabla azul",category:"Interactivos",media:Ei,content:{type:"table-component",attributes:{"data-table-theme":"blue"}}},{id:"table-orange",label:"Tabla naranja",category:"Interactivos",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
        ...
        </svg>`,content:{type:"table-component",attributes:{"data-table-theme":"orange"}}}];function _i(i){const e="table-component";Li(),i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Tabla",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!0,removable:!0,propagate:["editable","selectable","hoverable","droppable","highlightable"],tableData:null,tableTheme:"blue",attributes:{"data-gjs-type":e,"data-table-theme":"blue"},components:Oe(Ne(Z(3,3),"blue"),"blue"),script:Si(),traits:[{type:"button",name:"edit-table",label:"Editar tabla",text:"Abrir editor de tabla",command(t){const r=t.getSelected();r&&window.__openTableAdminModal&&(r.get("tableData")||r.set("tableData",Z(3,3)),window.__openTableAdminModal(r))}},{type:"select",name:"data-table-theme",label:"Color del tema",options:[{id:"blue",name:"Azul"},{id:"orange",name:"Naranja"}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e});const t=this.getAttributes()["data-table-theme"]||"blue";this.set("tableTheme",t),this.get("tableData")||(this.set("tableData",Z(3,3)),ue(this)),this.on("change:attributes",(r,a)=>{const l=a["data-table-theme"];l&&l!==this.get("tableTheme")&&(this.set("tableTheme",l),ue(this))})}}}),Ii(i,e),Ti(i,e)}function Ii(i,e){i.on("component:mount",t=>{const r=t.getEl();if(r?.getAttribute?.("data-gjs-type")===e){t.set("type",e);const a=r.getAttribute("data-table-theme")||"blue";t.set("tableTheme",a),t.get("tableData")||t.set("tableData",Z(3,3))}}),i.on("storage:end:load",()=>{setTimeout(()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getAttributes()["data-table-theme"]||"blue";t.set("tableTheme",r),t.get("tableData")||t.set("tableData",Z(3,3))})},800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Ti(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r&&!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
                    outline: 2px dashed rgba(0,59,113,0.5) !important;
...
<a href="#"
   ...
   class="dld-simple-${e}">
...
</svg>`,Xi=[{id:"badge-outline-blue",label:"Badge azul outline",category:"Badges",media:Vi,content:'<span class="inline-block border border-[#003B71] rounded-full px-4 py-1 text-base font-medium bg-transparent text-[#003B71]">Etiqueta</span>'},{id:"badge-outline-orange",label:"Badge naranja outline",category:"Badges",media:Wi,content:'<span class="inline-block border border-[#E97300] rounded-full px-4 py-1 text-base font-medium bg-transparent text-[#E97300]">Etiqueta</span>'},{id:"badge-outline-white",label:"Badge blanco outline",category:"Badges",media:Yi,content:'<span class="inline-block border border-white rounded-full px-4 py-1 text-base font-medium bg-transparent text-white">Etiqueta</span>'}],Gi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    ...
    <rect x="3" y="25" width="7" height="2.5" fill="#dee2e6" rx="0.5"/>
</svg>`;function Ji(){return function(){const i=this,e="agencies-map-component",t="/api/agencies/active",r="agencies";let a=[],l=[],c={},n=null,h=[];const f=async()=>{try{u(),await m(),await b(),C(),_(),z(),Y(),x()}catch(o){console.error("Error initializing map:",o),ae("Error al cargar las agencias"),x()}};function u(){const o=i.querySelector(`.${e}-list`);o&&(o.innerHTML=`
                ...
                `);const d=i.querySelector(`.${e}-map-container`);d&&(d.style.opacity="0.5")}function x(){const o=i.querySelector(`.${e}-map-container`);o&&(o.style.transition="opacity 0.3s ease",o.style.opacity="1")}async function m(){try{const d=await(await fetch(t)).json(),p=r?d[r]:d;Array.isArray(p)?(a=p.filter(v=>v.latitude&&v.longitude&&!isNaN(v.latitude)&&!isNaN(v.longitude)),l=[...a],s()):(a=[],l=[])}catch(o){console.error("Error loading items:",o),a=[],l=[]}}function s(){const o=[...new Set(a.map(p=>p.zone).filter(Boolean))].sort(),d=[...new Set(a.map(p=>p.department).filter(Boolean))].sort();c={zone:o,department:d},setTimeout(()=>{const p=i.querySelector(`.${e}-filters-container`);p&&!p.hasChildNodes()&&(p.innerHTML=`
                    ...
                    `,g())},100)}function g(){const o=i.querySelector(`.${e}-zone-filter`),d=i.querySelector(`.${e}-department-filter`);o&&c.zone&&c.zone.forEach(p=>{const v=document.createElement("option");v.value=p,v.textContent=p,o.appendChild(v)}),d&&c.department&&c.department.forEach(p=>{const v=document.createElement("option");v.value=p,v.textContent=p,d.appendChild(v)})}async function b(){if(!document.getElementById("leaflet-css")){const o=document.createElement("link");o.id="leaflet-css",o.rel="stylesheet",o.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css",document.head.appendChild(o)}if(typeof window.L>"u"&&await new Promise((o,d)=>{const p=document.createElement("script");p.src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js",p.onload=o,p.onerror=d,document.head.appendChild(p)}),typeof window.L<"u"&&window.L.Icon&&window.L.Icon.Default){const o=window.L.icon({iconUrl:"data:image/svg+xml;base64,"+btoa(`
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 36" width="24" height="36">
                            <path fill="#f0872a" d="M12 0C7.6 0 4 3.6 4 8c0 5.4 8 16 8 16s8-10.6 8-16c0-4.4-3.6-8-8-8z"/>
                            <circle cx="12" cy="8" r="3" fill="#fff"/>
                        </svg>
                    `),iconSize:[30,45],iconAnchor:[15,45],popupAnchor:[0,-45]});window.customOrangeIcon=o}}function C(){const o=i.querySelector(`.${e}-map`);if(!o||!window.L)return;o._leaflet_id&&o._map&&(o._map.remove(),delete o._map),n=window.L.map(o).setView([13.7942,-88.8965],8),window.L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(n),o._map=n,n.whenReady(function(){w(),setTimeout(()=>{n&&n.invalidateSize&&n.invalidateSize()},300)})}function w(){$(),T(),B()}function $(){h.forEach(o=>{o.marker&&n.removeLayer(o.marker)}),h=[]}function T(){l.forEach((o,d)=>{if(o.latitude&&o.longitude){const p=L(o),v=window.L.marker([o.latitude,o.longitude],{icon:window.customOrangeIcon||new window.L.Icon.Default}).addTo(n).bindPopup(p);h.push({marker:v,item:o,index:d})}})}function L(o){let d=`<div style="min-width: 200px;"><h4 style="font-weight: 700; margin-bottom: 12px; color: #0d3f6a; font-size: 16px; border-bottom: 2px solid #f0872a; padding-bottom: 8px;">${o.name}</h4>`;if(o.address&&(d+=`<p style="margin: 8px 0 0 0; font-size: 14px; display: flex; align-items: start; gap: 8px;"><i class="ri-map-pin-line" style="color: #f0872a; font-size: 16px; margin-top: 2px;"></i><span style="color: #4b5563;">${o.address}</span></p>`),o.municipality||o.department){const p=[o.municipality,o.department].filter(Boolean).join(", ");d+=`<p style="margin: 0 0 8px 0; font-size: 13px; color: #6b7280; padding-left: 24px;">${p}</p>`}return o.schedule&&(d+=`<p style="margin: 8px 0; font-size: 14px; display: flex; align-items: start; gap: 8px;"><i class="ri-time-line" style="color: #f0872a; font-size: 16px; margin-top: 2px;"></i><span style="color: #4b5563;">${o.schedule}</span></p>`),d+="</div>",d}function B(){if(l.length>0){const o=l.filter(d=>d.latitude&&d.longitude).map(d=>[d.latitude,d.longitude]);o.length>1?n.once("moveend",function(){setTimeout(()=>{try{n&&n._loaded&&typeof n.fitBounds=="function"&&n.fitBounds(o,{padding:[50,50],maxZoom:12,animate:!1})}catch(d){console.warn("Error fitting bounds:",d)}},100)}):o.length===1&&n.setView(o[0],14)}}function _(){const o=i.querySelector(`.${e}-list`);if(!o)return;if(l.length===0){j(o);return}const d=l.map((p,v)=>y(p,v)).join("");o.innerHTML=d,O()}function y(o,d){const p=o.phones&&o.phones.length>0?o.phones.map(E=>`
                    ...
                    `).join(""):"",v=`https://www.google.com/maps/search/?api=1&query=${o.latitude},${o.longitude}`;return`
                <div class="${e}-item p-5 bg-white/10 border border-white/20 rounded-2xl cursor-pointer transition-all duration-200" data-item-index="${d}">
                    ...
                    ${p}
                    <div class="mt-3">
                        <a href="${v}" target="_blank" rel="noopener noreferrer" class="agency-maps-btn block w-full text-center px-4 py-2.5 bg-primary text-white font-semibold rounded-lg hover:opacity-90 transition-all duration-200" onclick="event.stopPropagation();">
                            Ver ubicación
                        </a>
                    </div>
                </div>
            `}function j(o){o.innerHTML=`
            ...
            `}function O(){const o=i.querySelectorAll(`.${e}-item`);o.forEach(d=>{d.addEventListener("click",p=>{if(p.target.closest(".agency-maps-btn"))return;p.preventDefault();const v=parseInt(d.dataset.itemIndex),E=l[v];if(!E)return;o.forEach(k=>{k.classList.remove("bg-white","border-secondary","selected-agency"),k.classList.add("bg-white/10","border-white/20");const A=k.querySelector(".agency-title"),re=k.querySelectorAll("i"),le=k.querySelectorAll(".agency-phone-link"),ge=k.querySelectorAll("p:not(:has(.agency-phone-link))"),he=k.querySelector(".agency-maps-btn");A&&(A.classList.remove("text-secondary"),A.classList.add("text-white")),re.forEach(U=>{U.classList.remove("text-secondary","text-gray-300","text-white"),U.classList.add("text-primary")}),le.forEach(U=>{U.classList.remove("text-secondary"),U.classList.add("text-white")}),ge.forEach(U=>{U.classList.remove("text-secondary"),U.classList.add("text-gray-200")}),he&&(he.classList.remove("bg-secondary"),he.classList.add("bg-primary"))}),d.classList.remove("bg-white/10","border-white/20"),d.classList.add("bg-white","border-secondary","selected-agency");const q=d.querySelector(".agency-title"),F=d.querySelectorAll("i"),X=d.querySelectorAll(".agency-phone-link"),R=d.querySelectorAll("p:not(:has(.agency-phone-link))"),P=d.querySelector(".agency-maps-btn");if(q&&(q.classList.remove("text-white"),q.classList.add("text-secondary")),F.forEach(k=>{k.classList.remove("text-secondary","text-gray-300","text-white"),k.classList.add("text-primary")}),X.forEach(k=>{k.classList.remove("text-white"),k.classList.add("text-secondary")}),R.forEach(k=>{k.classList.remove("text-gray-200"),k.classList.add("text-secondary")}),P&&(P.classList.remove("bg-primary"),P.classList.add("bg-secondary")),n&&E&&n._loaded)try{n.flyTo([E.latitude,E.longitude],14,{animate:!0,duration:1});const k=h.find(A=>A.item.id===E.id);k&&k.marker&&k.marker.openPopup()}catch(k){console.warn("Error updating map view:",k)}})})}function z(){const o=i.querySelector(`.${e}-search-input`),d=i.querySelector(`.${e}-zone-filter`),p=i.querySelector(`.${e}-department-filter`),v=i.querySelector(`.${e}-no-results`),E={Occidental:["Ahuachapán","Santa Ana","Sonsonate"],Paracentral:["Chalatenango","Cuscatlán","La Paz","Cabañas","San Vicente"],Central:["La Libertad","San Salvador"],Oriental:["Usulután","San Miguel","Morazán","La Unión"]},q=()=>{if(!d||!p)return;const X=d.value,R=p.value;if(!X)p.innerHTML='<option value="">Todos los Departamentos</option>',[...new Set(a.map(k=>k.department).filter(Boolean))].sort().forEach(k=>{const A=document.createElement("option");A.value=k,A.textContent=k,p.appendChild(A)}),R&&(p.value=R);else{const P=E[X]||[];p.innerHTML='<option value="">Todos los Departamentos</option>',P.forEach(k=>{const A=document.createElement("option");A.value=k,A.textContent=k,p.appendChild(A)}),P.includes(R)&&(p.value=R)}},F=()=>{const X=o?o.value.toLowerCase().trim():"",R=d?d.value:"",P=p?p.value:"";l=a.filter(k=>{let A=!0,re=!0,le=!0;return X&&(A=Object.values(k).some(ge=>String(ge).toLowerCase().includes(X))),R&&(re=k.zone===R),P&&(le=k.department===P),A&&re&&le}),_(),w(),v&&v.classList.toggle("hidden",l.length>0)};o&&o.addEventListener("input",F),d&&d.addEventListener("change",()=>{q(),F()}),p&&p.addEventListener("change",F)}function Y(){const o=i.querySelector("[data-title]");if(o){const v=H("map-title")||"Nuestras Agencias";o.textContent=v}const d=i.querySelector(`.${e}-search-input`);if(d){const v=H("search-placeholder")||"Buscar...";d.setAttribute("placeholder",v)}const p=i.querySelector(`.${e}-no-results`);if(p){const v=H("no-results-text")||"No se encontraron agencias";p.textContent=v}}function H(o){return i.closest(`[data-gjs-type="${e}"]`)?.getAttribute(o)}function ae(o){const d=i.querySelector(`.${e}-list`);d&&(d.innerHTML=`
                    <div class="flex flex-col items-center justify-center p-12 text-center">
                ...
                `)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",f):f()}}const Zi=[{id:"agencies-map",label:"Mapa de Agencias",category:"Interactivos",media:Gi,content:{type:"agencies-map-component"}}];function Ki(i){const e="agencies-map-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute&&t.getAttribute("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Mapa de Agencias",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{class:"py-12 bg-secondary","data-gjs-type":e},components:`
                ...
                `,script:Ji()},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e})}}}),Qi(i,e),ea(i,e)}function Qi(i,e){i.on("component:selected",t=>{if(t.get("type")===e){const r=t.getEl();if(r){const a=r.querySelector(`.${e}-map`);a&&a._map&&setTimeout(()=>{a._map.invalidateSize()},100)}}}),i.on("storage:end:load",()=>{setTimeout(()=>{Te(i,e)},1e3)}),i.on("component:mount",t=>{const r=t.getEl();r&&r.getAttribute&&r.getAttribute("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&r&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();if(r){const a=r.querySelector(`.${e}-map`);a&&a._map&&(a._map.remove(),delete a._map),setTimeout(()=>{const l=t.get("script");l&&typeof l=="function"&&l.call(r)},500)}}}),i.on("canvas:render",()=>{setTimeout(()=>{Te(i,e)},800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(a=>{a.set("type",e),a.addAttributes({"data-gjs-type":e})})})}function Te(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(a=>{a.set("type",e);const l=a.getEl();if(l&&l.isConnected){const c=a.get("script");c&&typeof c=="function"&&c.call(l)}})}function ea(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument.head;if(!r.querySelector("#leaflet-css")){const a=document.createElement("link");a.id="leaflet-css",a.rel="stylesheet",a.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css",r.appendChild(a)}if(!r.querySelector(`#${e}-css`)){const a=document.createElement("style");a.id=`${e}-css`,a.innerHTML=`
                .leaflet-container {
                    height: 100%;
                    width: 100%;
            ...
            `,r.appendChild(a)}})}const ta=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
@media(max-width:480px){
    .banner-btn{flex:1 1 100%;min-width:0;}
            ...
            `;const o=i.ownerDocument??document;if(!o.getElementById("banner-skeleton-styles")){const d=o.createElement("style");d.id="banner-skeleton-styles",d.textContent=`
                ...
                `,o.head.appendChild(d)}}function C(o){return t[o]?t[o]:o==="outline-blue"||o==="outline-orange"?t["outline-white"]:t["fill-white"]}function w(o,d,p,v){const E=C(p),q=d?"a":"span",F=d?`href="${d}"${v?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${q} ${F}
                class="banner-btn"
                data-bg="${E.bg}"
                data-color="${E.color}"
                ...
                ${o}
            </${q}>`}function $(){s.innerHTML=a.map((o,d)=>`
                <div class="banner-slide" data-index="${d}">
                    <div class="banner-slide-inner">
                        <div class="banner-bg">
                            <img src="${o.image_url}"
            ...
            `).join(""),T(),L(),B()}function T(){i.querySelectorAll(".banner-btn").forEach(o=>{const d=o.style.borderColor;o.addEventListener("mouseenter",()=>{o.style.background=o.dataset.hoverBg,o.style.color=o.dataset.hoverColor,o.style.borderColor=o.dataset.hoverBg}),o.addEventListener("mouseleave",()=>{o.style.background=o.dataset.bg,o.style.color=o.dataset.color,o.style.borderColor=d})})}function L(){s.addEventListener("mousedown",_),s.addEventListener("touchstart",_,{passive:!0}),s.addEventListener("mousemove",y),s.addEventListener("touchmove",y,{passive:!0}),s.addEventListener("mouseup",j),s.addEventListener("touchend",j),s.addEventListener("mouseleave",j)}function B(){a.forEach(o=>{const d=new Image;d.src=o.image_url})}function _(o){n=!0,h=o.touches?o.touches[0].clientX:o.clientX,f=0}function y(o){n&&(f=(o.touches?o.touches[0].clientX:o.clientX)-h)}function j(){n&&(n=!1,Math.abs(f)>=u&&(z(f<0?(l+1)%a.length:(l-1+a.length)%a.length),H()),f=0)}function O(){const o=i.querySelector(".banner-stripe");if(!o||a.length<=1)return;const d=document.createElement("div");d.className="banner-dots",a.forEach((p,v)=>{const E=document.createElement("button");E.className="banner-dot",E.dataset.index=String(v),E.setAttribute("aria-label",`Banner ${v+1}`),E.addEventListener("click",()=>{z(v),H()}),d.appendChild(E)}),o.innerHTML="",o.appendChild(d)}function z(o,d=!0){const p=s.querySelectorAll(".banner-slide"),v=i.querySelectorAll(".banner-dot");p.forEach((E,q)=>{const F=q===o;d||(E.style.transition="none"),E.classList.toggle("banner-slide--active",F),d||requestAnimationFrame(()=>{E.style.transition=""})}),v.forEach((E,q)=>{E.classList.toggle("banner-dot--active",q===o)}),l=o}function Y(){a.length<=1||!x||(c=setInterval(()=>{z((l+1)%a.length)},5e3))}function H(){x&&(clearInterval(c),Y())}function ae(){const o=i.querySelector(".banner-wrapper");o&&(o.innerHTML=`
                    <div style="display:flex;align-items:center;justify-content:center;
                                min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;">
                        Sin contenido.
                    </div>`)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",g):g()}}const ra=[{id:"banner-hero",label:"Banner Slider",category:"Banners",media:ta,content:{type:"banner-hero-component"}}];function la(i){const e="banner-hero-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Slider",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-autoplay":"true","data-category":""},components:`
                ...
                `,script:aa(),traits:[{type:"select",name:"data-autoplay",label:"Avance automático",options:[{id:"true",name:"Activado"},{id:"false",name:"Desactivado"}],changeProp:!1},{type:"select",name:"data-category",label:"Filtrar por categoría",options:[{id:"",name:"Todas las categorías"}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const r=this.get("script");r&&typeof r=="function"&&setTimeout(()=>r.call(t),100)})}}}),na(i,e),sa(i,e),oa(i,e)}async function oa(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",a=await(await fetch(t)).json();if(!Array.isArray(a))return;const l=[...new Set(a.map(f=>f.category).filter(Boolean))].sort();if(l.length===0)return;const c=i.DomComponents.getType(e);if(!c)return;const h=c.model.prototype.defaults.traits.find(f=>f.name==="data-category");if(!h)return;h.options=[{id:"",name:"Todas las categorías"},...l.map(f=>({id:f,name:f}))]}catch{}}function na(i,e){i.on("storage:end:load",()=>{setTimeout(()=>ze(i,e),1e3)}),i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>ze(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function ze(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})}function sa(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#banner-hero-styles")){const a=document.createElement("style");a.id="banner-hero-styles",a.textContent=ia,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
            ...
            `,r.appendChild(a)}}})}const ca=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    <rect x="1" y="6" width="30" height="20" fill="#003B71" rx="2"/>
    <rect x="1" y="22" width="30" height="4" fill="#E97300" rx="0"/>
    <rect x="3" y="9" width="12" height="2" fill="rgba(255,255,255,0.9)" rx="1"/>
...
@media(max-width:480px){
    .bsingle-btn{flex:1 1 100%;min-width:0;}
                    ...
                    <div class="bsingle-content">
                        ...
                        ${s.btn_primary_text||s.btn_secondary_text?`
                            <div class="bsingle-buttons">
                                ${s.btn_primary_text?f(s.btn_primary_text,s.btn_primary_url,s.btn_primary_style,s.btn_primary_external):""}
                                ${s.btn_secondary_text?f(s.btn_secondary_text,s.btn_secondary_url,s.btn_secondary_style,s.btn_secondary_external):""}
                            </div>`:""}
                    </div>
                </div>`,x()}function x(){i.querySelectorAll(".bsingle-btn").forEach(s=>{const g=s.style.borderColor;s.addEventListener("mouseenter",()=>{s.style.background=s.dataset.hoverBg,s.style.color=s.dataset.hoverColor,s.style.borderColor=s.dataset.hoverBg}),s.addEventListener("mouseleave",()=>{s.style.background=s.dataset.bg,s.style.color=s.dataset.color,s.style.borderColor=g})})}function m(){l.innerHTML=`
                <div style="display:flex;align-items:center;justify-content:center;
                            min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;">
                    No hay banner seleccionado o no está activo.
                </div>`}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",c):c()}}const ga=[{id:"banner-single",label:"Banner Individual",category:"Banners",media:ca,content:{type:"banner-single-component"}}];function ha(i){const e="banner-single-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Individual",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-banner-id":""},components:`
                ...
                `,script:fa(),traits:[{type:"select",name:"data-banner-id",label:"Banner a mostrar",options:[{id:"",name:"Cargando banners..."}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const r=this.get("script");r&&typeof r=="function"&&setTimeout(()=>r.call(t),100)})}}}),ma(i,e),ua(i,e),pa(i,e)}async function pa(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",a=await(await fetch(t)).json();if(!Array.isArray(a)||a.length===0)return;const l=i.DomComponents.getType(e);if(!l)return;const n=l.model.prototype.defaults.traits.find(h=>h.name==="data-banner-id");if(!n)return;n.options=[{id:"",name:"— Seleccionar banner —"},...a.map(h=>({id:String(h.id),name:h.category?`[${h.category}] ${h.title}`:h.title}))]}catch{}}function ma(i,e){i.on("storage:end:load",()=>{setTimeout(()=>Ae(i,e),1e3)}),i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>Ae(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Ae(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})}function ua(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#banner-single-styles")){const a=document.createElement("style");a.id="banner-single-styles",a.textContent=da,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
                    outline: 2px dashed rgba(240,135,42,0.6) !important;
            ...
            `,r.appendChild(a)}}})}function ba(i){S.registerBlocks(nt),S.registerBlocks(ut),S.registerBlocks(ra),S.registerBlocks(ga),S.registerBlocks(ci),S.registerBlocks(Xt),S.registerBlocks(kt),S.registerBlocks(qt),S.registerBlocks(Ot),S.registerBlocks(Pt),S.registerBlocks(jt),S.registerBlocks(li),S.registerBlocks(Tt),S.registerBlocks(Rt),S.registerBlocks(gi),S.registerBlocks(bi),S.registerBlocks(vi),S.registerBlocks(Bi),S.registerBlocks(ji),S.registerBlocks(Ri),S.registerBlocks(Xi),S.registerBlocks(Zi),S.applyToEditor(i),Ki(i),la(i),ha(i),Lt(i),Ui(i),Gt(i),_i(i)}function xa(i,e,t){i.on("component:add",()=>e.markAsDirty()),i.on("component:remove",()=>e.markAsDirty()),i.on("component:update",()=>e.markAsDirty()),i.on("style:update",()=>e.markAsDirty());const r=document.getElementById("save-button");r&&r.addEventListener("click",async()=>{await ya(i,e,t,r)}),document.addEventListener("keydown",a=>{(a.ctrlKey||a.metaKey)&&a.key==="s"&&(a.preventDefault(),r&&!r.disabled&&r.click())})}async function ya(i,e,t,r){r.disabled=!0,r.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{t.needsTitle()?await va(i,e,t):await Pe(i,e,t)}catch(a){He(a.message,"error")}finally{r.disabled=!1,r.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}async function va(i,e,t){return new Promise((r,a)=>{Ue({title:"Título de la Página",description:"Ingresa un título descriptivo para identificar esta página",placeholder:"Ej: Acerca de Nosotros",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async l=>{if(!l?.trim()){a(new Error("El título es obligatorio"));return}try{await Pe(i,e,t,l),r()}catch(c){a(c)}},onCancel:()=>{a(new Error("Guardado cancelado"))}})})}async function Pe(i,e,t,r=null){const l={...e.getEditorContent(i),is_published:t.isPublished};r&&(l.title=r);const c=await e.savePage(i,l,t.storeUrl,t.getHttpMethod());c.success&&(e.markAsClean(),He(c.message,"success"),!t.isEditMode&&c.page?(t.updatePageInfo(c),t.updateTitle(c.page.title)):r&&t.updateTitle(r))}function He(i,e){typeof window.showNotification=="function"&&window.showNotification(i,e)}document.addEventListener("DOMContentLoaded",async()=>{const i=new Ve,e=new at;new rt(i);const t=We();if(t.on("load",()=>{ba(t),Ye(t),Xe(),Ge(),Je(t),Ze(t),Ke(t),Qe(t),et(t),tt(t),wa(t),ka(t),Ba(t),setTimeout(()=>{t.runCommand("sw-visibility"),t.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),e.isEditMode)try{await i.loadPageContent(t,e.loadUrl),De("Contenido cargado correctamente","success")}catch(r){De("Error al cargar el contenido","error"),console.error(r)}xa(t,i,e)});function wa(i){const e=[{cmd:"set-device-desktop",device:"Desktop"},{cmd:"set-device-tablet",device:"Tablet"},{cmd:"set-device-mobile",device:"Mobile"}];e.forEach(({cmd:t,device:r})=>{i.Commands.add(t,{run:a=>{a.setDevice(r),e.forEach(({cmd:l})=>{a.Panels.getButton("devices-c",l)?.set("active",l===t)})}})})}function ka(i){i.Commands.add("canvas-clear",{run:e=>{it({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{e.DomComponents.clear(),e.CssComposer.clear()}})}})}function Ba(i){const e=i.Canvas.getFrameEl();if(!e?.contentDocument?.head)return;const t=e.contentDocument.createElement("style");t.id="gjs-dashed-fix",t.textContent="body,body.gjs-dashed{margin:0!important;padding:0 0 160px 0!important;}",e.contentDocument.head.appendChild(t)}function De(i,e="info"){typeof window.showNotification=="function"&&window.showNotification(i,e)}
