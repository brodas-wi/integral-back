/* empty css                   *//* empty css                    *//* empty css                          *//* empty css                           */import{M as He,j as Oe,E as Fe,i as Re,t as Ue,d as Ve,f as Ye,e as We,s as Xe,g as Ge,c as Ze,b as Ke,a as Qe,h as Je}from"./editor-commands-C7f79RQP.js";import{a as q}from"./url-DaqOO3yL.js";import"./_commonjsHelpers-CqkleIqs.js";class et{constructor(){this.pageId=this.getElementValue("page-id"),this.pageSlug=this.getElementValue("page-slug"),this.pageTitle=this.getPageTitle(),this.loadUrl=this.getElementValue("page-load-url"),this.storeUrl=this.getElementValue("page-store-url"),this.isPublished=this.getElementValue("page-is-published")==="1",this.isEditMode=!!this.pageId,this.appUrl=(document.querySelector('meta[name="app-url"]')?.content??"").replace(/\/$/,"")}getElementValue(e,t=""){const r=document.getElementById(e);return r?r.value.trim():t}getPageTitle(){const e=document.getElementById("editor-title");if(!e)return"";const r=e.textContent.match(/(?:Editando:|Nueva Página)\s*(.+)?/);return r&&r[1]?r[1].trim():""}updatePageInfo(e){if(e.page)if(this.pageId=e.page.id,this.pageSlug=e.page.slug,this.pageTitle=e.page.title,this.isEditMode=!0,document.getElementById("page-id").value=this.pageId,document.getElementById("page-slug").value=this.pageSlug,e.page.update_url?this.storeUrl=e.page.update_url:this.storeUrl=`${this.appUrl}/pages/${this.pageSlug}`,document.getElementById("page-store-url").value=this.storeUrl,e.page.load_url?this.loadUrl=e.page.load_url:this.loadUrl=`${this.appUrl}/pages/${this.pageSlug}/load`,document.getElementById("page-load-url").value=this.loadUrl,e.page.edit_url)window.history.replaceState(null,"",e.page.edit_url);else try{const t=new URL(window.location.href);t.pathname=t.pathname.replace(/\/pages\/create\/?$/,`/pages/${this.pageSlug}/edit`),window.history.replaceState(null,"",t.toString())}catch(t){console.error("Error updating browser URL:",t)}}updateTitle(e){this.pageTitle=e;const t=document.getElementById("editor-title");t&&(t.textContent=`Editando: ${e}`),document.title=`Editar: ${e} - Editor`}getHttpMethod(){return this.isEditMode?"PUT":"POST"}needsTitle(){return!this.isEditMode}}class tt{constructor(e){this.editorService=e,this.setupBackButton(),this.setupBeforeUnload()}setupBackButton(){const e=document.querySelector("#editor-navbar a[href]");e&&e.addEventListener("click",t=>{this.editorService.shouldPreventUnload()&&(t.preventDefault(),this.showUnsavedChangesModal(()=>{this.editorService.startNavigation(),window.location.href=e.href}))})}setupBeforeUnload(){window.addEventListener("beforeunload",e=>{if(this.editorService.shouldPreventUnload())return e.preventDefault(),e.returnValue="Tienes cambios sin guardar. ¿Estás seguro de que quieres salir?",e.returnValue})}showUnsavedChangesModal(e){const t=document.createElement("div");t.style.cssText=`
        ...
        `;const c=document.createElement("button");c.textContent="Salir sin guardar",c.style.cssText=`
            ...
            font-family: inherit;
        `,[d,c].forEach(g=>{g.addEventListener("mouseenter",()=>{g.style.opacity="0.85"}),g.addEventListener("mouseleave",()=>{g.style.opacity="1"})});const h=()=>t.remove();d.addEventListener("click",h),c.addEventListener("click",()=>{h(),e()}),t.addEventListener("click",g=>{g.target===t&&h()}),o.appendChild(d),o.appendChild(c),r.appendChild(a),r.appendChild(o),t.appendChild(r),document.body.appendChild(t)}}const it="Básico";class at{constructor(){this.blocks=new Map}registerBlock(e,t){this.blocks.has(t.category)||this.blocks.set(t.category,[]),this.blocks.get(t.category).push({id:e,...t})}registerBlocks(e){e.forEach(t=>{this.registerBlock(t.id,t)})}applyToEditor(e){this.blocks.forEach(t=>{t.forEach(r=>{const{id:a,...o}=r;e.BlockManager.add(a,o)})}),this.applyCollapseSettings(e),this.hideDefaultCategories(e)}applyCollapseSettings(e){setTimeout(()=>{e.BlockManager.getCategories().each(t=>{t.set("open",t.get("label")===it)})},500)}hideDefaultCategories(e){setTimeout(()=>{const t=["Basic","Extra"];document.querySelectorAll(".gjs-block-category").forEach(r=>{const a=r.querySelector(".gjs-title");a&&t.includes(a.textContent.trim())&&(r.style.display="none")})},100)}}const L=new at,rt=[{id:"heading1",label:"Título H1",category:"Básico",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,content:{type:"image",attributes:{src:q("images/placeholder.svg"),alt:"Imagen"},classes:["max-w-full","h-auto","block"],activeOnRender:1}},{id:"unordered-list",label:"Lista no ordenada",category:"Básico",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,I='<div class="col-cell"></div>',U=`
<style>
...
</style>`,ht=[{id:"columns-1",label:"1 Columna",category:"Columnas",media:lt,content:`
...
${U}`},{id:"columns-2-1",label:"1/2 — 1/4 — 1/4",category:"Columnas",media:ft,content:`
<div class="col-section">
    ...
    <div class="ab-grid">
        ...
        <div class="ab-image-col">
            <div class="ab-image-wrap">
                <img src="${q("images/placeholder.svg")}" alt="Imagen de sección">
            </div>
        </div>
    </div>
</section>
${ue}`},{id:"about-mvv-section",label:"Misión, Visión y Valores",category:"Contenido",media:mt,content:`
...
${ue}`}],wt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<div class="pc-card">
    <div class="pc-card-img-wrap">
        <img src="${q("images/placeholder.svg")}" alt="Producto" class="pc-card-img">
    </div>
...
<\/script>`,Bt=[{id:"product-cards-section",label:"Sección de productos",category:"Productos y Servicios",media:wt,content:`
...
${xe}
${kt}`},{id:"product-card",label:"Tarjeta de producto",category:"Productos y Servicios",media:vt,content:`${K}${xe}`}],Et=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,$t=[{id:"dual-card-section",label:"Sección dos tarjetas",category:"Productos y Servicios",media:Et,content:`
...
${he}`},{id:"dual-card-dark",label:"Tarjeta azul oscuro",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
        ...
        </svg>`,content:`${ye}${he}`},{id:"dual-card-light",label:"Tarjeta blanca",category:"Productos y Servicios",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
        ...
        </svg>`,content:`${we}${he}`}],Ct=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</style>`,St=[{id:"cta-section",label:"Llamada a la acción",category:"Llamadas a la acción",media:Ct,content:`
<section class="relative overflow-hidden w-full bg-white py-16 px-6 md:px-16">
    <div class="cta-watermark-left">
        <img src="${q("images/brand-logo.png")}" alt="">
    </div>
...
${Lt}`}],jt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,zt=[{id:"icon-grid-hero",label:"Sección de características",category:"Productos y Servicios",media:jt,content:`
<section class="ig-section relative overflow-hidden">
    <div class="ig-watermark">
        <img src="${q("images/brand-watermark.png")}" alt="">
    </div>
...
${_t}`},{id:"icon-card",label:"Tarjeta con icono",category:"Productos y Servicios",media:It,content:`${X}`}],ve=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Dt=[{id:"product-link-cards",label:"Tarjetas de productos con enlace",category:"Productos y Servicios",media:ve,content:`
...
${pe}`},{id:"product-link-card-orange",label:"Tarjeta producto naranja",category:"Productos y Servicios",media:ve,media:Tt,content:`${Q}${pe}`},{id:"product-link-card-blue",label:"Tarjeta producto azul",category:"Productos y Servicios",media:At,content:`${J}${pe}`}],ke=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
<a href="#" class="il-item flex flex-col items-center text-center gap-4 no-underline">
    <div class="w-14 h-14 flex items-center justify-center">
        <img src="${q("images/placeholder.svg")}" alt="" class="w-full h-full object-contain">
    </div>
...
</style>`,Mt=[{id:"icon-links-strip",label:"Tira de iconos con enlace",category:"Productos y Servicios",media:ke,content:`
...
${Be}`},{id:"icon-link-item",label:"Ítem icono con enlace",category:"Productos y Servicios",media:ke,content:`${ee}${Be}`}],qt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,Pt=[{id:"promo-cta",label:"Promo con CTA y precio",category:"Llamadas a la acción",media:qt,content:`
...
${Nt}`}],Ht=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<style>${be}</style>`,Rt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
</svg>`,Ut=[{id:"tabs-cards",label:"Sección con tabs",category:"Contenido",media:Ht,content:{type:"tabs-cards-component"}},{id:"tab-card-single",label:"Tarjeta de tab",category:"Contenido",media:Rt,content:`${D()}`}];function Vt(i){const e="tabs-cards-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Sección con tabs",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!0,removable:!0,attributes:{"data-gjs-type":e,"data-tab-count":"5"},components:Ft,script:Ot(),traits:[{type:"number",name:"data-tab-count",label:"Número de tabs (2-10)",min:2,max:10,changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",(t,r)=>{const a=parseInt(r["data-tab-count"]);isNaN(a)||this.updateTabCount(a)})},updateTabCount(t){const r=Math.min(10,Math.max(2,t)),a=o=>{const d=Array.from({length:o},()=>'<button class="tabs-btn inline-block px-6 py-2 rounded-full border-2 border-[#003B71] bg-transparent text-[#003B71] text-sm font-medium">Lorem Ipsum</button>').join(`
`),c=Array.from({length:o},(h,g)=>`<div class="tabs-panel${g===0?" active":""} grid-cols-3 gap-5">
                            ${D()}
                            ${D()}
                            ${D()}
                            ${D()}
...
`);return`
                    ...
                    <style>${be}</style>`};this.components(a(r)),setTimeout(()=>{const o=this.get("script"),d=this.getEl();o&&typeof o=="function"&&d&&o.call(d)},200)}}}),Yt(i,e),Wt(i,e)}function Yt(i,e){i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},300))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},300)}}),i.on("storage:end:load",()=>{setTimeout(()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})},800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Wt(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#tabs-section-styles")){const a=document.createElement("style");a.id="tabs-section-styles",a.textContent=be,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
            ...
            `,r.appendChild(a)}}})}const Xt=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
...
<div class="w-full rounded-2xl overflow-hidden">
    <img src="${q("images/placeholder.svg")}"
...
${ri}`,li=[{id:"stats-strip-left",label:"Franja estadísticas - izquierda",category:"Heroes",media:ii,content:$e(!0)},{id:"stats-strip-right",label:"Franja estadísticas - derecha",category:"Heroes",media:ai,content:$e(!1)}],oi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,si=[{id:"service-cards-section",label:"Sección de servicios",category:"Productos y Servicios",media:oi,content:`
...
${Ce}`},{id:"service-card",label:"Tarjeta de servicio",category:"Productos y Servicios",media:ni,content:`${se}${Ce}`}],ci=`<svg viewBox="0 0 32 32" width="32" height="32">
...
</style>`,hi=[{id:"contact-info-row",label:"Información de contacto (fila)",category:"Contacto",media:gi,content:`
<section class="ci-section">
    <div class="ci-row">
        ${G("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
        ${G("ri-mail-fill","Correo","servicio.cliente@bancointegral.com","mailto:servicio.cliente@bancointegral.com")}
    </div>
</section>
${ce}`},{id:"contact-info-left",label:"Info de contacto (izquierda)",category:"Contacto",media:ci,content:`
<section class="ci-section">
    <div class="ci-list">
        ${G("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
        ${G("ri-mail-fill","Correo","servicio.cliente@bancointegral.com","mailto:servicio.cliente@bancointegral.com")}
...
${ce}`},{id:"contact-info-right",label:"Info de contacto (derecha)",category:"Contacto",media:di,content:`
<section class="ci-section">
    <div class="ci-list ci-list--right">
        ${G("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090",!0)}
        ${G("ri-mail-fill","Correo","servicio.cliente@bancointegral.com","mailto:servicio.cliente@bancointegral.com",!0)}
    </div>
</section>
${ce}`},{id:"contact-info-item",label:"Ítem de contacto",category:"Contacto",media:fi,content:`
<section class="ci-section">
    <div class="ci-list">
        ${G("ri-phone-fill","Contáctenos","(503)-2250-6090","tel:+50322506090")}
    </div>
</section>
${ce}`}],pi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#003B71" rx="2"/>
...
</style>`,bi=[{id:"rich-list",label:"Lista con título y descripción",category:"Contenido",media:pi,content:`
...
${Le}`},{id:"rich-list-item",label:"Ítem de lista con descripción",category:"Contenido",media:mi,content:`
<section class="rl-section">
    <div class="rl-list">
        ${ie}
    </div>
</section>
${Le}`}],ui=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#ffffff" rx="2" stroke="#e5e7eb" stroke-width="1"/>
...
`,yi=[{id:"contact-form",label:"Formulario de contacto",category:"Formularios",media:ui,content:`
...
<\/script>`}],wi=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    ...
    <line x1="21" y1="2" x2="21" y2="29" stroke="#003B71" stroke-width="1" stroke-opacity="0.3"/>
            ...
            <th colspan="${i.cols}" class="p-3 align-middle text-center text-base font-bold ${t.headerBg} ${t.headerText}">
        ...
        </tr>`,i.headers?.length&&(r+="<tr>",i.headers.forEach((d,c)=>{const h=c<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";r+=`<th class="p-3 align-middle text-sm font-semibold ${t.subheaderBg} ${t.subheaderText} ${h} border-b border-[${t.borderColor}] text-${d.align||"center"}">${d.text||""}</th>`}),r+="</tr>"),r+="</thead>"):i.headers?.length&&(r+="<thead><tr>",i.headers.forEach((d,c)=>{const h=c<i.headers.length-1?`border-r border-[${t.borderColor}]`:"";r+=`<th class="p-3 align-middle text-sm font-semibold ${t.headerBg} ${t.headerText} ${h} border-b border-[${t.borderColor}] text-${d.align||"center"}">${d.text||""}</th>`}),r+="</tr></thead>"),r+="<tbody>";const a=i.rows.length,o={};return i.rows.forEach((d,c)=>{r+="<tr>";let h=0;d.forEach(g=>{for(;o[`${c}-${h}`];)h++;const B=g.colspan||1,$=g.rowspan||1;for(let b=c;b<c+$;b++)for(let S=h;S<h+B;S++)(b!==c||S!==h)&&(o[`${b}-${S}`]=!0);const m=B>1?`colspan="${B}"`:"",n=$>1?`rowspan="${$}"`:"",p=g.isHeader?t.labelBg:c%2===0?t.rowEvenBg:t.rowOddBg,y=g.isHeader?"font-semibold":"font-normal",T=g.isHeader?t.labelText:t.rowText,x=`text-${g.align||"center"}`,E=c+$>=a,C=h+B>=i.cols?"":`border-r border-[${t.borderColor}]`,v=E?"":`border-b border-[${t.borderColor}]`,j=`${C} ${v} p-3 align-middle text-sm ${p} ${y} ${T} ${x}`;g.image?r+=`<td ${m} ${n} class="${j}">
                ...
                </td>`:r+=`<td ${m} ${n} class="${j}">${g.text||""}</td>`,h+=B}),r+="</tr>"}),r+="</tbody></table>",r}function Z(i,e){return{title:"Título de la tabla",cols:i,headers:Array.from({length:i},(t,r)=>({text:`Columna ${r+1}`,align:"center"})),rows:Array.from({length:e},()=>Array.from({length:i},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})))}}function qe(i,e){return`<div class="w-full overflow-x-auto rounded-2xl border-2 border-[${(de[e]||de.blue).borderColor}]">${i}</div>`}function Se(i,e){const t={};return i.forEach((r,a)=>{let o=0;r.forEach(d=>{for(;t[`${a}-${o}`];)o++;const c=Math.min(d.colspan||1,e-o),h=d.rowspan||1;for(let g=a;g<a+h;g++)for(let B=o;B<o+c;B++)(g!==a||B!==o)&&(t[`${g}-${B}`]=`${a}-${o}`);o+=c})}),t}const vi=`
...
.tam-img-selected-info{font-size:0.8rem;color:#6b7280;}`;function ki(){if(document.getElementById("tam-img-modal"))return;const i=document.createElement("div");i.id="tam-img-modal",i.innerHTML=`
        ...
        </div>`,document.body.appendChild(i);let e=null,t=null;async function r(c=""){const h=document.getElementById("tam-img-grid");h.innerHTML='<div class="tam-img-loading"><div class="tam-img-spinner"></div><span>Cargando...</span></div>';try{const g=new URLSearchParams({type:"image",per_page:50});c&&g.append("search",c);const B=document.querySelector('meta[name="media-api-url"]')?.content??"/media/api",n=(await(await fetch(`${B}?${g}`,{headers:{"X-Requested-With":"XMLHttpRequest",Accept:"application/json"}})).json()).items||[];if(!n.length){h.innerHTML='<div class="tam-img-loading"><i class="ri-image-line" style="font-size:2rem;"></i><span>No se encontraron imágenes</span></div>';return}h.innerHTML="",n.forEach(p=>{const y=document.createElement("div");y.className="tam-img-card",y.innerHTML=`<img src="${p.url}" alt="${p.filename}"><p title="${p.filename}">${p.filename}</p>`,y.addEventListener("click",()=>{h.querySelectorAll(".tam-img-card").forEach(T=>T.classList.remove("selected")),y.classList.add("selected"),e=p.url,document.getElementById("tam-img-selected-info").textContent=`Seleccionada: ${p.filename}`,document.getElementById("tam-img-confirm").disabled=!1}),h.appendChild(y)})}catch{h.innerHTML='<div class="tam-img-loading"><i class="ri-error-warning-line" style="font-size:2rem;color:#f87171;"></i><span style="color:#dc2626;">Error al cargar imágenes</span></div>'}}function a(c){t=c,e=null,document.getElementById("tam-img-selected-info").textContent="Ninguna imagen seleccionada",document.getElementById("tam-img-confirm").disabled=!0,document.getElementById("tam-img-search-input").value="",i.classList.add("open"),r()}function o(){i.classList.remove("open"),e=null,t=null}document.getElementById("tam-img-close").addEventListener("click",o),document.getElementById("tam-img-cancel").addEventListener("click",o),document.getElementById("tam-img-confirm").addEventListener("click",()=>{e&&t&&(t(e),o())});let d;document.getElementById("tam-img-search-input").addEventListener("input",c=>{clearTimeout(d),d=setTimeout(()=>r(c.target.value),300)}),i.addEventListener("click",c=>{c.target===i&&o()}),window.__openTableImagePicker=a}function Bi(i,e){if(document.getElementById("table-admin-modal"))return;const t=document.createElement("style");t.id="table-admin-modal-styles",t.textContent=vi,document.head.appendChild(t),ki();const r=document.createElement("div");r.id="table-admin-modal",r.innerHTML=`
        <div class="tam-container">
            ...
            </th>`).join("")}</tr>`,n.innerHTML=o.rows.map((x,E)=>`<tr>${Array.from({length:p},(C,v)=>{const j=T[`${E}-${v}`];if(j)return`<td class="tam-cell is-spanned" data-row="${E}" data-col="${v}">
                        <div class="tam-spanned-label">Combinada con [${j}]</div>
                    </td>`;const b=x[v]||{text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null},S=b.colspan||1,N=b.rowspan||1,z=S>1||N>1;return`<td class="tam-cell ${b.isHeader?"is-header-cell":""} ${b.image?"has-image":""} ${z?"has-span":""}"
                    ...
                    <div class="tam-cell-actions">
                        ...
                        <div class="tam-cell-span-group">
                            ...
                            <input type="number" class="tam-cell-span-input" min="1" max="${y-E}"
                                value="${N}" data-action="rowspan" data-row="${E}" data-col="${v}">
                        </div>
                    </div>
                </td>`}).join("")}</tr>`).join(""),n.querySelectorAll("input[data-action=colspan], input[data-action=rowspan]").forEach(x=>{x.addEventListener("change",()=>{const E=parseInt(x.dataset.row),_=parseInt(x.dataset.col),C=Math.max(1,parseInt(x.value)||1);o.rows[E]?.[_]&&(x.dataset.action==="colspan"?o.rows[E][_].colspan=Math.min(C,p-_):o.rows[E][_].rowspan=Math.min(C,y-E),g())})}),n.querySelectorAll("button[data-action]").forEach(x=>{x.addEventListener("click",E=>{E.preventDefault(),E.stopPropagation();const _=x.dataset.action,C=parseInt(x.dataset.row),v=parseInt(x.dataset.col);if(!(isNaN(C)||isNaN(v)||!o.rows[C]?.[v])){if(_==="header"){o.rows[C][v].isHeader=!o.rows[C][v].isHeader;const j=n.querySelector(`td[data-row="${C}"][data-col="${v}"]`);j&&(j.dataset.isheader=o.rows[C][v].isHeader?"1":"0",j.classList.toggle("is-header-cell",o.rows[C][v].isHeader)),x.classList.toggle("active",o.rows[C][v].isHeader),x.textContent=o.rows[C][v].isHeader?"✓ Etiqueta":"Etiqueta";return}if(_==="image"){window.__openTableImagePicker&&window.__openTableImagePicker(j=>{o.rows[C][v].image=j;const b=n.querySelector(`td[data-row="${C}"][data-col="${v}"]`);if(b){b.dataset.image=j,b.classList.add("has-image");let S=b.querySelector(".tam-cell-img-preview");S||(S=document.createElement("img"),S.className="tam-cell-img-preview",b.insertBefore(S,b.firstChild)),S.src=j;const N=b.querySelector("[data-action=image]");if(N&&(N.innerHTML='<i class="ri-image-line"></i> Cambiar'),!b.querySelector("[data-action=clear-image]")){const z=document.createElement("button");z.type="button",z.className="tam-cell-btn tam-cell-btn-clear",z.dataset.action="clear-image",z.dataset.row=C,z.dataset.col=v,z.textContent="✕ Quitar",z.addEventListener("click",Y=>{Y.preventDefault(),Y.stopPropagation(),o.rows[C][v].image=null,b.dataset.image="",b.classList.remove("has-image"),S.remove(),z.remove();const H=b.querySelector("[data-action=image]");H&&(H.innerHTML='<i class="ri-image-line"></i> Imagen')}),b.querySelector(".tam-cell-actions").appendChild(z)}}});return}_==="clear-image"&&(o.rows[C][v].image=null,$())}})})}document.getElementById("tam-close").addEventListener("click",c),document.getElementById("tam-cancel").addEventListener("click",c),r.addEventListener("click",m=>{m.target===r&&c()}),document.getElementById("tam-rebuild").addEventListener("click",()=>{const m=parseInt(document.getElementById("tam-cols").value)||3,n=parseInt(document.getElementById("tam-rows").value)||3;for(B(),h();o.headers.length<m;)o.headers.push({text:`Col ${o.headers.length+1}`,align:"center"});for(o.headers=o.headers.slice(0,m),o.cols=m;o.rows.length<n;)o.rows.push(Array.from({length:m},()=>({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null})));o.rows=o.rows.slice(0,n).map(p=>{for(;p.length<m;)p.push({text:"",align:"center",isHeader:!1,colspan:1,rowspan:1,image:null});return p.slice(0,m)}),$()}),document.getElementById("tam-apply").addEventListener("click",()=>{h();const m=document.getElementById("tam-theme").value;a&&(a.set("tableData",JSON.parse(JSON.stringify(o))),a.set("tableTheme",m),a.addAttributes({"data-table-theme":m}),me(a)),c()}),window.__openTableAdminModal=d}function me(i){const e=i.get("tableData"),t=i.get("tableTheme")||"blue";e&&i.components(qe(Me(e,t),t))}function Ei(){return function(){}}const $i=[{id:"table-blue",label:"Tabla azul",category:"Interactivos",media:wi,content:{type:"table-component",attributes:{"data-table-theme":"blue"}}},{id:"table-orange",label:"Tabla naranja",category:"Interactivos",media:`<svg viewBox="0 0 32 32" width="32" height="32">
            <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
            ...
            `,r.appendChild(a)}})}const ji=`<svg viewBox="0 0 80 32" width="80" height="32">
...
<a href="#"
   ...
   class="dld-simple-${e}">
    ...
    <rect x="3" y="25" width="7" height="2.5" fill="#dee2e6" rx="0.5"/>
</svg>`;function Yi(){return function(){const i=this,e="agencies-map-component",t="/api/agencies/active",r="agencies";let a=[],o=[],d={},c=null,h=[];const g=async()=>{try{B(),await m(),await y(),T(),j(),z(),Y(),$()}catch(l){console.error("Error initializing map:",l),ae("Error al cargar las agencias"),$()}};function B(){const l=i.querySelector(`.${e}-list`);l&&(l.innerHTML=`
                ...
                `);const s=i.querySelector(`.${e}-map-container`);s&&(s.style.opacity="0.5")}function $(){const l=i.querySelector(`.${e}-map-container`);l&&(l.style.transition="opacity 0.3s ease",l.style.opacity="1")}async function m(){try{const s=await(await fetch(t)).json(),f=r?s[r]:s;Array.isArray(f)?(a=f.filter(u=>u.latitude&&u.longitude&&!isNaN(u.latitude)&&!isNaN(u.longitude)),o=[...a],n()):(a=[],o=[])}catch(l){console.error("Error loading items:",l),a=[],o=[]}}function n(){const l=[...new Set(a.map(f=>f.zone).filter(Boolean))].sort(),s=[...new Set(a.map(f=>f.department).filter(Boolean))].sort();d={zone:l,department:s},setTimeout(()=>{const f=i.querySelector(`.${e}-filters-container`);f&&!f.hasChildNodes()&&(f.innerHTML=`
                    ...
                    `,p())},100)}function p(){const l=i.querySelector(`.${e}-zone-filter`),s=i.querySelector(`.${e}-department-filter`);l&&d.zone&&d.zone.forEach(f=>{const u=document.createElement("option");u.value=f,u.textContent=f,l.appendChild(u)}),s&&d.department&&d.department.forEach(f=>{const u=document.createElement("option");u.value=f,u.textContent=f,s.appendChild(u)})}async function y(){if(!document.getElementById("leaflet-css")){const l=document.createElement("link");l.id="leaflet-css",l.rel="stylesheet",l.href="https://unpkg.com/leaflet@1.9.4/dist/leaflet.css",document.head.appendChild(l)}if(typeof window.L>"u"&&await new Promise((l,s)=>{const f=document.createElement("script");f.src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js",f.onload=l,f.onerror=s,document.head.appendChild(f)}),typeof window.L<"u"&&window.L.Icon&&window.L.Icon.Default){const l=window.L.icon({iconUrl:"data:image/svg+xml;base64,"+btoa(`
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 36" width="24" height="36">
                            <path fill="#f0872a" d="M12 0C7.6 0 4 3.6 4 8c0 5.4 8 16 8 16s8-10.6 8-16c0-4.4-3.6-8-8-8z"/>
                            <circle cx="12" cy="8" r="3" fill="#fff"/>
                        </svg>
                    `),iconSize:[30,45],iconAnchor:[15,45],popupAnchor:[0,-45]});window.customOrangeIcon=l}}function T(){const l=i.querySelector(`.${e}-map`);if(!l||!window.L)return;l._leaflet_id&&l._map&&(l._map.remove(),delete l._map),c=window.L.map(l).setView([13.7942,-88.8965],8),window.L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}).addTo(c),l._map=c,c.whenReady(function(){x(),setTimeout(()=>{c&&c.invalidateSize&&c.invalidateSize()},300)})}function x(){E(),_(),v()}function E(){h.forEach(l=>{l.marker&&c.removeLayer(l.marker)}),h=[]}function _(){o.forEach((l,s)=>{if(l.latitude&&l.longitude){const f=C(l),u=window.L.marker([l.latitude,l.longitude],{icon:window.customOrangeIcon||new window.L.Icon.Default}).addTo(c).bindPopup(f);h.push({marker:u,item:l,index:s})}})}function C(l){let s=`<div style="min-width: 200px;"><h4 style="font-weight: 700; margin-bottom: 12px; color: #0d3f6a; font-size: 16px; border-bottom: 2px solid #f0872a; padding-bottom: 8px;">${l.name}</h4>`;if(l.address&&(s+=`<p style="margin: 8px 0 0 0; font-size: 14px; display: flex; align-items: start; gap: 8px;"><i class="ri-map-pin-line" style="color: #f0872a; font-size: 16px; margin-top: 2px;"></i><span style="color: #4b5563;">${l.address}</span></p>`),l.municipality||l.department){const f=[l.municipality,l.department].filter(Boolean).join(", ");s+=`<p style="margin: 0 0 8px 0; font-size: 13px; color: #6b7280; padding-left: 24px;">${f}</p>`}return l.schedule&&(s+=`<p style="margin: 8px 0; font-size: 14px; display: flex; align-items: start; gap: 8px;"><i class="ri-time-line" style="color: #f0872a; font-size: 16px; margin-top: 2px;"></i><span style="color: #4b5563;">${l.schedule}</span></p>`),s+="</div>",s}function v(){if(o.length>0){const l=o.filter(s=>s.latitude&&s.longitude).map(s=>[s.latitude,s.longitude]);l.length>1?c.once("moveend",function(){setTimeout(()=>{try{c&&c._loaded&&typeof c.fitBounds=="function"&&c.fitBounds(l,{padding:[50,50],maxZoom:12,animate:!1})}catch(s){console.warn("Error fitting bounds:",s)}},100)}):l.length===1&&c.setView(l[0],14)}}function j(){const l=i.querySelector(`.${e}-list`);if(!l)return;if(o.length===0){S(l);return}const s=o.map((f,u)=>b(f,u)).join("");l.innerHTML=s,N()}function b(l,s){const f=l.phones&&l.phones.length>0?l.phones.map(k=>`
                    ...
                    `).join(""):"",u=`https://www.google.com/maps/search/?api=1&query=${l.latitude},${l.longitude}`;return`
            ...
            `}function N(){const l=i.querySelectorAll(`.${e}-item`);l.forEach(s=>{s.addEventListener("click",f=>{if(f.target.closest(".agency-maps-btn"))return;f.preventDefault();const u=parseInt(s.dataset.itemIndex),k=o[u];if(!k)return;l.forEach(w=>{w.classList.remove("bg-white","border-secondary","selected-agency"),w.classList.add("bg-white/10","border-white/20");const A=w.querySelector(".agency-title"),re=w.querySelectorAll("i"),le=w.querySelectorAll(".agency-phone-link"),ge=w.querySelectorAll("p:not(:has(.agency-phone-link))"),fe=w.querySelector(".agency-maps-btn");A&&(A.classList.remove("text-secondary"),A.classList.add("text-white")),re.forEach(R=>{R.classList.remove("text-secondary","text-gray-300","text-white"),R.classList.add("text-primary")}),le.forEach(R=>{R.classList.remove("text-secondary"),R.classList.add("text-white")}),ge.forEach(R=>{R.classList.remove("text-secondary"),R.classList.add("text-gray-200")}),fe&&(fe.classList.remove("bg-secondary"),fe.classList.add("bg-primary"))}),s.classList.remove("bg-white/10","border-white/20"),s.classList.add("bg-white","border-secondary","selected-agency");const M=s.querySelector(".agency-title"),O=s.querySelectorAll("i"),W=s.querySelectorAll(".agency-phone-link"),F=s.querySelectorAll("p:not(:has(.agency-phone-link))"),P=s.querySelector(".agency-maps-btn");if(M&&(M.classList.remove("text-white"),M.classList.add("text-secondary")),O.forEach(w=>{w.classList.remove("text-secondary","text-gray-300","text-white"),w.classList.add("text-primary")}),W.forEach(w=>{w.classList.remove("text-white"),w.classList.add("text-secondary")}),F.forEach(w=>{w.classList.remove("text-gray-200"),w.classList.add("text-secondary")}),P&&(P.classList.remove("bg-primary"),P.classList.add("bg-secondary")),c&&k&&c._loaded)try{c.flyTo([k.latitude,k.longitude],14,{animate:!0,duration:1});const w=h.find(A=>A.item.id===k.id);w&&w.marker&&w.marker.openPopup()}catch(w){console.warn("Error updating map view:",w)}})})}function z(){const l=i.querySelector(`.${e}-search-input`),s=i.querySelector(`.${e}-zone-filter`),f=i.querySelector(`.${e}-department-filter`),u=i.querySelector(`.${e}-no-results`),k={Occidental:["Ahuachapán","Santa Ana","Sonsonate"],Paracentral:["Chalatenango","Cuscatlán","La Paz","Cabañas","San Vicente"],Central:["La Libertad","San Salvador"],Oriental:["Usulután","San Miguel","Morazán","La Unión"]},M=()=>{if(!s||!f)return;const W=s.value,F=f.value;if(!W)f.innerHTML='<option value="">Todos los Departamentos</option>',[...new Set(a.map(w=>w.department).filter(Boolean))].sort().forEach(w=>{const A=document.createElement("option");A.value=w,A.textContent=w,f.appendChild(A)}),F&&(f.value=F);else{const P=k[W]||[];f.innerHTML='<option value="">Todos los Departamentos</option>',P.forEach(w=>{const A=document.createElement("option");A.value=w,A.textContent=w,f.appendChild(A)}),P.includes(F)&&(f.value=F)}},O=()=>{const W=l?l.value.toLowerCase().trim():"",F=s?s.value:"",P=f?f.value:"";o=a.filter(w=>{let A=!0,re=!0,le=!0;return W&&(A=Object.values(w).some(ge=>String(ge).toLowerCase().includes(W))),F&&(re=w.zone===F),P&&(le=w.department===P),A&&re&&le}),j(),x(),u&&u.classList.toggle("hidden",o.length>0)};l&&l.addEventListener("input",O),s&&s.addEventListener("change",()=>{M(),O()}),f&&f.addEventListener("change",O)}function Y(){const l=i.querySelector("[data-title]");if(l){const u=H("map-title")||"Nuestras Agencias";l.textContent=u}const s=i.querySelector(`.${e}-search-input`);if(s){const u=H("search-placeholder")||"Buscar...";s.setAttribute("placeholder",u)}const f=i.querySelector(`.${e}-no-results`);if(f){const u=H("no-results-text")||"No se encontraron agencias";f.textContent=u}}function H(l){return i.closest(`[data-gjs-type="${e}"]`)?.getAttribute(l)}function ae(l){const s=i.querySelector(`.${e}-list`);s&&(s.innerHTML=`
                    <div class="flex flex-col items-center justify-center p-12 text-center">
                ...
                `)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",g):g()}}const Wi=[{id:"agencies-map",label:"Mapa de Agencias",category:"Interactivos",media:Vi,content:{type:"agencies-map-component"}}];function Xi(i){const e="agencies-map-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute&&t.getAttribute("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Mapa de Agencias",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{class:"py-12 bg-secondary","data-gjs-type":e},components:`
...
@media(max-width:480px){
    .banner-btn{flex:1 1 100%;min-width:0;}
            ...
            `;const l=i.ownerDocument??document;if(!l.getElementById("banner-skeleton-styles")){const s=l.createElement("style");s.id="banner-skeleton-styles",s.textContent=`
                ...
                `,l.head.appendChild(s)}}function T(l){return t[l]?t[l]:l==="outline-blue"||l==="outline-orange"?t["outline-white"]:t["fill-white"]}function x(l,s,f,u){const k=T(f),M=s?"a":"span",O=s?`href="${s}"${u?' target="_blank" rel="noopener noreferrer"':""}`:"";return`<${M} ${O}
                class="banner-btn"
                data-bg="${k.bg}"
                data-color="${k.color}"
                data-hover-bg="${k.hoverBg}"
                data-hover-color="${k.hoverColor}"
                style="background:${k.bg};color:${k.color};border:2px solid ${k.border};">
                ${l}
            </${M}>`}function E(){n.innerHTML=a.map((l,s)=>`
                        ...
                        <div class="banner-content">
                            ...
                            ${l.btn_primary_text||l.btn_secondary_text?`
                                <div class="banner-buttons">
                                    ${l.btn_primary_text?x(l.btn_primary_text,l.btn_primary_url,l.btn_primary_style,l.btn_primary_external):""}
                                    ${l.btn_secondary_text?x(l.btn_secondary_text,l.btn_secondary_url,l.btn_secondary_style,l.btn_secondary_external):""}
                                </div>`:""}
                        </div>
                    </div>
                </div>
            `).join(""),_(),C(),v()}function _(){i.querySelectorAll(".banner-btn").forEach(l=>{const s=l.style.borderColor;l.addEventListener("mouseenter",()=>{l.style.background=l.dataset.hoverBg,l.style.color=l.dataset.hoverColor,l.style.borderColor=l.dataset.hoverBg}),l.addEventListener("mouseleave",()=>{l.style.background=l.dataset.bg,l.style.color=l.dataset.color,l.style.borderColor=s})})}function C(){n.addEventListener("mousedown",j),n.addEventListener("touchstart",j,{passive:!0}),n.addEventListener("mousemove",b),n.addEventListener("touchmove",b,{passive:!0}),n.addEventListener("mouseup",S),n.addEventListener("touchend",S),n.addEventListener("mouseleave",S)}function v(){a.forEach(l=>{const s=new Image;s.src=l.image_url})}function j(l){c=!0,h=l.touches?l.touches[0].clientX:l.clientX,g=0}function b(l){c&&(g=(l.touches?l.touches[0].clientX:l.clientX)-h)}function S(){c&&(c=!1,Math.abs(g)>=B&&(z(g<0?(o+1)%a.length:(o-1+a.length)%a.length),H()),g=0)}function N(){const l=i.querySelector(".banner-stripe");if(!l||a.length<=1)return;const s=document.createElement("div");s.className="banner-dots",a.forEach((f,u)=>{const k=document.createElement("button");k.className="banner-dot",k.dataset.index=String(u),k.setAttribute("aria-label",`Banner ${u+1}`),k.addEventListener("click",()=>{z(u),H()}),s.appendChild(k)}),l.innerHTML="",l.appendChild(s)}function z(l,s=!0){const f=n.querySelectorAll(".banner-slide"),u=i.querySelectorAll(".banner-dot");f.forEach((k,M)=>{const O=M===l;s||(k.style.transition="none"),k.classList.toggle("banner-slide--active",O),s||requestAnimationFrame(()=>{k.style.transition=""})}),u.forEach((k,M)=>{k.classList.toggle("banner-dot--active",M===l)}),o=l}function Y(){a.length<=1||!$||(d=setInterval(()=>{z((o+1)%a.length)},5e3))}function H(){$&&(clearInterval(d),Y())}function ae(){const l=i.querySelector(".banner-wrapper");l&&(l.innerHTML=`
                    <div style="display:flex;align-items:center;justify-content:center;
                                min-height:480px;color:rgba(255,255,255,0.5);font-size:14px;padding:40px;">
                        Sin contenido.
                    </div>`)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",p):p()}}const ea=[{id:"banner-hero",label:"Banner Slider",category:"Banners",media:Ki,content:{type:"banner-hero-component"}}];function ta(i){const e="banner-hero-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Slider",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-autoplay":"true","data-category":""},components:`
                ...
                `,script:Ji(),traits:[{type:"select",name:"data-autoplay",label:"Avance automático",options:[{id:"true",name:"Activado"},{id:"false",name:"Desactivado"}],changeProp:!1},{type:"select",name:"data-category",label:"Filtrar por categoría",options:[{id:"",name:"Todas las categorías"}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const r=this.get("script");r&&typeof r=="function"&&setTimeout(()=>r.call(t),100)})}}}),aa(i,e),ra(i,e),ia(i,e)}async function ia(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",a=await(await fetch(t)).json();if(!Array.isArray(a))return;const o=[...new Set(a.map(g=>g.category).filter(Boolean))].sort();if(o.length===0)return;const d=i.DomComponents.getType(e);if(!d)return;const h=d.model.prototype.defaults.traits.find(g=>g.name==="data-category");if(!h)return;h.options=[{id:"",name:"Todas las categorías"},...o.map(g=>({id:g,name:g}))]}catch{}}function aa(i,e){i.on("storage:end:load",()=>{setTimeout(()=>Te(i,e),1e3)}),i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>Te(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Te(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})}function ra(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#banner-hero-styles")){const a=document.createElement("style");a.id="banner-hero-styles",a.textContent=Qi,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
            ...
            `,r.appendChild(a)}}})}const la=`<svg viewBox="0 0 32 32" width="32" height="32">
    <rect width="32" height="32" fill="#f8f9fa" rx="2"/>
    <rect x="1" y="6" width="30" height="20" fill="#003B71" rx="2"/>
    <rect x="1" y="22" width="30" height="4" fill="#E97300" rx="0"/>
    <rect x="3" y="9" width="12" height="2" fill="rgba(255,255,255,0.9)" rx="1"/>
...
@media(max-width:480px){
    .bsingle-btn{flex:1 1 100%;min-width:0;}
                ...
                </div>`}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",d):d()}}const sa=[{id:"banner-single",label:"Banner Individual",category:"Banners",media:la,content:{type:"banner-single-component"}}];function ca(i){const e="banner-single-component";i.DomComponents.addType(e,{isComponent:t=>t.getAttribute?.("data-gjs-type")===e?{type:e}:!1,model:{defaults:{name:"Banner Individual",tagName:"section",draggable:!0,droppable:!1,editable:!1,stylable:!1,resizable:!1,selectable:!0,hoverable:!0,layerable:!0,highlightable:!1,copyable:!1,removable:!0,attributes:{"data-gjs-type":e,"data-banner-id":""},components:`
                ...
                `,script:na(),traits:[{type:"select",name:"data-banner-id",label:"Banner a mostrar",options:[{id:"",name:"Cargando banners..."}],changeProp:!1}]},init(){this.set("type",e),this.addAttributes({"data-gjs-type":e}),this.on("change:attributes",()=>{const t=this.getEl();if(!t)return;const r=this.get("script");r&&typeof r=="function"&&setTimeout(()=>r.call(t),100)})}}}),ga(i,e),fa(i,e),da(i,e)}async function da(i,e){try{const t=document.querySelector('meta[name="api-banners-url"]')?.content??"/api/banners/active",a=await(await fetch(t)).json();if(!Array.isArray(a)||a.length===0)return;const o=i.DomComponents.getType(e);if(!o)return;const c=o.model.prototype.defaults.traits.find(h=>h.name==="data-banner-id");if(!c)return;c.options=[{id:"",name:"— Seleccionar banner —"},...a.map(h=>({id:String(h.id),name:h.category?`[${h.category}] ${h.title}`:h.title}))]}catch{}}function ga(i,e){i.on("storage:end:load",()=>{setTimeout(()=>Ae(i,e),1e3)}),i.on("component:mount",t=>{const r=t.getEl();r?.getAttribute?.("data-gjs-type")===e&&(t.set("type",e),setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500))}),i.on("component:clone",t=>{if(t.get("type")===e){const r=t.getEl();r&&setTimeout(()=>{const a=t.get("script");a&&typeof a=="function"&&a.call(r)},500)}}),i.on("canvas:render",()=>{setTimeout(()=>Ae(i,e),800)}),i.on("storage:start:store",()=>{i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e),t.addAttributes({"data-gjs-type":e})})})}function Ae(i,e){i.getWrapper().find(`[data-gjs-type="${e}"]`).forEach(t=>{t.set("type",e);const r=t.getEl();if(r?.isConnected){const a=t.get("script");a&&typeof a=="function"&&a.call(r)}})}function fa(i,e){i.on("load",()=>{const t=i.Canvas.getFrameEl();if(!t)return;const r=t.contentDocument?.head;if(r){if(!r.querySelector("#banner-single-styles")){const a=document.createElement("style");a.id="banner-single-styles",a.textContent=oa,r.appendChild(a)}if(!r.querySelector(`#${e}-editor-css`)){const a=document.createElement("style");a.id=`${e}-editor-css`,a.textContent=`
                [data-gjs-type="${e}"] * { pointer-events: none !important; }
                [data-gjs-type="${e}"].gjs-selected,
                [data-gjs-type="${e}"].gjs-hovered {
                    outline: 2px dashed rgba(240,135,42,0.6) !important;
            ...
            `,r.appendChild(a)}}})}function ha(i){L.registerBlocks(rt),L.registerBlocks(ht),L.registerBlocks(ea),L.registerBlocks(sa),L.registerBlocks(li),L.registerBlocks(Ut),L.registerBlocks(yt),L.registerBlocks(Bt),L.registerBlocks(zt),L.registerBlocks(Dt),L.registerBlocks(Mt),L.registerBlocks($t),L.registerBlocks(ti),L.registerBlocks(St),L.registerBlocks(Pt),L.registerBlocks(si),L.registerBlocks(hi),L.registerBlocks(bi),L.registerBlocks(yi),L.registerBlocks($i),L.registerBlocks(Pi),L.registerBlocks(Ui),L.registerBlocks(Wi),L.applyToEditor(i),Xi(i),ta(i),ca(i),Hi(i),Vt(i),Ci(i)}function pa(i,e,t){i.on("component:add",()=>e.markAsDirty()),i.on("component:remove",()=>e.markAsDirty()),i.on("component:update",()=>e.markAsDirty()),i.on("style:update",()=>e.markAsDirty());const r=document.getElementById("save-button");r&&r.addEventListener("click",async()=>{await ma(i,e,t,r)}),document.addEventListener("keydown",a=>{(a.ctrlKey||a.metaKey)&&a.key==="s"&&(a.preventDefault(),r&&!r.disabled&&r.click())})}async function ma(i,e,t,r){r.disabled=!0,r.innerHTML='<i class="ri-loader-4-line animate-spin"></i><span>Guardando...</span>';try{t.needsTitle()?await ba(i,e,t):await Ne(i,e,t)}catch(a){Pe(a.message,"error")}finally{r.disabled=!1,r.innerHTML='<i class="ri-save-line"></i><span>Guardar</span>'}}async function ba(i,e,t){return new Promise((r,a)=>{Oe({title:"Título de la Página",description:"Ingresa un título descriptivo para identificar esta página",placeholder:"Ej: Acerca de Nosotros",icon:"ri-file-text-line",iconBg:"#dbeafe",iconColor:"#2563eb",confirmLabel:"Guardar",onConfirm:async o=>{if(!o?.trim()){a(new Error("El título es obligatorio"));return}try{await Ne(i,e,t,o),r()}catch(d){a(d)}},onCancel:()=>{a(new Error("Guardado cancelado"))}})})}async function Ne(i,e,t,r=null){const o={...e.getEditorContent(i),is_published:t.isPublished};r&&(o.title=r);const d=await e.savePage(i,o,t.storeUrl,t.getHttpMethod());d.success&&(e.markAsClean(),Pe(d.message,"success"),!t.isEditMode&&d.page?(t.updatePageInfo(d),t.updateTitle(d.page.title)):r&&t.updateTitle(r))}function Pe(i,e){typeof window.showNotification=="function"&&window.showNotification(i,e)}document.addEventListener("DOMContentLoaded",async()=>{const i=new Fe,e=new et;new tt(i);const t=Re();if(t.on("load",()=>{ha(t),Ue(t),Ve(),Ye(),We(t),Xe(t),Ge(t),Ze(t),Ke(t),Qe(t),ua(t),xa(t),ya(t),setTimeout(()=>{t.runCommand("sw-visibility"),t.Panels.getButton("options","sw-visibility")?.set("active",!0)},100)}),e.isEditMode)try{await i.loadPageContent(t,e.loadUrl),De("Contenido cargado correctamente","success")}catch(r){De("Error al cargar el contenido","error"),console.error(r)}pa(t,i,e)});function ua(i){const e=[{cmd:"set-device-desktop",device:"Desktop"},{cmd:"set-device-tablet",device:"Tablet"},{cmd:"set-device-mobile",device:"Mobile"}];e.forEach(({cmd:t,device:r})=>{i.Commands.add(t,{run:a=>{a.setDevice(r),e.forEach(({cmd:o})=>{a.Panels.getButton("devices-c",o)?.set("active",o===t)})}})})}function xa(i){i.Commands.add("canvas-clear",{run:e=>{Je({title:"Limpiar canvas",description:"¿Estás seguro de que quieres eliminar todo el contenido del canvas? Esta acción no se puede deshacer.",icon:"ri-delete-bin-line",iconBg:"#fef2f2",iconColor:"#dc2626",confirmLabel:"Limpiar todo",confirmColor:"#dc2626",onConfirm:()=>{e.DomComponents.clear(),e.CssComposer.clear()}})}})}function ya(i){const e=i.Canvas.getFrameEl();if(!e?.contentDocument?.head)return;const t=e.contentDocument.createElement("style");t.id="gjs-dashed-fix",t.textContent="body,body.gjs-dashed{margin:0!important;padding:0 0 160px 0!important;}",e.contentDocument.head.appendChild(t)}function De(i,e="info"){typeof window.showNotification=="function"&&window.showNotification(i,e)}
